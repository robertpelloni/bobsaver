...
// res_r = (res_r + nhg_blur * s) / (1.0 + s);
// res_g = (res_g + nhb_blur * s) / (1.0 + s);
// res_b = (res_b + nhr_blur * s) / (1.0 + s);

/**/
// Random Weights
    float mpw0 = dvmd[48] * s * sign(dvmd[51]) * 4.0;
    float mpw1 = dvmd[49] * s * sign(dvmd[51]) * 4.0;
    float mpw2 = dvmd[50] * s * sign(dvmd[51]) * 4.0;
