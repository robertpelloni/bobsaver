...
// original https://www.shadertoy.com/view/4Xfyz8
...
void main(void)
{
    ...
    vec4 O2=O;
    vec3 dir = ray_dir( 45.0, resolution.xy, C.xy );
    
    // default ray origin
    vec3 eye = vec3( 0.0, 0.0, 4.4 );

    // rotate camera
    mat3 rot = rot3xy( vec2( -DEG_TO_RAD * 30.0, time * 0.5 ) );
    dir = rot * dir;
    eye = rot * eye;
