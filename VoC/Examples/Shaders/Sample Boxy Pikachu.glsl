...
// original https://www.shadertoy.com/view/ldyBWD
...
{
    glFragColor = vec4(0);
    vec2 t = gl_FragCoord.xy / resolution.xy * 2. - 1.;
    ...
    // Backdrop colour.
    glFragColor.rgb=vec3(.5,15,.3)*.9;
    
    if(groundt<dt)
    {
        // Ground is nearer.
        vec3 rp=ro+rd*groundt;
        glFragColor.rgb*=mix(.7,1.,linstep(0.,.1,box(rp.xz,vec2(1.))-.5));
    }
    else if(dt<20.)
    {   
        // SDF is nearer.
