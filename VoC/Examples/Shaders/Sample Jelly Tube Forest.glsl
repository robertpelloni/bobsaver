...
// original https://www.shadertoy.com/view/tsBSzc
...
void main(void)
{
    ...
    for(int y = 0; y < AA; ++y)
        for(int x = 0; x < AA; ++x)
        {
            vec2 t = uv.xy + vec2(x, y) / float(AA) * vec2(2.2) / resolution.x;
...

            {
                ...
                glFragColor.rgb += (vec3(cos(u), cos(u * 2.), cos(u * 3.)) * .5 + .5) * mix(.9, 1., fresnel);

                vec3 rp2 = rp * 10.;
                glFragColor.rgb = mix(glFragColor.rgb, vec3(1), .1 - .1 * smoothstep(.0, .05, length(rp2 - (floor(rp2) + cos(time + floor(rp2.zxy) * 9.) * .4 + .5)) - .1));

                vec3 rp3 = rp * 3.;
                glFragColor.rgb *= mix(.7, 1., smoothstep(.0, .01, length(rp3 - (floor(rp3) + cos(floor(rp3.zxy) * 9.) * .25 + .5)) - .2));

                glFragColor.rgb *= rp.y / 10. + 1.5;

                glFragColor.rgb += 1.5 * spec * fresnel + fresnel / 15.;
            }

            acc += clamp(glFragColor.rgb, 0., 1.);
            wsum += 1.;
        }
    glFragColor.rgb = pow(acc / wsum, vec3(1. / 2.2));
}
