...
// original https://www.shadertoy.com/view/lt2Bz3
...
Shape sdKirby (vec3 pos) {
    ...
    p = pos;
    p.y -= .3;
    p.y *= 1./smoothstep(.0,.1,abs(sin(time)));
    p.x = abs(p.x) - .2;
    p.x *= 2.;
    p.y *= .75;
...
vec3 camera (vec3 p) {
    //float click = 0.0;//clamp(mouse*resolution.xy.z, 0., 1.);
    //p.yz *= rot(click*(-.25*PI*(mouse*resolution.y/resolution.y-.5)));
    //p.xz *= rot(click*(-.5*PI*(mouse*resolution.x/resolution.x-.5)));
    //p.xz *= rot((1.-click)*(.5*PI*(.4*sin(time*.1))));
    return vec3(0.0,0.5,6.0);
}
...
vec3 raymarch (vec2 uv) {
    ...
    float y = uv.y + sin(uv.x*3.-time+sin(uv.x*6.+time))*.05;
...
void main(void)
{
    ...
    glFragColor = vec4(raymarch(uv),1.0);
}
