...
{
    float t = time * 0.4 + instance * 10.0;
...
float map(vec3 pos)
{
    ...
    rp.x += sin(instance * 17.0 + time * 0.9) * 0.05;
    rp.y += sin(instance * 10.0) * 0.4 + pow(cos(instance * 17.0 + time), 2.0) * 0.08;
...
vec3 trace(vec3 rayPos, vec3 rayDir)
{
    ...
    #ifdef PARTICLES
    float wt = snoise(vec3(gl_FragCoord.xy, time)) * 0.05 + 0.1;
    for (int i = 0; i < 50; i++)
    {
    ...
    vec3 tex = texture2D(backbuffer, uv).rgb;
...
vec3 camera(vec2 px)
{
    ...
    float x = sin(time*1.0) * 0.1 + 0.1;
    float y = cos(time*1.0) * 0.05 - 1.1;
    vec3 rayPos = vec3(x, y, time);
