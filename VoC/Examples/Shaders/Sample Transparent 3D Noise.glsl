...
// original https://www.shadertoy.com/view/lstGRB
...
//
// If not loaded, you need to load up the noise texture with this one.
...
float noise3D(vec3 p){
    ...
    // Setting up the stride vector for randomaization and interpolation, kind of. 
...
float map(vec3 p){
    ...
    // You need to load up the noise texture with this one.
...
void main(void) {
    ...
    float thD = .025; // + smoothstep(-0.2, 0.2, sin(time*0.75 - 3.14159*0.4))*0.025;
    ...
    uv = abs(gl_FragCoord.xy/resolution.xy - .5); // Wasteful, but the GPU can handle it.
