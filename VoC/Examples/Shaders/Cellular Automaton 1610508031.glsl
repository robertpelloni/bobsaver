...
vec2 nh_val(float nhsz, float rnsz, vec2 sinmod, int col) {
    float nhsz_c = 0.0;
    float val = 0.0;
    float nh_val = 0.0;
    for(float i = -nhsz; i <= nhsz; i += 1.0) {
...
void main() {
    ...
    float nh0s = 4.0;
    vec2 nh0 = nh_val ( nh0s, 1.0, vec2( 1.0, 1.0 ), 0 );
    float nh0v = nh0[0] / nh0[1];

    float nh1s = 6.0;
    vec2 nh1 = nh_val ( nh1s, 3.0, vec2( 1.0, 1.0 ), 0 );
    float nh1v = nh1[0] / nh1[1];

    float val_0 = clamp(pow(1.0-abs(nh0v-nh1v),9.4),0.0,1.0);

    float res_0 = nh0v*val_0*(1.67+cs*10.0);
    float res_1 = nh1v*val_0*(1.53);

    float val_2 = abs(gv(0.0,0.0,0)-res_0);
    float val_3 = abs(gv(0.0,0.0,0)-res_1);

    float res_3 = (val_2<val_3) ? res_0 : res_1;

    res_r = pow(res_3 * (0.95),1.0);
