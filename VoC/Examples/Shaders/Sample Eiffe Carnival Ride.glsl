...
//original https://www.shadertoy.com/view/Xd2GR1
...
float CE(in vec3 z0){
    ...
    if(abs(d2-d)<0.001)mcol+=vec4(1.0,0.1,0.0,0.2);
        ...
        d+=sin(-time2*100.0+100.0*pow(length(z0.xz),0.17))*0.01;
...
void main() {
    vec3 ro=vec3(-2.25,1.5+sin(time2*0.9),-4);
