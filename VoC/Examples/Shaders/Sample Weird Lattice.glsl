...
// original https://www.shadertoy.com/view/MtVGDt
...
void main(void)
{
    ...
    vec3 cu = vec3(0,1,0);                                            // Camera Up
      vec3 cv = vec3(path(time2 + .1),time2 + .1);                     // Camera View
    vec3 ro = vec3(path(time2),time2);                                // Camera Origin
    ...
    // gamma correction for add some contrast without saturation
       glFragColor = sqrt(f*f*f*2.); // gamma correction
}
