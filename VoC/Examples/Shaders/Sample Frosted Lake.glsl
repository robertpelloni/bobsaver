...
// original https://www.shadertoy.com/view/MsXyzN
...
float getSnowMask(in vec2 p) {
    ...
    p *= 0.02;
    p.x *= 0.5;
    float wind = getSnowWindMask(p,time * 0.25);
    wind = max(wind, getSnowWindMask(p,time * 0.25 + 1.5));
    wind *= wind;
    wind *= 0.5;    
    return max(f, wind);
#else
...
void main(void) {
    ...
    vec2 mouse = mouse*resolution.xy.xy / resolution.xy * 4.0 - 2.0;
    ...
    //if(mouse*resolution.xy.z > 0.0) ang = vec3(0.0,clamp(2.0-mouse*resolution.xy.y*0.01,-0.3,PI),mouse*resolution.xy.x*0.01);
