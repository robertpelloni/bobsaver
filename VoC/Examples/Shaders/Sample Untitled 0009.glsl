...
void main( void ) {
    ...
    c + wave1*wave2;
    glFragColor = vec4(c + wave1*wave2,c,c,1.);
    ...
    glFragColor.z = pow(clamp(1.-1.5*length(uv  - vec2(1.15*cos(time*.2)+.5,1.*sin(time*.1)+.5) + d),0.,1.),4.0);
    glFragColor.y = glFragColor.z*1.5 + glFragColor.x*0.3;
