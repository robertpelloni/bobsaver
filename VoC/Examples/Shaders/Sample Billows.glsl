...
// original https://www.shadertoy.com/view/4l3SRM
...
float circle(vec2 st) {
    return length(st) - abs(sin(time*0.1))*2.;
}
...
void main(void) {
    float phase = mod(time, 1.0);
    
    vec2 st = gl_FragCoord.xy/resolution.xy;
    st.x *= resolution.x/resolution.y;
    
    // Domain warping
    float angle = fbm(vec3(st*3.,time*0.1))*2.*3.1415;
    ...
    float f = fbm(vec3(st*3., time*0.05));
    float f2 = fbm(vec3(st*4., time*0.05));
