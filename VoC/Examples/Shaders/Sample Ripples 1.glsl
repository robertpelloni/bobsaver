...
void main( void ) {
    ...
    for(float i = 0.0; i < N; i++) {
        ...
        g+= cos( ((position.x-resolution.x*res/2.0) * cos(a/res) + (position.y-resolution.y*res/2.0) * sin(a/res) + time*1.1)/res/res);
        b+= cos( ((position.x-resolution.x*res/2.0) * cos(a/res) + (position.y-resolution.y*res/2.0) * sin(a/res) + time*1.2)/res/res);
    }
