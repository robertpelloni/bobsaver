...
// original https://www.shadertoy.com/view/wlyXDW
...
//-------------------------------------------------------------------------------------
vec4 p6mm_pattern_animated( vec2 p )
{
    ...
    float       a   = (time*2.0*pi)/t,
                ca  = cos(a),
                sa  = sin(a),
                s   = 5.2 + 5.0*sin( time/2.0 );
    mat2        rot = mat2( ca, -sa, sa, ca );
    vec2        d   = 0.003*vec2( time, 0.2*time );
...
vec2 dc_to_ndc( vec2 dc )
{
    return (dc-0.5*resolution.xy)/(0.5*resolution.yy) ;
}

//-------------------------------------------------------------------------------------

vec4 inversion_dc( vec2 p_dc )
{
    vec2  p_ndc = dc_to_ndc( p_dc ) ;
    float rd_sq  = dot( p_ndc, p_ndc );
    
    if ( rd_sq > 1.0 )
    {
       vec2  q = (p_dc-0.5*resolution.xy)/(0.5*resolution.xx) ;
       float b = q.x*q.x + q.y*q.y ; 
       return (1.0-b)*vec4(0.5,0.5,0.55,1) + b*vec4( 0.0, 0.0, 0.2, 1.0 );
    }
    float fac1 = 1.0/sqrt(1.0-rd_sq);
    vec2 p_plane = fac1*p_ndc ;
    return p6mm_pattern_animated( p_plane );    
}
