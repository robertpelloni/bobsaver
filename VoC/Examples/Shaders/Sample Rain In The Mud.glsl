...
// original https://www.shadertoy.com/view/Wd23Wc
...
vec2 getRainNormal(vec2 position)
{
    ...
    for(float i=0.0; i<15.0 * RAIN_INTENSITY; i++)
    {
        ...
        offset = mod(time + offset*3.0, 3.0)-1.0;
...
vec3 getWaterNormal(vec2 position)
{
    ...
    vec3 bpos = vec3(position.x, position.y, time*400.0);
...
void main(void)
{
    ...
    vec2 glFragCoord = Rotate(gl_FragCoord.xy - resolution.xy*0.5, time*0.06);
    glFragCoord.xy += vec2(sin(time*0.015), cos(time*0.03))*3000.0;

    // light dir
    vec3 l = normalize(vec3(1.0, -1.0, 2.0));    
    l.xy = Rotate(l.xy, time*0.06);

    vec3 groundBase =  GetFBM((glFragCoord.xy)*0.0005);
    vec3 water = getWaterNormal(glFragCoord.xy);
    ...
    vec2 normal2D = getRainNormal(gl_FragCoord.xy + groundBase.yz*50.0 *(1.0 - waterLimitSharp))*0.5 + water.yz*1.0;

    // flowmap
    float timing1 = fract(time*0.25);
    vec3 waterFlowing = Noise2DDer((gl_FragCoord.xy + groundBase.yz*timing1 * 150.0) * 0.07) * (1.0 - abs(timing1*2.0 - 1.0));
    float timing2 = fract(time*0.25 + 0.5);
    waterFlowing += Noise2DDer((gl_FragCoord.xy + vec2(500.0) + groundBase.yz*timing2 * 150.0) * 0.07) * (1.0 - abs(timing2*2.0 - 1.0));
    ...
    vec3 ground =  GetFBM((gl_FragCoord.xy + normal2D * 500.0 * waterDepth + waterFlowing.yz * RAIN_INTENSITY * 800.0 * (1.0 - flatGround))*0.0005);
