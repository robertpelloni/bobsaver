...
// original https://www.shadertoy.com/view/ld3yzS
...
void main(void) {
    ...
    vec2 mouse= (mouse*resolution.xy.xy - resolution.xy * 0.5) / resolution.x;
    //if (mouse*resolution.xy.z < 1.0) mouse = vec2(0.0);
    ...
    // dithering
    vec3 dither = hash33(vec3(gl_FragCoord.xy, frames));
    
    // get the sine of the angular extent of a pixel
    float sinPix = sin(FOV / resolution.x);
    ...
    glFragColor.rgb = acc.rgb + (dither - 0.5)*0.01;
    glFragColor.a = 1.0;
}
