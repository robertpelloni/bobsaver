...
// original https://www.shadertoy.com/view/lt2fD3
...
void main(void)
{
    ...
    vec2 ot=uv*2.-1.;
    ot.y*=resolution.y/resolution.x;
    ...
    glFragColor.rgb=mix(vec3(.15,0,0),vec3(1),ao)*shad*diff*1.1;
    glFragColor.rgb+=emit;

    glFragColor.rgb=pow(glFragColor.rgb,vec3(1./2.4));
}
