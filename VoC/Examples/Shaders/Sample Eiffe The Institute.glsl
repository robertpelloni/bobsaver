...
// original https://www.shadertoy.com/view/Xll3Wn
...
float DE(in vec3 p){
    float layout=(length(p.xy+vec2(8.0,-6.0))-45.0);
    layout=smin(layout,abs(p.y-28.0+sin(p.x*0.01)*40.0)-5.0,20.0)*0.02;
    float h=fbm(p.xy*0.1)*10.0*pow(clamp(layout,0.0,1.0),0.7),d=-p.z+7.0-h,dG=d;
    for (int n = 0; n < 4; n++) {
    ...
    }else if(prp.x<0.0)prp=vec4(10.0,h,layout,1.0);
    return d;
}
...
void main() {
    ...
    glFragColor = vec4(1.5*col*dimmer*dimmer,1.0);
}
