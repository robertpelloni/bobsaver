...
// original https://www.shadertoy.com/view/4tXcRl
...
/* repeated triangles, =x on 0,1 */ \
type tri( type x ) { return tri0( type(1.0) - x ); } \
...
vec2 get_screen_normalized_pos_from_view_pos( vec3 position, float aspect, float tan_half_fovy )
{
    vec3 a = -position / position.z;
    return a.xy / ( vec2( aspect, 1.0 ) * tan_half_fovy );
}
...
// trigger neighbouring tiles eval within a border threshold e, branchy and doesn't work so well...
// e needs to be so large it's not worth it (and still doesn't deal with diagonals)

    vec3 p2 = p;
    vec2 index = floor( p.xy );
    p2.xy = p.xy - index;

    float d = sd_TechTilesTestsSub0( p2, lod, t, ray, index, targs  );

#if ( TILE_MARCH_MODE == 1 )
    return d; // 1 eval, artifacts at edges
#endif

    if ( p2.x < e )
    {
        vec3 p3 = p2; p3.xy += vec2( 1, 0 );
        float ret2_d = sd_TechTilesTestsSub0( p3, lod, t, ray, index - vec2( 1, 0 ), targs );
        d = min( d, ret2_d );
    }
    ...
    if ( p2.y < e )
    {
        vec3 p3 = p2; p3.xy += vec2( 0, 1 );
        float ret2_d = sd_TechTilesTestsSub0( p3, lod, t, ray, index - vec2( 0, 1 ), targs  );
        d = min( d, ret2_d );
    }
...
float overshootstep2( float x, float df0, float a, vec3 args )
{
    ...
    float y = x > 0.0 ? overshoot( x, args, df0 ) : -( 1.0 - exp( x * a ) ) * y0; // look their is a smiley in that calculation
...
//        mm = ( mouse*resolution.xy.xy - resolution.xy * 0.5 ) / ( min( resolution.x, resolution.y ) * 0.5 );
//#endif
...
float intersect_plane( Ray ray, Plane plane )
{
    ...
    float vdotn = dot( ray.d, plane.normal );
    if ( abs( vdotn ) <= epsilon ) return FLT_MAX;
    float t = -dot( ray.o - plane.base, plane.normal ) / vdotn;
    return t;
}

vec2 intersect_plane2( Ray ray, Plane plane )
{
    float epsilon = 1e-4;
    float vdotn = dot( ray.d, plane.normal );
    float d = dot( ray.o - plane.base, plane.normal );
...
vec2 mm3( vec2 t )
{
    ...
    if ( t.y < 0.0 )
    {
        if ( t.x < 0.0 ) return vec2( t.x, FLT_MAX );
        return vec2( -FLT_MAX, t.x );
    }

    if ( t.x < 0.0 ) return vec2( -FLT_MAX, t.x );
    return vec2( t.x, FLT_MAX );
#else
// simplify if tree above
    if ( t.y * t.x > 0.0 ) return vec2( t.x, FLT_MAX );
    return vec2( -FLT_MAX, t.x );
#endif
}
...
//    return 0.0;
#if 0
    vec2 t0 = intersect_plane2( ray, mkplane( vec3( 0, 0, ROUGH_SHELL_D ), vec3( 0, 0, 1 ) ) );
    vec2 t1 = intersect_plane2( ray, mkplane( vec3( -TRENCH_HALF_WIDTH + ROUGH_SHELL_D, 0, 0 ), vec3( 1, 0, 0 ) ) );
    vec2 t2 = intersect_plane2( ray, mkplane( vec3( TRENCH_HALF_WIDTH - ROUGH_SHELL_D, 0, 0 ), vec3( -1, 0, 0 ) ) );
    vec2 t3 = intersect_plane2( ray, mkplane( vec3( 0, 0, -TRENCH_DEPTH + ROUGH_SHELL_D ), vec3( 0, 0, 1 ) ) );
#else
// we can hardcode a bit more
...
// m1 here is a 0,1 param that doesn't seem to affect "chrominance"
...
vec3 starfield( vec3 viewvec )
{
    ...
    vec2 sci = floor( sc / scd );
    vec2 scf = ( sc - sci * scd ) / scd;
//    return vec3( scf, 0.0 ); // view cells
//    vec4 r = vec4(0.5); // aligned stars
    vec4 r = hash42( sci );
//    return vec3( r ); // color cell by hash
    vec2 scc = ( ( sci + 0.5 ) * scd ); // cell center
//    if ( mod( sci.x + sci.y, 2.0 ) == 1.0 ) return RED;
    vec2 r2 = hash22( sci ); // x: discard probability, y: color distribution uniform input
//    return vec3( r2.y ); // debug
...
//    if ( r2.x > sin( scc.x ) ) return RED; // visualize the decimation
    ...
    vec3 v = zup_spherical_coords_to_vector( sc2.x, sc2.y );
//    return v;
    v = normalize( v ); // we also need to renormalize already normal vector here else broken on GTX1060 laptop
    float c = 1.0 - dot( v, viewvec ); // 0->2
//    return vec3( 1.0 - smoothstep( 0.0, 1.0, c ) ); // debug: show the full disks (max glow radius)
    vec3 col = star_color( cdf_inv( r2.y * cdf( 1. ) ) );
//    return vec3( chrominance( col ) );
#if 1
    c *= 1e+6;
    col = mix( col, vec3( maxcomp( col ) ), 0.5 ); // after all those efforts we lerp towards monotone... :-]
    float s = smoothstep( 512.0, 1280.0, resolution.x );
    return star_glare( c * 0.01, 0.7, 40.0 + s * 40.0 ) * col * 20.;
...
//    glFragColor.xyz = star_color( uv.x ).xyz; return;
//    glFragColor.xyz = laser_heatmap( uv.x ); return;
...
//            if ( to.t > 30 ) break;
    ...
    glFragColor.rgb = shade( view_ray.o, view_ray.d, camera, p, n, l
                           , traced_shadow, sun_shadow, first_bounce, to, ao, exposure );
#ifdef LASERS
    glFragColor.rgb += lasers( view_ray, 0.0, time, to.t );
    glFragColor.rgb += lasers( view_ray, 1.0, time, to.t );
    glFragColor.rgb += lasers( view_ray, -1.0, time, to.t );
    glFragColor.rgb += lasers( view_ray, -0.5, time, to.t );
    glFragColor.rgb += lasers( view_ray, +0.5, time, to.t );
//    glFragColor.rgb += lasers( view_ray, +0.8, time, to.t );
#endif
    glFragColor.rgb *= fade;
    glFragColor.rgb = mix( glFragColor.rgb, debug_color.rgb, debug_color.a );
}
