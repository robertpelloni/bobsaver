...
vec4 cylinder_radialrepeat( vec3 p, float r )
{
    ...
    p *= parms2.w;
    {
        ...
        dp.zx = rot2d( dp.xz, 0.3*time );
        vec2 ds = sdHalfCircle( dp );

        dp.xy = rot2d( dp.xy, -0.7 * time );
        ds = sel_min ( ds, sdHalfCircle( dp.xyz*1.3 ) );

        dp.xz = rot2d( dp.xz, 1.1*time );
        ds = sel_min ( ds, sdHalfCircle( dp.xyz*1.8 ) );

        dp.xz = rot2d( dp.xz, -1.3*time );
...
vec4 scene1( vec3 p )
{
    vec4 d = vec4( FAR_CLIP, 0.0, 0.0, -1.0 );
...
void main(void)
{
    ...
    vec2 texcoord0 = gl_FragCoord.xy / resolution.xy - vec2(0.5);
    vec2 uv = texcoord0;
    ...
    float rnd = nrand( texcoord0.xy + 0.01 * fract( time ) );
    ...
    hsv.x -= 0.1 * pow(0.9-gl_FragCoord.xy.y / resolution.y, 2.0);
