...
void main( void )
{
    ...
    camPos = pi*(camPos * cos(ang) + cross(qAxis, camPos) * sin(ang) + qAxis * dot(qAxis, camPos) * (1.0 - cos(ang)));
    ...
    temp = map((camPos + rayDir * (r)));
        ...
        tv = vt+sfin(camDir+sAxis*rcpi*(camPos + rayDir * (r)));
             vt = normalize(vt+(tv/(float(i)*r))*(e/(e+exp((tv.x+tv.y+tv.z)))))*0.5;
        show = abs(length(sin(tv+tv)+vt));
        break;
