...
// original https://www.shadertoy.com/view/XdKyzD
...
/*RayResult main_object(vec3 p, float scale)
{
    ...
    RayResult box = opRepBox(p2, vec3(0.05), vec3(0.15), Object(1));
...
RayResult stereographicProj ( in World _world, in vec3 _p )
{
    ...
    Ray raySTC; 
    raySTC.p = onSphere;
    raySTC.d =  normalize( _world.object_trf[3].xyz - onSphere );
    
    return intersect(raySTC, Object(2), 0.0000001, 2.0);
}
...
{
    vec2 ratioUV = _uv.xy / resolution.xy;
    vec2 ratio = 2.0*resolution.xy/vec2(min(resolution.x,resolution.y));
...
vec3 computeMaterial ( in World _world, inout RayResult _rayResult )
{
    vec3 _material;
...
void main(void)
{
    ...
    mat4 rot = rotationZ ( time*1.278914) * rotationY(time) * rotationX(time*0.332567);
    mat4 inverseRot = rotationX(-time*0.332567) * rotationY(-time) * rotationZ ( -time*1.278914) ;
    
    world.object_scale = 0.33;
...
#ifndef DEBUG
    eye = mouse*resolution.xy.xy/resolution.xy;
#endif
    
    Camera cam = cameraLookAt ( vec3(2.5-5.0*(eye.x),2.0+5.0*(eye.y),2.5), vec3(0.0,0.5,0.0), vec3(0.0,1.0,0.0) );
    
    // compute raytrace
    vec2 xy = imageToScreen ( gl_FragCoord.xy );    
        
    Ray ray;
    
    screenToRay ( cam, xy, ray );
    
    RayResult res = intersect ( ray, Object(-1), 0.00001, 100.0 );
    
    vec3 pos = ray.p + ray.d * res.t;
    vec3 nor = normal( pos );
    vec3 ref = reflect( ray.d, nor );
     
    vec3 col = computeMaterial ( world, res );   
    
    // if plane => stereographicProj to obtain color
    if ( res.o.id == 2 )
    {
        RayResult stInter = stereographicProj ( world, pos );
        
        vec3 stereoCol = computeMaterial ( world, stInter );
