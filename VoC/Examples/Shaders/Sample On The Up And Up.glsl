...
// origdfgjinal https://www.shadertoy.com/view/4sGXzz
...
void main(void) {
    ...
    for(int i = 0; i < ITERATIONS; i++) {
        ...
        temp = opRep( ip, vec3(5.0 + 0.8 * sin(time), 5.0 + 0.1 * sin(time), 9.0 + 2.0 * sin(time)) );
        if( temp < findThresh ) {
    ...
    // glFragColor = vec4(ip, 1.0 - float(last_i) / float(ITERATIONS)/2. );
       glFragColor = vec4(ip - float(last_i) / float(ITERATIONS), 1.0);
