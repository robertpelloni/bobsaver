...
// original https://www.shadertoy.com/view/XsfGRH
...
float displace(vec3 p) {
    return ((sin(6.*p.x + time/4.1)*sin(6.*p.y)*sin(6.*p.z))*cos(time/4.1))*.5;
}
...
void main(void)
{
    ...
    vec3 near    = vec3((gl_FragCoord.x - 0.5 * resolution.x) / resolution.x * 2.0  * aspect,(gl_FragCoord.y - 0.5 * resolution.y) / resolution.y * 2.0,0.0);
