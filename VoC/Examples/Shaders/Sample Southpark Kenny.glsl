...
// original https://www.shadertoy.com/view/ltGGD3
...
float dfHat(vec3 rp) {
    float osphere = sdSphere(rp, vec3(0.0, 0.0, 0.0), 1.6);
    float oellipsoid = sdEllipsoid(rp, vec3(1.6, 1.6, 1.8));
    float isphere = sdSphere(rp, vec3(0.0, 0.0, 0.0), 1.55);
    float iellipsoid = sdEllipsoid(rp, vec3(1.55, 1.55, 1.75));
    rp += vec3(0.0, 0.0, -1.8);
    float box1 = sdBox(rp, vec3(1.8));
    rp += vec3(0.0, 0.0, 3.6);
    float box2 = sdBox(rp, vec3(1.8));
    rp += vec3(0.0, 0.0, -5.2);
    float box3 = sdBox(rp, vec3(1.8));
    osphere = max(osphere, -box1);
    oellipsoid = max(oellipsoid, -box2);
    isphere = max(isphere, -box1);
    iellipsoid = max(iellipsoid, -box2);
    float hood = max(min(osphere, oellipsoid), -min(isphere, iellipsoid));
    return max(hood, -box3);
}

float dfTorso(vec3 rp) {
    rp += vec3(0.0, 2.7, 0.0);
    float body = sdEllipsoid(rp, vec3(1.5, 2.0, 1.4));
    rp += vec3(0.0, 3.2, 0.0);
...
vec3 hoodColour(vec3 rp) {
    vec3 hoodCol = vec3(0.8, 0.4, 0.0);
    if (rp.z > 1.5) {
        hoodCol = vec3(0.54, 0.27, 0.07);
    }
    return hoodCol;
}
