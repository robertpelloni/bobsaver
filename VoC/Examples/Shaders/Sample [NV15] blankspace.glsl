...
// original https://www.shadertoy.com/view/Mlj3zR

// This shader prints its own code, including the code for printing its own code.
...
void main(void)
{
    //perspective lines converge at (resolution.x/2,h)
    //
    vec2 uv=2.0*(gl_FragCoord.xy-resolution.xy*0.5)/resolution.x;
    ...
    float anglee=clamp(1.1-time*0.004,0.0,1.1);
    ...
    uv=0.5*p.xz-vec2(-0.5,-time*0.07);
    
    if(uv.x<0.0 || uv.x>1.0 || uv.y<0.0){
        glFragColor=vec4(fog,fog,fog,1.0);
        return;
    }
    uv*=256.0;
