...
// original https://www.shadertoy.com/view/ldfGWn
uniform float time;
...
//    vec3 col=(sin(vec3(z,z+pi/3.0,z+pi*2.0/3.0))+2.0)/3.0;
    ...
    vec3 env=vec3(1.0);//textureCube(iChannel0,reflected*reflected*reflected).xyz;

    glFragColor=vec4(col*light+0.1*env,1.0);
}
