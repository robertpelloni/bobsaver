...
// original https://www.shadertoy.com/view/NdKfRD
...
/*
    ...
    of concentric-circle Truchet rings on a randomly subdivided grid. 
...
void main(void) {
    ...
    // Smoothing factor.
    float sf = 1.5/resolution.y;
    
    // Canvas coordinates with translation.
    vec2 p = uv + vec2(1, .5)*time/16.;
    
    // Scene field calculations.
 
     
    // Randomly subdivided grid object.
    float sc = resolution.y>360.01? 1./4. : 1./4.*450./360.; // Canvas size scaling.
    ...
    for(int i = 0; i<2; i++){
        ...
        col = mix(col, vec3(0), (1. - smoothstep(0., sf*resolution.y/450., tr2[i]))*.9);
