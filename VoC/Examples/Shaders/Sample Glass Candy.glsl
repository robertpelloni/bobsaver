...
// original https://www.shadertoy.com/view/sljBRG
...
void main(void)
{
    ...
    vec3 noise = vec3(0.0); //texture(iChannel0, gl_FragCoord.xy/1024.).rgb;
    ...
    pos += ray * (.2+.19*sin(time*2.+length(uv)*6.)); // jumpy bounce
    for (index = count; index > 0.; --index)
    {
        float dist = map(pos);
        if (dist < .001) break;
