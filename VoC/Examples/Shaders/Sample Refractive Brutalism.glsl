...
// original https://www.shadertoy.com/view/wt33RN
...
vec4 texNoise(vec2 uv){ float f = 0.; //f+=texture(iChannel0, uv*.125).r*.5;
    ...
    //f+=texture(iChannel0,uv*1.).r*.125;f=pow(f,1.2);
    return vec4(f*.45+.05);
...
void main(void)
{
  ...
  tt=mod(time,59.66)+4.72; //modulo time to avoid glitchy artifact and also nicely reset camera / scene
  ...
  glFragColor = vec4( pow(co,vec3(.45)),1); // naive gamma correction
}
