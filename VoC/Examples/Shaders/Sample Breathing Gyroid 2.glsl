...
// original https://www.shadertoy.com/view/ssj3Dy
...
float scene (vec3 p, out int id)
{
    ...
    float thickness1 = .025 + .025*(cos (10.*time)*.5 + .5);
    float thickness2 = .05 + .05*(cos (13.*time)*.5 + .5);
    float thickness3 = .1 + .1*(cos (16.*time)*.5 + .5);
    float bias1 = 1.3;
    float bias2 = .3;
    float bias3 = .025;
    float scale = 7.;
    ...
    float r1 = 1.35 - .6*(cos (.5*time)*.5+.5);    // 1.35
    float r2 = 1.1 - .4*(cos (time + 1.)*.5+.5);   // 1.1
    float r3 = .7 + .2*(cos (2.*time + 2.)*.5+.5); // .7 + .2
...
void main(void)
{
    ...
    vec2 yaw = .75*vec2 (PI*cos (4.*mouse.x*resolution.xy.x/resolution.x),
                         PI*sin (4.*mouse.y*resolution.xy.x/resolution.x));
    float pitch = 1.;
    ...
    for (int x = 0; x < AA_SIZE; ++x) {
        for (int y = 0; y < AA_SIZE; ++y) {
            ...
            // normalize and aspect-correct UVs
            uv = 2.*((gl_FragCoord.xy + pixelOffset)/resolution.xy) - 1.;
            uv.x *= resolution.x/resolution.y;
