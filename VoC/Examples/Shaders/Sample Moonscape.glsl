...
// original https://www.shadertoy.com/view/3slfW2
...
void main(void)
{
    ...
    if (id < 0.5) {
        ...
        //vec3 sunPos = vec3(8.0 - 16.0 * mouse.x*resolution.xy.x / resolution.x, 6.0 - cos(time * 0.2), -1.0 - mouse.y*resolution.xy.y / 360.0 * 20.0);
        vec3 sunPos = vec3(8.0 - 16.0, 6.0 - cos(time * 0.2), -1.0);
        vec3 n = calcNormal(p);
        ...
        if (id > 1.5) {
            ...
            vec3 cloud = vec3(1.0);
            float landish = smoothstep(0.4, 0.52, fbm(n.xy * 3.1 + vec2(time * 0.05, 0.0)));
            col = mix(sea, land, landish);
            
            float cloudish = smoothstep(0.8, 0.95, n.y) * smoothstep(0.1, 0.8, fbm(n.xz * 10.0 + vec2(time * .1, 0.0)));
