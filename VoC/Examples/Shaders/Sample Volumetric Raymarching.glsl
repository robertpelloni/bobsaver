...
// original https://www.shadertoy.com/view/tsScDG
...
OrbLightDescription GetLight(int lightIndex)
{
    ...
    float theta = time * 0.7 + float(lightIndex) * PI * 2.0 / float(NUM_LIGHT_COLORS);
    float radius = 18.5f;
#else
    float theta = time * 0.4 * (float(lightIndex) + 1.0f);
...
{    
    ...
    minDist = min(
        sdSmoothUnion(
            sdSmoothUnion(
                sdSmoothUnion(
                    sdSphere(pos, vec3(-8.0, 2.0 + 20.0 * sin(time), -1), 5.6),
                    sdSphere(pos, vec3(8.0, 8.0 + 12.0 * cos(time), 3), 5.6),
                    3.0),
                sdSphere(pos, vec3(5.0 * sin(time), 3.0, 0), 8.0),
                3.0) + 7.0 * fbm_4(fbmCoord / 3.2),
            sdPlane(pos + vec3(0, 0.4, 0)),
            22.0),
        minDist);
...
float GetShadowFactor(in vec3 rayOrigin, in vec3 rayDirection, in float maxT, in int maxSteps, in float marchSize)
{
    ...
    {                       
        ...
        if(t > maxT || shadowFactor < ABSORPTION_CUTOFF) break;
        ...
        {
            shadowFactor *= BeerLambert(ABSORPTION_COEFFICIENT * GetFogDensity(position, signedDistance), marchSize);
        }
    }
    return shadowFactor;
}
...
void CalculateLighting(vec3 position, vec3 normal, vec3 reflectionDirection, Material material, inout vec3 color)
{
    for(int lightIndex = 0; lightIndex < NUM_LIGHTS; lightIndex++)
    {
        ...
        if(!IsColorInsignificant(lightColor))
        {
            ...
            shadowFactor = GetShadowFactor(position, lightDirection, lightDistance, MAX_OPAQUE_SHADOW_MARCH_STEPS, shadowMarchSize); 
        }
        #endif
        
        color += shadowFactor * lightColor * pow(max(dot(reflectionDirection, lightDirection), 0.0), 4.0);
        color += shadowFactor * lightColor * Diffuse(normal, lightDirection, material.albedo);
...
vec3 Render( in vec3 rayOrigin, in vec3 rayDirection)
{
    ...
    if(volumeDepth > 0.0)
    {
        ...
        for(int i = 0; i < MAX_VOLUME_MARCH_STEPS; i++)
        {
            ...
            if(volumeDepth > depth || opaqueLightFactor < ABSORPTION_CUTOFF) break;
            ...
            if(signedDistance < 0.0f)
            {
                ...
                opaqueLightFactor *= BeerLambert(ABSORPTION_COEFFICIENT * GetFogDensity(position, signedDistance), marchSize);
                ...
                for(int lightIndex = 0; lightIndex < NUM_LIGHTS; lightIndex++)
                {
                    ...
                    float lightFactor = GetShadowFactor(position, lightDirection, lightDistance, MAX_VOLUME_LIGHT_MARCH_STEPS, lightMarchSize); 
                    volumetricColor += absorptionFromMarch * lightFactor * volumeAlbedo * lightColor;
                }
    ...
    if( materialID != INVALID_MATERIAL_ID && opaqueLightFactor > ABSORPTION_CUTOFF)
    {
        vec3 position = rayOrigin + t*rayDirection;
    ...
    return min(volumetricColor, 1.0f) + opaqueLightFactor * opaqueColor;
}
...
{
    return 0.0;//100.0;// * (mouse.y*resolution.xy.y / resolution.y);
}

float GetRotationFactor()
{
    return 0.0;//mouse.x*resolution.xy.x / resolution.x;
}
...
void main(void)
{
    ...
    glFragColor=vec4( GammaCorrect(clamp(color, 0.0, 1.0)), 1.0 );
}
