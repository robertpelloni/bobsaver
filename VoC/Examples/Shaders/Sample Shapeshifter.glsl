...
// original https://www.shadertoy.com/view/lds3DH
...
float dist(vec3 p) //the distance function for raymarching
{
    ...
    float time2 = p.x*sin(time*2.312)*sin(time)*.9+
        p.z*cos(time*3.12)*sin(time)*.9+
        time+max(mod(time*.1,1.0),mod(-time*.1,1.0));
    
    //rotate the space, bp <- transform(p)
    vec3 bp = rotate_z(p,time2*.6);
    bp = rotate_y(bp,time2*.7);
    bp = rotate_x(bp,time2*.5);
    ...
    float change = sin(time2)*0.99;
    float change2 = sin(time2*.4)*0.99;
...
void main(void)
{
    ...
    vec3 p = vec3(sin(7.0)*2.1,0.0,cos(7.0)*2.1); //ray position
    vec3 d = rotate_y(vec3(uv,1.0),3.14159-7.0); 
    d = normalize(d); //ray direction
    
    light = normalize(vec3(sin(time),sin(time*.44)+1.2,sin(time*.24)));
    ...
    glFragColor = vec4(cc(c*.4+hash(uv.xy+c.xy)*.007,.8,0.9),1.0); //post process
    //glFragColor = vec4(c,1.0); //uncomment this to remove post processing
}
