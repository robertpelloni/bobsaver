...
// original https://www.shadertoy.com/view/lsGcRV
...
vec3 path(float time) {
    float x = time * 8.;
    return vec3(x, -2. - 1.5 * cos(x / 7. * PI), sin(time / 2.) * 2.);
}

void update_world() {
    light_pos = vec3(3.5 * cos(time / 3.), 3.5 * sin(time / 3.), 4.);
    origin = path(time);
    up = normalize(vec3(0., cos(origin.x / 7. * PI), 6.));
    light_pos = path(time + 1. + 0.8 * cos(time / 3.));
...
vec3 render(vec3 origin, vec3 direction, float pixel_radius) {
    ...
    float expected_angle = hash(vec2(z_plane, 5.)) * 2. * PI + time * 2. * pow(hash(vec2(z_plane, 4.)), 2.);
    ...
    float fade = max(0.0, min(1.0, 5. * cos(time * 0.1 * hash(vec2(z_plane, 2.))) - 2. + 5. * hash(vec2(z_plane, 3.)) ));
...
void main(void) {
    ...
    vec2 cursor = (2. * mouse*resolution.xy.xy - resolution.xy) / resolution.y;
    ...
    float pixel_radius = FOV_scale / resolution.y / 2.;
    ...
    for (int i = 0; i < AA; i++) {
        for (int j = 0; j < AA; j++) {
            ...
            vec2 pixel_coord = FOV_scale * (gl_FragCoord.xy + subpixel_coord - 0.5 * resolution.xy) / resolution.y;
