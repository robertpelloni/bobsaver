...
void main() {
    ...
    float r0 = res_r;
    vec3 n00r = nhd(vec2(4.0,1.0),psn,0.0,0);
    vec3 n01r = nhd(vec2(8.0,3.0),psn,0.0,0);
    ...
    vec3 n11r = nhd(vec2(12.0,0.0),psn,0.0,0);
    ...
    vec3 n30r = nhd(vec2(24.0,20.0),psn,0.0,0);
    float n30rw = n30r[0] / n30r[2];
    float z = 0.000+cs;
    float r4 = res_r;
    if( n30rw >= 0.080+z && n30rw <= 0.120+z ) { r4 += s*(5.0); }
    if( n30rw >= 0.000 && n30rw <= 0.070+z ) { r4 -= s; }
    if( n30rw >= 0.130+z && n30rw <= 1.000 ) { r4 -= s; }
    r4 = (res_r*0.1+r4+nmrw*0.05)/1.15;
    r4 -= 0.005;

    res_r = (r3+r4*(0.25))/(1.25);
