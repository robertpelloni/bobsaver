...
// original https://www.shadertoy.com/view/4tlXWs
...
{
    float d = length(pos.xz - vec2(0.0, 2.0) + 0.5*cos(2.0*pos.xz + vec2(3.0, 1.0) * time)) +  pos.y + 0.2 * cos(pos.y - time);
    float border = 4.0 * (sin(time) + 1.0);
 
    //vec3 c = gridColor(pos);
    vec3 c = hexagonColor(pos);
    c = mix(c, vec3(0.01), smoothstep(border - 0.5, border, d));
