...
// original https://www.shadertoy.com/view/wlGGzV
...
void arm(vec3 pos, vec3 r, vec3 n)
{
    ...
    float tt = fract((time*2.0)/PI/2.0);
    float aa = -cos(tt*tt*PI*2.0);
    
    float lf = 1.0;
    float sf = 1.0;
    float a  = -(1.0-aa)*17.0+4.0;
...
float map(vec3 p)
{
    ...
    float aa = cos(time*2.0);
    
    head (vec3(0,1.0+0.3*aa,0));
...
void main(void)
{
    ...
    if (mat == NONE)  
    {
        vec2 guv = gl_FragCoord.xy - resolution.xy / 2.;
    ...
    float isDigit = digit(gl_FragCoord.xy / fontSize, iMs, 2.0, 0.0);
