...
// original https://www.shadertoy.com/view/tsfyzn
...
getDistResult getDist(vec3 p) {
    ...
    vec3 nuv = p * size + vec3(0., 0., time * speed);
...
vec3 getOthersideBackground(vec2 uv) {
    return backgroundStars(uv/8. + sin(time/(speed)));
}

vec3 getBackground(vec2 uv) {
    float set = 0. - clamp(sin(time/SUNSET_SPEED)*3., -1., 2.);

    float sunDist = length(uv + vec2(0., -2.5 - set));
    float sun = 1. - smoothstep(2.35, 2.5, sunDist);

    float gradient = sin(uv.y/4. - 3.14/32. + set/3.)*2.;
    float bands = abs(sin(uv.y * 8. + time*2.)) * (1. - step(2.5 + set, uv.y));
    ...
    if (sun < .5) {
        // stars
        vec2 nuv = uv*2.;// + vec2(time, 0.);
        vec2 rize = vec2(-10., 12.);
        nuv -= rize;
        nuv *= rot2d(mod(-time/15., 6.28));
        nuv += rize;
        uv = fract(nuv);
        vec2 id = floor(nuv);
        uv -= .5;
...
{
    vec2 p = uv * GRID_SIZE + vec2(0., time * speed);
...
vec3 getAlbedo(vec3 p, float material, float height, vec3 normal) {
    ...
    float sunSet = sin(time/SUNSET_SPEED)*.5 + .5;
...
float triangleMask(vec2 uv) {
    return polarTriangle(uv + vec2(0., -.1),3.14 + .5*sin(time));
}
...
void main(void) {
    ...
    if (tr.isHit) {
        ...
        float fd = fract(triangle - clamp(sin(time/3.), 0., 2.));
