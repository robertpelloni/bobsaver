...
vec3 flx(vec2 p,float ct){
     float t; vec3 q=vec3(p,p.x-p.y+time*0.3);
...
void main(void){
     ...
     p*=1.-length(p)*2.*(vr10-1.); p*=vr8; vec2 pp=p*2.; p.y*=3./asp; p*=1.2; float t=time*0.4;
     ...
     p.x*=3.; p=(0.5*p)+nd.xy*0.6*e; t=time*0.04;
     vec3 fx=flx(p,t)*flx(2.1*p+10.,0.333+t*1.1); fx*=vr1*2.;
     fx=pow(fx,vec3(vr3)); glFragColor.rgb=pow(s*fx*(0.5+0.5*e),co);
     float lf=ll*vr4-(vr6-1.2);
     vec3 dir=normalize(vec3(pp*vec2(asp*1.6,-1),-1.5)),q,r=vec3(0.,0.,4.);
     float d,ii=1.;e=0.02;
     for(float i=0.;i<64.;i++){d=length(r*vec3(0.1,.5,1.))-1.;q=r; q.y-=2.; d+=(pn(q+vec3(.0,time*2.,.0))+pn(q*3.)*.5)*.25*(q.y); d=min(100.-length(q),abs(lf+d))+e; r+=d*dir; if(d<e){ii=i/64.;break;}}
     glFragColor.rgb+=vr7*mix(vec3(0.),mix(vec3(1.,.5,.1),vec3(0.1,.5,1),r.y*.02+.4),pow(ii*2.,4.*vr5));
     }
