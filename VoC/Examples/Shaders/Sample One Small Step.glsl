...
// original https://www.shadertoy.com/view/tt3yRH
...
float sdBoot(vec3 p) {
    ...
    tp.z = mod(tp.z - .5, .4) - .2;
    float t = max(
        sdBox(tp, vec3(2., .16, .12 + tp.y * .25)), // Treadz.
        sdBox(p - vec3(0, 0, 1.1), vec3(2., .16, 1.7)) // Toe/heel.
        );
...
float map(vec3 p) {
    float bmp = fbm(p) // Surface texture.
        * (.5 + 2. * exp(-pow(length(p.xz - vec2(.5,2.2)), 2.) * .26)); // Raise surface around the footprint.
...
vec3 lights(vec3 p, vec3 rd, float d) {
    vec3 ld = normalize(vec3(6, 3, -10) - p), n = calcN(p, d) + n31(p * 79.0625) * .25 - .25;
    ...
    vec3 lig = ((l1 + l2) * ao + spe) * vec3(2, 1.8, 1.7);
...
void main(void)
{
    float t = mod(time * .2, 30.);
    vec3 ro = vec3(0, .2, -4);
    ro.yz *= rot(-sin(t * .3) * .1 - .6);
    ro.xz *= rot(1.1 + cos(t) * .2);

    vec2 uv = (gl_FragCoord.xy - .5 * resolution.xy) / resolution.y;
    glFragColor = vec4(vignette(pow(march(ro, getRayDir(ro, vec3(0,0,.8), uv)), vec3(.45)), gl_FragCoord.xy), mix(1.2, 0., (d + 1.)/ 8.));
}
