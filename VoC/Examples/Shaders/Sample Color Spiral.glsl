...
// original https://neort.io/art/bnicldk3p9f5erb53130
...
void main(void)
{
    ...
    vec2 f =  z;
    //Change f and play around!
    float r = length(f - round(f));
    float th = arg(cmul(f, cexp(vec2(0., t /length(z) * 2.))));

    glFragColor = vec4((hsvToRgb(th, 0.3/r, 0.3/r) + vec3(0., 0., 1.)) / 2., 1.);
}
