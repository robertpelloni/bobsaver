...
// original https://www.shadertoy.com/view/4tKXzm
...
float map(vec3 p) {
    ...
    float slice = p.z;
    float a = fract((floor(time * SPEED * SLICES)) / SLICES - .5) * PI * 2.;
    tRotate(p.xz, a);
...
void main(void) {
    ...
    // add this all up
    glFragColor.rgb = (ao * texture(p)) * (ambient * (2. - LIGHT_COLOR) * .5 + (specular + diffuse) * shadow * LIGHT_COLOR);
    glFragColor.w=0.0;
    
    // fog
    vec4 fogColor = vec4(vec3(0,.01,.014) * (2. - length(uv)), 1.);
    glFragColor = mix(glFragColor, fogColor, saturate(dist * dist * .05));
    
    // if we passed the cake, then apply a dark glow, this makes the cake pop out
    if (length(p) > .6) 
        glFragColor *= saturate(1. - sqrt(steps / float(MAX_STEPS)) * 1.5);
