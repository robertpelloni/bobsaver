...
// original https://www.shadertoy.com/view/4ttXzj
...
void main(void)
{
    ...
    // base color
    glFragColor = vec4(f);
    glFragColor.rgb *= vec3(1., .3 + B * .05, 0.1 + B * .05);
    ...
    S = pow(S, 4.0) * .2;
    glFragColor.rgb += S * vec3(.4, .7, .7);
    // rim
    float R = 1.0 - clamp(dot(grad, v), .0, 1.);
    glFragColor.rgb = mix(glFragColor.rgb, vec3(.8, .8, 1.), smoothstep(-.2, 2.9, R));
    // edges
    glFragColor.rgb = mix(glFragColor.rgb, vec3(0.), smoothstep(.45, .55, (max(abs(ouv.y * a), abs(ouv.x)))));
    
    // contrast
    glFragColor = smoothstep(.0, 1., glFragColor);
}
