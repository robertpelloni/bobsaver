...
// original https://www.shadertoy.com/view/NlBfWz\
...
float GetSceneDistance(vec3 point, out int obj)
{
    ...
    float bounce_freq = 9.;
    sphere.w += 0.1*sin(bounce_freq * time);
    ...
    if(obj == 0)
    {
        ...
        //d+= 0.1*pow(0.99, c) *sin(0.3*c- bounce_freq * time);
        
        d+= 0.3*pow(0.95, c) *
                sin(0.3*c- bounce_freq * time);
...
void main(void)
{
    ...
    float spin = 1.;
    float range = 10.;
    vec3 light_pos2 = light_pos + range* vec3(sin(spin *time), cos(spin *time), 0.);
