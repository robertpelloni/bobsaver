...
// original https://www.shadertoy.com/view/Xtf3zn
...
float map( const in vec3 p ) {
    ...
    float ch = hash( floor( (pd.z+18.*time2)/40. ) );
    ...
    vec3 pdc = vec3( pd.x, pd.y, mod( pd.z+18.*time2, 40.) - 20. );
...
void main( void ) {    
    ...
    if (q.y < .12 || q.y >= .88) {
        glFragColor=vec4(vec4(0.0));
        return;
    }
    
    // camera
    float z = time2;
    float x = -10.9+1.*sin(time2*0.2);
    vec3 ro = vec3(x,  1.3+.3*cos(time2*0.26), z-1.);
    vec3 ta = vec3(-8.,1.3+.4*cos(time2*0.26), z+4.+cos(time2*0.04));
    ...
    vec2 st = 256. * ( p* vec2(.5, .01)+vec2(time2*.13-q.y*.6, time2*.13) );
