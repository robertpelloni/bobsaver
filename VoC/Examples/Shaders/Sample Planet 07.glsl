...
const mat2 rotate2D = mat2(1.4323, 1.78231, -1.78231, 1.42);
...
{
    float fogAmount = clamp(dis* 0.00000, 0.0, 1.0);
...
vec3 TerrainColour(vec3 pos, vec3 normal, float dis)
{
    ...
    if (matPos.y < 0.0)
    {
        ...
        //d -= .00005*dot(watPos.xz,watPos.xz);
        watPos += -dir * d;
...
bool Scene(in vec3 rO, in vec3 rD, out float resT )
{
    ...
    for( int j=0; j<190; j++ )
    {
        if (t > 111540.0) return false; // ...Too far
        vec3 p = rO + t*rD;
        if (p.y > 1111155.0) return false; // ...Over highest mountain
...
void main(void)
{
    ...
    camTar.xz     = CameraPath(40.0).xz;
    camTar.y = 0.;
    cameraPos.y = (1.+cos(iGlobalTime*.3))*1000.+h;
