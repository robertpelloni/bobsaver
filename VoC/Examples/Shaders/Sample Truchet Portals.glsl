...
// original https://www.shadertoy.com/view/lldfDr
...
#define portals 0.2   //portal amount, from 0.0 to 1.0
...
void main(void)
{
    ...
    int num = 2;
    float time = abs(mod(time*0.4+50.0,100.0)-50.0);
    ...
    */
    ...
    //    rd.xz *= rot(mouse*resolution.xy.x/resolution.x*3.14*2.0-3.14);
    //}
    ...
    for (int i = 0; i < 100; i++) {
        ...
        len = map(ro);
        if (len < 1.0/resolution.y*(dist+1.0)||dist>FAR) {
            hit = dist < FAR;
            break;
        }
        ...
        if (smallest <= 0.0) {
            ...
            float t = time*random.x+random.y;
            ...
            if (portalsize < 0.4) {
                ...
                glFragColor = vec4(-mask*signdir*0.5+0.5,1);
                glFragColor = vec4(random.xzwy);
                return;
            } else {
                floorpos += mask*signdir;
            }
    ...
    if (hit) {
        ...
        glFragColor /= (dist*dist*0.0005*FAR+1.0);
