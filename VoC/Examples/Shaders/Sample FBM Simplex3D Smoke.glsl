...
// original https://www.shadertoy.com/view/4tSGDy
...
void main(void)
{
    ...
    ro*=rot(vec3(time-2.0*PI*mouse.y/resolution.y,time+2.0*PI*mouse.x/resolution.x,0.0));
    rd*=rot(vec3(time-2.0*PI*mouse.y/resolution.y,time+2.0*PI*mouse.x/resolution.x,0.0));
    vec3 rp = ro;
