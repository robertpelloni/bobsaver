...
// original https://www.shadertoy.com/view/3ssSD8
...
void main(void)
{
    vec2 uv = (gl_FragCoord.xy - vec2(.5 * resolution.x, 0.)) / resolution.y
        - vec2(0., .5);

    //uv *= 3. + 2. * cos(-time);
    
#if DEBUG_LINE
       glFragColor = vec4(vec3(1., 0., 0.) * line(uv + vec2(0., time)), 1.0);
#else
    const int fire_n = 10;
    ...
    glFragColor = vec4(fire_color(2. * fire_intensity / float(fire_n)), 1.0);
#endif
}
