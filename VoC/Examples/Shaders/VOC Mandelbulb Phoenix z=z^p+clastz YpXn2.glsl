vec3 TriplexAdditionPhoenixYpXn2(in vec3 z, in vec3 c) {
    vec3 znew;
    znew.x=z.x+c.x;
    znew.y=z.y+c.y;
    znew.z=z.z+c.z;
...
vec3 TriplexPowerPhoenixYpXn2(in vec3 z,in float p) {
    vec3 znew;
    radius=pow(radius,p);
    theta=theta*p;
    phi=phi*p;
...
vec3 TriplexMultiplicationPhoenixYpXn2(in vec3 z,in vec3 p){
...
void Iterate(inout vec3 z,in vec3 c) {
    ...
    znew=TriplexPowerPhoenixYpXn2(z,power);
    znew2=TriplexMultiplicationPhoenixYpXn2(c,lastz);
    znew=znew+znew2;
    lastz=z;
    z=znew;
}
