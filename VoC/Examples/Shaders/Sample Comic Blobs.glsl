...
// original https://www.shadertoy.com/view/wsXBWS
...
float bayer(ivec2 uv) {
    return 0.0;//texelFetch(iChannel0,uv % 8,0).x;
}
...
float scene(vec3 p) {
  float rad = 3.+p.z+sin(p.y/2.+time)+cos(p.x/3.+time*0.9);
  float dist = 10000.;
  for (int i = 0; i < 4; i++) {
    vec3 rot = hash33(vec3(float(i+1), cos(float(i)), sin(float(i))));
    float d = comp(p, rot, time/2.*(float(i+1)));
    dist = smin(dist, d, 1.);
  }
  return mix(dist, rad, mix(0.3,0.8+sin(time)*0.2, 0.1));
}
...
void main(void)
{
  ...
  vec3 init = vec3(-50,0,sin(time*0.37)*1.4);
  ...
  vec2 ao = vec2(0);
  for (int i = 0; i < 8; i++) {
    ivec2 id = (ivec2(gl_FragCoord.xy/16.+vec2(time*10.,time*20.)) % 2)*2-1;
    ao += vec2(march(p+n*0.1, r, bayer(ivec2(gl_FragCoord.xy)+i+ivec2(i/4,0))) , 1.);
  }
  ao.x/=ao.y;

  glFragColor.xyz = hit&&!outline ? vec3(ao.x) : vec3(0.);
  glFragColor.xyz = pow(smoothstep(vec3(0),vec3(1),sqrt(glFragColor.xyz)),vec3(1.7,1.6,1.5));
}
