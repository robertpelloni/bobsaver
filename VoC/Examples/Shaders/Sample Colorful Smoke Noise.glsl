...
// original https://www.shadertoy.com/view/wlSyDd
...
{
    float mouse = (mouse.x*resolution.xy.x / resolution.x);
    if (mouse == 0.0) mouse = 1.0 - (cos(time * 0.5) * 0.5 + 0.5);
...
void main(void)
{
    ...
    vec2 stretchedUV = (gl_FragCoord.xy - 0.5 * resolution.xy) / resolution.xy;
    ...
    //uv -= layeredNoise12(uv + 43.0) * 0.5;
      ...
      noise *= vignette;
      glFragColor = mix(col, col2 * 0.2, 1.0 - noise * 2.0);
    glFragColor = smoothstep(-0.14, 1.1, glFragColor);
}
