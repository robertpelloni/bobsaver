...
// original https://www.shadertoy.com/view/3ddGWM
...
void main(void)
{
  ...
  float time2 = mod(time, 200.0);

  vec3 s=vec3(0,-1.0,-3);
  vec3 t=vec3(0,-1.0 + sin(time2*0.2)*0.5,0);
  ...
  vec3 cx=normalize(cross(cz, vec3(sin(time2)*0.1,1,0)));
    ...
    if(abs(d)<0.003) {
      if(!isghost) {
        ...
        if(p.y>-0.02) {
          ...
          n.x += sin(d3*45.0 + time2) * 0.5 * fac;
          n.x += sin(d3*17.0 + time2*3.0) * fac;
          n.z += sin(d3*23.0 + time2*0.7) * 0.8 * fac;
  ...
  col += b1*0.005*vec3(0.3,0.5,1) * curve(time2+12.2, 0.9);
  col += b2*0.013*vec3(0.4,0.7,0.5) * curve(time2, 1.2);
  col += b3*0.014*vec3(0.8,0.2,0.5) * curve(time2+17.4, 1.7);
  col += emi;
