...
// original https://www.shadertoy.com/view/XdX3WN
...
void main(void)
{
    ...
    pointR.x += 0.32 * sin(1.32 * time);
    pointR.y += 0.3 * sin(1.03 * time);
    pointR.z += 0.4 * sin(1.32 * time);
    
    pointG.x += 0.31 * sin(0.92 * time);
    pointG.y += 0.29 * sin(0.99 * time);
    pointG.z += 0.38 * sin(1.24 * time);
    
    pointB.x += 0.33 * sin(1.245 * time);
    pointB.y += 0.3 * sin(1.41 * time);
    pointB.z += 0.41 * sin(1.11 * time);
    ...
    glFragColor.x = 1.0 - smoothstep(distToR, 0.0, pointRadius * pointR.z);
    glFragColor.y = 1.0 - smoothstep(distToG, 0.0, pointRadius * pointG.z);
    glFragColor.z = 1.0 - smoothstep(distToB, 0.0, pointRadius * pointB.z);
    glFragColor.w = 1.0;    
    ...
    glFragColor.x += (linkStrengthRG * contribRonRG) + (linkStrengthBR * contribRonBR);
    glFragColor.y += (linkStrengthGB * contribGonGB) + (linkStrengthRG * contribGonRG);
    glFragColor.z += (linkStrengthBR * contribBonBR) + (linkStrengthGB * contribBonGB);
    
    // Use an underlying texture to provide some noise
    float noiseMin = 1.0 - noiseStrength;
    glFragColor.xyz *= (1.0 - noiseStrength) + (noiseStrength);
}
