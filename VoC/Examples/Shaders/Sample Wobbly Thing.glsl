...
// original https://www.shadertoy.com/view/ldB3Dt
...
vec2 DE(vec3 p) {
    ...
    //distortion
    float d3=noise3(p*2.0 + time)*0.18;
    //shape
    ...
    p=q.xzy;
    float r=p.y*0.02+sin(time)*0.05;
        oprep2(p.zy,3.0,8.5, 0.0);
    float g=cylinder(p,vec2(1.15+r,17.0)) - sin(p.y*1.3 - time*4.0)*0.1 -d3;
    float g2=cylinder(p,vec2(1.05+r,18.0)) - sin(p.y*1.3 - time*4.0)*0.1 -d3;
