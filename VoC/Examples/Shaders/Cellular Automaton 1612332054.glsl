...
float cgol_test(float c) {
    vec3 nh = nhd(vec2(1.0,0.0),1.0,0.99,0);
    if(nh[0] <= 1.0) { c = 0.0; }
    if(nh[0] == 3.0) { c = 1.0; }
    if(nh[0] >= 4.0) { c = 0.0; }
    return c;
...
// float r0 = cgol_test(res_r);

    float s = 0.025;
    float m = 0.6;
    float r0 = res_r - s * ((5.56*m) + cs*10.0);

    vec3 nr0 = nhd( vec2(20.0,10.0), psn, 0.0, 0 );
    float nr0w = nr0[0] / nr0[2];

    res_g = 0.0;
    res_b = 0.0;

    if( nr0w >= 0.365 && nr0w <= 0.549 ) { r0 += s*(((5.62*0.4)*m) + cs*10.0); res_g = 0.3; }
    if( nr0w >= 0.385 && nr0w <= 0.410 ) { r0 += s*(((5.62*0.8)*m) + cs*10.0); res_g = 1.0; }

    if( nr0w >= 0.257 && nr0w <= 0.336 ) { r0 += s*(((7.86*1.1)*m) + cs*10.0); res_b = 0.0; }
    if( nr0w >= 0.292 && nr0w <= 0.312 ) { r0 += s*(((7.86*0.5)*m) + cs*10.0); res_b = 1.0; }

// r0 -= s*0.5;
// if( nr0w >= 0.000 && nr0w <= 0.256 ) { r0 -= s; }
// if( nr0w >= 0.549 && nr0w <= 1.000 ) { r0 -= s; }
