...
void main( void ) {
    ...
    // Base skin color
    glFragColor = vec4(0.7, 0.5, 0.4, 1.0);
    ...
    glFragColor.rgb = mix(innerMouth, glFragColor.rgb, im);
    
    // Draw lower lip
    float BLip = lipWidth * xpow(sin(3.1415 * 0.5 * (1.0 - abs(position.x) / mouthEdge)), 0.55);
    BLip = lim((abs(position.y - mouthB + BLip) - BLip) * 300.0);
    glFragColor.rgb = mix(LipColor * 1.15, glFragColor.rgb, lim(BLip+mouthFade));

    // Draw upper lip
    float TLip = lipWidth * sin(3.1415 * 0.65 * (1.0 - abs(position.x) / mouthEdge));
    TLip = lim((abs(position.y - mouthT - TLip) - TLip) * 300.0);
    glFragColor.rgb = mix(LipColor * 0.8, glFragColor.rgb, lim(TLip+mouthFade));
    
    // Darken inner mouth lip edges
    //glFragColor.rgb *= min(1.0, 0.5 + mouthFade + 500.0*abs(position.y - mouthT));
    //glFragColor.rgb *= min(1.0, 0.5 + mouthFade + 500.0*abs(position.y - mouthB));
}
