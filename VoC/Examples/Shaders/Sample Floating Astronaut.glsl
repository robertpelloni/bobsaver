...
// original https://www.shadertoy.com/view/WtKGW3
...
void poseFloating()
{
    ...
    float s0 = sin(time*0.125);
    float s1 = sin(time*0.25);
    float s2 = sin(time*0.5);
    float s3 = sin(time*1.0);
    float s4 = sin(time*4.0);
    
    if (camrot)
        eulerPivot(pHip, sin(time*0.1)*15.0, 0.0, sin(time*0.2)*25.0);
...
void main(void)
{
    ...
    //float mx = 2.0*(mouse.x*resolution.xy.x/resolution.x-0.5);
    //float my = 2.0*(mouse.y*resolution.xy.y/resolution.y-0.5);
    float md = -12.5;
    ...
    */
    {
        ...
        uv = (gl_FragCoord.xy+ao-0.5*resolution.xy)/resolution.y;
    ...
    vec2 muv = (mouse*resolution.xy.xy-0.5*resolution.xy)/resolution.y;
    
    col = mix(col, white,  digit(0,    0, framesRate,    2.0));
    col = mix(col, blue,   digit(0,   40, time,         4.1));
    col = mix(col, yellow, digit(0,   80, iCompile,      2.2));
    col = mix(col, red,    digit(0,  120, float(option), 1.0));
    col = mix(col, green,  digit(150,  0, mouse*resolution.xy.y,      5.0));
    col = mix(col, red,    digit(150, 40, mouse*resolution.xy.x,      5.0));
    col = mix(col, green,  digit(250,  0, muv.x,         2.2));
    col = mix(col, red,    digit(250, 40, muv.y,         2.2));
        
    if (frag.x >= 350. && frag.x < 500. && frag.y < 160.)
    {
        uv = (mouse*resolution.xy.xy-0.5*resolution.xy)/resolution.y;
        rd = normalize(uv.x*uu + uv.y*vv + pCam.z);
        d  = rayMarch(pCam.p, rd);
        if (d < MAX_DIST)
        {
