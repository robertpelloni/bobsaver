...
// original https://www.shadertoy.com/view/llj3RD
...
float worldDensity(vec3 position)
{
    ...
    vec3 offset = vec3(0.0, 0.0, time * 0.010);
...
void main(void)
{
    uv = vec2(gl_FragCoord.x / resolution.x, gl_FragCoord.y / resolution.x);
    uv.y += (resolution.y / resolution.x) * 0.4;
