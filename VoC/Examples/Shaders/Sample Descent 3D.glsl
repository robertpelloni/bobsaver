...
// original https://www.shadertoy.com/view/wdfGW4
...
vec3 addSign(vec3 color, vec3 pos, float side, vec2 id) {
    ...
    float charCount = floor(1.0 + 8.0 * hash1(id + 0.7));
    if (center.y - p.y > 2.0 * halfWidth * (charCount + 1.0)) {
        center.y -= 2.0 * halfWidth * (charCount + 1.5 + 5.0 * hash1(id + 0.8));
        charCount = floor(2.0 + 12.0 * hash1(id + 0.9));
        id += 0.05;
    }
    ...
    color *= step(0.25, outline);

    vec2 charPos = 0.5 * (p - center + halfSize) / halfWidth;
    float char2 = -3.0 + 8.0 * noise(id + 6.0 * charPos);
    charPos = fract(charPos);
    char2 *= smoothstep(0.0, 0.4, charPos.x) * smoothstep(1.0, 0.6, charPos.x);
    char2 *= smoothstep(0.0, 0.4, charPos.y) * smoothstep(1.0, 0.6, charPos.y);
    color = mix(color, signColor, step(outline, 0.01) * clamp(char2, 0.0, 1.0));

    outline = smoothstep(0.0, 0.25, outline) * smoothstep(0.5, 0.25, outline);
    return mix(color, outlineColor, outline);
}
