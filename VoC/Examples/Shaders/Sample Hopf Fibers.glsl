...
// original https://www.shadertoy.com/view/ttVXWt
...
#define MS mouse*resolution.xy
//#define PHASE smoothstep(.2, .8, MS.z > 0. ? MS.y/RES.y : .5+.5*cos(.4*time))
//#define ANG (MS.z > 0. ? 2.*PI*MS.x/RES.x : PI + 1.5*time)
#define PHASE smoothstep(.2, .8, .5+.5*cos(.4*time))
#define ANG (PI + 1.5*time)
...
void march(in vec3 p, in vec3 nv, out vec4 color) {
    ...
    mat3 m = glRotate( normalize(vec3(1.,sin(time),1.)), -ANG );
...
void main(void) {
    ...
    vec4 objColor;
    march(camPos + rand(gl_FragCoord.xy)*nvCamDir*STEP_D, nvCamDir, objColor);
