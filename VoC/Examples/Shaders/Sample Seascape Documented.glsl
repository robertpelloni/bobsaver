...
// original https://www.shadertoy.com/view/llsXD2
...
float noise( in vec2 p ) {
    ...
    // bteitler: This is a smooth wave function with input between 0 and 1
...
float sea_octave(vec2 uv, float choppy) {
    ...
    // Note that this function is called with a specific frequency multiplier (SEA_FREQ=0.16) which will stretch out the wave:
...
// using a low(er) resolution noise function (less iterations).
float map(vec3 p) {
    float freq = SEA_FREQ;
    ...
    for(int i = 0; i < ITER_GEOMETRY; i++) {        
        ...
        d += sea_octave((uv-SEA_TIME)*freq,choppy); // bteitler: perhaps for more asymmetry? Not sure cost is justified.
        h += d * amp;        
        uv *= octave_m; freq *= 1.9; amp *= 0.22; // Black magic / Guess and check most likely :)
...
// bteitler: Compute the distance along Y axis of a point to the surface of the ocean
// using a high(er) resolution noise function (more iterations).
...
void main(void) {
    ...
    uv.x *= resolution.x / resolution.y; // bteitler: Aspect ratio correction - if you don't do this your rays will be distorted
    float time = time * 0.3 + mouse.x*0.01; // bteitler: Animation is based on time, but allows you to scrub the animation based on mouse movement
