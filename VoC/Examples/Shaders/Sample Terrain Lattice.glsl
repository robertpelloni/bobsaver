...
// original https://www.shadertoy.com/view/XslyRH
...
float triTerrain(vec2 p){ 
    ...
    vec2 f = fract(p); // Fractional grid value.
    p -= f; // Integer value. Used for the unique corner height values.

    
    //f *= f*(3.-2.*f); // Weird Gouraud-looking triangles, or smooth quad.
    ...
    float s = step(1., f.x + f.y); // Determines which side of the diagonal we're on.
    ...
    hVal = vec3(hm(p+s), hm(p + vec2(1, 0)), hm(p+vec2(0, 1)));
    
    // A simple, interpolated quad. It's not really flat, but the edge-joins are straight,
    // so it looks that way.
    //return mix(mix(hm(p), hVal.y, f.x), mix(hVal.z, hm(p+1.), f.x),f.y);
    ...
    //
    vec3 b = abs(vec3(1.0 - f.x - f.y, f.x - (f.x - f.y + 1.)*s, f.y - (f.y - f.x + 1.)*s));
...
/*
    ...
    float s = step(f.x, f.y);
    hVal = vec3(hm(p), hm(p + vec2(1. - s, s)), hm(p+1.));

    //return mix(mix(hVal.x, hm(p+vec2(1, 0)), f.x), mix(hm(p+vec2(0, 1)), hVal.z, f.x),f.y);

    vec3 b = abs(vec3(1. - (1. - s)*f.x - f.y*s, (1.-2.*s)*(f.x - f.y), f.x*s + f.y*(1. - s)));
...
float map(vec3 p){
    ...
    // The diagonal tube lines. Makes things look too busy, but comment them out, if you fee like it.
...
float trace(vec3 ro, vec3 rd){
    ...
    for (int i=0; i<96; i++){
        ...
        t += d*.86;  // Using more accuracy, in the first pass.
    }
...
*/    
    ...
    if(t < FAR){
        ...
        if(svObjID>0.5) { // Terrain texturing.
            ...
            texCol = vec3(0.1,0.0,0.0);//tex3D(iChannel0, sp*tSize0, sn);
            //texCol = texture2D(iChannel0, sp.xz*tSize0).xyz;
            //texCol *= texCol;
            ...
            */
            ...
            texCol = vec3(0.0,0.0,0.4);//tex3D(iChannel0, sp*tSize0, sn)*.25;
    ...
    glFragColor = vec4(sqrt(clamp(sceneCol, 0., 1.)), 1.0);
