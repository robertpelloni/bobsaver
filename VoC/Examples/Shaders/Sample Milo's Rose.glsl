...
// original https://www.shadertoy.com/view/XsdyWr
...
void main(void)
{
    ...
    //if (mouse*resolution.xy.x > 0.0)
        rot = rotationXY(mouse*resolution.xy.yx / resolution.yx * vec2(PI, -2.0 * PI) + vec2(PI * -0.5, PI));
    ...
    float r = 1.2 - length((gl_FragCoord.xy / resolution.xy) - 0.5) * 1.0;
    glFragColor = vec4(c * r, 1.0);
}
