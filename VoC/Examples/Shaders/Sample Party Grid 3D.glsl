...
// original https://www.shadertoy.com/view/XtlfWs
...
vec3 shade(in vec3 ro, in vec3 rd) {
    ...
    float mtime = mod(time+t_trans, 2.0*t_phase);
    ...
    for (float i=0.; i<rsteps; ++i) {
        ...
        for (float j=0.; j<8.; ++j) {
            ...
            if (inbox(ctr, 0.0)) {
                ...
                vec3 h1 = hash33(ctr);
                vec3 r = 0.5*party*0.5*cos((h1+0.05)*4.0*time + 2.0);
...
void main(void) {
    ...
    float thetay = 0.1235*time;
    float thetax = 0.75 * smoothstep(0.0, 5.0, time);

    //if (max(mouse*resolution.xy.x, mouse*resolution.xy.y) > 20.0) { 
    //    thetax = (mouse*resolution.xy.y - .5*resolution.y) * -5.0/resolution.y; 
    //    thetay = (mouse*resolution.xy.x - .5*resolution.x) * 10.0/resolution.x; 
    //}
