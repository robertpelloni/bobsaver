...
// original https://www.shadertoy.com/view/lsfcDs
...
void main(void)
{
    ...
    vec2 R = resolution.xy,
          uv  = vec2(gl_FragCoord.xy-R/2.) / R.y; // Thanks Fabrice
    vec3    dir = camera(uv);
    ...
    #ifdef    BLACKBODY
    glFragColor.xyz = blackbody( ( 15.-inter.y+.051*inter.x ) * 75. );
    return ;
    #endif
    #ifdef    SINUSITE
    glFragColor.xyz = inter.y*.05*vec3(abs(sin(dir.x*3.14+t+1.04)), abs(sin(dir.y*3.14+t+2.09)), abs(sin(dir.z*3.14+t+3.14)));
    return ;
    #endif
    #ifdef    GEODESIC
    ...
    #endif
    #ifdef    GRIDS
       glFragColor.xyz = vec3(abs(sin(t+1.32+1.04*g)), abs(sin(t+1.32+2.09*g)), abs(sin(t+1.32+3.14*g)))*inter.x*.005;
    return ;
    #endif
    #ifdef    HEATWAVES
    glFragColor.xyz = vec3(abs(sin(t+1.04+(15.-inter.y*.05) )), abs(sin(t+2.09+(15.-inter.y*.05) )), abs(sin(t+3.14+(15.-inter.y*.05) )));
    return ;
    #endif
}
