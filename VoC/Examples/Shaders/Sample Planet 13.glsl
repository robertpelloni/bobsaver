...
// original https://www.shadertoy.com/view/WdB3Ry
...
vec3 RayTrace(vec3 ro, vec3 rd, vec4 noise) 
{
    ...
    if (dist<MAX_DIST)
    {
        ...
        localPos = RotateY(localPos, time*0.05);
        ...
        float foamNoise =  GetNoise(localPos*7.0*(0.75+0.25*sin(time*0.1)))*0.25;
        float foamLimit = smoothstep(0.6+foamNoise, 1.0, waterHeight);
        foamLimit = 0.7*foamLimit + 0.3*fract(foamLimit*4.0-time*.2)*foamLimit*smoothstep(1.0, 0.85, foamLimit);
        ...
        localPosClouds = RotateY(localPosClouds, time*0.055 + localPosClouds.y*0.85);
        float clouds = GetFBMClouds(localPosClouds*3.0-vec3(time*0.025)) * 0.6;

        vec3 localPosCloudsBis = (pos+l*0.1 - PLANETCENTER)*0.25;
        localPosCloudsBis = RotateY(localPosCloudsBis, time*0.055);
        float cloudsBis = GetFBMClouds(localPosCloudsBis*3.0-vec3(time*0.025)) * 0.6;
    ...
    else
    {
        ...
        else
        {
            ...
            float noise = GetFBM(pos*2.0 + vec3(0.0, 0.0, time*0.05));
...
void main(void)
{
    ...
    vec4 noise = vec4(0.0);//texture(iChannel0, gl_FragCoord.xy/1024.0); 
    vec3 offset =  vec3(sin(time*0.1)*3.0, 0.25+(cos(time*0.1)*.5), 0);
    //offset = mix(offset, vec3(4.0-mouse*resolution.xy.x/resolution.x*8.0, 4.0-(mouse*resolution.xy.y/resolution.y)*8.0, 0), step( 0.001, mouse*resolution.xy.z ));
