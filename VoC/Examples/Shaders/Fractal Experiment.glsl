...
// original https://www.shadertoy.com/view/ltyBRz
...
void main(void)
{
    ...
    vec2 uv = (gl_FragCoord.xy - resolution.xy/2.0) / max(resolution.xy.x, resolution.xy.y);
 
    float anim = (sin(time * 0.2) + 1.0) * 0.6;
    vec3 tot = vec3(0.0);
    vec3 ro;
    ro = vec3(1.0, 5.0, 5.0);
