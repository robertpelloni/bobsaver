...
void main( void ) {
    ...
    for( int i = 0; i < particles; i++){
        ...
        d = point - pos;
        totald += (scale * ((floati*.29*point.x)/float(particles))) / dot(d,d);
    }
    ...
    for( int i = 0; i < particles; i++){
        ...
        d = point - pos;
        totald2 += .05*(scale * ((floati*.29*point.x)/float(particles))) / dot(d,d);
    }
    glFragColor = vec4((totald+.1)*color + (totald2)*color2,0.);
