...
// original https://www.shadertoy.com/view/4t3BWl
...
void main(void)
{
    ...
    if (t8 < 0.125) {
        ...
        angle -= 1.;
        turn_sign = 1.;
        angle += first_turn_look_angle*0.5 + (-first_turn_look_angle*0.5+1.0+second_turn_drift_angle)*t1;
        rotate_on_turns = 0.;
        roll_on_turns = 0.;
    } else if (t8 < 0.375) {
        prev += dir + dirL;
        ...
        //dir = dir;
        turn_sign = 1.;
        angle += second_turn_drift_angle*sqrt(1.-t1);
        //roll_on_turns = 0.;
    } else if (t8 < 0.5) {
        prev += dir + dir + dirL;
        turn = dirR;
        ...
        //dir = dir;
        turn_sign = -1.;
        angle -= (fifth_turn_drift_angle + 1.) * smoothstep1(t1);
        rotate_on_turns = 0.;
        roll_on_turns = 0.;
    } else if (t8 < 0.875) {
        ...
        angle -= 1.;
        turn_sign = 1.;
        angle -= fifth_turn_drift_angle - smoothstep1(t1) * (fifth_turn_drift_angle * 2. + 1.);
        rotate_on_turns = 0.;
        roll_on_turns = 0.;
    } else {
        prev += dir + dir + dir;
    ...
    #ifdef MOUSE
    up_down += 0.7 * (1. - (mouse*resolution.xy.y / resolution.y) * 2.);
    #endif
    ...
    #ifdef MOUSE
    angle += (mouse*resolution.xy.x / resolution.x) * 2. - 1.;
    #endif
