...
// original https://www.shadertoy.com/view/7dVyz1
...
float map(vec3 rp)
{
    ...
    //pos.y-=t*.25;
    //pos.xy=abs(1.-mod(pos.xy-t,2.));
    pos = abs(pos.xyz + 1.0) - 1.0;
    //pos.xz = abs(pos.xz) - vec2(1); // this mirrors space along a diagnal in the xz plane.  
    //pos.xy = abs(pos.xy) - vec2(1);
    pos=pos*2./clamp(dot(pos.xyz,pos.xyz),.3,1.)-vec3(0.5,1.5,0.5);
    vec3 b = vec3(1., 1.0, 1.);
   
    //pos.xy *= rot(pos.z*0.2);
    pos.y += sin(pos.z + time + pos.x*1.0)*0.6;
    pos.x += cos(pos.y - pos.z * 1.0 + time)*0.3;
 
    pos = mod(pos, b)-0.5*b;
    // pos.z = fract(pos.z) -.5;
     
     //pos.x = clamp(fract(pos.x) -.5, -.5, .5);
     //sdBox(pos, vec3(0.1, 0.1, 2.0));
     
    res = sdBox(pos, vec3(0.1, 0.1, 2.0));
...
{
    p.xy *= rot(p.z*0.64);
    vec3 col = mix(vec3(1.0, 0.7, 0.), vec3(1.4, .2, 0.0),  smoothstep(1.0, 0., abs(p.z)-.3));
    return col;
}
...
void main(void)
{
    ...
    ro.y += sin(ro.z + time + ro.x*1.0)*0.6;
