...
// original https://www.shadertoy.com/view/3ssfW8
...
float sphere(vec3 p)
{
    ...
    p.y -= abs(sin(time + grid.x*2.)*.1);
    p.y -= abs(cos(time + grid.z*2.)*.1);
    
    float sphere = length(p) - .5;
    float amp = .05;
    
    grid = abs(grid);
    return sphere - amp*sin(time*5. + grid.z)*sin(time + grid.x)*sin(time + grid.z*7.) + amp;
}
