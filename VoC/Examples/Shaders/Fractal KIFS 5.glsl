...
// original https://www.shadertoy.com/view/ts3SDr
...
vec4 sceneSDF(vec3 p, vec2 uv) {
    ...
    float time = time + uv.y*20.0 - uv.x*12.0;
...
void main(void)
{
    ...
    vec3 diffuse = vec3(0.5); //texture(iChannel0, abs(0.4*result.yz + 0.005*time)).rgb;    // color from texture based on transformed point
