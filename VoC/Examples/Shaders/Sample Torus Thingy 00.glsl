...
// original https://www.shadertoy.com/view/4t2yW1
...
{  
    ...
    var =
        step(cos(floor( (atan(p.x,p.y))*10.)/1.+time*1.)*3.14, atan(q.x, p.z ))
        *
        step(atan(q.x, p.z )-2., cos(floor( (atan(p.x,p.y))*10.)/1.+time*1.)*3.14)
        ;
