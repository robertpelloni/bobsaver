...
//layer can be anything from 0 to 9 and will give a different random result
...
float read_array(int xp, int yp, int which_layer) {
	...
	**/
	switch (which_layer) {
		case 1:return_value=imageLoad( layer1, ivec2(xp,yp+y_res*int(mod(frames,2))) ).r; break;
		case 2:return_value=imageLoad( layer2, ivec2(xp,yp+y_res*int(mod(frames,2))) ).r; break;
		case 3:return_value=imageLoad( layer3, ivec2(xp,yp+y_res*int(mod(frames,2))) ).r; break;
	}
	return return_value;
}
...
void write_array(int xp, int yp, int which_layer, float value) {
	...
	**/
	switch (which_layer) {
		case 1:imageStore( layer1, ivec2(xp,yp+y_res*int(mod(frames,2))), vec4(value,0,0,1)); break;
		case 2:imageStore( layer2, ivec2(xp,yp+y_res*int(mod(frames,2))), vec4(value,0,0,1)); break;
		case 3:imageStore( layer3, ivec2(xp,yp+y_res*int(mod(frames,2))), vec4(value,0,0,1)); break;
	}
...
void main()
{
    ...
    if(frames<2) {
		...
		**/
		...
		write_array(x_pixel,y_pixel,1,random_lcg(vec2(x_pixel,y_pixel),0));
		write_array(x_pixel,y_pixel,2,random_lcg(vec2(x_pixel,y_pixel),1));
		write_array(x_pixel,y_pixel,3,random_lcg(vec2(x_pixel,y_pixel),2));
