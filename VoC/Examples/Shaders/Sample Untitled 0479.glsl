...
// original https://www.shadertoy.com/view/tsySDw
...
{ 
  ...
  else
  {
      ...
      {
        float intensity = 0.4 + (1.0 + sin( time * LIGHT_SPEED * 2.0 )) * 0.0;
...
{  
  return vec3(time * SPEED * 12.0, 3.0, 6.0);
}
...
{  
  ...
  glFragColor = vec4(col,1.0); // Output to screen
}
