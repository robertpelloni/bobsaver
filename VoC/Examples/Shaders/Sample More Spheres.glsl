...
// original https://www.shadertoy.com/view/lsX3DH
...
void main(void) {
    ...
    for( int j=0; j<SAMPLES + min(0,frames); j++ ) {
        float fj = float(j);
        
#ifdef MOTIONBLUR
        time2 = time + fj/(float(SAMPLES)*TARGETFPS);
#endif
        ...
        // camera    
        vec3 ro = vec3( cos( 0.232*time2) * 10., 6.+3.*cos(0.3*time2), GRIDSIZE*(time/SPEED) );
        vec3 ta = ro + vec3( -sin( 0.232*time2) * 10., -2.0+cos(0.23*time2), 10.0 );
        
        float roll = -0.15*sin(0.5*time2);
