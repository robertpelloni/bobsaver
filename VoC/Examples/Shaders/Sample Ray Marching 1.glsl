...
void main(void){
  ...
  vec3 E=vec3(-sin(time/2.)*3.0,4,cos(time/2.)*9.0); //Camera location; Change camera path position here
  ...
  if (f<MAX_DEPTH){
    ...
    glFragColor=vec4((c*b+pow(b,8.0))*(1.0-f*.01),1.0);//simple phong LightPosition=CameraPosition
  }
