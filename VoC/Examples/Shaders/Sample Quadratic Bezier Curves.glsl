...
//original https://www.shadertoy.com/view/XdjXD1
...
float DistanceQuadraticBezier(vec2 x, vec2 a, vec2 b, vec2 c)
{
    ...
    float t;
       ...
       float Dist1 = distance(x, QuadraticBezier(clamp(roots.y, 0.0, 1.0), a, b, c));
    float Dist2 = distance(x, QuadraticBezier(clamp(roots.x, 0.0, 1.0), a, b, c));
...
vec2 Mouse()
{
    vec2 a = 2.0 *(mouse.xy / resolution.xy) - 1.0;
...
void main(void)
{
    ...
    vec3 col = vec3(0.2, 0.2, 0.3) * length(q);
