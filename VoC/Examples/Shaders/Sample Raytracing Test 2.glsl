...
// original https://www.shadertoy.com/view/ll2XR1
...
void main(void)
{
    ...
    vec3 bgCol = background(time, rd); // get the bg color
    
    rd = reflect(rd, nml); // relect off of the normal nml (from the sphere)
    
    vec3 col = background(time, rd) * vec3(0.9, 0.8, 1.0); // bg color times the color we want the bg to be (so its not just black and white and grey)
    
    glFragColor = vec4( mix(bgCol, col, step(0.0, t)), 1.0 ); // interpolate the background color and the relected color
}
