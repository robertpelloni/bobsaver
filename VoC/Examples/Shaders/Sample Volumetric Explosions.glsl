...
{  
    ...
    #ifdef TONEMAPPING
        vec4 result = vec4(ToneMapFilmicALU(sum.xyz*2.2),1.0);
        vec3 resultHSV = rgb2hsv(result.rgb);
        
        glFragColor = vec4(hsv2rgb(resultHSV + vec3(20. + time / 30. * (uv.x / 800. + time / 7000.), 0., 0.)) * pow(result.a, 1.), pow(result.a, 1.));
    #else
        
        vec4 result = vec4(sum.xyz,1.0);
        vec3 resultHSV = rgb2hsv(result.rgb);
        glFragColor = vec4(hsv2rgb(resultHSV + vec3(20. + time / 30. * (uv.x / 8. + time / 30.), 0., 0.)) * pow(result.a, 1.), pow(result.a, 1.));
    #endif
}
