...
// original https://www.shadertoy.com/view/WsSGWG
...
void main(void)
{
    ...
    for(int i = 0; i < N; i++) {
        ...
        col = mix(col, INTENSITY, dirt(uv * max(0.0, t) * 0.1 + vec2(0.2, -0.2) * time + ii, 3.5));
        ...
        col = mix(col, INTENSITY * vec3(1.), dirt(uv * max(0.0, t) * 0.1 + vec2(-0.2, -0.2) * time + ii, 3.5));
