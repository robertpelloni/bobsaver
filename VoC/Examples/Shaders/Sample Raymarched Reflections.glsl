...
// original https://www.shadertoy.com/view/4dt3zn
...
// spherical boxes, because they're cheap and display the 
...
float map(vec3 p)
{
    ...
    // a randomized 3D offset, then add it to it's normal position. 
    // Simple.
...
float softShadow(vec3 ro, vec3 lp, float k){
    ...
    // I've added 0.5 to the final shade value, which lightens the shadow a bit. It's a preference thing. Really dark shadows look 
    // too brutal to me.
...
void main(void) {
    ...
    // Ray origin. Doubling as the surface postion, in this particual example.
    // I hope that doesn't confuse anyone.
    vec3 ro = vec3(0., 0., time*1.5);
    ...
    // the ray origin to describe the surface hit pointis kind of confusing. The reason 
    ...
    // see if the reflected poin was in shadow, and incorporate that too, but we're cheating to
