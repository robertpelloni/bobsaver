...
// original https://www.shadertoy.com/view/lll3zl
...
vec2 DDE(vec3 p, vec3 rd){
    float d1=DE(p);
    //if(gl_FragCoord.x<size.x*0.5)return vec2(d1,d1*COMPARE_FUDGE_FACTOR);
...
void main(){
    ...
    glFragColor=pow(pixel(gl_FragCoord.xy)*v, vec4(1./2.2));
