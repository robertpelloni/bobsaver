...
float boxDist(in vec3 p, float width) {
    p = fract(p);
    p -= 0.5;
...
float trigNoise3D(in vec3 p) {
    ...
    float boxWidth = 0.1;
    float n = boxDist(p * 5.0 + time * 2.1, boxWidth);
...
void main( void ) {
    ...
    color = mix(
        color,
        mix(vec3(0.4, 0.3, 0.2) * 3.0, vec3(0.2, 0.4, 0.7) * 0.9, sunF * sunF),
