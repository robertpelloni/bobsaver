...
float antialias(float diff) {
    return 1.0 - clamp(diff * 10.0, 0.0, 1.0);
}
...
void main( void ) {
    ...
    if (loadIndex < 2.0) {
        glFragColor = vec4(0.0, character0(scaledNormalCoords), 0.0, 1.0);
    }
    if (loadIndex < 1.0) {
        glFragColor = vec4(0.0, character1(scaledNormalCoords), 0.0, 1.0);
    }
    else {
        glFragColor = vec4(0.0, character0(scaledNormalCoords), 0.0, 1.0);
    }
