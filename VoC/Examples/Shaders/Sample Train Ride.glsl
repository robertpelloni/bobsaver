...
// original https://www.shadertoy.com/view/4s2Sz3
...
vec3 trainPos[5], trPos, qTrWin=vec3(0.0), sunDir, sunCol, moonDir, moonCol;
vec2 trkOffset;
float tCur, dirTrWin=0.0;
int idObj;
bool isNight;
...
vec4 GrndCol (vec3 p, vec3 n)
{
  ...
  if (p.y > 22.) {
    ...
    if (n.y > 0.65) {
      vec3 c = (n.y - 0.5) * (gCol6 * vec3 (Noisefv2 (0.4 * q),
         Noisefv2 (0.34 * q), Noisefv2 (0.38 * q)) + vec3 (0.02, 0.1, 0.02));
      col = mix (col, c, smoothstep (0.65, 0.75, n.y) *
...
float GrndRay (vec3 ro, vec3 rd)
{
  ...
  if (h < hDiff) {
    ...
    for (int j = 0; j < 5; j ++) {
      sm = 0.5 * (sLo + sHi);
      p = ro + sm * rd;
      if (p.y - GrndHt (p.xz, 0) < hDiff) sHi = sm;
      else sLo = sm;
    }
    dHit = sHi;
  }
...
void main (void)
{
  ...
  if (fixCam) {
    ...
    ro = TrackPath (tz);
    ro.y += 4.;
    vec3 vd = TrackPath (tz + dt) - TrackPath (tz - dt);
    vd.y = 0.;
    vd = normalize (vd);
