...
// original https://www.shadertoy.com/view/MlSBDt
...
void main(void) {
    ...
    uv.y *= 1.2;
    vol = 0.0;//texture(iChannel0, vec2(.5, .25)).r  * 4.; 
    float t2 = time;
    float 
        sk = sin(-t2 * .1) * 48.0, 
        ck = cos(-t2 * .1) * 48.0,
    ...
    glFragColor = vec4(clamp(sceneColor * (1. - length(uv) / 3.5), 0.0, 1.0), 1.0);
    glFragColor = pow(glFragColor, 1./vec4(1.2));
