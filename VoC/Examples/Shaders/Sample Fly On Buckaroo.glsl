...
// original https://www.shadertoy.com/view/3sSBDV
...
float scene(vec3 p) {
  p.z += sin(time);
  p = erot(p, vec3(0,1,0), cos(time)*.2);
  beamm = 0.9*(linedist(p, vec3(0), vec3(0,0,-10))-.3-sin(p.z*3.+time*4.)*.05 - sin(time)*.2);
  vec3 p2 =p;
...
void main(void)
{
  ...
  glFragColor.xyz = mix(clds, obj, fctr) + glow*glow*.9*sqrt(fctr*.5+.5);
  glFragColor.xyz = sqrt(glFragColor.xyz);
  glFragColor.xyz = abs(erot(glFragColor.xyz, normalize(sin(clp*.3+t)), 0.2));
}
