...
// original https://www.shadertoy.com/view/Mll3DH
...
vec3 pal[16];  //Palette form: http://www.pepto.de/projects/colorvic/
void setpal()
{
...
vec3 fx2(in vec2 p)
{
    vec3 rz = vec3(0);
    const float q = 5.;
    float x = sin(time*0.8+p.x*q)+sin(time+p.x*q)+sin(time*0.9+p.y*q)+sin(time*0.82+p.y*q*2.)+sin(p.x*1.+time*1.1)*3.;
    rz = sin(vec3(1,2,3)*x+time*2.);
    return rz;
}
...
{
    vec3 rz = vec3(0);
    float xx = abs(sin((p.x+p.y)*10.+time*5.))*abs(sin(time*2.+p.x+p.y));
    rz = hsv2rgb(vec3(0.6+(p.x+p.y)*0.3+time*0.1,.82,5.))*xx;
    return rz;
}
...
vec3 fx4(in vec2 p, in vec2 bp)
{
    ...
    rz *= step(fract(p.y*.5+time*0.015+rn*(time+10.)*0.15)+.1,0.5);
    return rz;
}
...
vec3 fx5(in vec2 p)
{
    ...
    vec3 col = abs(sin(vec3(1,3,5)+id*0.01+time*0.5))*0.75;
    col /= sin(.9*time+id)*2.;
    ...
    float g = (bp.y + time*0.05)*30.;
    vec3 bg = sin(vec3(1,2,3)+floor(g))*0.5+abs(sin(g-time*2.5)*.8);
...
vec4 balls(in vec2 p, in float of)
{
    ...
    p.y += mix(pd1,pd2,sin(time*1.+of*3.)*0.5+0.5);
...
void main(void)
{
    ...
    if(resolution.x*.625 > resolution.y)
    {
        p.x *= asp*0.625;
        p.x -= (asp*0.625-1.)*0.5;
    }
    ...
    float time = max((time*0.95)-3.5,0.);
