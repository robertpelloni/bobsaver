...
/*
    float nh0v = clamp( (nh0[0] / nh0[1])*1.045 ,0.0,1.0);
    float nh1v = clamp( (nh1[0] / nh1[1])*1.050 ,0.0,1.0);
    float nh2v = clamp( (nh2[0] / nh2[1])*1.058 ,0.0,1.0);
    float nh3v = clamp( (nh3[0] / nh3[1])*1.064 ,0.0,1.0);
    float nh4v = clamp( (nh4[0] / nh4[1])*1.076 ,0.0,1.0);
*/
    ...
    float rd02 = clamp( (nh0v * (2.0+cs)) - nh2v ,0.0,1.0);
    float rd20 = clamp( (nh2v * (2.0+cs)) - nh0v ,0.0,1.0);
    float rdr02 = (nh0[1]<nh2[1]) ? rd02 : rd20;

    float rd03 = clamp( (nh0v * (2.0+cs)) - nh3v ,0.0,1.0);
    float rd30 = clamp( (nh3v * (2.0+cs)) - nh0v ,0.0,1.0);
    float rdr03 = (nh0[1]<nh3[1]) ? rd03 : rd30;

    float rd04 = clamp( (nh0v * (2.0+cs)) - nh4v ,0.0,1.0);
    float rd40 = clamp( (nh4v * (2.0+cs)) - nh0v ,0.0,1.0);
    float rdr04 = (nh0[1]<nh4[1]) ? rd04 : rd40;

    float rd12 = clamp( (nh1v * (2.0+cs)) - nh2v ,0.0,1.0);
    float rd21 = clamp( (nh2v * (2.0+cs)) - nh1v ,0.0,1.0);
    float rdr12 = (nh1[1]<nh2[1]) ? rd12 : rd21;

    float rd13 = clamp( (nh1v * (2.0+cs)) - nh3v ,0.0,1.0);
    float rd31 = clamp( (nh3v * (2.0+cs)) - nh1v ,0.0,1.0);
    float rdr13 = (nh1[1]<nh3[1]) ? rd13 : rd31;

    float rd14 = clamp( (nh1v * (2.0+cs)) - nh4v ,0.0,1.0);
    float rd41 = clamp( (nh4v * (2.0+cs)) - nh1v ,0.0,1.0);
    float rdr14 = (nh1[1]<nh4[1]) ? rd14 : rd41;

    float rd23 = clamp( (nh2v * (2.0+cs)) - nh3v ,0.0,1.0);
    float rd32 = clamp( (nh3v * (2.0+cs)) - nh2v ,0.0,1.0);
    float rdr23 = (nh2[1]<nh3[1]) ? rd23 : rd32;

    float rd24 = clamp( (nh2v * (2.0+cs)) - nh4v ,0.0,1.0);
    float rd42 = clamp( (nh4v * (2.0+cs)) - nh2v ,0.0,1.0);
    float rdr24 = (nh2[1]<nh4[1]) ? rd24 : rd42;

    float rd34 = clamp( (nh3v * (2.0+cs)) - nh4v ,0.0,1.0);
    float rd43 = clamp( (nh4v * (2.0+cs)) - nh3v ,0.0,1.0);
...
/*
    float qdc0 = (d01<d34) ? sqrt(nh0v*nh1v) : sqrt(nh3v*nh4v);
    float qdc1 = (d02<d14) ? sqrt(nh0v*nh2v) : sqrt(nh1v*nh4v);

    float qd0 = abs(qdc0-q_01_34) * 2.5;
    float qd1 = abs(qdc1-q_02_14);

    float qdres = (qd0<qd1) ? q_01_34 : q_02_14;

    res_r = qdres;
    res_r = (q_02_14+q_01_34)/2.0;
    res_r = sqrt(q_02_14*q_01_34);
    res_r = q_01_34;
    res_r = q_02_14;
*/
...
// float qres = (q_02_14+q_01_34)/2.0;

// res_r = (qdr>1.0) ? 0.0 : qdr;
// res_r = (qdr<0.0) ? 1.0 : qdr;

    res_r = clamp(qdr,0.0,1.0);
