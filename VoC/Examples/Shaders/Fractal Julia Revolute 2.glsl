...
// original https://www.shadertoy.com/view/4sfczN
...
vec2 df(vec3 p)
{
    float a = fullAtan(p.xz)*0.5 +  time * 0.5; // axis y
    ...
    vec2 q = p.xz;
    float a0 = mix(0.,1.57,res.y/5. * mix(-.1,.1,sin(time * 0.5)*.5+.5));
...
void main(void)
{
    ...
    if (d<dMax)
    {
        ...
        glFragColor.rgb = mix( glFragColor.rgb, sky, 1.0-exp( -0.01*d*d ) ); 
    }
    else
    {
        glFragColor.rgb = sky;
