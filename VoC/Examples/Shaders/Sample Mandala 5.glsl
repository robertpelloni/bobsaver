...
// original https://www.shadertoy.com/view/3ddyzn
...
{
    q.x = fbm( p + vec2(0.0+time2,0.0-time2) );
    q.y = fbm( p + vec2(5.2-time2,1.3) );
...
float render(vec2 uv){
    ...
    float d = length(uv*(2.5+m.y*25.))-time2*.05;
    vec2 p = vec2(d, a);
    vec2 q = vec2(0.);
    vec2 r = vec2(0.);
...
void main(void)
{
    ...
    float n = renderAA(uv);
    glFragColor = gradientColorAt(n, gradient, 0);
}
