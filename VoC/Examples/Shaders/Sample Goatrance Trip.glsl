...
// original https://www.shadertoy.com/view/wlVXRV
...
vec3 rdrPsy(vec2 uv)
{
  ...
  float fidx= mod(-time+length(uv)*4.+.2*-abs(sin(50.*atan(uv.y,uv.x)/PI))
    +abs(0.1*sin(time*5.+sin(uv.x*5.)*179.7*atan(uv.y,uv.x)/PI))
      ,4.);
  int curIdx=int(fidx);
  vec3 bubbles = vec3(uv,.5)*(1.-sat(50.*_cir(repeat(uv*(sin(time*.5)*.2+.5)*r2d(.5*sin(uv.y*2.+time*.5)),vec2(.1)),.02)));
...
vec3 rdr(vec2 uv)
{
  vec3 opsy = rdrPsy(uv*r2d(-time));
  uv = abs(uv);
  uv = uv*r2d(20.*atan(uv.y,uv.x)/PI);
  uv= uv+(vec2(.1)*r2d(-time));
  float sel = float(mod(time,.4)<.2);
  vec3 col = rdrDot((uv+vec2(.3))*r2d(time),.02,mix(.05,.01,sel));
  vec3 c2 = rdrCross(uv);
...
uv*=2.+(sin(time)*.5+.5);
  vec3 col = rdr(uv);
    ...
    vec3 col2 = vec3(0.0);//col*float(rad < texelFetch(iChannel0, ivec2(int((an)*512.), 0), 0).x)*.5;
    vec3 outcol = sat(1.-lenny(uv*.5))*col.zxy*.5+col2.zxy;
//    outcol = pow(outcol, vec3(1./2.2));
    
    float fadeIn = clamp(time,0.,3.)/3.;
  glFragColor = vec4(outcol*fadeIn, 1.0);
}
