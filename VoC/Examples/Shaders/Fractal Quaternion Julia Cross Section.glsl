...
// original https://www.shadertoy.com/view/3ddGWn
...
float getDE(DualQuaternion jc, vec3 position, out int object, out vec3 normal, out int iEscape)
{
    ...
    julia = max(julia, -crossSection);//subtract
    
    float ground = abs(position.y + 1.5);
    
    float ceiling = abs(position.y - 2.0);
...
{   
    ...
    //view basis
    mat3 viewTransform = rotationXZ(time * 0.0);// * rotationYZ(-tau32 / 24.0);
    vec3 viewPosition = viewTransform * vec3(0, 0, 1.6);
    ...
    for(int i = 0; i < 1; i++)  
    {
        ...
        switch(object)
        {
            case 1://julia
            {
                surfaceColor = hsv2rgb(vec3(float(escape) / 20.0, 1, 1));//vec3(1.0 ) * (1.0 - float(steps) / 100.0);
                break;
            }
            ...
            case 3:
            {
                surfaceColor = colorCheckered(intersection.xz) * 0.5 + 0.5;// * 0.999 + 0.001;
                break;
            }
        }

        vec3 lightColor = vec3(0.9, 0.9, 1.1) * 1.2;
        vec3 lightPosition = vec3(1, 1.5, 1.3);
        vec3 lightDirection = normalize(lightPosition - intersection);
        float lightIncidence = clamp(dot(normal, lightDirection), 0.0, 1.0);
        float lightBlocked;

        vec3 shrp = intersection + normal * 0.1;
        vec3 shrd = lightDirection;
        vec3 shIntersection;
        vec3 shNormal;

        bool hit = march(jc, shrp, shrd, object, shIntersection, shNormal, steps, escape);
        lightBlocked = hit ? 0.0 : 1.0;
        lightBlocked = lightBlocked * 0.8 + 0.2;
        
        color += energy * surfaceColor * lightColor * lightIncidence * lightBlocked;
        energy *= surfaceColor + lightIncidence;
        
        srp = shrp;
        srd = normalize(reflect(srd, normal));
    }
    
    color.z = color.z * 0.97 + 0.03;
    color = color * 0.97 + 0.03;
