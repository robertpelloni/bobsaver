...
// original https://www.shadertoy.com/view/XsdGzM
...
void main(void)
{
    vec2 c=vec2(gl_FragCoord.xy/resolution.xy-.5)*2.;
    ...
    if(t>18.)
    {
        glFragColor.rgb=mix(vec3(1),vec3(.6,.6,1),rd.y);
        return;
    }
    ...
    glFragColor.rgb=sqrt(mix(vec3(1),alb*irr+spec,exp2(-t/25.)));
}
