...
// original https://www.shadertoy.com/view/MttBz8
...
void main(void)
{
    vec2 surfacePosition = 0.5 * (2.0 * gl_FragCoord.xy - resolution.xy) / min(resolution.x, resolution.y);
    
    float mouseY = 0.0;//mouse*resolution.xy.y == 0.0 ? 0.0 : mouse*resolution.xy.y / resolution.y - 0.5;
    ...
    for(float i = 0.0; i<1024.0; i+=1.0) {
        ...
        float a = sin(time * 1.5 + i * 2.0) * 0.3 + i * 1.3;
    ...
    if (it == 0.0) {
        ...
        float diagonal = length(resolution.xy);
