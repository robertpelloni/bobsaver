...
// original https://www.shadertoy.com/view/llSyDh
...
/*
    ...
    Roughly speaking, it's not much different to a circle. In that case you use the familiar 
    polar coordinates (r = length(u), a = atan(u.y, u.x)). The only difference in this case 
    is that you use a modified radial coordinate (r = min(min(r1, r2), r3)) - or to put it
    another way, you use the distance field value... It's a bit difficult to explain, but easy 
    to perform. When I get time, I'm going to produce a more robust Truchet texturing example.
...
void main(void) {
    ...
    float res = clamp(resolution.y, 300., 600.); 
    
    // Aspect correct screen coordinates.
    vec2 u = (gl_FragCoord.xy - resolution.xy*.5)/res;
    
    // Scaling and moving the screen coordinates.
    vec2 sc = u*4. + s.yx*time/8.;
    ...
    //float aRnd = sin(rnd*6.283 + time*1.5)*.5 + .5; // Animating the random number.
    
    #ifdef INTERLACING
    // Random vec3 - used for some overlapping.
    //vec3 lRnd = vec3(rnd, fract(rnd + .333), fract(rnd + .666));
    vec3 lRnd = vec3(hash21(h.zw + .13), hash21(h.zw + .26), hash21(h.zw + .39));
    #endif
    ...
    // Using the two off-centered hex coordinates to give the background a bit of highthing.
