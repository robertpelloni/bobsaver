...
// original https://www.shadertoy.com/view/4scXzn
...
float map(in vec3 p){
    ...
    float light = max(max(q.y, q.z) - .22, q.x-.05); // Box.
    light = max(light, max(abs(p.x) - .5, abs(p.y) - .22)); // Chopping off anything outside the tunnel... Kind of.
    ...
    objID = step(tube, light) + step(tube, walls) + step(light, tube)*step(light, walls)*3.;
   

    // Returning the minimum of the three objects.
    return min(min(tube, walls), light);
}
...
// I tried to make it as concise as possible. Whether that translate to speed, or not,
// I couldn't say.
...
vec3 rotHue(vec3 p, float a){
    ...
    mat3 hr = mat3(0.299,  0.587,  0.114,  0.299,  0.587,  0.114,  0.299,  0.587,  0.114) +
              ...
              mat3(0.168,  0.330, -0.497, -0.328,  0.035,  0.292,  1.250, -1.050, -0.203) * s;
                             
    return clamp(p*hr, 0., 1.);
}

// Screen pattern. Simple, but effective. The idea to go with this was inspired by Dmitry Andreev's really
// cool "pixelScreen" shader, here: https://www.shadertoy.com/view/XdG3Wc
//
// His pixel pattern is a little fancier, mainly because he's using way more code... The fact that he won 
...
float DotPattern(vec2 p){
    ...
    float c = dot(sin(p/4. - cos(p.yx/.2 + time/4.)), vec2(.5));
...
void main(void) {
    ...
    // Camera Setup.
    vec3 ro = camPath(time*1.5); // Camera position, doubling as the ray origin.
    vec3 lk = camPath(time*1.5 + .1);  // "Look At" position.
    vec3 lp = camPath(time*1.5 + 2.) + vec3(0, 2, 0); // Light position, somewhere near the moving camera.
    ...
    if(t<FAR){
        ...
        // I should probably get rid of that "if" sigh later, but it seems OK for now.
