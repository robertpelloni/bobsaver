...
// original https://www.shadertoy.com/view/3tfGWs
...
float tunnel(vec3 p)
{
    p.x *= 1. + cos(p.z + time)/7.;
    p.y *= 1. + sin(p.z + time)/7.;
    float size = .2;// + mouse*resolution.xy.x/resolution.x*.5;
...
vec3 doLighting( in vec3 pos, in vec3 nor, in vec3 rd, in float dis, in vec3 mal , in vec3 li)
{
    ...
    // fog    
    //----------------------------
    //col = mix(hueShift(col, mouse*resolution.xy.y/resolution.y*2.0), col, exp(-0.03*dis*dis));
