...
// original https://www.shadertoy.com/view/3tX3R8
...
Todo: get the AA size with the derivative of groundP instead of this %&*$ty
...
float ground(vec2 pin)
{
    pin.x = abs(pin.x);
    vec2 p = groundP(pin);
    vec2 p2 = groundP(pin-vec2(1./resolution));
...
void main(void)
{
    ...
    smoothLevel = 0.7+(4.5-0.7)*(0.5-0.5*cos(time*0.2));
    ...
    float aaSize = 22.*zoom*(1.0/length(resolution))/tilesize;
    ...
    // Film grain effect
    float hash = hash13(vec3(gl_FragCoord.xy, time*1500.+50.));
    ret.z *= 1.-hash*0.13*(1.-ret.y);
    ret.x += hash*0.13*ret.y;

    // Convert to RGB
    glFragColor = vec4(hsv2rgb_smooth(ret), 1.0);
}
