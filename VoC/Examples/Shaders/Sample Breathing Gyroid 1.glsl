...
// original https://www.shadertoy.com/view/ssBGDG
...
float scene (vec3 p)
{
    ...
    if (length(p) - 1.1 < ground) {
        p.xz *= r2d (5.*time);
        p.yx *= r2d (7.*time);
        vec2 modulation = vec2 (2. + .75*cos(time),
                                1.23 + .55*sin(time));
        float thickness = .125 *(1./(1. + 5.*( cos(time)*.5 + .5 )));
        float bias1 = 1.3;
        float bias2 = .3;
        float scale = 5.;
        ...
        float r = 1.125 + .05*cos (20.*p.y + 5.*time);
        float ball = ball (p, r);
...
void main(void)
{
    ...
    vec2 yaw = .75*vec2 (PI*cos (4.*mouse.x*resolution.x/resolution.x),PI*sin (4.*mouse.y*resolution.x/resolution.x));
    float pitch = 1.;
