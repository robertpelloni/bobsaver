...
// original https://www.shadertoy.com/view/td2SRW
...
float shape(vec3 p)
{
    ...
    float r = 1. + pow(cos(time*(BPM/60.)*pi*2.)*.5+.5,6.)*.2;
    return min(
        max(
            sdOctahedron(p,2.),
            -min(
...
float scene(vec3 p)
{
    ...
    float mode = clamp(sin(time*.3)*6.,0.,1.);
    
    float radial=-atan(p.x,p.z);
    
    p.xz = rotate(p.xz, radial * mode * 2.);
    
    for(float i=0.;i<mod(time*BPM/60.,3.);++i)
        p=p.yzx;
    
    p.xy = rotate(p.xy, tick(time*BPM/60.)*pi*.5);
...
vec3 trace(vec3 cam, vec3 dir)
{
    ...
    for(int bounce=0;bounce<3;++bounce)
    {
        ...
        if(abs(k)<.001)
        {
            ...
            {
                h = mod(abs(h)-vec3(0,0,.5+tick2(time*BPM/120.)),2.)-1.;
            ...
            {
                float fresnel = pow(1.-dot(-dir,n),5.);
                vec3 color = sin(time+vec3(0,1,2)+h)*.5+.5;
