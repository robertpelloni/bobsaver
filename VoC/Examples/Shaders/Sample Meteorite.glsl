...
void main()
{
    ...
    glFragColor.a = 1.0;
    if (fd<line) 
         glFragColor.xyz = d*mix(vec3(0.21,0.19,0.0),vec3(1.0,0.99,1.0),noise3(rt*128.0)*0.9+noise3(rt*8.0)*0.1);
    else glFragColor.xyz = d*mix(mix(vec3(1.0,1.0,0.9),vec3(0.8,0.79,0.7),noise3(rt*128.0)*0.9+noise3(rt*8.0)*0.1),vec3(1.0),pow(fd+0.5,10.0));  
