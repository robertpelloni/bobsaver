...
// original https://www.shadertoy.com/view/NtVXDh
...
void voxel (vec2 uv, vec2 id, vec2 lc, vec2 sh, inout vec4 C)
{
    ...
    float th = mix (                                                                                     //sphere track depth
         ...
         , s =  pow(noise(vec2(h21(ii)*ii+time*.5)),8.)*.75 //small picks of altitude 
         , hg = (pow(noise(ii*.2*rot(1.) - time*.02),4.)-.5)*2.     //large noise
                 + s;
...
void Draw (vec2 uv, inout vec4 C)
{
    ...
    vec2    st = vec2((uv.x+uv.y*1.73),(uv.x-uv.y*1.73)) //isometic coordinates
         ,  sc = toSqr(st+sp)-uv+vec2(.0,2.+noise(vec2(time*5.,0.))*.2) //sphere center
...
void main(void)
{
    ...
    vec2 rz = resolution.xy
        ,uv = (g+g-rz)/-rz.y;
    uv += -.5;
    uv *= 1.+sin(time*.3)*.25; //camera scale
    sm = 3./resolution.y;
    
    T = -time*4.-((sin(time*.5)+1.)*5.); //local timer with speed variation
