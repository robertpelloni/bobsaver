...
const int   complexity  = 3; //color level  
...
void main( void )
{
    ...
    vec2 pos = (2.0*gl_FragCoord.xy/max(resolution.x, resolution.y)-1.0)*pi;
        vec3 camPos = (vec3(cos(t*(-trpi)), sin(t*(rcpi)), 1 ))*(pisq+sin(time/picu)*pi);
    ...
    {
            vec3 tv = rotate(camPos + (rayDir*(i)),camDir,i*sin(time/pi)*pi);
