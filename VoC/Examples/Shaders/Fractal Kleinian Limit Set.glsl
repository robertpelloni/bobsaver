...
// original https://www.shadertoy.com/view/7dK3WD
...
void main(void)
{
    ...
    float time = mod(0.2*time, float(NUM_PARAMS));
    ...
    float l   = length(p);
    float hue = fract(0.75*l-0.3*time) + .5;
    float sat = tanh(4.*l);
    ...
    col = postprocess(col, gl_FragCoord.xy/resolution.xy);
