...
// original https://www.shadertoy.com/view/3ssXR8
...
out vec4 glFragColor;
 ...
 vec2 raygl_sdf_map_1(in vec3 p) {
   ...
   for(int i = 0; i < 4; ++i) {
     ...
     if(r > 2.0) continue;
     theta = atan(z.y / z.x) ;
     phi = asin(z.z / r)  + time * 0.3  + 0.9 * mix(raygl_BMOD_1.x,1. - raygl_BMOD_1.y, 0.66 + r4.x * 0.33);
 ...
 float raygl_shape_1( out vec3 pt, out vec3 nor) {
   raygl_CMOD_1 = vec3(2.9 ,2.9,0. );
   vec3 ro = vec3(4. + time * 0.33, 1.7 , 2.9);
...
void main(void) {
   ...
   vec3 pt;
   vec3 nor;
   float m = raygl_shape_1(pt, nor);
   raygl_Surface_4 s;
   float z = 0.;
   float mz = raygl_sdf_map_1(vec3( pt.x, 1.0, pt.z)).x;
   float q = raygl_fbm_(pt.xz * 0.1);
   z =  raygl_fbm_(pt.xz * 0.2 + vec2(0.01, 0.02) * time + vec2(0.2 * bms.y,0.2 * bms.z)  + q);
