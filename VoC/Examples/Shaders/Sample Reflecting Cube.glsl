...
// original https://www.shadertoy.com/view/Mdf3DH
...
float function(vec3 position) {
    ...
    vec3 pos = rotateY(rotateX(position,time2*0.7),time2*0.8)+vec3(-0.6,0.6,-0.6)+vec3(60.0*sin(time2*0.1),0.0,0.0);
...
vec3 pos2 =rotateY(rotateX(position+vec3(0.0,0.0,0.2),time2*1.7),time2*1.8);
...
float l=length(nearPlanePosition - cameraPosition);
    ...
    glFragColor = vec4(vec3(((color-0.5)*1.7+0.5)+0.2),1.0); 
