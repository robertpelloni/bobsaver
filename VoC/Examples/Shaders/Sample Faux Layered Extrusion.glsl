...
// original https://www.shadertoy.com/view/Wsc3Ds
...
/*
    ...
    a separate sample. The main object, in this case, is an octagonal diamond 
    Truchet, but it can be anything simple. I've also sprinkled in some extra 
    beacon-like objects between the layers to enhance the illusion a bit.
    ...
    only takes a few minutes to figure it out.
...
// A diamond tile with have two circles cut out at opposite ends, and an octagon
...
//
//Uncomment the "SHOW_GRID" define and refer to imagary get better idea.
vec3 distFieldT(vec2 q){
    
    
    float d = 1e5;
    
    // Offsetting to make the tiles up with the grid lines. Normally, this extra step
    ...
    // Side length. Due to the symmetry, it's the side length of
    // octagon and diamond.
    float s = 1./(1. + sqrt2);
...
void main(void) {
    ...
    vec2 p = uv*gSc - vec2(cos(time/3.)*1.5, -.5*time);
    ...
    vec3 tx = vec3(1.0);//texture(iChannel0, pTx/gSc).xyz; tx *= tx;
    tx = min(smoothstep(-.1, .3, tx), 1.);
    tx *= lnPat*.5 + .7;
    
    // Top layer texture, combined with the top layer line pattern.
    vec3 tx2 = vec3(1.0);//texture(iChannel0, pTx2/gSc).xyz; tx2 *= tx2;
    ...
    float sFunc = (n2D3G(p/gSc*3. - time/4.)*.67 + n2D3G(p/gSc*6. - time/2.)*.33)*.2;
    ...
    // Without it, everything looks extruded, but flat.
    col = mix(col, min(col*1.5, 1.), (1. - smoothstep(0., sf*4., max(-d3d.x, d.x)))*.9);
    col = mix(col, vec3(1.5)*tx2, (1. - smoothstep(0., sf*4., max(-d3d.x, d.x + vsd)))*.9);
    ...
    if(rnd<.5 && rndS<.5){
        ...
        col = mix(col, min(col*3., 1.), (1. - smoothstep(0., sf*4., max(-d3d2.x, d2.x + vsd2)))*.9);
