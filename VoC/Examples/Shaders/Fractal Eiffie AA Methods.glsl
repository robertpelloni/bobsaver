...
// original https://www.shadertoy.com/view/XsSXDt
...
vec3 Light(vec3 so, vec3 rd, float px, float dist){
    ...
    float shad=ShadAO(so,light_dir,px),dif=dot(norm,light_dir)*0.5+0.5;
...
vec3 getRay(int RAY,float zoom){//creates the unrotated camera ray
    vec2 aa=vec2(floor((float(RAY)+0.1)*0.5),mod(float(RAY),2.0));
    return normalize(vec3((2.0*gl_FragCoord.xy+aa-size.xy)/size.y,zoom));
}
void main(){
    float zoom=5.0,px=2.0/(size.y*zoom);//find the pixel size
    ...
    vec3 col=vec3(0.0),bcol=vec3(gl_FragCoord.y/size.y);
    float t,d;
    
#ifdef USE_STANDARD_AA
    float t0=DE(ro)*rndStart(gl_FragCoord.xy)*0.5;
    ...
    for(int RAY=0;RAY<4;RAY++){
        ...
        for(int i=0;i<48;i++){
            ...
            t+=step;
            if(t>10.0 || d<0.0001)break;//hard stop
        }
    ...
    //march
    t=DE(ro)*rndStart(gl_FragCoord.xy);d=1.0;
    ...
    for(int i=0;i<64;i++){
        ...
        if(t>10.0 || od<0.0001)break;//hard stop
    }
    ...
    rd=cam*getRay(0,zoom);
    if(gl_FragCoord.x>size.x*0.5){//for comparison
        if(d<10.0*px*t)col=Light(ro+rd*t,rd,px*t,d);//single hit
        else col=bcol;
    }
    col=mix(vec3(0.0,1.0,0.0),col,smoothstep(0.0,1.0,abs(gl_FragCoord.x-size.x*0.5)));
#endif
