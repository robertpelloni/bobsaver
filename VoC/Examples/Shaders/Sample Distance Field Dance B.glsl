...
// original https://www.shadertoy.com/view/4ttBRX
...
void main(void)
{
  ...
  st = st* mat2(cos_factor, sin_factor, -sin_factor, cos_factor)+time/5.;
    
  d = length(abs(sin(abs(st*2.)+time))*
             (sin(abs(cos(st.x+sin(time))*sin(st*5.))*.8)/2.));
