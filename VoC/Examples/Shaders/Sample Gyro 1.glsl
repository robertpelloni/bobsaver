...
// original https://www.shadertoy.com/view/ltsXz7
...
void main(void) {
    vec4 p = vec4(gl_FragCoord.xy,0,1)/resolution.yyxy-.5, d,c; p.x-=.2; // init ray 
...
#define cyl(a,b,c) {v = vec4(a,b,c,0)/SQRT2_3; x1 = length(t.xyz-dot(t,v)*v.xyz)-.2*j; x2 = max(-x1,x2);}            
     ...
     glFragColor += vec4(1,0,0,0) * exp(-closest)*(.5+.5*cos(.5*T)); // thanks kuvkar ! 
}
