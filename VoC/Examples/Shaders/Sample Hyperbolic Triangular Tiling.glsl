...
// original https://www.shadertoy.com/view/4dyXzw
...
void main(void) {
    ...
    for(float i = 0. ; i < sampleNum ; i++){
        ...
        position = position * 2.;
        position *= 0.03 + abs(1. * sin(time) * sin(time));
        position *= 1.0;
        ...
        if(d == 0){
            ...
            if(mod(float(d), 2.) == 0.){
                sum += hsv2rgb(vec3(0.01 * float(d), 1., 1.));
            }else{
                sum += hsv2rgb(vec3(0.7 + 0.01 * float(d), 1., 1.));
            }
        }
    }
