...
// res_b = (res_b - fract(res_b) ) / psn;
    ...
    if( nhdr0_avg >= 0.175 && nhdr0_avg <= 0.280) { r0 += 0.03; }

    if( nhdr0_avg >= 0.000 && nhdr0_avg <= 0.084 ) { r0 -= 0.03; }
    if( nhdr0_avg >= 0.085 && nhdr0_avg <= 0.192 ) { r0 -= 0.03; res_b = 0.5; }
    if( nhdr0_avg >= 0.142+cs && nhdr0_avg <= 0.220+cs ) { r0 -= 0.01; res_b = 1.0; }
    if( nhdr0_avg >= 0.219 && nhdr0_avg <= 0.302 ) { r0 -= 0.03; res_g = 0.5; }
    if( nhdr0_avg >= 0.400 && nhdr0_avg <= 1.000 ) { r0 -= 0.01; res_g = 1.0; }
