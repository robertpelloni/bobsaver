...
// original https://www.shadertoy.com/view/4t3BW4
...
void main(void) {
    ...
    vec2 oP = uv*5.;    
    oP *= r2(sin(time/8.)*3.14159/8.);    
    oP -= vec2(cos(time/8.)*0., -time);
    ...
    float pat3 = clamp(sin((oP.x - oP.y)*6.283*resolution.y/24.)*1. + .9, 0., 1.)*.25 + .75;
    // Resolution based falloff... Insert "too may different devices these days" rant here. :D
    float fo = 5./resolution.y;
