...
// original https://www.shadertoy.com/view/3lyXDm
...
vec2 z,v,e=vec2(.000035,-.000035);float t,tt,b,g,g2,bb; vec3 np,bp,pp,po,no,al,ld;//global vars 
...
{ 
  ...
  vec2 h,t=vec2(bo(p,vec3(5,1,3)),3); //Blue box base, east Anglia's finest shape since Allan Partidge's striped ties.
  t.x=max(t.x,-(length(p)-2.5)); //Dig %&*$ing hole in box base, because sanctions against the city of Norwich for breeding hipsters should be debated in parliament.
  t.x=max(abs(t.x)-.2,(p.y-0.4));  //Onion trick + cut that %&*$ with horizontal plane . 
  h=vec2(bo(p,vec3(5,1,3)),6);  //Another %&*$ing box this time white. Nothing exciting, neither here nor in Norwich. 
  h.x=max(h.x,-(length(p)-2.5)); //Dig another %&*$ing hole, this time in the white box. 
  h.x=max(abs(h.x)-.1,(p.y-0.5)); //Onion trick + cut that %&*$ with horizontal plane.
  ...
  h.x=max(h.x,-(length(p)-2.5)); //Bored of all this digging hole bull%&*$ but had to be this way with the onion trick %&*$ing with my drilling rig
  t=t.x<h.x?t:h; //Merge geometries while retaining material ID
...
vec2 mp( vec3 p )
{ 
  pp=bp=p; //Setup bull%&*$ new postions pp and bp based on p,  
  bp.yz=p.yz*=r2(sin(pp.x*.3-tt*.5)*.4); //add twist along z axis (due to line below bull%&*$ x axis will become z axis. I know, dumb as %&*$, but not as dumb as a vegetarian sausage)
  p.yz*=r2(1.57);//This is some real bull%&*$, whoever thought it was a good %&*$ing idea to rotate the whole %&*$ing thing rather than flip variables in fb, is a %&*$ing bell-end... Ah, yes, that would be me.
  ...
  p.x=mod(p.x-tt*2.,10.)-5.; //Modulo along z axis (x is now z, yeah I know, still dumb as %&*$, but not as dumb as giving 2.5k fines to the homeless. Yeah I'm looking at you, Oxford, you cruel bastard.
  vec4 np=vec4(p*.4,.4); //make new position np a vec4 for fractal so we can track the scale changes in the .w scalar and reuse that %&*$ later to spank the distance field into shape to avoid artifact 
  for(int i=0;i<4;i++){ //Less is more with iterations, and also prison time.
    np.xyz=abs(np.xyz)-vec3(1,1.2,0); //Kaleidoscopic mandelbull%&*$
  ...
  vec2 h,t=fb(abs(np.xyz)-vec3(2,0,0),5.);//%&*$ it we still gonna abs symetry kaleidoscope that %&*$ one more time for more details
  ...
  g2+=0.1/(0.1*h.x*h.x*(1000.-b*998.)); //Balkhan's super sick glow trick, with my added sweep along z axis hot sauce. Things are getting too %&*$ing spicy for the pepper!
  t=t.x<h.x?t:h; //Merge geometries while retaining material ID
  ...
  t=t.x<h.x?t:h; //Merge geometries while retaining material ID
  h=vec2(length(cos(bp.xyz*.6+vec3(tt,tt,0)))+0.003,6); //Make it %&*$ing rain sparkly little %&*$ing particles. Because everything looks better when sprinkled with glitter, even Norwich city centre.
...
void main(void)
{
  ...
  co=fo=vec3(.1,.2,.3)-length(uv)*.1-rd.y*.2; //background is blueish with vignette and subtle vertical gradient based on ray direction y axis. It's grey like the heart of people from Norwich.
  ...
  if(z.y>0.){ //Yeah we hit something, unlike you trying it with that mediocre looking waitress
    ...
    al=mix(vec3(0.1,0.2,.4),vec3(0.1,0.4,.7),.5+0.5*sin(bp.y*7.)); //by default albedo is mix between white and blue but the mix is done with noise of terrain so give really nice details in hues of blue
    if(z.y<5.) al=vec3(0); //material ID < 5 makes it black
    if(z.y>5.) al=vec3(1); //material ID > 5 makes it white
    if(z.y>6.) al=mix(vec3(1,.5,0),vec3(.9,.3,.1),.5+.5*sin(bp.y*7.)); //Material ID 7 gives us greadient orange -> red colour
    ...
    co=mix(sp+mix(vec3(.8),vec3(1),abs(rd))*al*(a(.1)*a(.2)+.2)*(dif+s(2.)),fo,min(fr,.2));//Building the final lighting result, compressing the %&*$ outta everything above into an RGB %&*$ sandwich
    co=mix(fo,co,exp(-.0003*t*t*t));//Fog soften things, but it won't stop failed art students from becoming dictators.
  }
  glFragColor = vec4(pow(co+g*.2+g2*mix(vec3(1.,.5,0),vec3(.9,.3,.1),.5+.5*sin(bp.y*3.)),vec3(0.65)),1);// Naive gamma correction
} 
