...
// original https://www.shadertoy.com/view/wtdXzN
...
float fbm(vec2 p){
    float n = Noise(p*4.-vec2(time*0.1,0));
    n += Noise(p*8.+vec2(time*0.3,0))*0.5;
    n+= Noise(p*16.-vec2(time*0.5,0))*0.25;
    n+= Noise(p*32.+vec2(time*0.8,0))*0.125;
    n+= Noise(p*62.-vec2(time*2.,0))*0.0625;
    return (n/2.);
}
