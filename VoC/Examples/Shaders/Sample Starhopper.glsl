...
// original https://www.shadertoy.com/view/wdjGRz
...
float hop() {
    return pow(max(0.45 - 0.55 * cos(speed * time), 0.0), 2.0);
}
...
float throttle() {
    return smoothstep(0.95, 0.2, cos(speed * time));
}
...
float drawGround(vec2 pos) {
    ...
    shade *= 1.0 + power * 2.0 * fbm1(vec3(6.0 * pos / (r + 1.0 / (1.0 + r)), 0.5 * r - 15.0 * time));
...
vec3 drawFlame(vec3 color, vec3 eye, vec3 ray, float dist) {
    ...
    for (int i = 0; i <= 20; ++i) {
        ...
        float diamonds = flameDiamonds * pow(z, 1.3) * (1.0 + 0.05 * hash1(time));
        diamonds = abs(cos(0.5 * tau * diamonds));
        r = r * (1.2 - 0.2 * diamonds);
        z *= 1.0 + 0.3 * hash1(time + 10.0);
        float a = 1.0 - z * z - r * r;
...
void main(void) {
    ...
    float roll = 0.1 * sin(3.0 * speed * time) * smoothstep(0.0, 0.5, h);
    float c = cos(roll);
    float s = sin(roll);
    ...
    glFragColor = vec4(mix(backColor, hopperColor, hit), 1.0);
}
