...
// original https://www.shadertoy.com/view/llGGzh
...
/*
    ...
    face sections in accordance to the surface normal orientation. It's standard way to apply 2D patterns to 
    a cuboid-based surface, and is contained in the "texFaces" function.
...
float map(in vec3 p) {
   ...
   vec3 q = p - ip; // Break space into cubes. Equivalent to: fract(p) - .5.
   ...
   //
   // Reusing "p" to save an extra declaration. That's my oldschool brain at work... 
   // probably making things worse. :)
   p = abs(q); 
   q = step(p.yzx, p.xyz)*step(p.zxy, p.xyz)*sign(q); // Used for cube mapping also.
   ...
   // directions... See Fizzers article explanation. It's better. :) By the way, it's
