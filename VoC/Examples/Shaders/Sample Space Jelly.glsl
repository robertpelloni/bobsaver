...
// original https://www.shadertoy.com/view/WlBGR1
...
float map(vec3 p) {
  ...
  p.xz *= rot(p.y*sin(time3*12.5 + length(p.xz)*max(0.2,sin(time3))*0.8)*0.02 * smoothstep(0.0,0.5,abs(fract(time3*0.2)-.5)));
    
  float dist=100.0;
  p = (fract(p/dist+.5)-.5)*dist;

  float d=10000.0;
  float s=8.0 + curve(time3, 0.7)*10.0;
  for(int i=0; i<5; ++i) {
    
    float t=tick(time3, 0.8 + 0.7*float(i)) * 0.25;
    p.xy *= rot(t);
    p.yz *= rot(t*.7);
    p.xy=abs(p.xy);
...
void cam(inout vec3 p) {
  ...
  float t=time*.4 + curve(time3, 1.9) * 3.0;
  p.xy *= rot(t);
  p.yz *= rot(t*1.2);
}
...
{    
  ...
  float time2 = time*1.0 + 23.0;
  time3 = tick(time2*0.3, 1.7) + time2*0.3;

  vec3 s=vec3(0,0,-50);
  vec3 r=normalize(vec3(-uv, 0.5 + curve(time3, 0.8)));
  
  cam(s);
  cam(r);
  
  float dither = mix(1.0,rnd(uv + fract(time3)),0.1);
