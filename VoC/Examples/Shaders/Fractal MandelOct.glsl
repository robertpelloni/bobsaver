...
// original https://www.shadertoy.com/view/wlcfR8
...
// MandelOct by nimitz (twitter: @stormoid)
...
{    
    ...
    mo*=4.14;
    mo.y *= 0.6;
    mo.x += sin(time*0.2) + 2.5;
    
    vec3 ro = vec3(12.,-0.0, 70. + sin(time*0.5 + .5)*10.);
    vec3 rd = normalize(vec3(p,-fov));
    
    //an = smoothfloor((time + 20.)*FLIP_SPEED, FLIP_CURVE)*6.2831853*FLIP_REV;
    an = smoothfloor((time + 6.)*FLIP_SPEED, FLIP_CURVE)*6.2831853*FLIP_REV;
