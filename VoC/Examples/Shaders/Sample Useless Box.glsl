...
// original https://www.shadertoy.com/view/XdBSD3
...
float focus = 9.0+2.0*cos(0.35*time2);
float far = 30.0;
...
float alienHand(vec3 p, float cycle)
{
    ...
    float a1 = sin(3.6*time2);
    float a2 = sin(0.7*time2+1.333)*a1;
...
void main(void) 
{
    ...
    vec3 cp = vec3(d*cos(t), 6.0+sin(0.2*time2), d*sin(t)); // circle center
