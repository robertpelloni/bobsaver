...
// original https://www.shadertoy.com/view/Mtj3DV
...
vec3 shade( in vec3 ro, in vec3 rd ){
    ...
    if (t < 0.0) {
        ...
        vec3 k1 = rotX(time*0.7) * (rotY(time*0.3)*vec3(0, 0, 1.0));
        vec3 k2 = rotX(time*1.3) * (rotY(time*0.9)*vec3(0, 0, 1.0));
...
void main(void) {
    ...
    if (max(mouse.x, mouse.y) > 20.0) { 
        thetax = (mouse.y - .5*resolution.y) * 5.0/resolution.y; 
        thetay = (mouse.x - .5*resolution.x) * -10.0/resolution.x; 
    }
