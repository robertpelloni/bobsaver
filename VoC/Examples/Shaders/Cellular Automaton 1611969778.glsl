...
// res_b = (res_b - fract(res_b) ) / psn;
    ...
    vec3 nhdr0 = nhd( vec2(8.0,3.0), psn, 0.0, 0 );
    ...
    if( nhdr0_avg >= 0.194 && nhdr0_avg <= 0.271 ) { r0 += 0.03; }

    if( nhdr0_avg >= 0.000 && nhdr0_avg <= 0.124 ) { r0 -= 0.03; }
    if( nhdr0_avg >= 0.125 && nhdr0_avg <= 0.199 ) { r0 -= 0.03; res_b = 1.0; }
    if( nhdr0_avg >= 0.226 && nhdr0_avg <= 0.298 ) { r0 -= 0.03; res_g = 0.5; }
    if( nhdr0_avg >= 0.337 && nhdr0_avg <= 0.850 ) { r0 -= 0.01; res_g = 1.0; }
