...
// original https://www.shadertoy.com/view/XtVGDc
...
vec4 pixelColor(vec2 gl_FragCoord) {
    gl_FragCoord *= tan(radians (FOV) / 2.0);
    ...
    vec3 
        ...
        v = cross(vpn, u),
        vcv = (ro + vpn),
        scrCoord = (vcv + gl_FragCoord.x * u * resolution.x/resolution.y + gl_FragCoord.y * v),
...
void main(void) {
    glFragColor = pixelColor(gl_FragCoord.xy / resolution.xy - .5);
}
