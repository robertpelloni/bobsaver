...
// original https://www.shadertoy.com/view/wd2yDm
...
vec3 shade(vec3 p, vec3 dir, float h) {
    vec3 col=vec3(1.,0.,0.)*m*pow(smoothstep(.3,.5,abs(.5-fract(mc*.5+h*.5+time*.5))),2.);
    ldir=normalize(ldir);
    vec3 n=normal(p.xz);
...
vec3 march(vec3 from, vec3 dir) {
    ...
    col=mix(col,vec3(.2*(1.-p.y*.25)),f)*min(1.,time*.5);
