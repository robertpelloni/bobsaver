...
vec3 nhvec( vec2 nbhd, float psn, float thr, int col ) {
    float dist = 0.0;
    float cval = 0.0;
    float c_total = 0.0;
    float c_valid = 0.0;
        ...
        for(float j = -nbhd[0]; j <= nbhd[0]; j += 1.0) {
            ...
            if( dist <= nbhd[0] && dist > nbhd[1] && dist != 0.0 ) {
                ...
                if( cval > thr ) {
                    ...
                    vx += ( ((i > 0.0) ? (psn) : (i < 0.0) ? -(psn) : 0.0) * (1.0) );
                    vy += ( ((j > 0.0) ? (psn) : (j < 0.0) ? -(psn) : 0.0) * (1.0) );

                } } } }

    return vec3( vx, vy, c_total );
}
vec3 export_nh() {
...
void main() {
    ...
    vec3 rv0 = nhvec( vec2(12.0,0.0), psn, 0.5, 0 );

    float rx = (rv0[0] / (rv0[2]*0.5)) + 0.5;
    float ry = (rv0[1] / (rv0[2]*0.5)) + 0.5;
