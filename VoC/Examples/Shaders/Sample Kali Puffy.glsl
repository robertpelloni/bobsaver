...
// original https://www.shadertoy.com/view/ltdyDl
...
void main(void) {
  ...
  // Set globals
  time2 = mod(time, 600.);
  ani_mouth = sin(time2 * 6.);
  puff = -.03+.5*smoothstep(.945, .95, abs(sin(time2 * .1)))+ani_mouth*.04;
  ...
  // Camera
  vec2 mouse = (mouse*resolution.xy.xy / resolution.xy - .5) * 4.;
  float tcam = (time2+67.)*.05;
  float zcam = smoothstep(.7, 1., cos(tcam)) * 1.8 - .3;
  zcam -= smoothstep(.7, 1., -cos(tcam)) * 1.6;
  //if (mouse*resolution.xy.z < .1) mouse = vec2(sin(time * .15)*ar, zcam);
