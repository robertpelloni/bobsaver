...
// original https://www.shadertoy.com/view/flSyDt
...
float caustic(vec2 uv,int octaves,float st)
{
    ...
    for(int i = 0; i < 10; i++)
    {
        val = cloud(vec3(uv.xy,time),octaves);
        uv -= val.xy*st;
    }
    return exp(cloud(vec3(uv.xy,time),octaves).w*caustic_roughness - caustic_roughness/2.0);
}

void main(void)
{
    vec2 uv = (3. * gl_FragCoord.xy - resolution.xy) / resolution.y;

    // Output to screen
    glFragColor = vec4(caustic(uv,24,caustic_strength+caustic_chormatic_aberation),caustic(uv,24,caustic_strength),caustic(uv,24,caustic_strength-caustic_chormatic_aberation),1.);
}
