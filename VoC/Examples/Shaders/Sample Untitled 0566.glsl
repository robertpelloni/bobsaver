...
// original https://www.shadertoy.com/view/WtscW4
...
void main(void)
{
    ...
    vec3 matcol = mix(vec3(0.9,0.4,0.3), vec3(0.3,0.4,0.8), smoothstep(-1.,1.,sin(d1*5.+time*2.)));
    matcol = mix(matcol, vec3(0.5,0.4,1.0), smoothstep(0.,1.,sin(d2*5.+time*2.)));
    if (dl) matcol = mix(vec3(1),matcol,0.1)*.2+0.1;
    vec3 col = matcol*fact*ss + pow(fact,10.);
    if (lz) col = vec3(4);
    glFragColor.xyz = col*atten + glo*glo + fogcol*glo;
    
    glFragColor.xyz = mix(glFragColor.xyz, fogcol, fog);
    if(!dl)glFragColor.xyz = abs(erot(glFragColor.xyz, normalize(sin(p*2.)),0.2*(1.-fog)));
    if(!trg&&!dl)glFragColor.xyz+=dlglo*dlglo*.1*vec3(0.4,0.6,0.9);
    glFragColor.xyz = sqrt(glFragColor.xyz);
    glFragColor.xyz = smoothstep(vec3(0),vec3(1.2),glFragColor.xyz);
}
