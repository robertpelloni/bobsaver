...
void main() {
    ...
    vec2 nhg = nh_val ( 3.0, 1.0, 1 );
    vec2 nhb = nh_val ( 6.0, 5.0, 2 );
    float nhgv = (nhg[0] / nhg[1]);
    float nhbv = (nhb[0] / nhb[1]);

    float dev0 = (cvr-(nh1v / ( sqrt(nh1[1]/ 200.0) ) ));
    float rd01 = clamp( (nh1v * (2.0)) - nhbv ,0.0,1.0);
    float rd10 = clamp( (nhbv * (2.0)) - nh1v ,0.0,1.0);
    float rdr01 = (nh1[1]<nhb[1]) ? rd01 : rd10;
