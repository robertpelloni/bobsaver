...
void main( void ) {
    ...
    Sphere_radius = min(resolution.x, resolution.y)*0.15;
    ...
    if (dist_from_center < Sphere_radius) { 
        // Wouter van Nifterick, 2013-08-15: we only have to calculate surfaceZ when we hit the sphere
        float surfaceZ = sqrt(Sphere_radius*Sphere_radius - dist_from_center*dist_from_center);
        SurfacePosition = vec3(gl_FragCoord.xy, surfaceZ);
