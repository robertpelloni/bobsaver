...
// original https://www.shadertoy.com/view/ll2GRt
...
void main(void) {
    ...
    #ifdef INVERT
        glFragColor = 1.0-1.2*vec4( vec3(0.5*fbm(uv*val*8.0)+0.02*r(uv.x,uv.y)), 1.0 );
    #else
        glFragColor = 1.6*vec4( vec3(0.5*fbm(uv*val*8.0)+0.02*r(uv.x,uv.y)), 1.0 );
    #endif
}
