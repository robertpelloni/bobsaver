...
// original https://www.shadertoy.com/view/tdfGW4
...
// Fork of "The Cave" by BoyC. https://shadertoy.com/view/MsX3RH
// 2018-12-30 22:01:35
...
vec3 nmap(vec2 t, sampler2D tx, float str)
{
    ...
    float xy=0;//texture(tx,t).x;
    float x2=0;//texture(tx,t+vec2(d,0)).x;
    float y2=0;//texture(tx,t+vec2(0,d)).x;
...
void main(void)
{
    float time=time/3.0+291.0;//+43.63/3.0;
   ...
   //mux   
   ctx =max(ctx, zStripes*(sin(1.6+0*resolution.x/100.)));

    if (mouse*resolution.xy.xy==vec2(0)) //startup
    {
       ctx =mix(ctx,ctxDots,sin(time/2.));
    }
    else
       ctx =mix(ctx,ctxDots,0*resolution.y/resolution.y);  
    ...
    glFragColor = col*min(1.0,fade*10.0)+w; //*diff
