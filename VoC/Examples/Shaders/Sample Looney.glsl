...
// original https://www.shadertoy.com/view/XssXz2
...
float prim(vec2 p, float d, float n) 
{
    ...
    #ifdef ACID
    p.x += n*0.01+sin(time)*0.5;
    p.y += n*0.01+sin(time)*0.5;
    #endif
...
vec4 draw(in vec2 p, in float n, in float x)
{
    ...
    float rot = x*0.1;
    #endif
    float t2 = log(size)*sin(time*0.4)*.5;
...
void main(void)
{
    vec2 p = (gl_FragCoord.xy/resolution.xy-0.5)*2.;
