...
// original https://www.shadertoy.com/view/WlKyzW
...
float map2(vec3 p) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // Offset space
        float anim = mod((time - dot(a.xy, vec2(1,-1)) / 6.) / 3., 1.);
...
float map(vec3 p) {
    //if (mouse*resolution.xy.x > 0. && mouse*resolution.xy.y > 0.) {
        pR(p.yz, (.5 - mouse.x*resolution.xy.y / resolution.y) * PI / 2.);
        pR(p.xz, (.5 - mouse.y*resolution.xy.x / resolution.x) * PI * 2.);
    //}
    return map2(p);
}
