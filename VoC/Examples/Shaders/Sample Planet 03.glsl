...
// original https://www.shadertoy.com/view/4tjGRh
...
float       SEA_EPSILON_NRM    = 0.1 / resolution.x;
...
vec3 llamelGetNormal( in vec3 ro ) {
    ...
    float d0=llamelMap(ro);
    vec3 dn=vec3(llamelMap(ro-e.xyy),llamelMap(ro-e.yxy),llamelMap(ro-e.yyx));
    vec3 dp=vec3(llamelMap(ro+e.xyy),llamelMap(ro+e.yxy),llamelMap(ro+e.yyx));
    return (dp-dn)/(length(dp-vec3(d0))+length(vec3(d0)-dn));
}
