...
// original https://www.shadertoy.com/view/ltBfDt
...
float flare(in float angle,in float alpha,in float time){
    ...
    float rotate = (0.1+sin(t+alpha*20.0 + n*1.5)+angle*9.5)*(1.5+0.2*n);
...
void main(void) {
    ...
    if(alpha >= 1.0){
        ...
        f = flare(angle,alpha*0.7,time+0.5);
        f *= 1.1;
    }else if(alpha < 0.001){
        f = alpha;
    }else{
        f = flare(angle,alpha,time)*1.8;
    }
    glFragColor = vec4(vec3(f,f*alpha,alpha),1.0);
}
