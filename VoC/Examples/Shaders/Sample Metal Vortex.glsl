...
// original https://www.shadertoy.com/view/wssXzr
...
float cog(vec3 p, vec3 tp ){
    ...
    bp.xy=rotate(bp.xy,sin(-time-tp.z*.05-p.x*.01)*10.);
...
vec3 lighting(vec3 sp, vec3 camPos, int reflectionPass){
    ...
    //lighting stuff
    //light pos
    vec3 lp = vec3(sin(time)*50.,cos(time)*50.,0.+time*10.);//cos(time));
    
    //light direction and color
    vec3 ld = lp - sp;
    //subtly change color
    vec3 lcolor = vec3(1.*sin(time*.2),1.*cos(time*.2),1.)/3.+vec3(1.);
...
//    dir.xy=rotate(dir.xy,-time/10.);
    ...
    glFragColor.rgb = (lighting(rsp,p,1)*.1+l)/clamp((d.x*.015),1.,10.);//d.yzw
}
