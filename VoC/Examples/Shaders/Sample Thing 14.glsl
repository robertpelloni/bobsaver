...
// original https://www.shadertoy.com/view/XtyfRc
...
float map(vec3 p)
{
    ...
    p.x -= 1.5*cos(time*.25);
    p.y += 1.5*sin(time*.5);
    rotate(p.xz, .25*sin(time));   
    vec3 op = p;
    
    float a = p.z*1.+ time*.0;
...
//    p.xy += vec2(cos(a), sin(a))*.6;
    rotate(p.xy, a*.1-time*.5);

    
    p.xy = fract(p.xy*.25)-.5;
    rotate(p.xy, p.z*1.5+time*10.*.25+sin(p.z+time)*3.14);
    p.xy = abs(p.xy)-.1051;
    rotate(p.xy, -p.z*1.5+time*-5.*.25+sin(-p.z+time*.5)*3.14);
    p.xy = abs(p.xy)-.051;
    
    
    float s = length(p.xy)-.0151+(.0071+1.0*.027*cos(p.z*1.*.5-(2.+time)));
...
void main(void)
{
    ...
    h -= h ;
    vec2 R = resolution.xy,u = (f-R*.5)/R.y;
