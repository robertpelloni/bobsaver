...
// original https://www.shadertoy.com/view/lsXGz8
...
{
    // get time
    float mils = fract(date.w);
    float secs = mod( floor(date.w),        60.0 );
    float mins = mod( floor(date.w/60.0),   60.0 );
    float hors = mod( floor(date.w/3600.0), 24.0 );
