...
// original https://www.shadertoy.com/view/lll3D8
...
const float PRECISSION =             0.76;            // larger radius needs smaller precission
...
vec3 scene_color(in vec3 ray, in vec3 norm)
{
    ...
    vec3 col = .25 + .25 * vec3(sin(a), sin(a*1.5), sin(a*2.));
#else
    vec3 col = vec3(.45, .5, .4);
#endif
