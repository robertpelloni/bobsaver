...
// original https://www.shadertoy.com/view/wlSSDt
...
vec3 render(vec3 ro, vec3 rd)
{
    ...
    for (int i = 1; i < LIGHT_NUM; ++i)
    {
        lights[i].pos.xy = 2.0 * vec2(sin(time+an),cos(time+an));        
        if (i%3==0) an += dt;
    }
...
void main(void)
{
    ...
    color = pow(color, vec3(0.95+0.4*sin(time)));
