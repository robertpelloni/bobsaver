...
void main() {
    ...
    vec3 nr2 = nhd( vec2(12.0,6.0), psn, 0.0, 0 );
    vec3 nr3 = nhd( vec2(28.0,18.0), psn, 0.0, 0 );
    vec3 nr4 = nhd( vec2(38.0,28.0), psn, 0.0, 0 );
    vec3 nr5 = nhd( vec2(58.0,52.0), psn, 0.0, 0 );
...
// ardf[i+j*nc] = arrw[i] - arrw[j];
    ...
    for(int i = 0; i < nc*nc; i++) {
        for(int j = 0; j < nc*nc; j++) {
            if( abs(ardf[j]) < abs(ardf[i]) && abs(ardf[j]) > 0.004 ) { 
                minvar4 = minvar3; 
                minvar3 = minvar2; 
                minvar2 = minvar1; 
                minvar1 = minvar0; 
    ...
    res_g = (res_g*res_g*r0);*/

    res_g += sn(ardf[minvar1])*s;
    res_g -= sn(ardf[minvar3])*s;
    res_g = (
        res_g * wt[0] +
        nrmw * wt[1] +
        nr0w * wt[2] +
    ...
    res_b += sn(ardf[minvar2])*s;
    res_b -= sn(ardf[minvar3])*s;
    res_b = (
        res_b * wt[0] +
        nrmw * wt[1] +
        nr0w * wt[2] +
