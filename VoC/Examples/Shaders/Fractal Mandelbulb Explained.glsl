...
// original https://www.shadertoy.com/view/XsXXWS
...
Color3 trace(vec2 coord) {
    float zoom = pow(200.0, -cos(time * 0.2) + 1.0);
    
    Point3 rayOrigin = Point3(2.0 * coord / resolution.xy - 1.0, -cameraDistance);

    // Correct for aspect ratio
    rayOrigin.x *= resolution.x / resolution.y;
    ...
    if (hit) {
        ...
        return mix(backgroundGradientCenterColor, backgroundGradientRimColor, sqrt(length((coord / resolution.xy - vec2(0.66, 0.66)) * 2.5)));
    }
}

void main() {
    // Euler-angle animated rotation    
    float pitch = sin(time * 0.2);
    float yaw   = cos(time * 0.3);
