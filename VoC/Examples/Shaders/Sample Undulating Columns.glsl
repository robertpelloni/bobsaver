...
// original https://www.shadertoy.com/view/WlXXDH
...
float pieceDist(vec3 p, float th, int n, float r, float rs)
{
    ...
    for(int i = 0; i < n; ++i)
    {
        r0 = (.07 + cos(float(i + n) + time / 2.) * .04) * rs;
        y += delta.y * r0;
        r += delta.x * r0;
...
void render( out vec4 glFragColor)
{
    ...
    glFragColor.rgb = diff;
    
    glFragColor.rgb *= mix(.5, 1., sh) * vec3(max(0., .6 + .4 * dot(n, ld)));
    
    float fr = pow(clamp(1. - dot(n, -rd), 0., 1.), 2.);
    
    //glFragColor.rgb += textureLod(iChannel0, r, 5.).rrr * fr * sh2;
    glFragColor.rgb += smoothstep(.4, .5, dot(ld, r)) * fr * sh2 * 1.6;
...
void main(void)
{
    ...
    //    {
            vec4 rc;
            render(rc);
            glFragColor.rgb += clamp(rc.rgb, 0., 1.);
    //    }
    
    glFragColor.rgb /= float(AA * AA);
    glFragColor.rgb /= (glFragColor.rgb + 1.5)*.43;
