...
// original https://www.shadertoy.com/view/cs2GWD
...
vec4 map(vec3 p,Grid grid){
  p-=grid.c;
  p.z+=.4*sin(2.*time+1.*fract(grid.h.z*28.)+.3*(grid.c.x+grid.c.y));
  ...
  if(grid.i==0){//2x2
    if(grid.h.y<.3){//speaker
      ...
      pt.y-=.5-smoothstep(-.5,.5,sin(time+c.x+grid.h.z*100.));
      ...
      float ani=smoothstep(-.5,.5,sin(time+grid.h.z*100.));
      ...
      if(d3<d){
        float led=1.-ani;
        led*=.5+.5*sin(time*exp2(3.+3.*grid.h.z));
    ...
    if(grid.h.y<.4){//fader
      ...
      float ani=smoothstep(-.2,.2,sin(time+grid.h.z*100.));
      pt.x-=mix(-.8,.8,ani);
      ...
      float ani=smoothstep(.8,.9,sin(10.*time-c.x*2.2+grid.h.z*100.));

      vec4 blah=vec4(d,0,0,0);
      float d3=length(pt-vec3(0,.22,.04))-.05;
      
      if(d3<blah.x){
        float led=ani;
        blah=vec4(d3,2,led,0);
      }
      ...
      if(d2<blah.x){
        blah=vec4(d2,5,fract(grid.h.z*8.89),0);
      }
    ...
    if(grid.h.y<.5){//knob
      ...
      float ani=smoothstep(-.5,.5,sin(2.*time+grid.h.z*100.));
      pt.xy*=r2d(PI/6.*5.*mix(-1.,1.,ani));
      ...
      float ani=sin(2.*time+grid.h.z*100.);
...
void main(void)
{
  ...
  float canim=smoothstep(-.2,.2,sin(time));
  ...
  if(march.isect.x<1E-2){
    ...
    if(mtl==0.){
      ...
      float lv=0.0;//texture(iChannel0,vec2(march.grid.h.z,0)).x*1.;
      ...
      led*=vec3(2,1,2);
      float plasma=sin(length(c)*10.-10.*time+march.grid.h.z*.7);
