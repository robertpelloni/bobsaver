...
// original https://www.shadertoy.com/view/Ms3SRS
...
vec3 lighting(vec3 p, vec3 lp, vec3 rd, float prec) 
{
    ...
    //if (mouse*resolution.xy.z> 0.) atten = exp( -0.001*d*d )-0.5;
    l /= d;
...
void main(void)
{
    ...
    glFragColor.rgb = vec3(0.47,0.6,0.76) * lighting(p, ro, rd, 0.1); 
    glFragColor.rgb = mix( glFragColor.rgb, vec3(0.5,0.49,0.72), 1.0-exp( -0.01*d*d ) ); 
}
