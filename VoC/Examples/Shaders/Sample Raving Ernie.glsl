...
//original https://www.shadertoy.com/view/XdBGzh
...
#define time2 (time*1.0)
#define tczu (time*8.0)
...
vec4 scene(vec3 p){
  ...
  float b=tcz/64.0,t=(1.0+pow(sin(b*4.0*PI),2.0))*0.5*0.25,s=(sin(time2*8.0*PI)*0.5*t),m=0.0,m1=0.0,m2=0.0;
...
vec3 shade(vec3 pos,vec3 n,vec3 eyePos,float m){
  ...
  if(r<1.0){
    c+=lightShade(pos,n,eyePos,color,vec3(1.0,0.25,0.25),vec3(cos(time2*4.0*PI)*5.0,1.0,sin(time2*4.0*PI)*5.0),s)*((sin((tcz/8.0)*PI*2.0)*0.5)+0.5);
  }else if(r<2.0){
    c+=lightShade(pos,n,eyePos,color,vec3(0.25,1.0,0.25),vec3(cos(time2*2.0*PI)*5.0,1.0,sin(time2*2.0*PI)*5.0),s)*((sin(time2*PI*2.0)*0.5)+0.5);
  }else{
   c+=lightShade(pos,n,eyePos,color,vec3(0.25,0.25,1.0),vec3(cos(time2*1.0*PI)*5.0,1.0,sin(time2*1.0*PI)*5.0),s)*((sin(time2*PI*1.0)*0.5)+0.5);
  }
  return c;
}
...
void main(void){
  ...
  float asp=resolution.x/resolution.y,m=0.0,roty=0.0,rotx=0.0;
  vec3 rd=vec3(0.0),ro=vec3(0.0),c=vec3(0.0);
  rd=normalize(vec3(asp*pixel.x,pixel.y,-2.0));
  ro=vec3(0.0,1.5,8.0);
  roty=(sin(time2*PI*0.125)*0.5);
  rotx=(cos(time2*PI*0.5)*0.25)-0.25;
