...
// original https://www.shadertoy.com/view/MlsfRX
...
vec3 scene(vec2 uv) {
    ...
    uv = vec2(sign(uv.y)*uv.x,abs(uv.y))-vec2(0.,.4+.05*cos(time+uv.x*6.28)); // 1
...
void main(void) {
    ...
    glFragColor = vec4( scene(uv), 1.0 );
    // uncomment to see the original pattern    
    //glFragColor = vec4( pattern(uv*2.), 1.0 ); 
}
