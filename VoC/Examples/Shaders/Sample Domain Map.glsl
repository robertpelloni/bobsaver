...
void main( void ) {
    ...
    glFragColor.zw = vec2(0);
    glFragColor.xy = uv_distorted3; // domain map, third iteration
    
    
    glFragColor = mix(glFragColor, vec4(0), vec4(border(uv_distorted2, 0.125)*0.5)); // blend domain borders from second iteration first
    
    glFragColor = mix(glFragColor, vec4(1), vec4(border(uv_distorted, 0.125))); // blend domain borders from first iteration
    
    //glFragColor = vec4(circle(uv, 5.)* 0.5); // seed
    //glFragColor += texture2D(backbuffer, uv_distorted)*mix(1., 0.6, filter); // that would be feedback from the first iteration
}
