...
// original https://www.shadertoy.com/view/4ss3DM
...
vec3 trace(vec3 rpos, vec3 rdir) {
    ...
    // Shade sky
    float fade = sqrt(length((gl_FragCoord.xy / resolution.xy) - vec2(0.7, 0.5)));
    vec3 sky = vec3(0.01, 0.01, 0.05) * (1.0 - fade);
    vec3 final = mix(sky, col.xyz, col.w) + sparkCol * brightness;
    return final + vec3(rand(vec2(gl_FragCoord.x * gl_FragCoord.y, time))) * 0.002;
}

// Ray-generation
vec3 camera(vec2 px) {
    vec2 rd = (px / resolution.yy - vec2(resolution.x/resolution.y*0.5-0.5, 0.0)) * 2.0 - 1.0;
    vec3 rdir = normalize(vec3(rd.x*0.5, rd.y*0.5, 1.0));
    return trace(vec3(-40.0, 20.0, -150), rdir);
}

void main(void) {
    glFragColor = vec4(pow(camera(gl_FragCoord.xy), vec3(0.4545)), 1.0);
}
