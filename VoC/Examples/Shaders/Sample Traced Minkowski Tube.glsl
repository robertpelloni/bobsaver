...
// original https://www.shadertoy.com/view/4lSXzh
...
void main(void) {
    ...
    rd.xy *= rot(sin(time*0.25)*0.5); // Very subtle look around, just to show it's a 3D effect.
    ...
    //if(sDist>1e-8){
        ...
        vec3 sn = -sign(sp)*vec3(pow(abs(sp.xy)*scale, vec2(power-1.)), 0.); // Minkowski normal.
