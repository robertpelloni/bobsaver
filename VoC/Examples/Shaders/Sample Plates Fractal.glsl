...
// original https://www.shadertoy.com/view/WtK3D3
...
#define time (sin(((sin(float(__LINE__))*GR/PI+GR/PI/E)*time+100.0)/100.0)*100.0)
...
void main(void)
{
    ...
    vec4 sample1 = vec4(0.0);//texture(iChannel1, uv);
    ...
    for(int i = 0; i < max_iterations; i++)
    {
        ...
        uv = iterate_square(uv, .5/resolution.xy, magnification);
        ...
        float stripes = map*1.0*PI;//*floor(log(max(resolution.x, resolution.y))/log(2.0));
        ...
        if(i > 0)
        {
            uv = iterate_mobius(uv, .5/resolution.xy, magnification, multa, offa, multb, offb);
            antispeckle *= smoothstep(0.0, 1.0/TIMES_DETAILED, sqrt(1.0/(1.0+magnification)));
            border = max(border, (smoothstep(1.0-1.0/GR/E/PI/2.0, 1.0, (cos(uv.y*PI)))));
    ...
    glFragColor.rgb = (color+accum/sum)*(1.0-border);
    glFragColor.a = 1.0;
}
