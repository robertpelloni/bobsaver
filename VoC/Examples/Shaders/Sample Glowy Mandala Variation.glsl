...
// original https://www.shadertoy.com/view/wsdyWf
...
// just applied some log polar mapping 

// Fork of "25 - Glowy Mandala" by Krabcode. https://shadertoy.com/view/tsdyDX
// 2020-10-05 16:19:40
...
float getSineNeighboursSum(vec2 cv){
    float sum = 0.;
    cv.y = fract(cv.y*4.)+.5;
    for(float y = cv.y-2.; y <= cv.y+2.; y++){
       sum += cubicPulse(0., .4, abs(getSine(cv.x, y))/lg);
    }
    return sum;
}
...
void main(void)
{
    ...
    colorPoint[colorsPerGradient] gradient = colorPoint[](
        colorPoint(0.0, hexToRgb(0x6a2c70)*.0),
        colorPoint(0.25, hexToRgb(0x2a2c70)),
    ...
    t = time;
    lg = log(length(cv));
    vec2 mandalaCoord = vec2(cos(22.*atan(cv.y, cv.x)), lg - time*0.5);
    
    mandalaCoord.x -= sin(time*0.2)*lg*(2. + length(cv)*1.) + sin(time);
    
    
    //mandalaCoord.y *= 1. + sin((mandalaCoord.y)*.02)*0.2;
    //mandalaCoord.x += 0. + sin(time + floor(mandalaCoord.y)*44.2 + floor(mandalaCoord.x*3.14) )*5.4;
