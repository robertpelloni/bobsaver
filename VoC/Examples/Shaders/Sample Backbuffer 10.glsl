...
void main( void ) {
    ...
    float circle = 0.4-distance((position*5.0),vec2(mouse*5.0));
    circle = clamp(circle*Perlin(position*3.0),0.0,1.0)*0.3;
    circle += float(texture2D(backbuffer,position+vec2(0.0,-0.002)+(gradient*0.01)+(-(Perlin(position*10.0+time)-0.5)*0.005)));
    circle *=0.97;
    glFragColor = vec4(circle);
