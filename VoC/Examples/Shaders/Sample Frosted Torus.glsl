...
// original https://www.shadertoy.com/view/Mds3Rr
...
float f(vec3 p) {
    ...
    float md = mix( cf(p), d, .72 + sin(time) * .185 );
...
void main(void)
{
    ...
    vec3 lp = normalize(vec3(sin(time)*10.0, 1.0+abs(sin(time*0.5))*20.0, 10.0*cos(time))-p);
    ...
    glFragColor.xyz = clamp(csb(mix(cA, cB, m), 1.2, 1.0, 1.0).xyz, 0., 1.);
    glFragColor.w = 1.;    
}
