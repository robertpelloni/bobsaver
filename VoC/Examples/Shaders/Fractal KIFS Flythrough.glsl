...
// original https://www.shadertoy.com/view/XsKXzc
...
// code, will give it more meaning. I'd also recommend reading Syntopia's blog. He explains things
// really well.
float map(vec3 p){
    
    
    const vec3 offs = vec3(1, .75, .5);
    ...
    for(int i=0; i<2; i++){
        ...
        // comparison  and a cube and minimum, but all kinds of 
...
// convert the 3D ray coordinates (rd) to polar coordinates and index into a repeat texture. 
// It can be pretty convincing (in an abstract way) and allows environment mapping without 
// the need for a cube map, or a reflective pass.
//
// More sophisticated environment mapping:
// UI easy to integrate - XT95    
// https://www.shadertoy.com/view/ldKSDm
vec3 envMap(vec3 rd, vec3 sn, float lod){

    vec2 uv = vec2(atan(rd.y,rd.x)/6.283, acos(rd.z)/3.14159);
    uv = fract(uv);
   
    vec3 col = vec3(0.0,0.0,0.0);//texture2D(iChannel1, uv/4.).zyx;//*(1.-lod*.8)
...
void main(void) {
    ...
    vec3 rd = (vec3(2.*gl_FragCoord.xy - resolution.xy, resolution.y));
    ...
    if(t<FAR){
        ...
        vec3 oCol = vec3(0.0,0.0,0.0);//tex3D(iChannel0, sp*sz, sn); // Texture color at the surface point.
        ...
        vec3 env = envMap(reflect(rd, sn), sn, .5).xyz;
        col += env;
