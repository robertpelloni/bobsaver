...
// original https://www.shadertoy.com/view/Wsc3W4
...
float map(vec3 p)
{
    ...
    vec3 r=q;
    r.xz=abs(r.xz*rot(2.*time))-1.9;    
    r.yz=abs(r.yz*rot(2.*time))-1.9;
...
void main(void)
{
    ...
    rnd=hash12(gl_FragCoord.xy)*0.1+0.9;
    
    vec3 ro=vec3(0.,-1.,10.+cos(time));
    vec3 ta=vec3(0.,1.,0.);
    
    ro.xz*=rot(.5*time); 
     // debugging camera
    float x_rot=-mouse.x*resolution.xy.x/resolution.x*PI*2.0;
    float y_rot=mouse.y*resolution.xy.y/resolution.y*3.14*0.5 + PI/2.0;
    //if(mouse*resolution.xy.z>0.||mouse*resolution.xy.w>0.)
    ...
    for(int n=0; n<AA; n++)
    {
        ...
        p=(-resolution.xy + 2.0*(gl_FragCoord.xy+o))/resolution.y;
#else    
