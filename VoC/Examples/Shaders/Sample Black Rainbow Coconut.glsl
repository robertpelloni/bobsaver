...
vec3 renderColor( vec3 ro , vec3 rd, in vec3 color, vec3 currPos )
{
    ...
    //color = hsv( sin(lambert*40.0)*0.05+0.8, 0.6, 1.0 );
  
    //color = vec3( sin(currPos.z * 60.0 + time*10.0) );
    
    //color *= vec3(mix(0.0,1.0,clamp(ao,0.0,1.0)));
    //color += vec3(rim) * 0.2;
    //color = vec3(0.2,0.3,0.4);
    //color += vec3(1.0,0.4,0.3) *vec3(ao)*0.8;
    ...
    else
    {
        ...
        //float lambert = pow( dot( normal, lightDir ), 2.0);
        //color *= vec3(ao)*2.0;
        vec4 pos = vec4( currPos*15.0, 1.0 );
        
        //float noiseVal = snoise(pos);
        
        //vec3 normalShiny = refract( normal, normalize(- vec3(noiseVal, 0.0, 1.0) ), 1.0);
        color = normal*0.5+vec3(0.5);
        //color = vec3( noiseVal );
        //color += vec3(ao)*1.5;
    ...
    //color = mix( vec3(0.2,0.3,0.4), vec3(1.0,0.9,0.3), ao );
