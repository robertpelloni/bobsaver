...
// original https://www.shadertoy.com/view/WljcR3
...
vec2 GetBaseFlow( const vec2 vPos ) {
    return vec2( 1.0, 0.);// GetRiverMeanderDx(vPos.x) );
}
...
vec2 GetFlowRate( const vec2 vPos ) {
    vec2 vBaseFlow = GetBaseFlow( vPos ); 
    vec2 vFlow = vBaseFlow; 
    float fFoam = 0.0;
    float fDepth = -GetTerrainHeight( vec3(vPos.x, 0.0, vPos.y) );
...
vec3 GetScene( const in vec3 vRayOrigin,  const in vec3 vRayDir, const in float near, const in float far) {
    ...
    vec2 vFlowRate = GetFlowRate( waterInt.m_pos.xz );
...
void initCamera(out vec3 ro, out vec3 rd,out float near, out float far) {
    ...
    //if( mouse*resolution.xy.z > 0.0 )  {
        float fHeading = mouse.x*resolution.xy.x * 10.0 / resolution.x;
        float fDist = 5.0 - mouse.y*resolution.xy.y * 5.0 / resolution.y;
...
void main(void) {
    ...
    vResult = ApplyVignetting( gl_FragCoord.xy / resolution.xy, vResult );    
    vec3 vFinal = Tonemap(vResult * 3.0);
    vFinal = vFinal * 1.1 - 0.1;
    glFragColor = vec4(vFinal, 1.0);
}
