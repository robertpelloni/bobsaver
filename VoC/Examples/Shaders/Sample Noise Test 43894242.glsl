...
// original https://www.shadertoy.com/view/NtjcDh
...
float GetDist(vec3 p) {
    vec2 uv = vec2(p.x + 0.1 * time, p.z);
    float n = n2(uv);
    p.y += -pow(n, 8.);
    p.y += 0.1 * cos(length(p) * 1.5 + 100. * pow(n,16.) - time);
    float d = p.y;
...
void main(void)
{
    ...
    if(d<MAX_DIST) {
        ...
        col += fresnel2;
        col += 0.35 * thc(6., 100. * exp(-length(p)) - time);
    }
