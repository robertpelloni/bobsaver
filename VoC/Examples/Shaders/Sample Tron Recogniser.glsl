...
// original https://www.shadertoy.com/view/MdX3Dn
...
vec3 xform(vec3 v)
{
    return rotateY(time * 0.3 + (mouse.x / resolution.x - 0.5) * 4.0, rotateX(0.4 + (mouse.y / resolution.y - 0.5), v));
}
...
void main()
{
    ...
    glFragColor.rgb = mix(glFragColor.rgb, c * vec3(1.0, 0.0, 0.1), step(f, 10.0)) * 2.0;
}
