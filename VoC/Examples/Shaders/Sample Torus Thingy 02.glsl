...
// original https://www.shadertoy.com/view/llsBRH
...
float    scene(vec3 p)
{
    ...
    */
    ...
    rotate(q.xy, +time*2.+ata*3.);
    
    q.xy = abs(q.xy)-.25;
    
    rotate(q.xy, -time*2.+ata*8. );
    q.xy = abs(q.xy)-.1; // .25 == butterflys
    rotate(q.xy, +time*2.+ata*4. );
    q.xy = abs(q.xy)-.051;
    balls = mylength(q)+(-.0305-.011*(abs(ata)-3.00));//sin(6.28*ata+time)*1.;
    p.y -= 2.;
    rotate(p.xz , time*1.);
    float    light = dot(p,p);
