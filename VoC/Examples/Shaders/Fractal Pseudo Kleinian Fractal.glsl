...
// original https://www.shadertoy.com/view/tdfSzM
...
/*  MIT License
  ...
  A simple desktop version of this shader is awailable at

  https://github.com/neozhaoliang/pywonderland/blob/master/src/fractal3d/fractal3d.py

  There are already several "pseudo kleinian" fractals
  implemented on shadertoy, but I'm not satisfied with
  them, so here is a new one.
  
  This is an experiment based on other people's work
  (e.g. knighty, etc), I'm not the creator, so
  if you find it interesting, credit it to all those
  who share their shader code and idea on the web.

  1. You should set AA>=3 for making wall paper images.
  2. It runs quite slow for AA>=3 :sad:
  3. change fovdist to 2.0 to see a full view.
