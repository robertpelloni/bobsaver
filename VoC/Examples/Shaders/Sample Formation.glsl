...
// original https://www.shadertoy.com/view/4sdyz4
...
vec3 map(vec3 p) {
    ...
    p.y += noise(p * 6.1 + vec3(10, -time * .3, 0).xyy) * 0.175 - 1.;       
    //p.y += noise(p * 2. + time * .2) * .1;
...
void main(void) {
    ...
    vec3 
        vuv = vec3(0., 1., 0.), // up
        ro = vec3(-t, .0 - cos(2.+time * .05) * .4, sin(time * .1)),//-vec3(mouse*resolution.xy.x / 100. - 1.,mouse*resolution.xy.y / 100. - 1., 1.), // pos
        vrp =  vec3(cos(time * .21), -5., 1.) + ro, // lookat    
