...
// ardf[i+j*nc] = arrw[i] - arrw[j];
    ...
    if( nr2w >= 0.230 && nr2w <= 0.380 ) { r0 += s*3.5; }
    if( nr2w >= 0.130 && nr2w <= 0.180 ) { r0 += s*0.4; }
    if( nr2w >= 0.000 && nr2w <= 0.178 ) { r0 -= s*8.0; }
    if( nr2w >= 0.263 && nr2w <= 0.380 ) { r0 -= s*5.0; }
