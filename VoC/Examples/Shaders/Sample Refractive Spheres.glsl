...
// original https://www.shadertoy.com/view/MsSGWz
...
void main(void)
{
    lightPos_ = vec3(-cos(time)*-12.0,3.5+sin(time*2.05)*8.0,(sin(time)*12.0-5.4));
    bgLight = normalize(lightPos_);
    lightPos = bgLight * 9999.0;

    vec2 aspect = vec2(resolution.x / resolution.y, 1.0);
    vec2 uv = (1.0 - 2.0 * (gl_FragCoord.xy / resolution.xy)) * aspect;
    ...
    for (int i=0; i<16*4; i++) {
        ...
        if (dist != 10000.0) {
            ...
            if (!getBit(rayCount, rayBounceCount)) {
                ...
                r.transmit *= (1.0+f)*vec3(0.9, 0.6, 0.5);
            } else {
    ...
    glFragColor = vec4(1.0 - exp(-r.light/rayCount * 2.5), 1.0);
}
