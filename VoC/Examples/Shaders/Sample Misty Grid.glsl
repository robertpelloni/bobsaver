...
// original https://www.shadertoy.com/view/wl2Szd
...
vec3 fr(vec3 p, float t) {
  ...
  float s = 0.7 - smoothstep(0.0,1.0,abs(fract(time2*0.1)-0.5)*2.0)*0.3;
  for(int i=0; i<5; ++i) {
...
float map(vec3 p) {
  ...
  p.xy *= rot((p.z*0.023+time2*0.1)*0.3);
