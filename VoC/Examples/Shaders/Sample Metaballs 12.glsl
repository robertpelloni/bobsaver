...
// original https://www.shadertoy.com/view/wsSXzc
...
////////////////////////////////////////////////////////////////////////////////
//
// Metaballs in "ribbons" part 2 - variation of older shader I wrote once
//
...
float map (in vec3 p, out int id) {
    ...
    vec3 boxCenter = p + time * vec3 (.0, .75, .0);
...
vec3 shade (in vec3 ro, in vec3 rd, in float d, in int id) {
    ...
    for(int i = 0; i < 2; ++i) 
    {
        ...
        float attenuation = 6./(ldist*ldist);
    ...
    vec3 irradiance = vec3(0.0); //.2*texture (iChannel0, N).rgb;
...
vec3 background (in vec2 uv) {
    vec2 p = uv - ceil (resolution.xy*.5);
    p *= r2d (34.*time);
    vec3 col = vec3 (.2, .5, 1.);
...
void main(void) {
    ...
    float dist = 2.75 + cos (1.5*(time + 2.));
    vec3 ro = vec3 (dist * cos (angle), cos (time), dist * sin (angle));
    vec3 aim = vec3 (.0);
    float zoom = 2.;
    float d = .0;
    vec3 col = vec3 (.0);    
    ...
    for (int x = 0; x < AA_SIZE; ++x) {
        for (int y = 0; y < AA_SIZE; ++y) {
            ...
            uv = (gl_FragCoord.xy + pixelOffset)/resolution.xy;
            uv = uv*2. - 1.;
    ...
    if (id == 3) col = background (gl_FragCoord.xy);
