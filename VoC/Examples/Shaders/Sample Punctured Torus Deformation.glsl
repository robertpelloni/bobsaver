...
// original https://www.shadertoy.com/view/ssG3DK
...
//this takes in the pixel coordinates on the screen (gl_FragCoord.xy) and rescales
...
vec2 normalizedFragCoord(){
 ...
 // Normalized the pixel coordinates (from -0.5 to 0.5)
    ...
    uv = 4.*vec2(1,resolution.y/resolution.x)*uv;
...
void main(void)
{
    ...
    setUpFD(osc(0.01,1.,time/3.),osc(0.5,2.5,time/1.));
