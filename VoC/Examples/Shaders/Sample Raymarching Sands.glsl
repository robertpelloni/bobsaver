...
// original https://www.shadertoy.com/view/3ts3Wf
...
float sandDistanceMap ( vec3 point ) {
    float distance = min(sdfPlane(point, vec4(0, -1, 0, 3)) 
        + 1.1 *  twoDnoise(vec2(point.x, point.z - time)), sdfPlane(point, vec4(0, 2, 0, 3)) 
        +  1.2 * twoDnoise(vec2(point.x, point.z - time)));
...
void main(void) {
    ...
    vec2 q = (gl_FragCoord.xy - .5 * resolution.xy) / resolution.y;
    vec3 rayDirection = normalize(vec3(q, 0.) - rayOrigin);
    glFragColor = vec4(trace(rayOrigin, rayDirection), 1.0);
}
