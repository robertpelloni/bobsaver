...
// "Glass Eel" by Kali
// https://www.shadertoy.com/view/MsfGWS
...
void main(void)
{
    ...
    coord.xy*=mat2(cos(time2),sin(time2),-sin(time2),cos(time2));
    float fov=.5;
    vec3 from = vec3(-3.,-1.,sin(time2)*4.-1.);
    ...
    float ref=0.;
    vec3 light1=normalize(vec3(sin(time2),sin(time2*2.)*.5,1.5));
    vec3 light2=normalize(vec3(sin(time2+2.),sin((time2+2.)*2.)*.5,1.5));
    ...
    //lights
    vec3 tcoor=vec3((dir.xy*(2.-sin(time2)*.8))+sin(coord.xy*20.+time*10.)*.007,1.);
    vec3 li=vec3(0.15);
