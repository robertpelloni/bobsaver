...
// original https://www.shadertoy.com/view/XdBGDt
...
hit scene (const in vec3 p, const in vec3 d) {
    ...
    if (all (greaterThan (vec2 (t, h.t), vec2 (0, t)))) {
        ...
        h.m.light *= sin (time2 * 0.2) * 0.5 + 0.5;
    }
    t = (4.0 - p.x) * invd.x;
    if (all (greaterThan (vec2 (t, h.t), vec2 (0, t)))) {
        vec3 r = p + d * t;
...
void main (void) {
    ...
    vec2 tmp = mod (gl_FragCoord.xy + time2, dithering);
    seed1 += tmp.x + tmp.y / dithering;
    
    //seed1 += mod (gl_FragCoord.x + gl_FragCoord.y * 2.0, dithering);
    
    vec2 cr = (gl_FragCoord.xy - resolution.xy * 0.5) / resolution.x;
    vec3 p = vec3 (sin (time2) * 2.0, sin (time2) + 1.25, cos (time2) * 2.0);
    ...
    for (int i = 0; i < samples; i++) {
        vec3 d = rayDirection (
            ...
            cr + (vec2 (rndValue (), rndValue ()) * 2.0 - 1.0) * antialiasing / resolution.x);
        
        color += rayColor (p, d);
    }
    color = pow (color / float (samples), vec3 (gamma));
    glFragColor = vec4 (color * brightness, 0.0);
}
