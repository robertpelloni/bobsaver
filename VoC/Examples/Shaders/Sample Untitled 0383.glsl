...
// original https://www.shadertoy.com/view/ts2XRR
...
void init_scene() {
  ...
  //    transparent(vec3(0.2, 0.5, 0.2), vec3(0.3, 0.7, 1.0))
   ...
   for(int i=0; i<20; ++i) {
     float beta = float(frames)/30.0 + float(i)*6.28/19.0;
     float s = sin(beta);
     float c = cos(beta); 
...
void main(void) {
   ...
   for(int k=0; k<5; ++k) {
       if(nearest_intersection(R, P, N, material)) {
          ...
          glFragColor.rgb += Kr_cumul * vec3(0.5, 0.5, 1.0);
          break;
       }
   }  
}
