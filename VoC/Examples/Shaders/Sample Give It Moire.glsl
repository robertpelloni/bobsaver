...
// original https://www.shadertoy.com/view/3lj3zy
...
vec2 map(vec2 st, float t)
{
    ...
    float freq = mix(1.5,2.25,cell_random(sin(PI + time*.15),5.0));
    float amount = mix(0.08,0.35,cell_random(sin(TWO_PI*.75+time*.0025),10.0));
    float c = length(st);
    float sq = mix(1.0,10.0, cell_random(sin(PI + time*.005),50.0));
...
void main(void)
{
    ...
    st = st * 2.0 - 1.0;
    st *= rotate(PI*0.25);
    st *= rotate(TWO_PI*time*0.0015);

    vec2 m = map(st,t);

    float r = cell_random(length(m)+time*0.023,10.0);

    float stripes = sin(m.x*50.0);
    float d = dot(m,st)+0.1;

    float c_repeats = mix(20.0,100.0,cell_random(sin(time*.035),10.0));

    vec3 c_sin = cos(vec3(0.15,.32,0.3)+d*c_repeats + time );
