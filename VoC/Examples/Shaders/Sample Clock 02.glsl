...
// original https://www.shadertoy.com/view/ldS3Wt
...
void setPos(float r, vec2 pos, out float ang, out float radius) {
    ...
    pos.x /= sin(6.283*time/10.*dt);
    ang = mod(.5-atan(pos.y,pos.x)/6.283,1.); radius = length(pos);
}

void main(void)
{
    vec2 pos = 2.*(gl_FragCoord.xy / resolution.y-vec2(0.9,.5));
    float ang = mod(.5-atan(pos.y,pos.x)/6.283,1.), radius = length(pos);

    // tunings
    float MY = 2.5/resolution.y,    MA = .5/resolution.y,
          MR = 64./resolution.y,   MR2 = 410./resolution.y;
    float AI = (1.+sin(6.283*time/100.))/2.;
    
    float year = date.x, month = date.y, day = date.z, seconds = date.a;
    ...
    col.r += smoothstep(MA,-MA,ang-v) *I *smoothstep(MR2,-MR2,abs(radius-.56)/.005-1.);
    col.g += smoothstep(MA,-MA,ang-v) *I *smoothstep(MR2,-MR2,abs(radius-.56)/.005-1.);
    ...
    col.r += smoothstep(MA,-MA,ang-v) *I *smoothstep(MR,-MR,abs(radius-.61)/.035-1.);
    col.g += smoothstep(MA,-MA,ang-v) *I *smoothstep(MR,-MR,abs(radius-.61)/.035-1.);
    ...
    col.g += smoothstep(MA,-MA,ang-v) *I *smoothstep(MR,-MR,abs(radius-.7)/.04-1.);
    col.b += smoothstep(MA,-MA,ang-v) *I *smoothstep(MR,-MR,abs(radius-.7)/.04-1.);
    ...
    col.r += smoothstep(MA,-MA,ang-v) *I *smoothstep(MR,-MR,abs(radius-.8)/.04-1.);
    col.b += smoothstep(MA,-MA,ang-v) *I *smoothstep(MR,-MR,abs(radius-.8)/.04-1.);
