...
// original https://www.shadertoy.com/view/lsjGRV
...
vec2 map(in vec3 pos) {
    ...
    pos = mod(pos + mod(time*vel, wrap), wrap);
    ...
    /*
    if (texture2D(iChannel0, vec2(KEY_G, 0.75)).x > 0.0) {
...
void main(void) {
    ...
    vec2 uv = (gl_FragCoord.xy - 0.5*resolution.xy) * yscl / resolution.y;
    ...
    if (max(iMouse.x, iMouse.y) > 20.0) { 
        ...
        thetay = (iMouse.x - 0.5*iResolution.x) * -3.14/iResolution.x; 
    }
    */
