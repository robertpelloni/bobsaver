...
// original https://www.shadertoy.com/view/3dGSWw
...
void get_penguin_sdf(vec3 p, inout float mindist, inout int material) {
    ...
    dist = op_smooth_union(dist,
        sd_ellipsoid(rotate_z(p - larm_pos, 0.7 + sin(time*10.0)*0.2), arm_radius), 0.1);
    dist = op_smooth_union(dist,
        sd_ellipsoid(rotate_z(p - rarm_pos,-0.7 + sin(time*10.0)*0.2), arm_radius), 0.1);
    ...
    dist = min(
        sd_ellipsoid(rotate_x(p - lfoot_pos, sin(time*10.0)*0.3), foot_radius),
        sd_ellipsoid(rotate_x(p - rfoot_pos, sin(time*10.0+M_PI)*0.3), foot_radius));
    ...
    // retina
    vec3 lretina_pos = vec3(-0.4 + sin(time)*0.1, 3.3, 1.63);
    vec3 rretina_pos = vec3(0.4 - sin(time)*0.1, 3.3, 1.63);
...
Hit get_sdf(vec3 p) {
    ...
    // penguin
    get_penguin_sdf(rotate_z(p, sin(time*10.0)*0.1), mindist, material);
...
vec3 get_material_diffuse(vec3 p, int material) {
    switch(material) {
        ...
        case MATERIAL_FLOOR: {
            float checker = clamp(sin(p.x)*sin(p.y)*sin(p.z+time*4.0), 0.0, 1.0);
...
void main(void) {
    ...
    vec2 uv = 2.0 * ((fragcoord-0.5*resolution.xy) / resolution.y);
