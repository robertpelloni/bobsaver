...
// original https://www.shadertoy.com/view/wssXWl
...
// "Science in my day was %&*$ing weldin' %&*$ing ships together, and sending them to the %&*$ing sea" - Limmy (The armadillo sketch)

vec2 sc,y,e=vec2(.000035,-.000035);float t,tt,b,bb,g1=0.,g2=0.;vec3 np,bp;//Some %&*$ing globals, about as exciting as a vegetarian sausage
...
float cz(vec3 p,float r){p.z=0.;return length(p)-r;}//Did you know you could ... I just %&*$ing told you, didn't I? Pay attention this isn't your ex's nitting class
...
vec2 fb(vec3 p,vec3 m, float s)//%&*$ing bits function which makes the %&*$ing bit/piece it is a base shape which we clone and repeate to create the whole geometry in mp function
{
  p.xz*=r2(s);//constantly giving it a little shake, like a juicing diet but without the pretention or the juices
  p.xy*=r2(s+bb*1.4);//Every so often we flip the %&*$ outta everything, it's like those brexit promises that got flipped into nothingness
  vec2 h,t=vec2(bo(abs(p)-vec3(0,0,1),vec3(2.2,0.3,0.7)),m.x);//set the tone make a bunch of %&*$ing boxes with abs symetry cloning
  t.x=min(0.6*cy(abs(abs(p)-vec3(0.5,0,0))-vec3(0.5,0,1),0.1),t.x);//Erecting major %&*$ing poles, because it's not a good shader if it doesn't have sexual innuendo
  ...
  h=vec2(bo(abs(p)-vec3(0,0,1),vec3(2.0,0.5,0.5)),m.y);//More %&*$ing boxes, double outline grapphitti style mother %&*$er, "yeah I come from the streets". Not really, i was born in a hospital
  t=(t.x<h.x)?t:h;//Blending two shapes while retinaing material ID, like a melting headbutt with colours punched in the face
  h=vec2(bo(p,vec3(1.8,0.2,2.8)),m.z);//yet more %&*$ing boxes, this time white one for structure and comfort, take off your shoes broski, we share slipers here. Gross, yes, but at least we're family
  t=(t.x<h.x)?t:h;//Blending two shapes while retinaing material ID, like a melting headbutt with colours punched in the face
...
{//Technique is to make a new position np and tweak it, clone it, rotate it and then pass np to fb to create complex geometry from simple piece
  ...
  for(int i=0;i<5;i++){//In the loop we push, rotate np into more complex "position"
    np=abs(np)-vec3(2.5,1.7,0);//symetry clone the %&*$er out
    np.xy*=r2(cos(p.y*0.05)*0.5);//rotate the bitch along xy axis
    np.xz*=r2(20.);//and again mega rotate the whole %&*$ along xz so it looks symetrical
  }
  vec2 h,t=fb(np,vec3(5,3,6),b);//push np to %&*$ing bit function to make complex geometry base don single piece, like flipping a coin in the air and getting a gram of coke back!
  ...
  h=fb(bp,vec3(7,8,9),0.1);//this line makes outter bits using same fb function but with different position "bp" this time, making it totally different, nifty %&*$ broski
  ...
  h=vec2(cz(p,0.5-b),8);//Make a %&*$ing wobbly tube that glows, bit like a fat wobbly glow stick. Well %&*$, i guess the metaphore is ironic, it's not the %&*$ing nineties anymore.
...
void main(void)
{
    ...
    uv -= 0.5;
    uv /= vec2(resolution.y / resolution.x, 1);//boilerplate code to get uvs in BONZOMATIC live coding software i use.
    tt=mod(time,100.);//MAin time variable, it's modulo'ed to avoid ugly artifact. Holding time in my hand: playing god is nearly as good as this crystal meth bag
    ...
    if(t>0.){//If t>0 then we must have hit some geometry so let's %&*$ing shade it. 
        ...
        spo=exp(1.+3.*noise(np/vec3(.1,.2,.4)));//TRICK making a gloss map from a 3d noise function is a thing of %&*$ing beauty
        ...
        co=mix(sp+al*(.8*ao+0.2)*(dif+sss),fo,fr);//Building the final lighting result, compressing the %&*$ outta everything above into an RGB %&*$ sandwich
    ...
    glFragColor = vec4(pow(co,vec3(0.45)),1);//Cheap tone mapping, even cheaper than the presents your ex used to get you for christmas
}
