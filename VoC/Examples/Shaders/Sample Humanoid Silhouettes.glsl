...
// original https://www.shadertoy.com/view/4scBWN
...
void main(void)
{
    ...
    uv = uv*2.-vec2(0,0);
    //float phase = floor(time*3.)*2.; // randomise
    uv.x += time;
    float w = .7;
    ...
    glFragColor = vec4(.5+.5*sdf/(abs(sdf)+.002)); // map SDF to slightly soft black & white image
}
