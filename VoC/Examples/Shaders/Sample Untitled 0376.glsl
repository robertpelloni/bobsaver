...
float subFunc(vec3 pos)
{
    ...
    return sdRoundCone(pos, 0.02,0.01, 0.015 );
}
...
vec3 rayMarching(vec2 pos) {
    vec3 cameraPos = vec3(0.0, 0.0, -10.0 + time * 0.015);
    Ray ray;
    ray.pos = cameraPos;
    ray.dir = normalize(vec3(pos * 13.0, 1.0));
    float d;
    for(int i = 0; i < 18; ++i)
    {
        d = distFunc(ray.pos);
...
void main( void ) {
    ...
    glFragColor = vec4(vec3(rayMarching(pos1)), 1.0);
    return;
    glFragColor = 
        vec4(
            (rayMarching(pos1) + 
