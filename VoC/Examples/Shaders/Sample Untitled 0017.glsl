...
void main(void)
{
    ...
    for (float i = 1.; i < 20.; i++) {
        ...
        if (psuedo_random == 0.0) {
            glFragColor += alpha * ((1.0 - smoothstep(dist_1, dist_1 + .25, distance(coord, pos))) + 
            ...
            glFragColor += alpha * ((1.0 - smoothstep(dist_2, dist_2 + .05, distance(coord, pos))) + 
                                  (1.0 - smoothstep(0.01, 0.02, distance(coord, pos))) +
                                  (1.0 - smoothstep(radius * 0.1, radius * 0.11, distance(coord, pos)))) * particle_color;            
        }
        
        else if (psuedo_random == 2.0) {
            glFragColor += alpha * ((1.0 - smoothstep(dist_3, dist_3 + .105, distance(coord, pos))) + 
