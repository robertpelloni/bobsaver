vec3 TriplexAdditionPhoenixXnYp1(in vec3 z, in vec3 c) {
    vec3 znew;
    znew.x=z.x+c.x;
    znew.y=z.y+c.y;
    znew.z=z.z+c.z;
...
vec3 TriplexPowerPhoenixXnYp1(in vec3 z,in float p) {
	vec3 znew;
    radius=pow(radius,p);
    theta=theta*p;
    phi=phi*p;
...
vec3 TriplexMultiplicationPhoenixXnYp1(in vec3 z,in vec3 p){
...
void Iterate(inout vec3 z,in vec3 c) {
    ...
    znew=TriplexPowerPhoenixXnYp1(z,power);
    znew2=TriplexMultiplicationPhoenixXnYp1(c,lastz);
    znew=znew+znew2;
    lastz=z;
    z=znew;
}
