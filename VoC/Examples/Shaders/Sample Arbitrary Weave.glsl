...
// original https://www.shadertoy.com/view/MtlBDs
...
vec3 doObject(vec3 col, float d, float c, vec2 d2, float objID, float rnd){
    ...
    float sh = abs(clamp(d2.x, -.13, .13))*16.; //sh = pow(sh, 2.)*1.4;
    float sh2 = abs(clamp(d2.y, -.13, .13))*16.; //sh2 = pow(sh2, 2.)*1.4;
    ...
    float blink = smoothstep(.5, .75, sin(rnd*6.283 + time*5.));
...
vec4 scene(vec2 p){
    ...
    // rn3.x = sin(rn3.x*6.283 + floor(time))*.5 + .5; // Peridically changes the patten formation.
    ...
    if(rnd.y>.8){ // Three small arcs segments. So-called standard hexagonal Truchet.
        ...
        d.y = abs(d.y) - th;
        a = atan(p2.y, p2.x);
        gRnd.y = hash21(h.zw + floor(a/6.283*3.) + 4.78);
        a = mod(a/3.14159265, 2.) - 1.;
...
void main(void) {
    ...
    //vec2 fC = floor(gl_FragCoord*4.)/4.;
    // Aspect correct screen coordinates.
    vec2 u = (gl_FragCoord.xy - resolution.xy*.5)/clamp(resolution.y, 300., 650.);
    
    vec2 p = u*4. + s.yx*time/4.;
    
    vec3 tx = vec3(0.0);//texture(iChannel0, p/5.).xyz;//r2(3.14159265/9.)*
    tx *= tx;
    ...
    // The seperate offset grid that the blocks are produce in.
    //float eDist2 = hex(getHex(p - vec2(0, .6)).xy);
