...
// original https://www.shadertoy.com/view/7ds3W2
...
vec2 func(in vec2 z) {
    //return mult(z - vec2(2.0f, 0.0), div(vec2(1.0,0.0), z + vec2(sin(time), 0.0)));
...
void main(void)
{
    ...
    vec2 uv = 4.0*(2.0*gl_FragCoord.xy/resolution.xy - 1.);
    ...
    //
    float n1 = 22.;
    float arg1 = uv.x-time/4.; //atan(uv.y, uv.x);
    float gEps1 = 0.001;
    ...
    //  background color
    float bc = 1.0-wrap(length(uv.x-time),1.);
    //  line color
    c = max(c,c1);
    vec3 lcol = vec3(1.0,0.0,0.0);
    //vec3 col = (1.0 - c) * bcol + c * vec3(abs(sin(uv.x-time)), 0.0, 0.0);
    vec3 col = vec3(0.);
    if (bc > c) {
