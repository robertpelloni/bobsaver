...
void main(){
    ...
    for(int i=0;i<72;i++){
        ...
        rayL+=dist;
        rayP=camP+ray*rayL;
        rayP.xy=rotate(rayP.xy, smoothstep(40.0+sin(time)*30., 0.0, length(ray)*rayL)*rayL*0.1);
    }
    
    if(abs(dist)<0.001){
        float fragR=dot(-ray,getNormal(rayP));
        float fragG=0.;
        float fragB=dot(vec3(1.),getNormal(rayP));
        glFragColor=vec4(pow(vec3(fragR,fragG,fragB)*15./rayL, vec3(1.5)),1.);
    }else{
