...
// original https://www.shadertoy.com/view/cs3GWf
...
void main(void)
{
    ...
    for( int nn=0; nn<AA; nn++ )
    {
        ...
        vec2 p2 = (2.0 * (gl_FragCoord.xy+o)/resolution.xy -1.0 ); 
        #else    
        vec2 p2 = (2.0 * gl_FragCoord.xy/resolution.xy -1.0 );
        #endif
        ...
        vec3 ro = vec3(3.0*sin(7.0*(m.x+(time/10.0))), 3.0*cos(4.0*(m.x+(time/10.0))),3.0*cos(7.0*(m.x +(time/10.0))));
