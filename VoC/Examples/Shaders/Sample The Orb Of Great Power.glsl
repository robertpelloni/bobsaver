...
// original https://www.shadertoy.com/view/Wsy3zm
...
void main(void)
{
  ...
  float fade1 = 1.0-pow(fract(time2*exspeed),10.0);
  float fade2 = 1.0-pow(fract(time2*exspeed+0.5),10.0);
  
  // pick a random color for each explosion
  vec3 c1 = rndcol(floor(time2*exspeed)+17.3);
  vec3 c2 = rndcol(floor(time2*exspeed+0.5)+37.5);
  ...
  float iterationcount = (90.0+min(time2,0.0));
