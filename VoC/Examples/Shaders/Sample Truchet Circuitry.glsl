...
// original https://www.shadertoy.com/view/dlsXW8
...
vec2 hash22(vec2 p){ 
    ...
    return fract(vec2(262144.*8.,262144)*sin(dot(p, vec2(89,61))));
}
...
void main(void) {
    ...
    col = mix(col, vec3(0., 0.3, 0.)*sh, 1. - smoothstep(0., fo, abs(d.x + 0.12) - .02));
      
    
     
    // UV color mixing.
    col = mix(col, col.xzy,0.);
