...
void main( void ) 
{
    ...
    vec2 position = positionPixel/resolution;
        ...
        vec4 source = subPixel(backbuffer, position);
        
    vec2 pos = gl_FragCoord.xy / resolution.xy;
        float random = rand(vec2(pos.x * time, pos.y));
        
        vec4 colour = source + ((random * 2.0) - 1.0) * 0.1;
    
        
        activator = averageForRadius(position, 2.0);
        inhibitor = averageForRadius(position, 5.0);
    ...
    glFragColor = vec4(col.r,col.g+activator,col.b-inhibitor,1.0);
}
