...
void main() {
    ...
    float nh0w = nh0[2] / nh0[1];
        if( nh0c >= 0.000 && nh0c <= 0.300+cs ) { res = 0.0; }
        if( nh0c >= 0.207 && nh0c <= 0.225 ) { res = 1.0; }
        if( nh0c >= 0.600 && nh0c <= 0.700 ) { res = 0.0; }

    float nh1s = 3.0;
    vec3 nh1 = nh_ring ( nh1s, 0.0, vec2(1.0, 0.0), vec2(0.0, 0.0) );
    float nh1c = nh1[0] / nh1[1];
    float nh1w = nh1[2] / nh1[1];
        if( nh1c >= 0.500 && nh1c <= 0.600 ) { res = 1.0; }

    float nh2s = 12.0;
    vec3 nh2 = nh_ring ( nh2s, nh2s/3.0, vec2(0.0, 0.0), vec2(0.5, 4.0) );
    float nh2c = nh2[0] / nh2[1];
    float nh2w = nh2[2] / nh2[1];
        if( nh2c >= 0.600 && nh2c <= 1.000 ) { res = 0.0; }
    ...
    if(frames<3){
        ...
        if (rnd1 > 0.45) { glFragColor = vec4(1.0); } else { glFragColor = vec4(0.0); }
        return;
    }
    /**
