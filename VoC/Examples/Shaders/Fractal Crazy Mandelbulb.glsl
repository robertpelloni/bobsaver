...
// original https://www.shadertoy.com/view/MljGDz
...
float x,y,z;
mat3 rx,ry,rz,objectRotation;
float aspectRatio,fovfactor,pixelScale,epsfactor;
vec3 w,u,v;
mat3 cameraRotation;
...
vec3 Mandelbulb(vec3 w)
{
    ...
    vec3 z = w;
    vec3 c = mix(w, offset, juliaFactor * sin(time * 0.03));
    vec3 d = w;
    float dr = 1.0;
    float r  = length(z);
    float md = 10000.0;
...
{
    vec2 p = (0.5 * resolution.xy - pixel) / vec2(resolution.x, -resolution.y);
    p.x *= aspectRatio;
...
void main(void)
{
    ...
    x = deg2rad(objRotation.x) * time * 0.05;
    ...
    pixelScale = 1.0 / min(resolution.x, resolution.y);
