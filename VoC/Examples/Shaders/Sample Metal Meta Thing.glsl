...
// original https://www.shadertoy.com/view/4ssGWS
...
// Kleber Garcia (c) 2017

float gAspect = resolution.x / resolution.y;
const float EPSILON = 0.001;
const vec3 AMBIENT = vec3(0.6, 0.4, 0.3);
vec3 gDirLight1 = normalize(vec3(-0.1, 0.2, -1));
vec3 gLightPos1 = vec3(-1, 1, -8);
vec2 gMouseHomogeneous = 4.0*((mouse*resolution.xy.xy / resolution.xy) * 2.0 - 1.0);
...
vec2 scene(vec3 pos, out vec3 intersectP)
{
    ...
    intersectP = pos + vec3(sin(pos.y + time),0,0);
    intersectP.y += 0.3*sin(pos.x + time);
    intersectP.x -= 0.5*cos(4.0*(pos.y + time));
    intersectP.y -= 0.5*cos(2.5+2.3*(pos.x + 3.0*time));    
    intersectP.y -= 0.5*sin(0.8+pos.x + 2.0*time);
    intersectP.z += 0.1*sin(0.9+pos.x + 1.35*time);
    intersectP.x -= 0.2*cos(1.9+0.2*pos.y + 1.4*time);    
    intersectP.x += 0.5*sin(1.9+4.2*pos.y + 0.3*time);
    intersectP.y += 0.2*cos(2.5+0.7*pos.y + 0.32*time);
    intersectP.x += 0.2*sin(4.6+6.5*pos.y + 1.45*time);
    intersectP.x -= 0.1*cos(1.9+0.6*pos.y + 0.6*time);
...
void main(void)
{
    ...
    float initAngle =  (HALF_PI / 150.0) * (sin(time * 2.7));
    float initAngleVet =  (HALF_PI / 150.0) * (cos(time * 3.0));
    float angle = HALF_PI + initAngle + (-gMouseHomogeneous.x) * 0.1;//(time * 0.9);
    float angleVert = HALF_PI + initAngleVet + (-gMouseHomogeneous.y) * 0.1;//(time * 0.9);;
    ...
    glFragColor = vec4(pow(finalCol,vec3(2.2)), 1.0);
}
