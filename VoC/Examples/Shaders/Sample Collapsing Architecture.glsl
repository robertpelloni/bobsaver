...
// original https://www.shadertoy.com/view/XltXWN
...
void main(void) {
    ...
    // add this all up
    glFragColor.rgb = (ao * texture(p)) * (ambient + (specular + diffuse) * shadow);
    glFragColor.w=0.0;
    
    // edge glow
    glFragColor *= sqrt(steps / float(MAX_STEPS));
    
    // fog
    glFragColor = mix(glFragColor, vec4(.9, .8, .7, 1.), saturate(dist * dist * .03));
