...
// original https://www.shadertoy.com/view/WssfzS
...
{
    float ani = pow(0.5 + 0.5 * sin(7.0 * time + q.y / 20.0), 4.0);
...
{
    float SINt = sin(time);
    float SINt03 = sin(time * 0.3);
    float COSt03 = cos(time * 0.3);
    ...
    float sphereD3 = sphereDist(pos, 0.5, vec3(3.8 * SINt03, cos(time), 3.8 * COSt03));
...
{
    vec3 pos = vec3(p.x, p.y, p.z - (0.7 * time));
...
void main(void)
{
    ...
    //vec2 mouse = 4.0 * (mouse*resolution.xy.xy / resolution.xy);
    vec3 rd = normalize(vec3(uv, -1.0));
    vec3 ro = vec3(0,1.0, 8.0);
