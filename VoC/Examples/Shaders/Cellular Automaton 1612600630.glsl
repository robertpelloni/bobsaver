...
/**/
/*
    vec3 nr0 = nhd( vec2(2.0,0.0), psn, 0.0, 0 );
    vec3 nr1 = nhd( vec2(5.0,3.0), psn, 0.0, 0 );
    vec3 nr2 = nhd( vec2(12.0,7.0), psn, 0.0, 0 );
    vec3 nr3 = nhd( vec2(22.0,15.0), psn, 0.0, 0 );
    vec3 nr4 = nhd( vec2(38.0,25.0), psn, 0.0, 0 );
    vec3 nr5 = nhd( vec2(56.0,40.0), psn, 0.0, 0 );
/**/
/* vec3 nr0 = nhd( vec2(2.0,0.0), psn, 0.0, 0 );
    vec3 nr1 = nhd( vec2(5.0,0.0), psn, 0.0, 0 );
    vec3 nr2 = nhd( vec2(12.0,0.0), psn, 0.0, 0 );
    vec3 nr3 = nhd( vec2(22.0,0.0), psn, 0.0, 0 );
    vec3 nr4 = nhd( vec2(38.0,0.0), psn, 0.0, 0 );
    vec3 nr5 = nhd( vec2(56.0,0.0), psn, 0.0, 0 );
/**/
/*
    vec3 nr0 = nhd( vec2(1.0,0.0), psn, 0.0, 0 );
    vec3 nr1 = nhd( vec2(3.0,0.0), psn, 0.0, 0 );
    vec3 nr2 = nhd( vec2(6.0,0.0), psn, 0.0, 0 );
    vec3 nr3 = nhd( vec2(11.0,0.0), psn, 0.0, 0 );
    vec3 nr4 = nhd( vec2(24.0,0.0), psn, 0.0, 0 );
    vec3 nr5 = nhd( vec2(42.0,0.0), psn, 0.0, 0 );
/**/
/**/
    vec3 nr0 = nhd( vec2(1.0,0.0), psn, 0.0, 0 );
    vec3 nr1 = nhd( vec2(2.0,1.0), psn, 0.0, 0 );
    vec3 nr2 = nhd( vec2(6.0,3.0), psn, 0.0, 0 );
    vec3 nr3 = nhd( vec2(11.0,8.0), psn, 0.0, 0 );
    vec3 nr4 = nhd( vec2(24.0,15.0), psn, 0.0, 0 );
    vec3 nr5 = nhd( vec2(42.0,35.0), psn, 0.0, 0 );
/**/
...
/**/
    ...
    for(int i = 0; i < nc*nc; i++) {
        if(abs(ardf[i]) < abs(ardf[minvars[0]]) && abs(ardf[i]) >= 0.005) { 
            minvars[5] = minvars[4];
            minvars[4] = minvars[3];
            minvars[3] = minvars[2];
            minvars[2] = minvars[1];
...
// Output
    ...
    if(frames<3){
        float rnd1 = mod(fract(sin(dot(position + (time+5.0) * 0.001, vec2(14.9898,78.233))) * 43758.5453), 1.0);
