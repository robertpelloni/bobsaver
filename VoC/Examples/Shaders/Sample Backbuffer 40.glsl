...
void main( void ) {
    ...
    vec2 rate = position.xy * texture2D(backbuffer, position.xy).br;
    rate *= texture2D(backbuffer, position).r;
    rate *= texture2D(backbuffer, position).g;
    ...
    if ((me.r < 0.05) && (me.g < 0.05) && (me.b < 0.05)) {
        ...
        //vec2 cen=vec2(1.0,1.0);
        vec2 mult = 1.0 - rate;

        // Jitter range
        vec2 jitter = vec2(1.01 / resolution.x,
                           1.01 / resolution.y);
