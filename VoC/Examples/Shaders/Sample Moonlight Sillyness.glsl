...
// original https://www.shadertoy.com/view/ld3czS
...
vec3 calcCameraRayDir(float fov, vec2 gl_FragCoord, vec2 resolution) 
{
    ...
    vec2 d = fx * (gl_FragCoord.xy * 2.0 - resolution);
...
float ghost1(vec2 uv)
{
    ...
    d = max(d, -eyeL);
    d = max(d, -mouth);
    float threshold = mix(0.04, 0.06, (0.5 +sin(time)*0.5));
...
void main(void)
{
      vec2 res = vec2(max(resolution.x, resolution.y));
