...
// original https://www.shadertoy.com/view/XdcfR8
...
void main(void) {
    ...
    //    dir.yz *= rot((mouse*resolution.xy.y-resolution.y*0.5)*0.01);
    //    dir.xz *= rot((mouse*resolution.xy.x-resolution.x*0.5)*-0.01);
    //}
    
    // get the sine of the angular extent of a pixel
    float sinPix = sin(FOV / resolution.y);
    ...
    // add fog
    glFragColor.rgb = clamp(acc.rgb, vec3(0), vec3(1));
    float fog = clamp(totdist/20.0, 0.0, 1.0);
    glFragColor.rgb = mix(glFragColor.rgb, vec3(0.4, 0.5, 0.7), fog);
    // gamma correction
    glFragColor.rgb = pow(glFragColor.rgb, vec3(1.0/2.2));
    // vignetting
    vec2 vig = gl_FragCoord.xy/resolution.xy*2.0-1.0;
    glFragColor.rgb = mix(glFragColor.rgb, vec3(0), dot(vig, vig)*0.2);
