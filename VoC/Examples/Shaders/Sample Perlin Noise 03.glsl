...
float InterpNoise(float x, float y)
{
    ...
    float v3 = SmoothNoise(integer_x_int,     integer_Y + 10.0);
    float v4 = SmoothNoise(integer_x_int + 1.0, integer_Y + 2.0);
...
void main( void ) {
    ...
    glFragColor = vec4( mix(prevColor.xyz, curr_color, 0.05), 1.0 );
