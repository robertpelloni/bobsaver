...
// original https://www.shadertoy.com/view/fsBcRm
...
float arcd(vec2 c, vec2 p1, vec2 p2, vec2 p) {
    ...
    vec2 v1 = p1 - c;
    vec2 v2 = p2 - c;
    // Optinonal: make sure p1, p2 are both on the circle
...
void main(void)
{
    ...
    // Minor relief for the GPU: exit early in black region
    if(bbox == 0.0) {
    ...
    float secs = floor(mod(date.w, 60.0));
    float mins = floor(mod(date.w, 3600.0)/60.0);
    float hrs = floor(date.w/3600.0);
    ...
    d = min(d, nixiecolon(p-wspace-nspace*1.45)-0.2);
    ...
    // More like hot filaments than "cold cathodes", but... oh, well.
    float halo = 1.0-smoothstep(-0.3, 0.3, d);

    // Brittle parameters! This scale/shift of p has a strong impact on
    // the pattern at the edges of the grid through "anodefade".
