...
// original https://www.shadertoy.com/view/wljfWz
...
float warpedNoise(vec2 p) {
    ...
    vec2 m = vec2(time, -time)*.5;//vec2(sin(time*0.5), cos(time*0.5));
...
void main(void)
{
    // Screen coordinates. Using division by a scalar, namely "resolution.y," for aspect correctness.
    ...
    glFragColor = vec4(sqrt(max(col, 0.)), 1.); // Rough gamma correction.
}
