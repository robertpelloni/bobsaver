...
// original https://www.shadertoy.com/view/wd2Szm
...
// well on duplicate code sections. So rather then cache a calcuation in
...
void main(void)
{
    ...
    // standard uv fixup
    vec2 p = (2.0*gl_FragCoord.xy-R.xy)/R.y + 0.5*vec2(sin(0.2*T),sin(0.3*T));
    ...
    // final coloring
    glFragColor = vec4(v*vec3(1.+sin(i+T+i),1.+sin(i+T+1.),1.+sin(i+T+2.)),1.);
}
