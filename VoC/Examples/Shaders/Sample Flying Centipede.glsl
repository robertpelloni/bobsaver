...
// original https://www.shadertoy.com/view/ttlGR4
...
float leg( vec3 p, const bool complex, out vec4 albedo, float parm, float phase, int end ) {
    ...
    // walk animation
    float xro = time*-3.0+phase*2.0*PI + parm*12.0;
    float sro = sin(xro);
    float cro = cos(xro);
...
vec2 body( vec3 p, const bool complex, out vec4 albedo, float parm, int index ) {
    ...
    if ( complex ) {
        float tex1 = 0.0; //textureLod(iChannel0, inShell.xz*1.0 + float(index)*0.1424, 0.0).r;
        float tex2 = 0.0; //textureLod(iChannel0, inShell.xz*1.4 + float(index)*0.1424, 0.0).r;
...
vec4 getColor( in vec2 gl_FragCoord, in vec3 from, in vec3 dir ) {
    
    // get random stuff
    vec3 rnd = hash33(vec3(gl_FragCoord, frames*2+0));
    
    // find the angular extent of this pixel
    vec3 ddir = normalize(dir+fwidth(dir));
    float sinPix = length(cross(dir, ddir))*0.5;
...
void mainVR( out vec4 glFragColor, in vec2 gl_FragCoord, in vec3 fragRayOri, in vec3 fragRayDir ) {
    ...
    vec4 col = getColor(gl_FragCoord, fragRayOri/VR_SCALE, fragRayDir);
    glFragColor.rgb = mix(getBackground(fragRayDir), col.rgb, col.a);
    // tonemapping
    glFragColor.rgb = glFragColor.rgb / (glFragColor.rgb + vec3(1));
    // gamma correction
    glFragColor.rgb = pow(glFragColor.rgb, vec3(1.0/2.2));
    // add noise
    vec3 rnd = hash33(vec3(gl_FragCoord, frames*2+1));
    glFragColor.rgb += (rnd-0.5)*0.08;
...
void main(void) {
    ...
    vec2 r = vec2(time*0.1, 0.3);
    
    /*
    if (mouse*resolution.xy.z > 0.25) {
        r = mouse*resolution.xy.xy - resolution.xy*0.5;
        r *= -0.01;
    }
    */
    ...
    mainVR(glFragColor, gl_FragCoord.xy, from*VR_SCALE, dir);
    
    // add some vignetting in non vr
    vec2 uvv = gl_FragCoord.xy / resolution.xy * 2.0 - 1.0;
    float vigD = dot(uvv, uvv);
    glFragColor.rgb = mix(glFragColor.rgb, vec3(0), vigD*0.3);
