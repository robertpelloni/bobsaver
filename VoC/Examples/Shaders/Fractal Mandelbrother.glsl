...
// original https://www.shadertoy.com/view/tlBSRh
...
void powN1(inout vec3 z, float r, inout float dr) {
    ...
    float power = Power*cos(time*0.001+length(mouse*resolution.xy)*0.009);
...
//void main(void)
{
  ...
  float mx=mouse.x*resolution.xy.x*PI*0.009;
  float my=mouse.y*resolution.xy.y*PI*0.009;
  ...
  if (f<maxd){
    ...
    glFragColor = mix(vec4((b*c+pow(b,16.0))*(1.0-f*.01),1.0), vec4(c,1.0),0.58);
  }
  else glFragColor=vec4(.9,.9,.9,1); //background color
}
