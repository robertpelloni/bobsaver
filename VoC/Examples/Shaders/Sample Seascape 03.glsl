...
float sea_octave(vec2 uv, float choppy) {
    uv += noise(uv);        
    vec2 wv = 1.0-pow(sin(uv), vec2(2.));
    vec2 swv = pow(cos(uv), vec2(2.));    
    wv = mix(wv,swv,wv);
...
void main(void) {
    ...
    uv.x *= (uv.y+1.8);
    float rt = time;
    float time = time * -1.8;//+1./(uv.y+0.05);
    ...
    vec3 ori = vec3(0.0,5.5+sin(rt)*3.,time*5.0);
