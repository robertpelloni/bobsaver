...
// original https://www.shadertoy.com/view/MtlXR4
...
{    
    ...
    vec3 diffuse = vec3(0.2 * cosa);
    vec3 phong = vec3(0.8 * pow(cosr, 8.0));
...
{    
    ...
    vec3 tex = vec3(1.0);
    if (pos.y > -5.95) tex = vec3(1.0,0.9,0.0);
...
void main(void)
{
    ...
    vec2 xy = (gl_FragCoord.xy - resolution.xy/2.0) / max(resolution.xy.x, resolution.xy.y);
    
    vec3 campos = vec3(15.0*cos(t/5.0),10.0,15.0*sin(t/5.0));
