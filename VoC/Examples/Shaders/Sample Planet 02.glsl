...
// original https://www.shadertoy.com/view/lssGRH
...
void main( void ) {
    vec2 p = 2.*(gl_FragCoord.xy-resolution.xy*.5)/resolution.y;
    float d = length(p);
    p *= (acos(d) - 1.57079632)/d;
    glFragColor = vec4(func(p)*max(1.-d*d*d, 0.), 1.0);
}
