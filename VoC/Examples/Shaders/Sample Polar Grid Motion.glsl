...
// original https://www.shadertoy.com/view/XtcfRH
...
/*
    ...
    The particle movement is pretty standard. Each obect moves in a circular path with a 
    ...
    to look like they form a continuous connected strand. Potential overlapping objects
    consist of three polar cells and each of their adjacent polar neighbors -- That's nine
    altogether, which is not very taxing on the average GPU, so I don't expect any frame
    rate problems.
...
void main(void) {
    ...
    // Hence, the confusing minimal variable names. :)
    U = (2.*U - R)/yRes;
    
    // A non-transformed copy of the original screen coordinates. Used just once near
    // the end.
    vec2 oU = U;
    ...
    vec3 bg = vec3(1, .9, .8)*.5;
    
    // Apply some light. The background screne
    float light = getLight( vec3(oU, 2.6), n,lp);
    bg *= light;
     
    // Apply som marbly subtle noise.
    float ns = fbm(U*5. + 17.3);
    ns = mix(ns, sin(ns*32. - cos(ns*34.)), .125);
    bg *= max(ns, 0.)*.4 + .8;
    
    // Apply a grainy randomized diagonal pattern.
    float pat = clamp(sin((U.x - U.y)*min(R.y, 800.)/1.75)*1. + .5, 0., 1.);
    float rnd = fract(sin(dot(oU, vec2(11.27, 112.43)))*43758.5453);
    if(rnd>.5) pat *= .7; 
    else pat *= 1.3;
    bg *= pat*.35 + .7;  
    ...
    // OBJECT MOVEMENT.
    //
    // Moving the objects around the rose curve path, and storing their positions
    // for rendering.
    
    vec4 p; // Utility storage for the object position and ID.
    
    // A bit of global rotion. You could rotate every cell object individually, but 
    // this is a little faster.
    float t = time/2., cs = cos(t), sn = sin(t);
    U *= mat2(cs, sn, -sn, cs);
    
    // Storing "atan" to save a couple of extra calls. Not overly necessary, but
    // my oldschool brain still thinks of it as an expensive function. :)
    float a0 = atan(U.y, U.x);
    
    // Due to overlap, polar neighbors need to be considered. If the cell objects
    // are smaller, then it's not a problem, but I wanted a bit of object density.
    //
    for(int i=0; i<3; i++){ // Adjacent polar angle cells.
        ...
        for(int j=0; j<3; j++) {  // Three intertwining overlapping revolutions.
            ...
            // detail, it's a circular path with a varying sinusoidal radius that give it
    ...
    // or something along those lines -- implenting a quick swap, the GPU shouldn't really 
    ...
    for(int i=0; i<9; i++){
        for(int j=i+1; j<9; j++){
            ...
            // done in this case. If anyone knows differently, feel free to let me know.
            if(c[i].z<c[j].z){ 
    ...
    for(int i=0; i<3; i++){ // Adjacent polar angle cells.
        ...
        for (int j=0; j<3; j++) {  // Three intertwining overlapping revolutions.
            ...
            col = mix(col, pCol*light, 1. - smoothstep(0., falloff, d + .01));
            col = mix(col, vec3(0), 1. - smoothstep(0., falloff, d + sz - sz*.4));
            col = mix(col, vec3(1)*light, 1. - smoothstep(0., falloff, d + sz - sz*.4 + .01));
