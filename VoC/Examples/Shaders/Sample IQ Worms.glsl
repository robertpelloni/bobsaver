...
// original https://www.shadertoy.com/view/XsjXR1#
...
vec3 map( vec3 p )
{
    ...
    p += 0.5*vec3(sin(ve*time+(p.y+ph)*0.53), 0.0, 
                  cos(ve*time+(p.y+ph)*0.32) );
...
{    
    ...
    if( t < maxdist )
    {
        ...
        //col *= 0.2 + 0.8*texture2D( iChannel0, vec2( 50.0*res.z - 1.0*ve*time,0.5) ).xyz;
        //col *= 2.0;
        col *= occ;

        float fl = mod( ve*1.0*time + fid*7.0 + res.y*13.0, 4.0 )/4.0;
