...
// original https://www.shadertoy.com/view/wtd3RN
...
vec3 map(vec2 p) {
    ...
    // moustache 1
    q -= vec2(45.0, -15.0);
    q.y += q.x*q.x*0.003*sin(time*1.0);
    d2 = sd_line(q, vec2(0.0,0.0), vec2(45.0,0.0)) - 0.2;
    d = min(d, d2);

    // moustache 2
    q.y += 6.0;
    q.y += q.x*q.x*0.003*sin(time*2.0);
    d2 = sd_line(q, vec2(0.0,0.0), vec2(45.0,0.0)) - 0.2;
    d = min(d, d2);
    ...
    // middle legs
    q = sp - vec2(6.0, -50.0);
    q = rotate(q, -(0.5+0.5*sin(time))*0.1);
    ...
    // outer legs
    q = sp - vec2(22.0, -60.0);
    q = rotate(q, -(0.5+0.5*cos(time))*0.1);
    ...
    q = p - vec2(-26.0, -60.0);
    q = rotate(q, PI*0.7);
    q.x -= sin(q.y*0.123+3.0)*4.0 + sin(q.y*0.1+time*10.0);
    ...
    // nose
    d = sd_circle(q - vec2(0.0,-12.0+sin(time)*0.5), 2.0);

    // mouth
    d2 = sd_arc(q - vec2(0.0,-18.0), PI+sin(time)*0.3, 0.3, 4.0, 0.4);
    d = min(d, d2);
    ...
    q = sp;
    q.y -= -12.0;
    q.y /= 1.0 - smoothstep(0.98,1.0,sin(time*3.0));
