...
// original https://www.shadertoy.com/view/wtl3zX
...
void main(void) {
    ...
    // Screen coordinate  .
    vec2 uv = (gl_FragCoord.xy - resolution.xy*.55)/resolution.y;  
    
     uv *= resolution.y/450.; // Maintaining cellular size.

    // VECTOR SETUP - surface postion, ray origin, unit direction vector, and light postion.
    //
    // Setup: I find 2D bump mapping more intuitive to pretend I'm raytracing, then lighting a bump mapped plane. 
    vec3 ro = vec3(vec2(time*.35, sin(time*.05)*2.), -1);
    ...
    vec3 lp = ro + vec3(cos(time)*0.66, sin(time)*0.33, -1); // Light position - Back from the screen.
