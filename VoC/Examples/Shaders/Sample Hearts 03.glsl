...
float Hit(vec2 p,float s)
{
    // pardon the bad code.. bad copy pasta <https://www.shadertoy.com/view/4lK3Rc>
    vec3 q = vec3(p.x, p.y+.1, 0.01);
    if(length(q) > 0.5) return 1e7;
    q *= 200.0;
    vec2 res = vec2( 1., 2.0 );
    float r = 15.0;
    q.y -= r;
...
float Pass( in vec2 p, float a, float s )
{
    p = rot(p,a);
    return Hit(p,s);    
}
...
vec3 background()
{
    ...
    float d = Pass(uv, rr*TAU-time*0.8, size);
