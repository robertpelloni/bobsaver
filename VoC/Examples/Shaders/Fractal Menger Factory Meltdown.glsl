...
// original https://www.shadertoy.com/view/ltySzh
...
void main(void) {
    ...
    // add this all up
    glFragColor.rgb = (ao * texture(p)) * (ambient * (2. - LIGHT_COLOR) * .5 + (specular + diffuse) * shadow * LIGHT_COLOR) + glow(p);
    glFragColor.w=0.0;
    
    // edge glow
    glFragColor *= .3 + sqrt(steps / float(MAX_STEPS)) * .7;
    
    // fog
    glFragColor = mix(glFragColor, vec4((1. - LIGHT_COLOR) * .02, 1.), saturate(dist * dist * .05));
