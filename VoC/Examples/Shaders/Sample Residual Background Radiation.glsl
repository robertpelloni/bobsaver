...
// original https://www.shadertoy.com/view/4sfyzN
...
void main(void) {
    ...
    // Mouse position
    vec2 m = 1.0 - vec2(mouse*resolution.xy.xy / resolution.xy);
    ...
    for(float i = 1.0; i <= OCTAVES; i++){
        brightness += cell(pos - (m * 8.0), time * 0.5 * i) / i;
        pos *= OCTAVES;
    }
    ...
    glFragColor = vec4(hsb2rgb(vec3(hue, 0.5, brightness)), 1.0);
}
