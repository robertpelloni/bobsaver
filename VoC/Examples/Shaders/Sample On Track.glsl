...
// original https://www.shadertoy.com/view/wd2Sz3
...
float trackPlanks(vec3 tp){
    ...
    tp.z = mod(tp.z, 1.) -0.5;
    float bmp2 = 0.0; //texture(iChannel1, tp.xz/4.).r*.015;
...
void main(void)
{
    ...
    if(abs(d) < EPS+.4){
        vec3 tx = vec3(0.0);//tex3D(iChannel2, surfPos/8., n);
        glFragColor = mix(vec4(1.0*diff), bg, fog);        //+ hit* -.04;
        if(abs(d2)<EPS+.01){
            glFragColor = vec4(1.0, 0.0, 0.0, 1.0)*diff;
        }
        else if(abs(d3) < EPS+0.1){
            vec4 tx = vec4(0.0);// texture(iChannel1, surfPos.xz);
            glFragColor = vec4(0.0, 1.0, 0.0, 1.0)*diff;
        }
    }
    else
        glFragColor = vec4(bg);
