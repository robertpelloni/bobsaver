...
// original https://www.shadertoy.com/view/ttKBDd
...
void main(void) {
    ...
    if(p.w > 0.){
        ...
        float n1 = 2.*octnse(cuv, 10, -time*.08 * ts) - 1.;
        float n2 = 2.*octnse((cuv+3.), 10, -time*.03 * ts) - 1.;
    
        vec2 os = vec2(n1, n2);
    
        float val = octnse((cuv + vec2(n1, n2)*3.6), 8, -time*.1 * ts);
        
        col += .35+.35*cos(vec3(1.4, .7, 0.9)*n1*10. + time*.35);
        col += .48+.37*cos(vec3(2.2, .1, 0.3)*n2*20. + vec3(.7, 1.2, .7));
        col += .48+.23*cos(vec3(1.4, .7, 0.9)*val*30. + vec3(.2, 0.8, 4.7)+ time*.25);
        col*=.38;
        
        vec3 ref = reflect(n, rd);
        float val2 = octnse((ref.xy + os*10. + val*5.), 8, -time*.1 * ts);
