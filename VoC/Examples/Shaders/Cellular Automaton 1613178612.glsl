...
void main() {
    ...
    vec3 nr1 = nhd( vec2(2.0,1.0), psn, 0.0, 0, div );
    vec3 nr2 = nhd( vec2(6.0,3.0), psn, 0.0, 0, div );
    vec3 nr3 = nhd( vec2(11.0,8.0), psn, 0.0, 0, div );
    vec3 nr4 = nhd( vec2(24.0,15.0), psn, 0.0, 0, div );
    vec3 nr5 = nhd( vec2(42.0,35.0), psn, 0.0, 0, div );
    vec3 nr6 = nhd( vec2(76.0,55.0), psn, 0.0, 0, div );
    vec3 nr7 = nhd( vec2(125.0,96.0), psn, 0.0, 0, div );
    vec3 nr8 = nhd( vec2(180.0,155.0), psn, 0.0, 0, div );
...
// ardf[i+j*nc] = arrw[i] - arrw[j];
    ...
    float wsum = 1.0 - (0.003 + cs); // Needs a slight boost to hit highest values
    float[nc+1] wt;
