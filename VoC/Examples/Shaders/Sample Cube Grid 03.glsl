...
// original https://www.shadertoy.com/view/4dXGRM
...
vec3 trans(vec3 p, out float rotout) {
    ...
    rotout = time*1.88 + (rep.x+rep.z+rep.y)*0.3;
    p = rot(p, rotout);
    return p;    
}
...
void main(void)
{
    ...
    if(d < 0.06) {
        ...
        glFragColor = vec4(mix(col, bg, fog),1.0);
    }
    else {
