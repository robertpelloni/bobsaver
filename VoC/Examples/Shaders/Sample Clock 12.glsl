...
// original https://www.shadertoy.com/view/4sdBDn
...
float moveto(){
    float timeval=date.w;
    //timeval=mouse*resolution.xy.x/resolution.x*86400.;//test1
    //timeval=time*30.;//test2
...
void main(void)
{
    vec2 uv = (gl_FragCoord.xy-.5*resolution.xy) / resolution.y*vec2(9./16.,1.);
    ...
    uv.x+=moveto();
    float hline1=1.-step(1.8,mod(uv.y/.002,resolution.y)); //horizontal line
    #ifdef AA
    ...
    glFragColor = vec4(min(rgradient,lgradient)*(colorMinutes*(1.-vline2)+colorHours*vline2)*vline1*(step(-0.051-0.05*vline2,uv.y))*(+1.-step(0.051+0.05*vline2,uv.y)),1. );
    glFragColor += vec4(tri*colorTri,1.);
    #ifdef text
    glFragColor += vec4(drawminutes(uv)*colorTextM*min(rgradient,lgradient)*(1.-tmpvline2),1.);
    glFragColor+=vec4(drawhours(uv)*colorTextH*tmpvline2*min(rgradient,lgradient),1.);
    #endif
}
