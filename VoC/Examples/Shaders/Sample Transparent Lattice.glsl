...
// original https://www.shadertoy.com/view/Xd3SDs
...
float map(vec3 p){
    ...
    p = (cos(p*.315*2.5 + sin(p.zxy*.875*2.5)));        // + time*.5
...
void main(void) {
    ...
    vec3 ro = vec3(0., 0., time*1.5), col=vec3(0), sp, sn, lp, ld, rnd;
    ...
    float thD = .0125; // + smoothstep(-0.2, 0.2, sin(time*0.75 - 3.14159*0.4))*0.025;
