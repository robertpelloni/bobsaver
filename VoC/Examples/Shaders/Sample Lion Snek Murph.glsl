...
// original https://www.shadertoy.com/view/tsXyz8
...
float bo(vec3 p,vec3 r){p=abs(p)-r;return max(max(p.x,p.y),p.z);} //box primitive function. Hey, did you know you shouldn't put people in a box? (Unless they %&*$ing fit)
...
vec2 fb( vec3 p, float s, float m, float b ) // fb "%&*$ing bit" function make a base geometry which we use to make robot faces and outter geometry using more complex positions defined in mp
{ //fb just does a simple triple outlined box, really. Nothing is simple when you dip into triple brews though: try Triple Karmeliet beer and then we'll talk about how %&*$ Budweiser is.
  ...
  t.x*=0.7; return t; //Tweak distance field to avoid artifact and return the whole %&*$
}
vec2 mp( vec3 p )
{ 
  p.xy*=r2(.3);//tilt the whole %&*$ing scene a bit forward, make it more menacing, like a cross armed BTP officers fining skateboarders in the Corporation of London
  ...
  at=min(0.,(length(p+vec3(sin(tt*.5)*50.,-sin(p.y*.5+tt*5.)*5.,0))-30.)/30.); //spherical field displacement reverse attractor bull%&*$, name it whatever you wish, I'm way too busy counting the grains of rice in this bag.
  np.xy*=r2(mix(0.,0.8,bb)); //rotate %&*$ to switch faces using bb variable
  for(int i=0;i<4;i++){ //main kifs loop cloning, pushing, pulling rotating each iteration, bit like a sex club but without the overweight bouncer trying to slot himself in.
    ...
    np.xz+=cos(p.y*.4+1.5); // this tweaks face into cheekbones and %&*$ like that. Man I have no idea I do these things under the influence and now I'm sober so none of this makes sense. I have no idea how you tea-total nerds do it.
    if(i<3){ //Bp is for the chin / jaw and we need less iteration this if makes sure of that, politely but with the kind of vigour that you would't wanna %&*$ with, especially if you are just a piece of code.
      bp=abs(bp)-vec3(1.5,0,1.-at*7.); //another boring kifs bull%&*$ to make more geometry out of nothing, once again eluding everyone as to my lack of Math understanding and lack of giving a %&*$ about it.
    ...
    pp=abs(pp)-vec3(5,5.5,0);//%&*$ing hell one more kifs? One trick poney bull%&*$ right there. Well %&*$ it I just needed one more kif for the structure at bottom and top
    pp.yz*=r2(0.45);//Rather dull simple %&*$ here brohs...
    pp.xz*=r2(0.1); //...I've seen more exciting stuff happen at a funeral
  }  
  vec2 h,t=fb(np,1.2,3.,0.); //Now we have all the new positions, we call %&*$ing bit function with the more complex position np to create faces
  h=fb(bp,3.+sin(bp.z*.5)*2.,6.,0.); t=t.x<h.x?t:h;//Then we call again %&*$ing bit this time with bp complex position to create the bottom jaw
  h=fb(pp,30.,6.,0.); t=t.x<h.x?t:h; //Finally we call %&*$ing bit a third time with pp complex position to create structure above and bellow
  ...
  h.x=max(h.x,length(p.xz)-(5.+sin(p.y*.1)*20.)); // Ah yes nice %&*$ing bounding trick here broski, nothing clever but this limits the particles to sphere above head rather than them being everywhere.
  ...
  pp.xz=abs(pp.xz)-vec2(0,8.-at*15.);  //%&*$ yeah Einstein! A cat has two ears, so we abs symetry the %&*$er to have two ears instead of one.
  ...
  h.x=abs(h.x)-.3; //Onion skin the %&*$ing ears to get edges and create ear holes (not to be confused with bum holes)
  ...
  t=t.x<h.x?t:h; return t; // Add cat ears and return the whole %&*$
}
...
void main(void)
{
  ...
  tt=mod(time,62.8318);  //Time variable, modulo'ed to avoid ugly artifact. Imagine moduloing your timeline, you would become a cry baby straight after dying a bitter old man. Christ, that's some %&*$ing life you've lived, Steve.
  ...
  if(z.y>0.){ //Yeah we hit something, just like me stumbling home after 7 pints
    ...
    co=mix(sp+al*(a(.1)*a(.5)+.2)*(dif+s(2.)*.55),fo,min(fr,.5)); //Building the final lighting result, compressing the %&*$ outta everything above into an RGB %&*$ sandwich
    co=mix(fo,co,exp(-.00002*t*t*t)); //Fog soften things, but it won't stop Sophie's dildo battery from running out.
  }
  glFragColor = vec4(pow(co+g*0.2*mix(vec3(.4,.3,.2),vec3(1,.4,.2),-at),vec3(.5)),1);// Naive gamma correction and glow applied at the end. Glow has a mix of colours to make it more interesting.
}
