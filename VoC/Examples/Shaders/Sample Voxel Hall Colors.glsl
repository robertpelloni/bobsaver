...
// original https://www.shadertoy.com/view/fsXczN
...
float map(vec3 p){
     ...
     float n = dot(sin(p*1. + sin(p.yzx*.5 + time*.0)), vec3(.25)); // Sinusoidal layer.
...
void main(void) {
    ...
    // Camera Setup.
    vec3 camPos = vec3(0., 0.5, time*8.); // Camera position, doubling as the ray origin.
    ...
    if(t<FAR){
        ...
        //tint =  .7 + .5*cos(6.28318*(vec3(1.,0.1,0.4)*time*.25 + vec3(0.5,0.15,0.25))) ;
        tint = .7 + .5*cos(6.28318*(vec3(2.0,1.,0.)*time*.25 + vec3(.5,.2,.25)));
