...
// original https://www.shadertoy.com/view/td23WG
...
float gold_noise(in vec2 coord, in float seed) {
    return fract(tan(distance(coord * (seed + PHI), vec2(PHI, PI))) * ROOT2);
}
...
vec2 nmouse() {
    return mouse*resolution.xy.xy / resolution.xy;
}
...
void main(void) {
    ...
    float t = (48. + time) * 32. * (nsin(time / 64.) + 0.5);
    float scale =  0.3 + 256. * nmouse().x + 0.5 * nsin(t / 32.);
    float asp = resolution.x / resolution.y;
    uv = gl_FragCoord.xy /resolution.yy - vec2(asp, 1.) * .5;
    float uvl = length(uv);        // save to use later framing viewport
    float uva = arg(uv);
    vec2 st = uv;
    uv = rotate(uv, t / 2048. * sign(mod(time, 128.) - 64.));
    ...
    // Make the frame a roundy polygon:
    float k = 0.0 + ncos(t / 400.);
    float temp = 0.1 * sin((uva + PI) * floor(3. + mod(time / 32., 10.)));
    col = step(uvl + temp, k) * col;
    if (uvl + temp > k) {
        for (int i = 0; i < 5; i++) {
               vec2 pos = vec2(asp * (gold_noise(vec2(floor(time / 43.), 2. * floor(time / 29.)), 1.) - 0.5),
                                    gold_noise(vec2(floor(time / 53.), floor(time / 37.)), 2.) - 0.5);
             col += (0.5 + 0.5 * cos(time + uv.xyx + vec3(0, 2, 4))) * step(distance(uv, pos), 0.5);
        }
    }    
