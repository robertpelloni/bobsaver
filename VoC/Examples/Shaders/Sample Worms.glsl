...
// original https://www.shadertoy.com/view/Xd3BW2
...
vec3 getObjectColor(vec3 p, vec3 n, vec2 mat) {
    ...
    if (mat.x == 2.0) return vec3(.8, .0, .4) + floor(.1 + fract(p.x * 14. - 3. * time));
...
void main(void) {
    ...
    vec3 
        vuv = normalize(vec3(0., 1., sin(time) * .3)), // up
