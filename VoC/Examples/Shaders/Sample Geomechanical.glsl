...
// original https://www.shadertoy.com/view/MdcXzn
...
struct repeatInfo
{
    vec3 samplex; //object-space, cyclic
...
repeatInfo DF_repeatHex(vec3 p)
{
    ...
    //Variation
    float period = fract(time/30.)*3.0;
...
DF_out DF_composition( in vec3 pos )
{
    ...
    oHexA.d = DF_RoundedHex( infoA.samplex-vec3(0,infoA.anchor.y,0),
    ...
    oHexB.d = DF_RoundedHex( infoB.samplex-vec3(0,infoB.anchor.y,0),
                             g_animationChannels.geometry_width*HEX_HALF_WIDTH, PRIM_HEIGHT );
...
float SAMPLER_trilinear(vec3 p)
{
    ...
    vec2 layer1_layer2 = vec2(0.0,0.0);//texture2D(iChannel0,layer_translation+(pixCoord.xz+t.xz+0.5)/TEXTURE_RES,-100.0).xy; //Note : +0.5 to fall right on pixel center
...
vec4 MAT_apply(vec3 pos, TraceData traceData)
{
    ...
    float jitter_01 = SAMPLER_trilinear(pos*10.0+time*50.0);
...
Cam CAM_animate(vec2 uv, float fTime)
{
    ...
    cam.lens = 1.2+0.3*sin(time*0.1);
    cam.zoom = 3.0+sin(time*0.1)/cam.lens;
    return cam;
}
