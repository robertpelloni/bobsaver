...
// original https://www.shadertoy.com/view/4l2XWw
...
void main(void)
{
    ...
    // VARIABLES
    float t = fract(time * .05) * PI;
    vec3 sp = vec3(uv - 0.4, 0.);
    vec3 lp = vec3(1.0,1.0,0.0); //mouse*resolution.xy.z < .5 ? vec3(sin(t*5.)*10.,cos(t*8.)*10., -1.5) : vec3(m, -2.);
