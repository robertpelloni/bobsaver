...
//original https://www.shadertoy.com/view/ldf3RN
...
void main( void ) {
    ...
    for (float z = -1.; z <= 1.; z++) {
        for (float x = -1.; x <= 1.; x++) {
            ...
            float scale = 1./(1./(q.z+14.) - 1./(mouse2.y*9.-4.5+14.1));
    ...
    glFragColor = vec4( vec3(sqrt(color)), 1.0 );
