...
// original https://www.shadertoy.com/view/ls3BWf
...
{
    x.y *= 15. * (1.+ cos(time)/15.); // scale of my voronoi
    x.x *= 15. * (1.- sin(time)/10.); 
...
float map(vec3 originPos)
{
    ...
    return length(originPos) - 1. - .5*ret.x * (-.5) * (.5+sin(time)/5.) ; //*(sin(time)/2.-2.);  // peaks variation   
}
...
void main(void)
{
    ...
    if(t < MAX_DIST)
    {
        ...
        col += sphere_map(normalize(intersection)) * vec3(1.,0.001,0.00005) * (1.+sin(time)/2.);
    ...
    else // background
    {
        ...
        light = (1.+cos(time)/10.)*0.1 / distance(normalize(uv)*(1.+cos(time)/5.)*0.68, uv);
