...
// original https://www.shadertoy.com/view/flsfzH
...
float map(vec3 p){
  ...
  float s1 =sdSphere(p+vec3(1.0,0.1,0.0), 1.7*(sin(time+p.x)*0.005+1.0));
  float s2 =sdSphere(p+vec3(-1.0,0.0,0.1), 1.8*(sin(time+p.x+1.0)*0.005+1.0));
...
void main(void)
{
  ...
  float time =sin( time*0.2)*0.2+1.3;
  // camera    
