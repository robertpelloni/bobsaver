...
// original https://www.shadertoy.com/view/XlXGD7
...
//    this wonderful site and the noise functions which I am borrowing.
...
float hash3(vec2 p) {
    float h = dot(p, vec2(127.1, 311.7));
    float f = fract(sin(h) * 43758.5453123);
    return f * f * f;
}
...
bool getPosition(in vec3 camera, in vec3 dir, out vec2 pos) {
    ...
    if (h > 0.0) {
        ...
        pos = p.xz + (time - 25.0) * vec2(0.005, 0.02);
    }
...
void main(void) {
    ...
    if (getPosition(camera, dir, position)) {
        ...
        population *= (noise3(p) + coast); p = m * p;
        population *= (noise3(p) + coast); p = m * p;
        population *= (noise3(p) + coast); p = m * p;
        population *= (noise3(p) + coast); p = m * p;
        population *= (noise3(p) + coast);
        ...
        float weather = fbm(20.0 * position) * (0.6 + 0.4 * noise(10.0 * position));

        vec2 wind = vec2(fbm(30.0 * position), fbm(60.0 * position));
        float clouds = 0.7 * smoothstep(0.35, 0.5, weather) * smoothstep(-0.6, 1.0, fbm(wind));
        earth = mix(ground, vec3(0.5, 0.5, 0.5), clouds); 

        float lightning = 0.0;
        vec2 strikeScreen = vec2(hash(time), hash(time + 1.0));
        vec2 strikePosition;
        if (getPosition(camera, getDir(strikeScreen), strikePosition)) {
            vec2 diff = position - strikePosition;
    ...
    atmosphere = mix(atmosphere, vec3(0.05, 0.15, 0.4), smoothstep(0.992, 1.004, horizon));
    ...
    glFragColor = vec4(mix(earth, atmosphere, horizon), 1.0);
}
