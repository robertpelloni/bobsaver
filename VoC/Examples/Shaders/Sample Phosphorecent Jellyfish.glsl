...
// original https://www.shadertoy.com/view/4sXBRn
...
// Phosphorecent Jellyfish by Martijn Steinrucken aka BigWings - 2017
...
vec3 render( vec2 uv, ray camRay, float depth ) {
    ...
    if(o.m>0.) {
        ...
        if(o.m==1.) {    // hood color
            ...
            vec4 volTex = vec4(1., .1, .5, density/VOLUME_STEPS); 
...
void main(void)
{
    ...
    col = pow(col, vec3(1.6));        // post-processing
