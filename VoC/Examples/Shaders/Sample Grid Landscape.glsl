...
// original https://www.shadertoy.com/view/3dlSz2
...
{
    return vec2(sin(p.x+time*0.25)*0.15, sin(p.y-time*0.125)*0.15);
}
...
void main(void)
{
    ...
    if(mouse*resolution.xy.xy == vec2(0,0))
    {
        ...
        angles.x = tau * (3.9 / 8.0) + sin(time * 0.1) * 0.3;
    }
    else
    {    
        angles = vec3((mouse*resolution.xy.xy / resolution.xy) * pi, 0);
    ...
    #ifdef DITHER_ENABLE
    color = dither(color, COLOR_MODE, gl_FragCoord.xy);
    #endif */
