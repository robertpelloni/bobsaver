...
// original https://www.shadertoy.com/view/4l23zc
...
void main(void)
{
    float animWings = 0.004 * cos(time*0.5);
    float animFlap = 0.011 * sin(time*1.0);    
    float timeVal = 56.48-20.1601;
    vec2 uv = gl_FragCoord.xy - resolution.xy*.5;
    uv /= resolution.x*1.5113*abs(sin(timeVal));
    uv.y -= animWings*5.0; 
    vec2 tuv = uv*125.0;
    ...
    float tapU = (1.0/ float(resolution.x))*25.5;//*cos(animFlap);
    float tapV = (1.0/ float(resolution.y))*25.5;//*cos(animFlap);
    
  
    glFragColor = vec4( JuliaFractal(uv+vec2(0.0,0.0), vec2(juliax, juliay), animWings, animFlap ) ,1.0);
    
    glFragColor += vec4( JuliaFractal(uv+vec2(tapU,tapV), vec2(juliax, juliay), animWings, animFlap ) ,1.0);
//    glFragColor += vec4( JuliaFractal(uv+vec2(tapU,-tapV), vec2(juliax, juliay), animWings, animFlap ) ,1.0);
//    glFragColor += vec4( JuliaFractal(uv+vec2(-tapU,tapV), vec2(juliax, juliay), animWings, animFlap ) ,1.0);
    glFragColor += vec4( JuliaFractal(uv+vec2(-tapU,-tapV), vec2(juliax, juliay), animWings, animFlap ) ,1.0);  
    glFragColor *= 0.3333;
    
    glFragColor.xyz = glFragColor.zyx;
    glFragColor.xyz = vec3(1)-glFragColor.xyz;
