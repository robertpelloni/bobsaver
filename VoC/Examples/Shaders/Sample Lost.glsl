...
// original https://www.shadertoy.com/view/3ssfWB
...
void main(void)
{
    ...
    float sp = 1.5;
    float ph = mod(time*sp, (pi*2.0));
    float cy = floor(time*sp/(pi*2.0));
    ...
    rp += vec2((noise(time*2.0+id.x)-0.5)*(0.5-abs(ease-0.5)),
               (noise(time*2.0+id.y)-0.5)*(0.5-abs(ease-0.5)));
    rp *= rot(theta);
