...
float deltax=8.0+sin(time*1.0)*3.8; // Time-dependent zoom-in-zoom-out.
...
vec2 f(vec2 z)
{
  ...
  vec2 p = vec2(sin(time),cos(time));
  p = mul(p,z);
  p = p+vec2(sin(time),cos(2.*time));
  p = mul(p,z);
  p = p;//+vec2(sin(time+10.)/10.,cos(.4*time+5.)/10.);
  p = mul(p,z);
  p = p+vec2(2.,0.);
  p = mul(p,z);
  p = p+vec2(1.,1.);
...
void main(void)
{
  ...
  glFragColor = hsv2rgb(vec3(h,s,v));
}
