...
// original https://www.shadertoy.com/view/4tBGRm
...
int showSolution = 0; // set to zero to show the final horizontal velocity of the ball et given starting locations
int MotionBlur = 1; 
...
float time0;
float bounceTan=0.0;
float bounceLineAX=0.0,bounceLineBX=0.0;
...
vec2 displayBallPos=vec2(0.0),displayBallVel=vec2(0.0);

void main(void)
{
    pixelsize = 1.0/resolution.x;
    linethickness = pixelsize*2.0*aspect;
    ...
    sp = gl_FragCoord.xy / resolution.xy;
    sp.y /= aspect;
    ...
    if (length(ballpos)==0.0)  
        ballpos = vec2(0.19-mod(floor(time/newBallTiming)*0.02211,0.12),0.57);
...
//            float x = ballpos.x + ballvel.x*t + gravslope.x*t*t*0.5;
   ...
   if (showSolution!=0) color = max(color,vec3(-ballvel.x*3.0,ballvel.x*3.0,0.0));
