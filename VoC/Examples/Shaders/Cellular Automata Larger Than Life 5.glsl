...
vec2 kernel(vec2 uv)
{
    ...
    for(int i = -KERNEL_SIZE;i <= KERNEL_SIZE;i++)
    {
        for(int j = -KERNEL_SIZE;j <= KERNEL_SIZE;j++)
        {
            ...
            fill += region * texture2D(backbuffer,mod((uv+offset)/resolution,1.0)).a;
...
void main( void ) 
{
    ...
    float cur = texture2D(backbuffer,uv/resolution).a;
    ...
    glFragColor = vec4(color, state);
    
    #ifdef VIEW_REGIONS
    glFragColor.rg = getRegion(uv-center);
    #endif
    #ifdef VIEW_LIFE_FUNCTION
    glFragColor.rg = vec2(lifeFunction(uv/resolution));
    #endif
}
