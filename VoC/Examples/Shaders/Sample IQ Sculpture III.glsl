...
void main( void ) 
{
    sca = 0.5 + 0.15*sin(iGlobalTime-10.0);

    vec2 p = (-iResolution.xy+2.0*gl_FragCoord.xy) / iResolution.y;
    vec2 q = gl_FragCoord.xy / iResolution.xy;
   
    //float g = (1.0+sin(iGlobalTime*0.2))*3.0;
    //grow = smoothstep( 0.0, 1.0, (g-vec4(0.0,1.0,2.0,3.0))/3.0 );
    grow.x = 0.5-0.5*cos(iGlobalTime * 0.235234);
    grow.y = 0.5-0.5*cos(iGlobalTime * 0.325423);
    grow.z = 0.5-0.5*cos(iGlobalTime * 0.427434);
    grow.a = 0.5-0.5*cos(iGlobalTime * 0.283673);
