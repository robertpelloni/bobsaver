...
// original https://www.shadertoy.com/view/Xsd3zf
...
void main(void)
{
    ...
    //if (mouse*resolution.xy.z>0.0) addrot=(mouse*resolution.xy.xy-resolution.xy*0.5)*res;
    ...
    for (int i=0; i<layersblob;i++) {
        ...
        if (noise3(seedn)>0.2 && i<int(layers)) {
            ...
            //vec4 cc=filterFlake(glFragColor,(pos+addpos)*mr,normalize(ray*mr+nray*0.1),normalize(ray1*mr+nray*0.1),normalize(ray2*mr+nray*0.1));
            vec4 cc = filterFlake(glFragColor,(pos+addpos)*mr,ray*mr,ray1*mr,ray2*mr);
            ...
            //cc+=refcolor*0.5;
            glFragColor=mix(cc,glFragColor,min(1.0,glFragColor.w));
        }
        ...
        if (noise3(seedn*10.0)>0.4) {
            ...
            glFragColor.xyzw += vec4(1.0,1.2,3.0,1.0)*pow(l,5.0)*(pow(0.6+raf,2.0)-0.6)*mxcl;
        }
        mxc -= 1.1/layers;
        pos.z += step;
        iteratorc += 2.0;
    ...
    glFragColor.xyz += mix((cr.xyz-glFragColor.xyz)*0.1,vec3(0.2,0.5,1.0),clamp((-p.y+1.0)*0.5,0.0,1.0));
