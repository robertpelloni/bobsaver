...
// original https://www.shadertoy.com/view/msjXzR
...
float map(vec3 p, float t, inout vec3 lgt) {
    ...
    if (abs(p.x) < 1.5) {
        // Bridge steps
        q   = p;
        q.z = lrep(q.z, .7, floor(time*8. - 14.));
        ...
        q.z = lrep(p.z, 1.5, floor(time*8.));
        lb2 = box(q - vec3(1.2,2. + sin(time + id3.z)*.5,0), vec3(.1,.3,.1));
        d   = min(d, b2);        
    ...
    if (abs(p.y) > 3.) {
        ...
        d     = min(d, b3);
        lgt  += c*patternsCol * light(b3, 20., 2.) * exp(-t*.1 - (cos(max(0., time - 9.)*.5)*.5+.5)*10.);
    }  
    ...
    b4   = max(b4, -abs(p.x) +2.);
    d    = min(d, b3);
    if (fract((id0.x+41.11)*sin(id0.y*44.7)) < smoothstep(5., 8., time))
        lgt += c*pillarsCol * light(b4, 40., 2.) * (sin(time+p.y*(2.*fract(id0.x*id0.y*47.44))+id0.x*id0.y)*.5+.5);
...
void main(void) {
    ...
    glFragColor = vec4(pow(lgt, vec3(.45)), 1.0);
}
