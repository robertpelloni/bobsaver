...
// original https://www.shadertoy.com/view/3dyXRz
...
float noise2dT(vec2 uv)
{
    ...
    vec2 fl = floor(uv);
    uv = smoothv + fl;
    return 0.0;//textureLod(iChannel0, (uv + 0.5)/iChannelResolution[0].xy, 0.0).y;    // use constant here instead?
}
float noise(vec3 uv)
{
...
void main(void)
{
    ...
    vec2 uv = gl_FragCoord.xy/resolution.xy * 2.0 - 1.0;// - 0.5;
    ...
    float mx=mouse.x*resolution.xy.x/resolution.x*PI*2.0 + time * 0.01;
    float my=-mouse.y*resolution.xy.y/resolution.y*10.0 + sin(time * 0.03)*0.2+0.2;
