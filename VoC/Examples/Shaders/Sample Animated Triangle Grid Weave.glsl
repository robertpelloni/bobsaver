...
// original https://www.shadertoy.com/view/7sycz3
...
/*
    ...
    // An extuded simplex weave, but with no animation. It takes a while to
...
// 2D photoshop effects onto a canvase. I've explained it in more detail
...
void main(void) {
    ...
    vec2 p = uv*gSc - vec2(-.57735, -1)*time/50.;
    ...
    for(int i = 0; i<3; i++){
        ...
        if(vDist<vert){
            ...
            vertID = id + vID[i]; // Nearest vertex ID.
        }
    ...
    vec3 ani = (abs(fract(ang/6.2831*aNum + time*dir*laneDir*.5) - .5)*2. - .66)/aNum0/2.;
    ang = max(arc + .025, ani);
