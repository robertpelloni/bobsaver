...
// original https://www.shadertoy.com/view/MtsGDf
...
float fractalNoise(in vec3 vl) {
    float persistance = 2.;
    float persistanceA = 2.;
    float amplitude = .5;
    float rez = 0.0;
    float rez2 = 0.0;
    vec3 p = vl;
    
    for (float i = 0.0; i < OCTAVES / 2.; i++) {
    ...
    for (float i = OCTAVES / 2.; i < OCTAVES; i++) {
