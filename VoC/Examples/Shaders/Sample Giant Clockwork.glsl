...
// original https://www.shadertoy.com/view/WddXWB
...
float allGears(vec3 p, float rot, float tilt)
{
    ...
    for(int i=0;i<N+1;i++)
    {
        ...
        float gearAng=time*(float(i%2)*2.-1.)*r0/r-c2*.25;
...
vec4 myenv(vec3 pos, vec3 dir, float period)
{
    ...
    vec4 r1 = vec4(0.0); //.5*(texture(RandTex,dir.xy*.01)-texture(RandTex,dir.xy*.017+.33));
    vec3 skyCol=vec3(.9,1,1.1)+.3*(r1.xxx*.5+r1.xyz*.5);
    //skyCol*=2.5;
    vec4 r2 = vec4(0.0); //.5*(texture(RandTex,dir.xy*.1)-texture(RandTex,dir.xy*.07-.33));
    ...
    vec3 r=vec3(0.0); //texture(RandTex,vec2(azim/PI2*.125,.5)).xyz;
    col*= 1.-clamp(((r.xxx*.7+r.xzz*.3)*2.-1.)*clamp(1.-abs(dir.z*1.6),0.,1.),0.,1.);
    return vec4(col*col*vec3(1.1,1,.9)/**clamp(1.+dir.x*.3,.9,1.2)*/,1);
    //#endif
}

float distTruchetCell(vec3 pos, float rot, float tilt)
{
    return min(min(
        allGears(fract(pos.xyz               )-vec3(0,0,.5),rot,tilt),
        allGears(fract(pos.zxy*vec3( 1,-1, 1))-vec3(0,0,.5),rot,tilt)),
        allGears(fract(pos.yzx*vec3(-1,-1, 1))-vec3(0,0,.5),rot,tilt));
}
...
float truchetDist(vec3 pos)
{
    ...
    return distTruchetCell(pos*fact*mp.x,1.,PI2*.25*(mp.x*.5+.5));
}

float getTime()
{
    return time;//texelFetch(iChannel0,ivec2(0,0),0).x;
}
...
{
    float motion=1.;
    vec2 myRes=resolution.xy;
    vec3 pos=vec3(time*.4*motion-.5,0,0);
    vec2 scoord=(gl_FragCoord.xy-myRes*.5)/myRes.x;
    float phi = time*.333*.3*motion;
    float th = cos(time*.23423*.3)*motion;
    //if(mouse*resolution.xy.x>.5) 
    //{
    //    phi+=-mouse*resolution.xy.x/resolution.x*PI2/2.*3.;
    //    th+=PI2/4.-mouse*resolution.xy.y/resolution.y*PI2/2.*3.;
    //}
    ...
    #ifdef DOLLY_ZOOM
    calcDollyZoom(pos,dir0,dir,.6*clamp(-cos(time*.1)*5.+.5,0.,1.));
    #endif
