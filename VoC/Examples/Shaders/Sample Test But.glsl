...
// original https://www.shadertoy.com/view/7dcyRr
...
vec3 col = vec3(0);
   ...
   for( int m=0; m<4; m++ )
   {
      ...
      float k = 2.0*pow(abs(sin(0.5*time)),0.2);
...
vec4 dist(vec3 p){
    vec3 p1 = p;
    float k = 2.;
    float ksstx = clamp(3.*(fract(0.1*time)-2./3.),0.0,1.0);
    float kssty = clamp(3.*(fract(0.1*time)-1./3.),0.0,1.0);
    float ksstz = clamp(3.*fract(0.1*time),0.0,1.0);
    ...
    vec3 ps = p1-vec3(1.);
    float ktx = 0.3;
    float kt = fract(time*ktx);
    float d1 = cnbox(ps,ease_in_out_cubic(kt),1.+mod(ease_in_out_cubic(kt)+floor(time*ktx),BOXITER));
