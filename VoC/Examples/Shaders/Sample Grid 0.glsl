...
// original https://www.shadertoy.com/view/MdVBWz
...
void main(void)
{
    ...
    t = time;
    vec3    col = vec3(0., 0., 0.);
    vec2    R = resolution.xy, uv  = (f-R*.5)/R.yy;
    vec3    dir = camera(uv);
    vec3    pos = vec3(-.0, .0, 2.-time);
...
float    scene(vec3 p)
{
    ...
    rotate(p.xy, p.z*.25 + 1.0*sin(p.z*.125 - time*0.5) + 0.*time);
    ...
    rotate(pr.xy, time);
    #else
    rotate(pr.xy, clamp( + (mod(id.x*.5, 2.)-1. <= 0. ? 1. : -1.) + (mod(id.y*.5, 2.)-1. <= 0. ? 1. : -1.), -2., 2.) * time*.5 );
    #endif
...
vec4    march(vec3 pos, vec3 dir)
{
    ...
    {
        dirr = dir;
        rotate(dirr.xy, .51025*dist.y+time*-.0 );
        p = pos + dirr * dist.y;
        dist.x = scene(p);
        dist.y += dist.x*.5;
        float    d = 5.;
