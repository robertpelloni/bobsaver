...
//----------------------------------------------------------------------------------------
// RemnantX.glsl   Real-time Shadertoy 3D fractal             2014-09-17  by David Hoskins
//                 see https://www.shadertoy.com/view/4sjSW1
// Thanks to boxplorer and the folks at 'Fractalforums.com'
// HD Video:- https://www.youtube.com/watch?v=BjkK9fLXXo0
// Binary subdivision finds the surface really well with this fractal.
// Two light sources with shadows and near surface glows.
// Hint: Change mouse X for time warp.
//----------------------------------------------------------------------------------------
...
vec3 surfaceColour3 = vec3(0.5, 0.3, 0.0);
...
float BinarySubdivision(in vec3 rO, in vec3 rD, float t, float oldT)
{
  float halfwayT = 0.0;
  halfwayT = (oldT + t ) * 0.5;
  if (Map(rO + halfwayT*rD) < .001) t = halfwayT; else oldT = halfwayT;
  halfwayT = (oldT + t ) * 0.5;
  if (Map(rO + halfwayT*rD) < .001) t = halfwayT; else oldT = halfwayT;
  halfwayT = (oldT + t ) * 0.5;
  if (Map(rO + halfwayT*rD) < .001) t = halfwayT; else oldT = halfwayT;
...
{
  // float t = .05 + 0.05 * texture2D(iChannel0, gl_FragCoord.xy / resolution.xy).y;
  float t = .05;
  vec3 p = vec3(0.0);
  float oldT = 0.0;
  bool hit = false;
...
{
  rgb = pow(rgb, vec3(0.47));   // Gamma first...
...
void main(void)
{
  float m = (mouse.x ) * 100.0;
  gTime = (time + m) * 0.002 + 15.0;
