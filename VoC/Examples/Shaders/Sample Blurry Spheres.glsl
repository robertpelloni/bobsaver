...
// original https://www.shadertoy.com/view/4scBW8
...
vec3 getSphereCenter( in vec2 c ) {
    return vec3(c, sin(c.x-c.y*4.3+time)*0.2);
}
...
void main(void) {
    ...
    float focal = 2.5;
    //if (mouse*resolution.xy.z > 0.5) focal = 1.0+mouse*resolution.xy.y/resolution.y*4.0;
    float sinPix = sin(FOV / resolution.y);
    vec4 acc = vec4(0, 0, 0, 1);
    vec3 dummy = vec3(0);
    float totdist = de(from, dir, 0.0, dummy)*hash13(vec3(gl_FragCoord.xy, frames));
    for (int i = 0 ; i < 100 ; i++) {
