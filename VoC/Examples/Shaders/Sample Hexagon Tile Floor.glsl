...
// original https://www.shadertoy.com/view/3dXXRj
...
Result scene (in vec3 p)
{
    ...
    float variance1 = .1*sin (noise2d (cellIndex1).x*time);
    float variance2 = .1*sin (noise2d (cellIndex2).y*time);
...
void main(void)
{
    ...
    vec3 ro = vec3 (5.*cos (.3*time), 3. + .1*cos (time), 5.*sin (.3*time));
    vec3 aim = vec3 (.0, .0, .0);
    float zoom = 1.;
