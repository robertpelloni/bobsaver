...
// original https://www.shadertoy.com/view/MlfGR4
...
void main(void) {
    ...
    modeAngle = dot (frag - vec2 (cos (time * 2.0), 0.0), vec2 (cos (modeAngle), sin (modeAngle)));
    ...
    #ifdef MOUSE
    yawAngle += 4.0 * PI * resolution.xy.x / resolution.x;
    pitchAngle += PI * 0.3 * (1.0 - resolution.xy.y / resolution.y);
    #endif
    ...
    float cameraDist = mix (300.0, 95.0 + 50.0 * cos (time * 0.8), mode3D);
    vec3 origin = (vec3 (0.0, 0.0, 40.0 * sin (time * 0.2)) - cameraOrientation [2] * cameraDist) / VOXEL_RESOLUTION;
    ...
    if (d.x < DELTA) {
        ...
        // Lighting
        vec3 l = normalize (mix (vec3 (1.0, 0.0, 0.0), vec3 (1.25 + cos (time * 0.2), 1.0, 1.0), mode3D));
