...
// original https://www.shadertoy.com/view/3tySRR
...
void main(void) {
  ...
  float xform_s = (BBOX_X[1] - BBOX_X[0]) / resolution.x;
  vec2 xform_t = vec2(
      BBOX_X[0],
      BBOX_Y1 - (BBOX_X[1] - BBOX_X[0]) * (resolution.y / resolution.x));

  vec2 p = frag_coord * xform_s + xform_t;
  bool mouse_down = false;//mouse*resolution.xy.z > 0.5;
  ...
  vec3 color = vec3(1.0);
  {
    ...
    // Main rendering
    //
    SceneInfo info = getSceneSdf(p, time);
