...
// original https://www.shadertoy.com/view/Xl2GDW
...
float map(in vec3 pos)
{
    ...
    float shape = mix(max(-negCircle, cube), cube, 0.5 + sin(time * 0.4) * 0.5);;
...
void main(void)
{
    ...
    vec3 ro = vec3(0.1 * cos(time * 0.4), 0.05 * sin(time * 0.25), -3.0);
    vec3 rd = normalize(vec3(p.x, p.y, 0.75));
    vec3 lightPos = vec3(cos(time * 0.1) * 3.0, 0.0, sin(time * 0.4) * 3.0);
