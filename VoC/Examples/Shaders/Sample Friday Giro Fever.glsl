...
// original https://www.shadertoy.com/view/wdXBzs
...
vec2 fb( vec3 p,float s,float m) // fb "%&*$ing bit" function make a base geometry which we use to make gyro and flying objects
{
  for(int i=0;i<4;i++){//bull%&*$ kifs rotation cloner. "Friday Giro Fever" is a reference to the UK umemployment benefits, the "giro". It usually lands on friday, meaning there is always a party for everyone, every friday, in England.
...
{ 
  vec2 h,t=vec2(cy(p,vec3(6,.5,1)),8);//Use my fancy %&*$ing hollow tube primitive function, I know some clever bastard probably thought about it before but this is me we're talking about: I once got into an argument with the neighbour's dog and lost.
  h=vec2(cy(p,vec3(6,.5,5)),3);//Make another hollow toob, this time black and...
  h.x=abs(h.x)-.1;//...whip that %&*$er into shape, push it against the edges, and other sexual innuendos...
  h.x=max(h.x,abs(p.y)-.6); //croperoo the %&*$eroo and whatch it groove
  t=t.x<h.x?t:h; //Make toobs touch boobs! (blend geometry together while retaining nipple colour, I mean material ID)
  ...
  h=vec2(cy(p,vec3(6.9,0,.1)),6); //One more hollow tube this time around the whole %&*$, make it glow, so it's nice a central focal point to the composition (look at me talking like I actually know what I'm doing... %&*$ing hipster)
...
{ 
  ...
  vec2 h,t=fb(bp*.6,0.,5.); //Make first gyro bit and spin the %&*$er
  ...
  h=fb(np,0.,5.); //Make another %&*$ing gyro and make it spin
  ...
  h=fb(bp*1.7,0.,5.);h.x/=1.7;t=t.x<h.x?t:h; //Yeah yeah one more gyro, blah blah blah, something about distance %&*$ing fields, man I'm so borded when is football restarting, being a geek is cool, but hooliganism is more rewarding.
  h=vec2(length(p)-.4);//Dumb %&*$ing sphere in middle, make it glow, call the Tate Modern, tell them about my struggle as a child in our anonymous post-post-modern society and give me the Turner Prize.
  ...
  pp.xz*=r2(sin(tt));//Rotate the %&*$ing whole thing back and forth
  ...
  g+=0.2/(0.1+h.x*h.x*(50.-sin(pp.z*.2+tt*2.)*45.)); //Glow the fat white line along z axis, I know, I know, it is inviting. Who doesn't like to buy a gram of fun and chat %&*$ to their friends?
...
vec4 texNoise(vec2 uv){ float f = 0.; //f+=texture(iChannel0, uv*.125).r*.5;
    ...
    f=pow(f,1.2);return vec4(f*.45+.05);
}
...
void main(void)
{
  ...
  tt=mod(time,62.82);  //Time variable, modulo'ed to avoid ugly artifact. Imagine moduloing your timeline, you would become a cry baby straight after dying a bitter old man. Christ, that's some %&*$ing life you've lived, Steve.
  ...
  z=tr(ro,rd,50.,128);t=z.x; //Trace the trace in the loop de loop. Sow those %&*$ing ray seeds and reap them %&*$ing pixels.
  if(z.y>0.){ //Yeah we hit something, unlike you trying to throw a spear at a pig. We wouldnt have survive the ice age with you and your nerdy mates.
    ...
    co=mix(sp+mix(vec3(.8),vec3(1.),abs(rd))*al*(a(.1)*a(.2)+.2)*(dif+s(13.)),fo,min(fr,0.2));//Building the final lighting result, compressing the %&*$ outta everything above into an RGB %&*$ sandwich
    if(z.y>7.||z.y==5.){ //If material id is 5 or more than 7 then we do reflections
      ...
      if(z.y>0.){ //If we hit something in reflection ray... (optimization and cleanup this)
        ...
        if(z.y>5.) al=vec3(1);//Still pretty boring %&*$
  ...
  glFragColor = vec4(pow(co+g*.2+gg*.3*vec3(.0,.1,.7),vec3(.45)),1);
}
