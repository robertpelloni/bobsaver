...
// original https://www.shadertoy.com/view/7dy3zt
...
void main(void)
{
    ...
    mask += mask1;
    vec3 col = mask * abs(sin(itime))* abs(sin(itime)) * vec3(p +.3);

    // Output to screen
    glFragColor = vec4(cos(time * col) * sin(time * col) + col,1.0);
}
