...
// original https://www.shadertoy.com/view/Wt2SRw
...
float map(vec3 p){
    ...
    float theta = 0.;
    float a = 0.2;
    float begin = sin(time * 2. * (0.2 + rand(theta)) + theta) * a;
    float prev = begin;
    float curr = 0.;
    ...
    for(int i = 1; i < 12; i ++){
        theta += PI / 6.0;
        curr = sin(time * 2. * (0.2 + rand(theta)) + theta) * a;
    ...
    theta += PI / 6.0;
    curr = sin(time * 2. * (0.2 + rand(0.)) + 0.) * a;
    res = min(res,line(p,
                      vec3(r * sin(theta),curr,r * cos(theta)),
                      vec3(r * sin(theta - PI / 6.0),prev,r * cos(theta - PI / 6.0)),
                      0.025));
