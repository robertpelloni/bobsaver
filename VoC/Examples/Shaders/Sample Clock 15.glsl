...
// original https://www.shadertoy.com/view/ldVBDD
...
void main(void)
{
    ...
    for(int i = 0; i < 6; i++)
    {
        P[i] = (pos - vec2(1.0 - 5.0*k * float(i), 0.0) - k*O) * rot(0.1 * sin(0.05 / (pow(0.5, float(i)) * T[i] * fract(date.w / T[i])) )) + k*O;
        num[i] = draw_num(P[i] / k, R / k, int(mod(date.w / T[i], t[i])));
        light += num[i];
    }
    for(int i = 0; i < 2; i++)
    {
        ...
        light += k * (point[2*i] + point[2*i+1]) * mod(floor(date.w), 2.0);
    }
    col *= light;
