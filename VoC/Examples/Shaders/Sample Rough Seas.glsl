...
// original https://www.shadertoy.com/view/dtXGW4
...
//------------------------------------------------------------------------------
void main(void) //WARNING - variables void ( out vec4 outCol, in vec2 coord ) need changing to glFragColor and gl_FragCoord.xy
{
	//vec4 out = vec4(0.0);
    ...
    vec2 uv = (coord-resolution.xy*.5)/resolution.y;

    vec2 mouse = vec2(0);
//#ifndef EXPORT_VERSON
//    if (mouse*resolution.xy.z > 0.) mouse = (mouse*resolution.xy.xy-resolution.xy*.5) /resolution.y;
//#endif
    ...
    camPos = vec3(time2*.01,100,0);
    float h = (sin(time2*.65)+1.0)*40.+ 10.;
    ...
    {
        oce += oceanFundamental(camPos.xz, 0.0, time2 + i, 7.0);
    }
    oce = (oce / 4.0)+140.0;
    oce= h-oce;
    if (oce < 0.0)
    ...
    vec3 col;
    vec3 dir  = vec3(0,0, 1.);
    dir = viewMat (uv.y -.3, uv.x-2.+time2*.25 + mouse.x*6.28) * dir;
    ...
    vec2 xy = coord/resolution.xy;
    colour *= 0.5 + .5*pow( 80.0*xy.x*xy.y*(1.0-xy.x)*(1.0-xy.y), .7);  // ...Vignette.
    // Fade out at five minutes...
#ifdef EXPORT_VERSON
    outCol = vec4(fader(0.0, 4.0, time2) * fader(299.0, 294.0, time2)*sqrt(colour), 1);
#else
    outCol = vec4(fader(0.0, 4.0, time2) * sqrt(colour), 1);
#endif
