...
// original https://www.shadertoy.com/view/Md23Rd
...
float DE2(vec3 z, out int mat) { 
 ...
 float inverse = -base + (1.0+cos(time2/4.0))*4.0;
 if (base>inverse) {
      mat = (sphere>-0.1) ? 0 : 2;
 } else {
     mat = (sphere>-0.1) ? 1 : 3;
...
vec4 rayMarch(in vec3 from, in vec3 dir) {
    ...
    float totalDistance = rand(gl_FragCoord.xy+vec2(time2));
...
{ 
    time2 = time + 38.0;
    scale = 0.8+(0.1*cos(time2/3.0));
    offset = vec3(1.0+0.2*(cos(time2/5.7)),0.3+0.1*(cos(time2/1.7)),1.).xzy;

    float angle = time2/5.0; 
    
    // Camera position (eye), and camera target
    vec3 camPos =  19.0*vec3(1.0,0.5*curve(time2*0.2),0.5*curve(7.0+0.3*time2));
    ...
    glFragColor = rayMarch(camPos, rayDir);
}
