...
// original https://www.shadertoy.com/view/XlfXR4
...
void main(void)
{
    a = vec3(1.85,1.25,1.85) + 0.1*cos(5.0+0.7*time + vec3(0.5,1.0,2.0) );
    b = vec3(0.60,0.60,0.60) + 0.1*cos(4.0+0.5*time + vec3(2.5,5.0,3.0) );
    c = vec3(0.40,0.40,0.40) + 0.1*cos(1.0+0.3*time + vec3(6.0,2.0,4.2) );
    m = cos( 0.11*time + vec3(2.0,0.0,5.0) );
    n = cos( 0.17*time + vec3(3.0,1.0,4.0) );
    ...
    for( int i=0; i<256; i++ )
    {
        ...
        vec3 cp = xb + sd.y*(xc-xb);
        col += gc*0.8*cc*exp(-2.0*sd.x)*exp( -0.05*length(cp-ro) );
        }
