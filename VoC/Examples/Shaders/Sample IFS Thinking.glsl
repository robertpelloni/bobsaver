...
// original https://www.shadertoy.com/view/WlXGzM
...
vec2 map(vec3 p)
{
    ...
    vec2 mouse = remap(mouse*resolution.xy.xy/resolution.xy, vec2(0.0), vec2(1.0), vec2(-pi), vec2(pi));
    for (int i = 0; i < ITERATION_NUM; i++) {
        p = abs(p) - offset * s;
        
        float phase = time * 4.0+float(i)*0.25+distFromCam*5.;
...
vec3 materialize(vec3 p, vec3 ray, float depth, vec2 mat)
{
    ...
    float t = mod(time * 3.0, float(ITERATION_NUM));
