...
// original https://www.shadertoy.com/view/tsBXRW
...
void main(void) {
  ...
  */
  ...
  vec2 rot = normalize(vec2(A,B));
  //z = clog(z);
  //z = 2.0*catanh(z);
  z = 2.0*(3.0*catanh(z) - 2.0*catanh(cmul(vec2(cos(0.1*time),sin(0.1*time)), z)));
  z = cmul(rot,z);
  z /= 2.0*PI/sqrt(A*A+B*B); // Alignment
  z *= K;
  z.y += 2.0*time;
  vec2 index = round(z);
  z -= index;
...
#endif  
  vec3 colx = h2rgb(hx);
  vec3 coly = h2rgb(hy+0.5);
#if defined CHECKERBOARD
  float k = z.x*z.y;
  vec3 col = mix(colx,coly, smoothstep(-0.1,0.1,sign(k)*sqrt(abs(k)))); // Checkerboard
#else  
  vec3 col = mix(coly,colx, smoothstep(0.15,0.16,d)); // Concentric
#endif
  col *= 1.0-smoothstep(0.38,0.4,d);
