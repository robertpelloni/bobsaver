...
// original https://www.shadertoy.com/view/wddfR2
...
/*
Code modified slightly from https://www.shadertoy.com/view/4dt3zn
...
float map(vec3 p)
{
    ...
    p = fract(p + rnd*sin(time*rnd*100.0));
    p = p   - .5; //* (1.0/(length(p)*time)+1.0)
...
void main(void) {
    ...
    for(int i=0 ; i<BOUNCES; i++) {
        sn = getNormal(ro);
        rd = reflect(rd, sn);
        t = trace(ro +  sn*.003, rd);
        ro += rd*t;
        refColor = refColor + doColor(ro, rd, sn, lp, t)*(1.0/(float(i)+1.0));
    }
