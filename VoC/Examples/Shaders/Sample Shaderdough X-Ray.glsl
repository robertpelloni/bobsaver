...
// original https://www.shadertoy.com/view/XtGyWh
...
vec3 icosahedronVertex(vec3 p) {
    vec3 sp, v1, v2, v3, result, plane, plane2, plane3;
    float split;
    sp = sign(p);
    v1 = vec3(PHI, 1, 0) * sp;
    v2 = vec3(1, 0, PHI) * sp;
    ...
    plane2 = vec3(-PHI - 1., 1, PHI) * sp;    
    plane3 = vec3(PHI, -PHI - 1., 1) * -sp;
    plane = mix(plane3, plane2, split);
    split = max(sign(dot(p, plane)), 0.);
