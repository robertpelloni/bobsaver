...
// original https://www.shadertoy.com/view/4dXBD2
...
vec3 camera (vec3 p) {
    p.xz *= rot((PI*(-.5)*step(0.5,0.0)));
    p.yz *= rot((PI*(-.5)*step(0.5,0.0)));
    p.yz *= rot(PI/2.);
    return p;
}
...
void main(void)
{
    ...
    vec2 uv = (coord.xy-.5*resolution.xy)/resolution.y;
    #ifdef DITHER
    vec2 dpos = ( coord.xy / resolution.xy );
    ...
    glFragColor = vec4(1);
    glFragColor.rgb = n*.5+.5;
    glFragColor.rgb *= 1.-shade/(STEPS-1.);
}
