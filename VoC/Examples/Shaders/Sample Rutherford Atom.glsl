...
// original https://www.shadertoy.com/view/lt2Gzh
...
vec2 map(in vec3 pos) {    
    ...
    pos = pos.xzy;
    pos = rotX(0.2)*pos;
    vec3 p0 = rotY(-0.1*time)*rotZ(-0.2*time)*pos;
    ...
    // Beryllium?
    pos = pos.xzy;
    vec3 p0 = rotY(-0.3*time)*rotZ(-0.2*time)*pos;
    ...
    for (int i=0; i<4; ++i) {
        vec3 pi = rotX(0.04)*rotZ(s*rz*time)*rotY(-s*rx)*rotX(s*rx*0.5+0.35)*pos;
...
void main(void) {
    ...
    cpos = vec3(0,0,3.5);
    vec2 uv = (gl_FragCoord.xy - .5*resolution.xy) * 1.25 / (resolution.y);
    ...
    if (max(mouse.x, mouse.y) > 20.0) { 
        thetax = (mouse.y - .5*resolution.y) * 3.14/resolution.y; 
        thetay = (mouse.x - .5*resolution.x) * -6.28/resolution.x; 
    }
