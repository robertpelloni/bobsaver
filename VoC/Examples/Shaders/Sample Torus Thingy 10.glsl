...
// original https://www.shadertoy.com/view/XtjBWm
...
{  
    ...
    var = sin(floor(sin(atan(p.x, p.y)*4.+time*2.)*3.14 + 3.14*sin(floor(atan(q.x, q.y)*4. + time*5.) + ((time*1.1))*.8 ) ));//+sin(floor(atan(p.x, p.y)*4. +floor(time*10.1)*.1 ) );
