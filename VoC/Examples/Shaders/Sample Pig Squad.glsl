...
// original https://www.shadertoy.com/view/WdBcRh
...
float sdPig(float3 p,float jump) {
    p*= 1.0 + vec3(-0.2,0.2,-0.2)*(0.5+0.5*sin(time*10.0+3.5));
...
float GetDist(float3 p) {
    ...
    //float2 h = hash22(id);
    float g = sdPig(p, max(sin(time * 10. /*+ h.x * 3.141529 * 2.*/), .0));
...
void main(void)
{
    ...
    vec3 ro = vec3(0, 5, -5);
    ro.yz *= Rot(-0.4);
    ro.xz *= Rot(time*0.5+2.);
    vec3 rd = GetRayDir(uv, ro, vec3(0), 1.);
    
    glFragColor = scene(ro,rd);
}
