...
// original https://www.shadertoy.com/view/ldfSDN
...
// Sort of a cross between https://www.shadertoy.com/view/MdXSD8 and
...
void main(void)
{
    ...
    if(id > 0.5 && id < 1.5)
    {
        ...
        vec3 noiseColor = getFragColor(cnoise(vec2(time + refractionVec.x + uv.x, refractionVec.y + uv.y))).rgb;
        col = mix(noiseColor, reflectColor, reflectColor);
    }
    else
        col = getFragColor(cnoise(vec2(time/2.0 + uv.x, uv.y))).rgb;
