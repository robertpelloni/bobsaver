...
float finger(vec3 p, float z){
    vec3 j4=vec3(0.7+sin(z+time2+sin(z*1.3))*0.25,abs(sin(z+time2))*0.6,0.0);
...
void main( void )
{
    ...
    randSeed=fract(sin(dot(gl_FragCoord.xy,vec2(13.434,77.2378))+time2*0.1)*41323.34526);
#endif
    vec3 ro=vec3(2.0*sin(time2*0.1),1.5+sin(time2*0.03)*0.6,time2*0.6+cos(time2*0.135));
...
//#endif
    ...
    glFragColor = vec4(clamp(col*2.0,0.0,1.0),1.0);
