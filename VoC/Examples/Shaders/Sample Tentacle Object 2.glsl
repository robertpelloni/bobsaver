...
// original https://www.shadertoy.com/view/wlfSRn
...
mat3 rot(float r)
{
    float t = time - r * 2.;
    float s = .5 + .5 * r;
    return rotX(cos(t / 1.5 * 4.) * s * .2) * rotY(sin(t / 3.) * s);
}

// Scene SDF
float dist(vec3 p)
{
    const float precis = 16.0;
    ...
    vec2 sf2 = inverseSF( normalize(p), precis * 10., q2 ); 
    vec2 sf3 = inverseSF( normalize(p), precis * 4., q2 ); 
    
    q *= k;
    
    float d = length(p), od = d;
    
    // Alternating tentacle lengths based on spiral point ID.
    float r3 = (mod(sf.x, 3.) < 1.) ? .7 : 1.5;
    float r2 = r3 / k;

    d = smax(sf.y - diff * 2.2 - .03 / dot(p, p), d - r3, 32. / 200.);
    ...
    // More tentacles and dimples
    d = min(d, smax(sf3.y - diff * 2.2 - .01 / dot(p, p), od - .9, 16. / 200.));
    d = min(d, smax(-d + .03, od - .7 + (1.-smoothstep(0.,.06,sf2.y)) * .1, 16. / 200.));
...
vec3 render()
{
    ...
    //time = time + texelFetch(iChannel1, ivec2(gl_FragCoord.xy * 2.) & 1023, 0).r * .025;
    
    vec3 glFragColor = vec3(0);
    
    vec2 uv = gl_FragCoord.xy / resolution.xy * 2. - 1.;
    uv.x *= resolution.x / resolution.y;

    vec3 ro = vec3(0, 0, 2), rd = normalize(vec3(uv, -1.2));
    ...
    // Background colour
    vec3 bg = vec3(.9) * mix(vec3(.5, .5, 1.), vec3(1), .6) * (1. - smoothstep(0., 4., length(uv)));

    if(spheret.x > spheret.y)
        return bg;

    float t = spheret.x, d = 0.;
    ...
    else
    {
        ...
        // Backlight / fake SSS
        glFragColor.rgb = bg * mix(vec3(.2, .5, 1.) / 2., vec3(1., .9, .8).bgr, specshad2 * pow(fr, .8));
        
        // Fake AO from center of body
        glFragColor.rgb *= vec3(pow(mix(.5 + .5 * dot(n, normalize(vec3(0) - rp)), 1., smoothstep(0., 1.5, l)), .5));
        
        vec3 c = glFragColor.rgb;
        
        glFragColor.rgb = mix(c.bbb * vec3(1.,1.,.7), glFragColor.rgb, smoothstep(.3, .7, tri(l * 4. - .45)));
        
        // Pink tips
        glFragColor.rgb = mix(glFragColor.rgb, c.bbb * vec3(1,.5,1), smoothstep(1.3, 1.5, l));
        
        // Pink tips self-illumination
        glFragColor.rgb += vec3(1,1,.4) * smoothstep(1.4, 1.5, l) * .11;

        // Specular highlight
        glFragColor.rgb += specshad * 2. * smoothstep(.7, 1., dot(r, normalize(vec3(1)))) * fr;

        // Shadows inside holes
        glFragColor.rgb *= mix(.2, 1., smoothstep(.3,.6,l));

        // Mist
        glFragColor.rgb += vec3(mix(vec3(.5, .5, 1.), vec3(0), exp(-t / 25.)));
    }
...
void main(void)
{
    ...
    // Contrast
    glFragColor.rgb = (glFragColor.rgb * 1.2 - .05);
    
    // Clamp, gamma, dither
    glFragColor.rgb = pow(clamp(glFragColor.rgb, 0., 1.), vec3(1. / 2.2));
}
