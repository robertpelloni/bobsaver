...
// original https://www.shadertoy.com/view/XtBXWy
...
void main(void)
{
    ...
    float m = mod(date.w / 60.0, 60.0);
    float h = mod(date.w / 3600.0, 12.0);
    ...
    if (xy.x > resolution.x - 148.0 && xy.y < 22.0)
    {
        c = drawChar(xy, vec2(resolution.x - 63.0, 0), num[10], c, c2, 21.0);
           c = drawChar(xy, vec2(resolution.x - 126.0, 0), num[10], c, c2, 21.0);
        if (s < 10.0)
            c = putNbr(xy, vec2(resolution.x - 42.0, 0), 21.0, c, c2, 0.0);
        if (m < 10.0)
            c = putNbr(xy, vec2(resolution.x - 105.0, 0), 21.0, c, c2, 0.0);
        c = putNbr(xy, vec2(resolution.x - (s < 10.0 ? 21.0 : 42.0), 0), 21.0, c,c2, s);
        c = putNbr(xy, vec2(resolution.x - (m < 10.0 ? 84.0 : 105.0), 0), 21.0, c, c2, m);
        c = putNbr(xy, vec2(resolution.x - 147.0, 0), 21.0, c, c2, h);
    }
