...
// original https://www.shadertoy.com/view/XdcSRH
...
void main(void)
{
    vec3 cam_pos = vec3(cos(time/5.)*20.0,sin(time/5.)*40.0,20.0);

    vec2 tx = gl_FragCoord.xy / resolution.xy;
    ...
    if (f<maxd) {
        ...
        //glFragColor = vec4(b, 0.1, sin(b), cos(time*time));//Set the screen pixel to that color
    } else {
