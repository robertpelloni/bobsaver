...
const float fogDistance = 2.0;
const vec3 fogColor = vec3(0.34, 0.37, 0.4);

const float windowSize = 0.03;
const float windowSpead = 0.2;
    ...
    for (int i = 0; i < 200; ++i) {
        ...
        if (h.x < prob) {
            ...
            if (t < dist) {
                ...
                t = distance(center, p);
                fog *= smoothstep(1.0, 0.5, cos(tau * (beaconFreq * time + h.y)));
...
void main(void) {
    ...
    if (res.w < 0.5) {
        glFragColor = vec4(fogColor + res.y * beaconColor, 1.0);
        return;
    }
    ...
    vec3 color = windowColor + windowSpead * (hash3(block) - 0.5);
    color *= smoothstep(0.1, 0.9, fract(2.5 * (x * y * z)));
    color += streetColor * exp(-p.z / streetDistance);

    vec3 level = 0.25 * streetColor * exp(-4.0 * p.z / streetDistance);
