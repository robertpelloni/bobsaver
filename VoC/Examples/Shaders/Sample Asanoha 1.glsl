...
// original https://www.shadertoy.com/view/wlGGWG
...
void main(void)
{
    ...
    glFragColor.rgb = mix(glFragColor.rgb, vec3(solid), anim.blendSolid);
    glFragColor.rgb = mix(glFragColor.rgb, lineColor, lineWidth * anim.blendLine);
    glFragColor.rgb = mix(glFragColor.rgb, meshColor, anim.blendMesh);
    
    // degamma
    glFragColor.rgb = pow(glFragColor.rgb, vec3(1.0 / 2.2));
    
    // debug
    //glFragColor.gb = vec2(0);
    //glFragColor.r = 1.0 - pow(1.0 - near.z, 5.0);
    //glFragColor.r = neighbor.z * 10.0;
    //glFragColor.r = normal.w * 7.0;
}
