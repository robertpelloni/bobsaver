...
// original https://www.shadertoy.com/view/tls3WB
...
float map(vec3 p) {
    ...
    float textureoffset = time*0.25;
    float final_offset;
    //if (mouse*resolution.xy.w < 1.)
        final_offset = offsetdiff*delay_sin(time*0.5+1.) + ring_offset;
    //else
    //    final_offset = 2.0;//(mouse*resolution.xy.y/resolution.y-0.5)*3.+2.;
    ...
    //else
        final_ringcount = ringdiff+ring_count;//ringdiff*(mouse*resolution.xy.x/resolution.x-0.5)*2. + ring_count;
...
{    
    ...
    vec3 v = cross(u, w);
    vec3 rd = normalize(w*fdist+(gl_FragCoord.x/resolution.x-0.5)*u+(gl_FragCoord.y-resolution.y/2.0)/resolution.x*v);
    
    vec2 record = raymarch(ro, rd);
    vec4 n = gradient(ro+rd*record.x);
    vec3 shadedcolor = shade(ro, rd, n, record);
    
    glFragColor = vec4(shadedcolor, 1.);
}
