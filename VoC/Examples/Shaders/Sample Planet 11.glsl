...
// original https://www.shadertoy.com/view/MdyfWw
...
void main(void)
{
    ...
    vec2 p = arrangeCoords(gl_FragCoord.xy);
     
    vec3 lookAt = vec3(0.);
    vec3 camPos = vec3(5. * sin(time*0.3), 3. , -4. * cos(time*0.3));
    camPos = vec3(2,2.1,2.);
    ...
    glFragColor = vec4(sphereColor-.1, 1);
}
