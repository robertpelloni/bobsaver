...
// original https://www.shadertoy.com/view/XssczX
...
float de( in vec3 p, out vec3 color1, out vec3 color2, out float roughness ) {
    ...
    // add ants
    vec3 inTorObj = vec3(abs(inTor.x), inTor.y, inTor.z + time*1.3 + sign(inTor.x));
    float ant = floor(inTorObj.z / 4.0);
    vec3 objCenter = vec3(0.6, 0, ant * 4.0 + 2.0);
    float phase = fract(time) + mod(ant, 9.0) / 9.0;
...
float computeSoftShadows( in vec3 from, in vec3 dir, in float theta ) {
    ...
    for (int i = 0 ; i < SOFT_STEPS ; i++) {
        ...
        if (acc <= 0.0) break;
        totdist += max(0.1, dist*0.95);
    }
    return acc;
}

// compute lighting at this position
vec3 computeColor( in vec3 p, in vec3 dir, in vec2 gl_FragCoord ) {
...
void main(void) {
    ...
    // user input
    //vec2 mouse=(mouse*resolution.xy.xy / resolution.xy - 0.5) * 0.5;
    //mouse *= step(1.0, mouse*resolution.xy.z);
    ...
    vec3 noise = vec3(1.0,1.0,1.0);//hash33(vec3(gl_FragCoord.xy, iFrame));
...
// nonetheless the border color is constant so we can cone trace it without any trouble
    ...
    if (hit) {
        vec3 p = camPos + totdist * dir;
        glFragColor.rgb = computeColor(p, dir, gl_FragCoord.xy);
    } else {
        glFragColor.rgb = vec3(0.8, 0.8, 0.9);
    }
    
    // add a black border
    borderAcc = pow(borderAcc, 8.0);
    glFragColor.rgb = mix(vec3(0), glFragColor.rgb, borderAcc);
    
    // vigneting
    vec2 p = gl_FragCoord.xy / resolution.xy * 2.0 - 1.0;
    glFragColor.rgb = mix(glFragColor.rgb, vec3(0), dot(p, p)*0.2);
    
    // add some noise
    glFragColor.rgb += noise * 0.08 - 0.04;
