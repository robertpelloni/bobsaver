...
// original https://www.shadertoy.com/view/3d2GDW
...
//    vec3 col = (vec3(snoise(vec4(uv.x*time*0.75, uv.y*time*0.75, uv.x*time*0.75, uv.y*time*0.75), grad)) * 0.5) + 0.5;
//    vec3 col = (vec3(snoise(vec4((uv.xyxy*10.0)*time*0.75), grad)) * 0.5) + 0.5;
//    vec3 col = (vec3(snoise(vec4((uv.xyxy*0.05)*time*0.75), grad)) * 0.5) + 0.5;
//    vec3 col = (vec3(snoise(vec4((uv.xyxy * vec4(time*0.001)))*time, grad)) * 0.5) + 0.5;
    float col = (snoise(vec4(time*0.5, uv.x + time*0.01, time*0.5, uv.y - time*0.05), grad) * 0.5) + 0.5;
    //float col = (snoise(vec4(uv, vec2(time*0.5)), grad) * 0.5) + 0.5;

    // Output to screen
    //glFragColor = vec4(grad.xy*vec2(col), grad.z, 1.0);
