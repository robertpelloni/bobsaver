...
// original https://www.shadertoy.com/view/fssfDB
...
float map(vec3 p)
{
    rot(p,vec3(cos(time*.3),sin(time*.5),.5*sin(time*.2)),time*.7);
    float t = float[](3.,4.,5.)[int(fract(time/8.)*3.)];
    p = fold(p,t);
    p.z-=1.;
    vec3 n=vec3(-.5,-cos(PI/t),0);
    n.z=sqrt(1.-dot(n,n));         // normalize
