...
// original https://www.shadertoy.com/view/lddfRl
...
vec3 background(vec2 coords, float level) {
    ...
    for(float s = 0.0; s < 100.0; s += 1.0) {
        ...
        float shade = sin((scoords.x - scoords.y * 0.7 + time * 0.03) * 30.0) * 0.5;
        shade += sin((0.7 * scoords.x + scoords.y + time * 0.1) * 100.0) * 0.1;
...
void main(void) {
    ...
    float iters[LEVELS];
    vec4 hit[LEVELS];
       coords[0] = (2.0 * gl_FragCoord.xy  - resolution.xy) / max(resolution.x, resolution.y);
    ...
    glFragColor = vec4(stackColor, 1.0);
}
