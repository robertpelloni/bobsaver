...
void main(void)
{
    ...
    vec3 normal, hitpos, chitpos=vec3(0.0), cpos=vec3(0.0), cnor=vec3(0.0);
    float hit=0.;
    mat3 crot=mat3(0.0);
    float root3=floor(pow(float(cubes),1./3.));

    // time manipulation
    float cyc=mod(time,15.);
    ...
    // camera rotation
    mat3 camrot=rotmat(normalize(vec3(1.,0.5,ti*.5)),time);
    ...
    // explosion shine
    float et=pow(ti,.7)+.1;
    float e=pow(max(0.,et-length(uv)-abs(sin(-time*8.+atan(uv.x,uv.y)*6.))*.025)/et,5.);
