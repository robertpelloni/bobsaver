...
void main( void ) {
    ...
    if (gl_FragCoord.y < 1.) {
        ...
        vec4 t2 = texture2D(backbuffer, vec2(4.+tt,0) / (resolution));
        ...
        if (gl_FragCoord.x < 1.) {
            ...
            float speed = .004;
            glFragColor = mix(vec4(cPrev.x+cos(propdir*6.283)*speed, cPrev.y+sin(propdir*6.283)*speed,propdir,fract((t+1.5)/256.)), mix(t0,vec4(0), step(.9,t1.w)), eatself);
            ...
            glFragColor = mix(mix(vec4(t1.xy+move*step(fract(time*41.752654)+.0001,s3), t1.zw), fract(vec4(time*17.34312532,time*31.634541,fract((s+1.5)/256.),0)), eaten), mix(vec4(t1.xyz,t1.w+.01),vec4(.2,.2,0,0),step(.9,t1.w)), eatself);
        ...
        float det = max(0.,1.-p.x*p.x-p.y*p.y);
        float q = (sqrt(det)+p.y)*sign(det);
        float edible = step(color.w,q)*q;
         color = mix(color,vec4(.0),step(color.w,q));
        
        glFragColor = vec4(edible + color.x+color.y*.3,color.x+color.z*.3,color.x*4.,1.);
    }
