...
// original https://www.shadertoy.com/view/ttjGWt
...
Hit sceneSDF(vec3 p) {
    ...
    vec3 p1 = doTranslate(p, vec3(10.0*cos(time), 1.0, 10.0*sin(time)));
...
void main(void) {
    ...
    vec3 col = vec3(0.0);
    vec2 mouse = vec2(0.01) + mouse*resolution.xy.xy  / resolution.xy ;
    mouse -= 0.5;
    ...
    Light lightInfo = Light(vec3(5.0 * sin(time), 10.0, 2.0), vec3(200.0));
