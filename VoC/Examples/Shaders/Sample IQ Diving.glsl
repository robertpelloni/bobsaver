...
// original https://www.shadertoy.com/view/3t2SzW
...
vec3 diver( in vec3 col, in vec2 p, in float px, float t)
{
    ...
    for( int i=0; i<10; i++ )
    {
        ...
        vec2 p1 = head + vec2( 0.20*sin(21.39*float(i)),0.7);
        float h = fract( 0.5*(t+float(i)*0.12331) );
        vec2 c = mix( p0, p1, h );
        float d = sdCircle( p, c, 0.03*sqrt(h) );
        
        col += (1.0-0.75*abs(d)/0.03) * 0.35 * 4.0*h*(1.0-h)*(1.0-smoothstep(-px,px,d));
    }
