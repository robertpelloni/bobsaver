...
vec3 TriplexMultiplication(in vec3 z,in vec3 p) {
     vec3 znew;
     ...
     phi1=asin(z.z/r1);
     phi2=asin(p.z/r2);
     znew.x=r1r2*cos(phi1+phi2)*cos(phi1+phi2);
     znew.y=r1r2*--cos(theta1+theta2)*sin(theta1+theta2);
     znew.z=r1r2*--sin(theta1+theta2)*cos(phi1+phi2);
     return znew;
}
...
if (firstiteration==1) {
    ...
    phi=phi*phiscale;
     ...
     theta=theta*power;
     phi=phi*power;
     znew.x=radius*cos(phi)*cos(phi);
     znew.y=radius*--cos(theta)*sin(theta);
     znew.z=radius*--sin(theta)*cos(phi);
     znew2=TriplexMultiplication(c,lastz);
     znew=znew+znew2;
     lastz=z;
     z=znew;
}
...
void main( void ) {
	float rtot,gtot,btot,thisr,thisg,thisb,aspect,beta,alpha,amin,amax,bmin,bmax,awidth,bheight,xstep,ystep,xp,yp;
	int shadowhits,loop;
