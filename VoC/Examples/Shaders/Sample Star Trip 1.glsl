...
void main( void )
{
    ...
    position += vec2(sin(time*4.0)/10.0, cos(time*4.0)/10.0); // use this for mouse panning
    ...
    glFragColor = vec4( (color+ vec3(texture2D(backbuffer, (gl_FragCoord.xy -  resolution.xy))))/1.5, 1.0);
}
