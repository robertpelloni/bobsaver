...
// original https://www.shadertoy.com/view/3lc3zS
...
void main(void)
{
    ...
    vec2 Ouv = gl_FragCoord.xy / resolution.x;
    ...
    //head
    float face = sdEllipse(uv - center, vec2(0.275 - 0.003*sin(time), 0.15));
    float ears = sdVesica(rot(uv - vec2(center), 0.1*sin(sin(time))) + vec2(0.13, 0.0), 0.28, 0.15);
    
    float earsInner = sdVesica(rot(uv - vec2(center), 0.1*sin(sin(time))) + vec2(0.13, 0.0), 0.27, 0.16);
    earsInner = max(earsInner,-sdEllipse(uv - center, vec2(0.275 - 0.003*sin(time), 0.15)));
    ...
    //black of face
    float eye = sdEllipse(rot(uv - center, -0.5 + 0.05*sin(time)) + vec2(0.14, 0.05), vec2(0.035, 0.045));
    // mouth
    float bF = min(eye, sdLine(rot(uv - center, -0.1*sin(time)) - vec2(0., 0.03 - 0.015*sin(time)), vec2(0.1, -0.02), vec2(-.015, -0.08),  2.2));

    //whiskers
    for (int i = 0; i<3; i++){
        
        bF = min(bF, sdLine(rot(rot(uv, -length(uv) + 0.35 - 0.02*sin(time)) - center, -float(i)*0.1), vec2(-0.26, -0.05), vec2(-.35, -0.1),  10.*length(uv-center)));
    
    }
    
    float eyeSpec = sdSphere(rot(uv - center, -0.25 + 0.06*sin(time)) + vec2(0.138, -0.009), 0.02);

    //nose
    float nose = sdTri(70.*(uv - center - vec2(0., 0.008 -0.005*sin(time))));
    ...
    glFragColor.rgb = mix(bg.xyz, bgKitties.rgb, bgKitties.a);

    glFragColor.rgb = mix(glFragColor.xyz, layer0.rgb, layer0.a);
    glFragColor.rgb = mix(glFragColor.rgb, layer0_5.rgb, layer0_5.a);
    glFragColor.rgb = mix(glFragColor.rgb, layer1.rgb, layer1.a);
    glFragColor.rgb = mix(glFragColor.rgb, layer2.rgb, layer2.a);
    glFragColor.rgb = mix(glFragColor.rgb, layer2.rgb, layer2.a);
    glFragColor.rgb = mix(glFragColor.rgb, layer3.rgb, layer3.a);
