...
// original https://www.shadertoy.com/view/WsfXW7
...
void main(void)
{
    ...
    vec2 uv = (gl_FragCoord.xy/resolution.xy) / vec2(1., 16./9.);
    uv -= vec2(.5, 9./16./2.);
    ...
    for(float i=0.; i<1.; i+=1./3.) {
        ...
        intensity3 += fade*t*dot_pattern_k(uv*z/1.3, time);
    }
    
    
    vec3 colorized = color(time, dist(uv));
    float intensity1 = dot_pattern(uv, time);
    float intensity2 = dot_pattern_k(uv, time);
    
    float time_stepper = sin(time) * 8. - 2.;
    ...
    // Output to screen
    glFragColor = vec4(output_intensity*colorized ,1.0);
}
