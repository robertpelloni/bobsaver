...
// original https://www.shadertoy.com/view/tdBXDt
...
vec2 distort(vec2 uv){
  return uv+fbm(uv+vec2(.24,.025)+time*.125);
}

vec2 distort2(vec2 uv){
  return uv+fbm(uv+vec2(.12,.52)+time*.324);
}
...
void main(void) {
  ...
  glFragColor=.25*wave1+.25*wave2+.25*wave3+.25*wave4;
}
