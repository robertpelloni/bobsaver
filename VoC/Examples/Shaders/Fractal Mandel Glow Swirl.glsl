...
// original https://www.shadertoy.com/view/4slXD7

// wait for it.  slow start.
...
{
    float F = fract(.02*time);
    
    vec2 C = (2.0*pos.xy-resolution.xy) / resolution.x;
...
{
    // anti-aliasing
    glFragColor.rgb  = fractal( gl_FragCoord.xy + vec2(5,1)/8.0 );
    glFragColor.rgb += fractal( gl_FragCoord.xy + vec2(1,3)/8.0 );
    glFragColor.rgb += fractal( gl_FragCoord.xy + vec2(7,5)/8.0 );
    glFragColor.rgb += fractal( gl_FragCoord.xy + vec2(3,7)/8.0 );
    glFragColor.rgb /= 4.0;
