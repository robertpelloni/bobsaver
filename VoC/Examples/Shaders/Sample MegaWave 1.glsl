...
// original https://www.shadertoy.com/view/4tjXDz
...
vec2 df(vec3 p)
{
    ...
    vec3 q = vec3(cos(p.x), p.y * 5. - 10. + 10. * cos(p.x / 7. + t) + 10. * sin(p.z / 7. + t) ,cos(p.z));
...
void main(void)
{
    ...
    vec2 mo = mouse*resolution.xy.xy;
    ...
    if (d<dMax)
    {
        ...
        if (s.y < 1.5) // mat 1 : plane
        {
            ...
            glFragColor.rgb = 0.4 + 0.1*r*vec3(1.0);
        }    
        else if (s.y < 2.5) // mat 2 : sphere
        {    
            glFragColor.rgb = OrenNayarLightModel(n, ld, vec3(.33));            
        }    

        glFragColor.rgb = lighting(glFragColor.rgb, p, n, rayDir, rayDir, d); // lighting    
    }
    else
    {
        glFragColor.rgb = GetSky(rayDir, ld, vec3(0.6));
    }
