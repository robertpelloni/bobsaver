...
// original https://www.shadertoy.com/view/3dcSzS
...
{
    return map( uv + offset / resolution.xy, muv );
}

float sampMandelbrot( vec2 uv, vec2 offset )
{
    return map( vec2( 0 ), uv + offset / resolution.xy );
}

void main(void)
{
    vec2 uv  = 2.*( gl_FragCoord.xy/resolution.yy - vec2(.5 * resolution.x/resolution.y, .5) ) - vec2(.5, 0);
    vec2 muv;
    
    //if( mouse*resolution.xy.z > 0.0 ) {
    //    muv = 2.*( mouse*resolution.xy.xy/resolution.yy - vec2(.5 * resolution.x/resolution.y, .5) ) - vec2(.5, 0);
    //} else {
        float t = .1*time;
