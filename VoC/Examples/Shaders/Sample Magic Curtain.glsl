...
// original https://www.shadertoy.com/view/DdXSzX
...
void main(void) { //WARNING - variables void (out vec4 col, vec2 gl_FragCoord.xy){ need changing to glFragColor and gl_gl_FragCoord.xy
    vec2 uv = gl_FragCoord.xy/resolution.y;
    float t = 0.5*time;
    uv.y += 0.03*sin(8.0*uv.x-t);
    float f = 0.6+0.4*sin(5.0*(uv.x+uv.y+cos(3.0*uv.x+5.0*uv.y)+0.02*t)+sin(20.0*(uv.x+uv.y-0.1*t)));
    float b = 1.7;
    vec4 col = vec4(0.2588235294*b, 0.1254901961*b, 0.2549019608*b, 1.)*vec4(f);
    ...
    float n = fadeLR * fadeTB * smoothstep(.50, 1.0, snoise(pos * resolution.y / 10.)) * 8.;
    ...
    col -= vec4((1.-uv.y)*0.1*k,0,0,1.);
    col -= (vec4(uv.y, uv.y * 0.8, uv.y, 1.)) / 8.0;
	glFragColor = col;
}
