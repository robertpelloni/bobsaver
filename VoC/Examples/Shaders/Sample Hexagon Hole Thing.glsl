...
// original https://www.shadertoy.com/view/7ljcDw
...
float GetDist(vec3 p) {
    float d = length(p + 0.*thc(2., (p.x + p.y + p.z) + time)) - 1.5;
    
    float pd =min( abs(p.x-p.y), abs(p.z - p.x));
    pd = min(pd, abs(p.y-p.z));
    
    p = erot(p, normalize(vec3(1)), -0.8 * time);
    float e = 2. * pi / 3.;
    p = erot(p, normalize(2. - cos(time + (0.5 + 0.5 * thc(4.,0.25 * time)) * vec3(0,e,-e))*p), pi + 0. * time);
    //p *= 1.5 + 0.5 * cos(time);
