...
// original https://www.shadertoy.com/view/tsscz4
...
float sample_noise(vec3 p, float octave)
{
    ...
    vec3 d2 = hash3(vec3(-1.0, -13.0, 0.0)) * time * .25;
...
void main(void)
{
    ...
    if (x0 < 1.0) for (i = 0.0; i < st0_count; i += 1.0)
    {
        ...
        {
            float st1_t = (j + 1.0 + 0.125*(hash3(vec3(gl_FragCoord.xy*0.001, 0.0)).x * 0.5 + 0.5))*dt1;
