...
// original https://www.shadertoy.com/view/tlXyR4
...
{    
    ...
    vec3 pt_n = pt;
      for (int i = 1; i <= depth; ++i)
      {
        ...
        mat4 mx1 = Rz(-0.2*sin(time+6.2))*posR*bendP*Disp(vec3(0,-2.*l - l/2.,0));

        mat4 wind = Rz(0.2*sin(time+6.2));
        mat4 mx2 = wind*negR*bendP*Disp(vec3(0,-2.*l,0));

        wind = Rz(0.2*sin(time+1.));
...
void main(void)
{
    ...
    vec3 position = vec3(cos(mouse.x*resolution.xy.x/100. - .8)*10., 3., sin(mouse.y*resolution.xy.y/100.-.3)*10.);
    
    mat4 mat = LookAtRH(position, vec3(0,3,0), vec3(0,1,0));
    vec3 pt = (mat*pixel).xyz;
    vec3 ray = normalize(mat*(pixel - vec4(0,0,-f,1))).xyz;
    
    glFragColor   = render(ray, pt);
}
