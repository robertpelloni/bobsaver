...
// original https://www.shadertoy.com/view/4sdfzS
...
void main(void)
{
    ...
    vec2 p = vec2(0.1,0.001) + (-resolution.xy+2.0*gl_FragCoord.xy)/resolution.y;
    ...
    if(length(p)<1.0){
        ...
        ball_pt = vec4(ball_pt.x * cos(time) + ball_pt.y * sin(time), -ball_pt.x * sin(time) + ball_pt.y * cos(time), ball_pt.z, ball_pt.w);
        ball_pt = vec4(ball_pt.x, ball_pt.y * cos(0.5*time) + ball_pt.z * sin(0.5*time), -ball_pt.y * sin(0.5*time) + ball_pt.z * cos(0.5*time), ball_pt.w);
        //ball_pt = (0.999-0.15*cos(time))*ball_pt;
