...
void main()
{
   ...
   // draw the floor
   glFragColor.rgb = traceFloor(ro, rd);
   ...
   if(hit.y == 1.0 && car_hit)
   {
      ...
      glFragColor.rgb = vec3(0.75) * vec3(pow(0.5 + 0.5 *
               dot(norm, rd), 1.5));
      ...
      // reflection of floor
      glFragColor.rgb += traceFloor(rp, refl) * 0.8;
   }
   else if(hit.y == 2.0 && car_hit)
   {
      ...
      glFragColor.rgb = 2.0 * vec3(0.5, 0.4, 0.2) *
            vec3(0.5 + 0.5 *
   ...
   for(float x = 0.0; x < 1.0; x += 0.04)
   {
      ...
      if(objpos.z > 0.0 && objpos.z < max_z)
      {
         ...
         glFragColor.rgb += r / distance(p.xy, proj) *
