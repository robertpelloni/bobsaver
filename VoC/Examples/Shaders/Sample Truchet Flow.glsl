...
// original https://www.shadertoy.com/view/XtfyDX
...
void main(void) {
    ...
    //uv -= sin(uv*3.14159/2. - cos(uv.yx*3.14159/4. + time)*3.14159)*.01;
    ...
    p = p - ip - .5; // Equivalent to "p = fract(p) - .5;."
    ...
    // This line requires a bit of explanation: The standard 2D Truchet tile consists of
    ...
    // Arc one, centered in the top right of the grid cell - or the bottom right if flipped.
    ...
    // Moving the grid coordinats to the corner, or the center of the arc - in order to
    ...
    float d2 = df2(mod(a - .15*dir, 6.283/aNum) - .5*6.283/aNum, .2);// - .05;
    d2 = max(d + .14, -d2); // Control the arrow line width  with the Truchet distance field.
    
    // Arrow head, or a triangle if you want to get technical. :) The triagles need to be
    ...
    float d4 = df2(mod(a - .15*dir, 6.283/aNum/3.) - .5*6.283/aNum/3., .03);// - .05;
    ...
    // front layer like so:
    //
    // col = mix(layer1, layer2, a);
    // col = mix(col, layer3, a)
    // col = mix(col, layer4, a)
    // etc.
    //
    ...
    // Mix the background with the border overlay. Note ".8" value for transparency.
