...
// original https://www.shadertoy.com/view/msX3Dl
...
// Fork of "generative art deco 4" by morisil. https://shadertoy.com/view/mds3DX
// 2022-10-28 16:14:01
...
float getColorComponent(in vec2 st, in float modScale, in float blur) {
    ...
    float angle = atan(modSt.x, modSt.y) + sin(time * .01) * 9.0;
...
void main(void) {
    ...
    for (float i = 0.; i < ITERATIONS; i++) {
        vec2 center = st + vec2(sin(time * .012), cos(time * .13));
...
//        if (color == vec3(1)) break;
    ...
    glFragColor = vec4(1. - pow(color.rgb, colorGrading), 1.);
