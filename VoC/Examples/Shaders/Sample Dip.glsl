...
// original https://www.shadertoy.com/view/Wd33Dr
...
float map(vec3 p) {
    ...
    vec3 p3 = p;p3.z*=1.12;
    //p3.x-=sin(time*2.0)/4.0;
    //p3.x-=sin(p3.y+time*2.0)/8.0;
...
void main(void)
{
    vec2 rotor = vec2(sin(time*2.0)/2.0,cos(time));
    ...
    float blink = clamp(1.0-abs(sin(time*0.45))*16.0,.0,1.0);
    ...
    blink = max(blink,clamp(1.0-abs(cos(time*0.45*blinkRate))*16.0/blinkRate,.0,1.0));
    
    //squint occasionally
    blink = max(blink,clamp(.35-abs(cos(time/4.0+.15)),.0,.10));
    ...
    //glFragColor.g+=step(.0,n.y)*.25*step(-.5,r.pos.y);
    //glFragColor.g+=step(.0,n.y)*.05*step(n.y,.01);
    
    //glFragColor.rgb=n.rgb;

    glFragColor.g=clamp(glFragColor.g+eyes*pupils,.0,1.0);
    ...
    float talkFreq = mix(sin(time*16.0),sin(time*13.0)-.8,clamp((cos(time*2.0+.15)+.5)*4.0,0.0,1.0));
    float talkH=clamp(talkFreq/16.0+.1,.015,.25);
    //float talkW=smoothstep(-1.0,1.0,clamp(-cos(time*2.0+.15)*8.0+.5,.0,1.0)*.24)*1.15-.34;
    float talkW=smoothstep(-1.0,1.0,clamp(-cos(time*2.0+.15)*8.0+.5,.0,1.0)*.24)*.65;

    float mouth = (sdEllipsoid(pm+vec3(.0,.425-pow(abs(pm).x,2.0)/1.0,-.5),vec3(talkW,talkH,.5)));

    
    
    glFragColor.g+=1.0-step(-.01,mouth);
    
    //cel shading
    //glFragColor*=mix(.5,1.0,clamp(glFragColor.g+round(dot(n,lDir)+.5),.0,1.0));
