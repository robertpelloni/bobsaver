...
// original https://www.shadertoy.com/view/3ddyz2
...
float map(vec3 p){
    ...
    float PosX, PosY;
    PosX = 0. + sin(time*20.)/4.;// - (mouse*resolution.xy.x)/resolution.x;//0.2*2.*(rechts - links);
    PosY = -0.1 + sin(time*2.)/4.;;//(mouse*resolution.xy.y)/resolution.y*4.;
    ...
    //guckmal
    float gu = clamp(sin(time*0.3), -0.4, 0.4);
    cs = cos(gu), si = sin(gu);
    //p.xz = mat2(cs, si, -si, cs)*p.xz;
    
    
    p*=0.6;
    p.y-=0.2;// + sin(time)*0.1;
