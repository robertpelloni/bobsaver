...
//original https://www.shadertoy.com/view/4sj3Rh
//britney video overlays removed
...
vec3 light(in vec3 p, in vec3 dir, in vec3 n, in float hid) {
    ...
    if (hid>.5) {
        ...
        col+=abs(.01-mod(p.y-time*.1,.02))/.01*ao;
        col*=max(0.,1.-pow(length(p2.yz-vec2(.25,1.)),2.)*3.5);
    } else{
        //vec3 c=(texture2D(iChannel0,mod(p.zx*2.+vec2(0.5),vec2(1.))).xyz);
        float cf=abs(.01-mod(p.x-time*.1*sign(p.x+1.),.02))/.01;
        vec3 c=vec3(cf,cf,cf);
...
vec3 move(inout mat2 rotview1,inout mat2 rotview2, inout mat2 rotview3) {
    vec3 go=path(t);
    vec3 adv=path(t+.7);
    float hd=de(adv).x;
    ...
    rotview2=mat2(cos(an),sin(an),-sin(an),cos(an));
        an=adv.x-go.x; an*=min(1.,abs(adv.z-go.z))*sign(adv.z-go.z)*.7;
    rotview3=mat2(cos(an),sin(an),-sin(an),cos(an));
    return go;
}
...
void main(void)
{
    ...
    mat2 rotview1, rotview2, rotview3;
    vec3 from=origin+move(rotview1,rotview2, rotview3);
