...
vec3 getColor2D(vec2 c,vec2 m) {
    ...
    ci =  1.0 - log2(.5*log2((1.5+m.y)*mean/C));
    //return vec3( .5+.5*cos(6.*ci +R),.5+.5*cos(6.*ci + G),.5+.5*cos(6.*ci +B) );
    return palette[int(ci*60.0)].rgb;
}

void main() {
 //vec2 m = vec2(0,0.3);
 c2 = vec2(-0.11766+0.1*sin(time/3.0),5.0571+0.1*cos(time/(5.0)));
 vec2 m = mouse.xy - vec2(0.5,0.5);
 vec2 pos = (gl_FragCoord.xy / resolution.xy) - vec2(0.5,0.5);
 vec2 p = pos*(0.5+m.x)*2.0*(5.0+1.0*cos(time/5.0));
 p.x -= 0.1;  
 
 glFragColor = vec4(getColor2D(p,m), 1.0);
}
