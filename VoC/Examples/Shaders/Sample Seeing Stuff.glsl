...
// original https://www.shadertoy.com/view/M3tGRS
...
float sdf(vec2 uv) {
    ...
    dir = cos(l - time) * dir + sin(l - time) * vec3(-dir.y, dir.x, dir.z);
    dir.yx = cos(-time + dir.y) * dir.xz + sin(-time * 1.3 - dir.x) * vec2(-dir.z, dir.x);
...
void main(void)
{
    ...
    vec2 uv = (gl_FragCoord.xy * 2.0 - resolution.xy) / mr * 3.0;
    uv /= pow(dot(uv, uv), 0.8);
    
    float d = sdf(uv) * 0.2;
    vec3 n = abs(get_normal(uv)) / (1.0 + 0.5 * sqrt(length(uv)));
    vec3 col = smoothstep(n, n + cos(n - vec3(2, 3, 1) + 1.5), vec3(hue_shift(n, d - time)));
    col.gb = col.bg;
