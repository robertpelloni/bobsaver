...
// original https://www.shadertoy.com/view/ltXyD7
...
{
    // spin
    float theta = pi/2.0;//0.05*time;
    float c = cos(theta);
    float s = sin(theta);
    uv = vec2((uv.x*c - uv.y*s), (uv.x*s + uv.y*c));
    
    // wave
    uv.x += 0.05*sin(10.0*uv.y + 0.17*time);
    uv.y += 0.05*sin(10.0*uv.x + 0.13*time);
    uv.x += 0.2*sin(1.0*uv.y + 0.31*time);
    uv.y += 0.2*sin(1.0*uv.x + 0.27*time);
    return uv;
}
