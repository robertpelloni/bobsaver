...
// original https://www.shadertoy.com/view/stXXWn
...
void main(void) {
    ...
    cuv.x = mix(cuv.x, 2.0/cuv.x, smoothstep(-1.0, 1.0, clamp(2.0*sin(0.5*time), -1.0, 1.0)));
    cuv.x -= cos(0.5*time);
    ...
    glFragColor = vec4((strip1+0.6*strip2)*td);
}
