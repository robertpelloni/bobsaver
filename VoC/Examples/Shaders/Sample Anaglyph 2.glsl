...
//     glFragColor = vec4(pow(radiance(ray) * exposure, vec3(1.0 / gamma)), 1.0);
// }
...
//    vec2 uv = 2. * gl_FragCoord.xy / resolution.xy - 1.;
  // vec2 uvs = uv * resolution.xy / max(resolution.x, resolution.y);
    ...
    Ray ray_l = Ray(vec3(-0.1, 2.5, 12.0), normalize(vec3(uv.x, uv.y, -1.0)));
    Ray ray_r = Ray(vec3(0.1, 2.5, 12.0), normalize(vec3(uv.x, uv.y, -1.0)));
    vec4 c_l = vec4(pow(radiance(ray_l) * exposure, vec3(1.0 / gamma)), 1.0);
    vec4 c_r = vec4(pow(radiance(ray_r) * exposure, vec3(1.0 / gamma)), 1.0);
    glFragColor  = vec4(c_r.x, c_l.x, c_l.z, 1.0);
}
