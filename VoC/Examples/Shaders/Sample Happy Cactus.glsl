...
// original https://www.shadertoy.com/view/3tSGWz
...
vec2 doModel( vec3 p )
{
    ...
    float b2 = step(fract(time*0.1), 0.9);
    if (b2<=0.0)
    ...
    // eyes    
    float blink = step(sin(time * 2.8 + cos(time * 2.0) * 2.0), 0.98);
    p.x = abs(p.x);
...
vec3 background(vec2 pos)
{
    ...
    float v = sin(sin(pos.x*16.0)+(vv+time*.1) * 42.0)+0.95;
    v*=0.5-d*d;
...
void main(void)
{
    ...
    // camera matrix
    mat3 camMat = calcLookAtMatrix( ro, ta, sin(time)*0.25 );
    ...
    vec3 col = background((gl_FragCoord.xy - 0.5 * resolution.xy) / resolution.y);
