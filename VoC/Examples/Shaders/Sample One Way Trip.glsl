...
// original https://www.shadertoy.com/view/WlSSWV
...
// global variables
float det=.005, fin=0., time2=time; // raymarching threshold, aux variable
...
vec4 march(vec3 from, vec3 dir, vec3 camdir) {
    ...
    col.rgb=mix(col.rgb,glow*suncol*.01+backcol,1.-smoothstep(0.,.8,abs(time2-10.5)));
...
// the path of the camera at a given point of time2
...
void main(void)
{
    ...
    time2=mod(time2,162.); // time2 loop
    fin=smoothstep(145.,147.5,time2); // aux variable used for the end sequence
    // camera accelerations and slow downs
    float acel1=smoothstep(11.,12.,time2)*7.31;
    float acel2=smoothstep(99.,100.,time2)*4.;
    float desacel1=smoothstep(77.,78.,time2)*5.;
    ...
    t+=smoothstep(125.,145.,time2)*243.;
    ...
    col.rgb*=1.-smoothstep(25.,26.,time2)+step(25.1,tt); // BW freeze frame sequence fading
    col.rgb*=1.+step(abs(tt-25.),.1);
    // PVM Logo color mixing
    vec4 pvm=logo(uv*1.5+vec2(.9,.5))*smoothstep(1.,3.,time2+uv.x*2.)*(1.-smoothstep(7.5,8.,time2+uv.x*2.));
    col.rgb=mix(col.rgb,pvm.rgb,pvm.a);
    // fade in from black
    col.rgb*=smoothstep(0.,4.,time2);
    // fade out to black
    col.rgb*=1.-smoothstep(160.,162.,time2);
    glFragColor = col;
