...
// original https://www.shadertoy.com/view/lstfW2
...
void main(void) {
    vec3 src = vec3(3. * (gl_FragCoord.xy - 0.5*resolution.xy) / resolution.yy, 2.0);
    vec3 dir = vec3(0., 0., -1.);
    ...
    if (value > CLOSENESS) glFragColor = vec4(0., 0., 0., 1.);
    else {
      ...
      glFragColor = vec4(1.0*color*totalLight, 1.0);
    }
    vec2 p = 2.0* (gl_FragCoord.xy / resolution.yy - vec2(0.5 /  resolution.y * resolution.x, 0.5));
    float q = max(0.1, min(1.0, dot(vec3(p, sqrt(1. - dot(p,p))), vec3(0., 2., 1.))));
        glFragColor += q * vec4(shad * max(0.0, pow(dot(normalize(src), normalize(vec3(0.,2., 1))),1.0)) * pow(atmos,1.5) * vec3(0.5, 0.5, 0.9), 1.0);

    //glFragColor += vec4(q, 0.0, 0.0, 1.0);
}
