...
// original https://www.shadertoy.com/view/3lsBD8
...
void main(void)
{
    ...
    float a = 0.0;// 10.0 * mouse.x*resolution.xy.x/resolution.x;
    float taDist = 2.0;
    vec3 ro = vec3( 8.0 * sin(a + float(time * 0.25)), 7.0, 8.0* cos(a + float(time * 0.25)));
    vec3 ta = vec3(0,0,0);
