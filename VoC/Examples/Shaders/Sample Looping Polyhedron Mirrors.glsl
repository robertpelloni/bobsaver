...
// original https://www.shadertoy.com/view/4XtGDn
...
void init() {
    ...
    truncation_type.x =0.5*sin(time*1.5)+0.5;
    truncation_type.y=0.5*sin(time*0.8)+0.5;
    truncation_type.z =0.5*sin(time*0.3)+0.5;
...
vec4 wallColor(vec3 dir, vec3 nor, vec3 eds) {
    ...
    // Texturing of walls
    vec3 albedo = vec3(0.0); //texture(iChannel1, vec2(eds.xy * 2.0)).rgb;
...
void main(void) {
    ...
    vec2 move = vec2(time * 0.2, sin(time * 0.2) * 0.5); // vec2(.65, 0);
    ...
    //    mouse = 2. * mouse*resolution.xy.xy / resolution.y - 1.;
