...
// original https://www.shadertoy.com/view/Xs3SzM
...
void main(void)
{
  ...
  //Light movement 
  lights[0].position = vec3(0.5, 0.5, 0.0); 
                    
  //Object movement
  float speed = 1.0;
  spheres[0].position = vec3(0.5, 0.24, -0.5) + 0.3 * vec3(sin(speed*(time)),0.0,cos(speed*(time)));
  spheres[1].position = vec3(0.5, 0.2, -0.5) + 0.23 * vec3(sin(speed*(time) + 10.0),0.0,cos(speed*(time)+ 10.0));
  spheres[7].position = vec3(0.5, 0.7, -0.5) + 0.2 * vec3(0, sin(speed*(time)), 0.0);
           
  //Raytrace objects
  vec2 pixelCoords = gl_FragCoord.xy / resolution.y;
  vec3 pixel = vec3(pixelCoords - vec2(0.5,0), 0);
  Ray ray = Ray(camera.position, normalize(pixel - camera.position));
  glFragColor = rayTrace(ray);
}
