...
// original https://www.shadertoy.com/view/NslGRN
...
//#define curvature (0.001+1.5-1.5*smoothstep(0.,8.5,mod((time+tshift)*0.44,20.))*(1.-smoothstep(10.,18.5,mod((time+tshift)*0.44,20.))))
...
// min(frames,0) does not speedup compilation in ANGLE
...
void main(void)
{
...
#ifdef MOUSE_control
    mouseY = (1.0 - 1.15 * mouse.y*resolution.xy.y / resolution.y) * 0.5 * PI;
    if(mouse.y*resolution.xy.y < 1.)
#endif
    mouseY = PI*0.49 - smoothstep(0.,8.5,mod((time+tshift)*0.33,25.))*(1.-smoothstep(14.,24.0,mod((time+tshift)*0.33,25.))) * 0.55 * PI;
    float mouseX = -2.*PI-0.25*(time+tshift);
#ifdef MOUSE_control
    mouseX+=-(mouse.x*resolution.xy.x / resolution.x) * 2. * PI;
#endif
    ...
    if (t > 0.)
    {
        ...
        glFragColor = vec4(col, cineshader_alpha);
    }
    else
    {
        glFragColor = vec4(background(eye, rd, l_dir), 0.15);
    }
}
