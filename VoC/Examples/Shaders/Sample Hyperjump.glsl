...
void main(void)
{
    vec2 uv = (gl_FragCoord.xy - resolution.xy * 0.5) / resolution.x * 2.0;
    vec2 mouseUV = (mouse*resolution.xy.xy - resolution.xy * 0.5) / resolution.x * 2.0;
    ...
    vec3 col = getTex(vec2(p.z, fract(atan(p.x, p.y) / PI * 0.5 + 0.5 + p.z / 100.0)), time* 30.0);
    
    //col = vec3(1.0 - p.z / 5.0);
    
    col += vec3(p.z / min(time * 10.0, 135.0));
