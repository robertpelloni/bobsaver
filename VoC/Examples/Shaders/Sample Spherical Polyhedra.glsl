...
// original https://www.shadertoy.com/view/4dBXWD
...
{    
    ...
    if (t > 0.)
    {
        ...
        if (sel == 0.)col = max(dod(rf)*.4,dod(pos)*2.);
        else if (sel == 1.) col = max(ico(rf)*0.4,ico(pos)*2.);
        else if (sel == 2.) col = max(cub(rf)*0.4,cub(pos)*2.);
        else if (sel == 3.) col = max(octa(rf)*0.4,octa(pos)*2.);
    }
