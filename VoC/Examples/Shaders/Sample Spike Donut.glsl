...
// original https://www.shadertoy.com/view/MsVfWV
...
vec4 trace(vec2 gl_FragCoord)  {
    ...
    for(int i=0; i <64; i++) {
        ...
        k=i;
            ...
            if(colorIndex == 2) color3 = vec4(0.3, 0.3, 0.3,1.0);//plasma(gl_FragCoord);
...
{    
    ...
    for(int i=0; i< 4; i++) {
        for(int j=0; j < 4; j++) {
            color += trace(gl_FragCoord.xy+(vec2(i,j)/4.0-0.5));
        }
    }
    
                                  
    glFragColor = color /16.0;                                  
}
