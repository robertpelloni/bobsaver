...
// original https://www.shadertoy.com/view/XlsXDB
...
vec3 render(vec2 uv)
{
    ...
    vec3 scol = rgb(255,155,102) * sun * 0.65;
    color += scol;
    // clouds
    vec3 cl1 = mix(rgb(156,142,158), rgb(166,191,224),uv.y);
    float d1 = mix(0.9,0.1,pow(uv.y, 0.7));
    color = mix(color, cl1, cloud(uv,2.,8.,d1,0.4,0.04));
    color = mix(color, vec3(0.9), 10.*cloud(uv,14.,18.,0.9,0.75,0.02) * cloud(uv,2.,5.,0.6,0.15,0.01)*uv.y);
    color = mix(color, vec3(0.8), 5.*cloud(uv,12.,15.,0.9,0.75,0.03) * cloud(uv,2.,8.,0.5,0.0,0.02)*uv.y);
    // post
    color *= vec3(1.0,0.95,0.83)*1.03;
    color = mix(0.8*rgb(255,205,161), color, smoothstep(-0.1,0.3,uv.y));
    return color;
}
