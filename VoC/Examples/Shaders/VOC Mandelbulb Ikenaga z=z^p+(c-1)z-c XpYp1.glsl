vec3 TriplexAdditionIkenagaXpYp1(in vec3 z, in vec3 c) {
    vec3 znew;
    znew.x=z.x+c.x;
    znew.y=z.y+c.y;
    znew.z=z.z+c.z;
...
vec3 TriplexPowerIkenagaXpYp1(in vec3 z,in float p) {
	vec3 znew;
    radius=pow(radius,p);
    theta=theta*p;
    phi=phi*p;
...
vec3 TriplexMultiplicationIkenagaXpYp1(in vec3 z,in vec3 p){
...
void Iterate(inout vec3 z,in vec3 c) {
    ...
    znew=TriplexPowerIkenagaXpYp1(z,power);
	znew2=c-vec3(1.0,1.0,1.0);
	znew2=TriplexMultiplicationIkenagaXpYp1(znew2,z);
	znew2=znew2-c;
	znew=znew+znew2;
	lastz=z;
	z=znew;
