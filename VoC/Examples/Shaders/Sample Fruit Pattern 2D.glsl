...
// original https://www.shadertoy.com/view/llsBD2
...
{
    vec2 bgUV = uv + vec2(time * backgroundSpeed);
...
float calcFruitType()
{
    float fruit = time * fruitTypeAnimSpeed;
...
{   
    ...
    vec2 imageUV = gl_FragCoord.xy / vec2(resolution.x) + vec2(0.0, -1.0);
    vec3 bg = background(imageUV, fruitType);
    
    vec2 mainUV = gl_FragCoord.xy / vec2(resolution.x) * vec2(1.0, -1.0);
