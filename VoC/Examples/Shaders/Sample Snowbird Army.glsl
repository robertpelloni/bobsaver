...
// original https://www.shadertoy.com/view/3dlGz4
...
{
    p.y -= sin(time * 4.0) * 0.33;
    p.y += sin(p.x * 1.1 + time * 4.0) * 0.3;
...
{
    p.y *= 1.0 + sin(time * 4.0 + p.x * 0.25 + p.z * 0.25) * 0.1;
...
void main(void)
{
    ...
    float rx = (mouse.x*resolution.x / resolution.x * 2.0 - 1.0) * 3.15 * 2.0;
    opRotate(ro.xz, rx);
    opRotate(rd.xz, rx);
