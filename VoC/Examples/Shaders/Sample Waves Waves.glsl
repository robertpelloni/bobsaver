...
// original https://www.shadertoy.com/view/XdtcD4
...
float map(vec3 p) {
    ...
    for (float i = 0.; i < 5.; i+= 1.) {
        n += wave(p.xz, vec2(sin(i) * RADIUS, cos(i) * RADIUS), 1., .4, i + time * 3.);
    }
...
void main(void) {
    ...
    glFragColor = pow(sceneColor.rgbb, 2./vec4(2.)) * 4.;
}
