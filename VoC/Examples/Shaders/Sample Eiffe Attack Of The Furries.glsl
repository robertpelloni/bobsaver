...
//original https://www.shadertoy.com/view/Mss3RM
...
float Limb2(vec3 p, vec3 p0, vec3 p2, vec3 rt, float d, float r){
    ...
    p1+=p0+normalize(cross(p1,rt))*(d*d-dot(p1,p1));
...
float DE(vec3 z){
    ...
    z.xz=mod(z.xz,1.0)-vec2(0.5)+0.25*vec2(sin(float(i)));
    float tim=(time+float(i)),arm=sin(float(i))*0.2;
    vec3 p5=vec3(-0.38+abs(arm),arm-0.1-abs(-sin(tim)*0.05),-0.1-cos(tim)*0.1);
    vec3 p8=vec3(0.38-abs(arm),arm-0.1-abs(sin(tim+3.1416)*0.05),-0.1+cos(tim)*0.1);
...
vec3 scene(vec3 ro, vec3 rd){
    ...
    for(int i=0;i<48;i++){
        ...
        atm+=noyz(ro+rayLen*rd-vec3(time))*0.03;
