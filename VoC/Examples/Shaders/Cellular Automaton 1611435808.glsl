...
// Blue: Spacing / (B >= nhr) && (B > nhg)
    ...
    vec2 nhb = vec2 ( nhcb, 1.0 );
            nhr = nh_val ( floor(8.0+(0.0*20.0)), floor(0.0+(0.0*20.0)), psn, 0 );
...
// Assemble
    ...
    float r = res_r;
    float a = ((res_r + dfrv*0.2)/1.2)-0.005;
    vec3 n0 = nh_ring(12.0,0.0,vec2(1.0,1.0),0.5);
    float b = n0[0]/n0[1];

    if(b >= 0.300 && b <= 0.500) { r += 0.150; }
    if(b < 0.500) { r -= abs(b-0.300)*(0.45+cs); }
    if(b > 0.300) { r -= abs(b-0.500)*(0.45+cs); }

    if(b >= 0.100 && b <= 0.200) { r += 0.150; }
    if(b < 0.200) { r -= abs(b-0.100)*(0.38); }
    if(b > 0.100) { r -= abs(b-0.200)*(0.38); }
