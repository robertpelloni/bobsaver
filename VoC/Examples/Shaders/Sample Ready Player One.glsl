...
// original https://www.shadertoy.com/view/sllSDS
...
vec3 render(vec3 ro, vec3 rd) {
    t = mod(time, 30.);
    return pow(max(vec3(0), march(ro, rd)), vec3(.45)) * sat(time);
}

void mainVR(out vec4 glFragColor, vec2 fc, vec3 ro, vec3 rd) {
    rd.xz *= rot(-1.57);
    glFragColor = vec4(render(vec3(-2, -4, -5), rd), 0);
}
...
void main(void) {
    ...
    glFragColor = vec4(vig(render(ro, rayDir(ro, lookAt, uv)), gl_FragCoord.xy), 0);
}
