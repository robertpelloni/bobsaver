...
// original https://www.shadertoy.com/view/Xdf3z4
...
void main()
{
    vec3 o = vec3 (.3, 5.25, 16.-time);
    vec3 rd = normalize(vec3((-1.+2.*gl_FragCoord.xy/resolution.xy)*vec2(resolution.x/resolution.y, 1.), -1.));
    vec3 pr = o - vec3(.05, 0., 0.);
    vec3 pc = o + vec3(.05, 0., 0.);
    float d1, d2;
    for(int i = 0; i < 64; i++)
    {
        d1 = scene(pr);
