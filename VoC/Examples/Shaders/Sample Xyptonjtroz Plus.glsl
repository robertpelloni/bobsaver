...
// original https://www.shadertoy.com/view/MtfGDB
...
float vine(vec3 p, in float c, in float h)
{
    ...
    return length(q) - h -sin(p.z*3.+sin(p.x*7.)*0.5+time*0.5)*0.13;
}
...
float fogmap(in vec3 p, in float d)
{
    p.x += time;
    //p.z -= time*.3;
    return triNoise3d(p*.1,0.2)*(smoothstep(.7,.0,p.y));
}
...
vec3 fog(in vec3 col, in vec3 ro, in vec3 rd, in float mt)
{
    ...
    for(int i=0; i<7; i++)
    {
        ...
        float grd =  clamp((rz - fogmap(pos,d)), 0.1, 1. );

        col = mix(col,vec3(.8, .6, .5),clamp(rz*smoothstep(d-0.4,d+2.+d*.75,mt),0.,1.) );
        d *= 1.8;
        if (d>mt)break;
    }
    return col;
...
{
    float n = sin(triNoise3d(p,0.0)*4.)*0.6;
    n += sin(triNoise3d(p*1.,0.0)*8.)*0.3;
    return (n*n)*0.01;
}
...
{    
    ...
    vec3 ro = vec3(smoothstep(0.,1.,tri(time*.45)*2.)*0.1, smoothstep(0.,1.,tri(time*.9)*2.)*0.07, -time*0.6);
    ro.y -= height(ro.zx)+0.07;
    mo.x += smoothstep(0.6,1.,sin(time*.6)*0.5+0.5)-1.5;
    vec3 eyedir = normalize(vec3(cos(mo.x),mo.y*2.-0.2+sin(time*.75*1.57)*0.1,sin(mo.x)));
    ...
    vec3 fogb = mix(vec3(.5, .4,.3), vec3(1.,.9,.8), min(pow(max(dot(rd,ligt), 0.0), 1.5)*2.25, 1.0));
    vec3 col = fogb;
    ...
    if ( rz < FAR )
    {
        ...
        float spe = pow(clamp( dot( reflect(rd,nor), ligt ), 0.0, 10.0 ),1.)*shd*.8;
        float fre = pow( clamp(1.0+dot(nor,rd),0.0,1.0), 1.5 );
        vec3 brdf = vec3(.5,.5,0.3);
        brdf += 1.*dif*vec3(1.00,.7,.5);
        col = mix(vec3(.9,0.5,.5),vec3(1.,1.,1),p.y*.3)*0.2+.1;
        col *= (sin(bnoise(pos*.2)*450.)*0.4+0.6);
        col = col*brdf + col*spe + fre*vec3(1.,1.0,1.)*.5*crv;
    }
    ...
    col *= 1.3;
    col *= clamp(1.2-smoothstep(0.1,2.,length(p)), 0.0, 1.0);
