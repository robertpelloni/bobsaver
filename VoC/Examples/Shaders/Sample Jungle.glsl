...
// original https://www.shadertoy.com/view/Wd23RD
...
{  
    ...
    float de = 100.;
    for(int i=0; i<3; i++){
        p.xy *= rotate(0.4+sin(time*0.2+ 0.3*sin(time*0.4))*0.2);
        p.yz *= rotate(0.4+sin(time*0.3+ 0.5*sin(time*0.5))*0.2);
        p = sabs(p); 
        p *= s;
...
void main(void)
{
    ...
    vec3 ro = vec3(0.2, 0.1, 0.5)+time*0.1;
    vec3 rd = normalize(vec3(p, 2));
    rd.xz *= rotate(sin(time*0.3)*0.6);
    rd.yz *= rotate(sin(time*0.2)*0.6);
    rd.xy *= rotate(sin(time*0.05));   
    vec3 col = mix(
        vec3(0.3, 0.7, 0.8),
        vec3(0.1, 0.1, 0.2),
    ...
    col= mix(col, vec3(col.x), clamp(sin(time*0.5 + sin(time*0.2)*0.5)*2.0-1.0, 0.0, 1.0));
