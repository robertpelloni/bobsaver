...
// original https://www.shadertoy.com/view/4sKyRD
...
{
    float ss = .01;
    vec2 p = (gl_FragCoord.xy - resolution.xy*.5)*ss;
    vec2 ev = edgeVoronoi(p);
    float tri = delaunayTriangulation(p);
    float dist = tri;//(mouse*resolution.xy.z > 0.) ? ev.y : tri;
    vec3 col = shade(tri, dist, ev, ss);
    glFragColor = vec4(gamma(col), 1);
}
