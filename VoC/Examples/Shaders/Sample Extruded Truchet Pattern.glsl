...
// original https://www.shadertoy.com/view/ttVBzd
...
float dist(vec2 p){
    ...
    //return pow(dot(pow(p, vec2(1.666)), vec2(1)), 1./1.666);
    //return (p.x + p.y)*.7071;
...
void main(void) {
    ...
    vec3 r = normalize(vec3(u, 1)), o = vec3(0, time*0.25, -3), l = o + vec3(.25, .25, 2.);
    
    // Rotating the camera about the XY plane.
    r.yz = rot2(.15)*r.yz;
    r.xz = rot2(-cos(time*3.14159/16.)/8.)*r.xz;
    r.xy = rot2(sin(time*3.14159/16.)/8.)*r.xy; 
    ...
    //c = oCol*oCol*1.5*(pow(df, 4.)*2.*sh + sp*sh*2. + .25)*at*ao;
