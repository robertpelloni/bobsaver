...
// original https://www.shadertoy.com/view/WdyBRm
...
void main(void)
 { 
    ...
    float SIZE = resolution.x / BUBBLE_SIZE;
        
    float SF = 1./min(resolution.x,resolution.y)*SIZE*.5;
    ...
    for(float y =- 2.0; y <= 2.0; y++ ) {
        for(float x =- 2.0; x <= 2.0; x++ ) {
            ...
            vec2 ruv = uv + vec2(x, y) + 
                vec2(0, mod(rid, 2.)*.5) + 
                vec2(0, sin(simplex_noise(vec3(rid*0.075, time))*5.)*1.0);                         
