...
// original https://www.shadertoy.com/view/wdjSDW
...
out vec4 glFragColor;
    ...
    #define time ((sin(float(__LINE__))/PI/GR+1.0/GR)*time+1000.0)
    ...
    float lowAverage()
    {
        ...
        {
            float sound = 0.0;//texture(iChannel1, vec2(float(i)/float(iters), 0.5)).r;
            smallest = 
    ...
    vec4 getEvil( in vec2 gl_FragCoord2 )
    {
        ...
        for(int i = 0; i < max_iterations; i++)
        {
            ...
            uv = (iterate(uv0, .5/resolution.xy, magnification)*2.0-1.0);
            ...
            border *= smoothstep(1.0-1.0/GR/E/PI, 1.0, 1.0-color.a);
            
            float weight = smoothstep(0.0, 0.25, magnification);
            antispeckle *= 1.0/magnification;
        }
        
        //glFragColor = vec4(everything(uv*2.0-1.0, o).a);
        //glFragColor = everything(uv*2.0-1.0, o);
    ...
    float map (vec3 pos) {
        ...
        // ground and ceiling
        float bump = 0;//texture(iChannel0, pos.xz*.1).r;
    ...
    void camera (inout vec3 p) {
        p.xz *= rot((-PI*(0./resolution.x-.5)));
    }

    vec4 getSoulMask( in vec2 uv )
    {
        uv = (uv.xy-.5*resolution.xy)/resolution.y;
        vec2 mouse = vec2(0.)/resolution.xy;
    ...
    vec4 getSmoke( in vec2 gl_FragCoord2 )
    {
      
        vec2 uv =  gl_FragCoord2.xy/resolution.x;
...
void main(void)
    {
        ...
        vec4 soulMask = vec4(1.0) - getSoulMask(gl_FragCoord.xy);
        vec4 smoke = getSmoke(gl_FragCoord.xy) * 0.25;
        vec4 evil = getEvil(gl_FragCoord.xy) * 0.35;
        glFragColor = vec4(soulMask.xyz,1.0);
        glFragColor.xyz = vec3(n) * 1.6 - glFragColor.xyz;
        
        glFragColor.x = max(evil.x,glFragColor.x);
        glFragColor.y = max(evil.y,glFragColor.y);
        glFragColor.z = max(evil.z,glFragColor.z);
        
        glFragColor.x = max(smoke.x,glFragColor.x);
        glFragColor.y = max(smoke.y,glFragColor.y);
        glFragColor.z = max(smoke.z,glFragColor.z);
