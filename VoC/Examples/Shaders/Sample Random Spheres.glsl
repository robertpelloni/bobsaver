...
//original https://www.shadertoy.com/view/4s23zR
...
{
    vec3 ray;
    ray.x = (2.0*gl_FragCoord.x/resolution.y - resolution.x/resolution.y);
    ray.y = (2.0*gl_FragCoord.y/resolution.y - 1.0);
    ray.z = (0.5/tan_fov) + 0.5*length(ray.xy)/(0.001+tan(fov*length(ray.xy)));
    
    ray = normalize(ray);//20.0;
    
    mat3 matrix = mat3(cos(0.1*time),sin(0.1*time),0.0,
    ...
    vec3 origin = vec3(0.6*sin(0.5*time), 0.6*cos(0.5*time),time);
    vec3 light = origin + vec3(sin(time)+0.3*sin(3.0*time), cos(time), 3.0+cos(time));
    
    vec3 sphere_colour = vec3(1.0,1.0,1.0);
    vec4 pixel = vec4(0.0,0.0,0.0,1.0);
    vec3 point;
    vec3 exact_point=vec3(0.0);
    vec3 norm;
    ...
    //glFragColor = vec4(to_camera/20.0, to_camera/20.0, to_camera/20.0, 1.0);
    
    //sphere_colour = direct*sphere_colour;
    
    //glFragColor = vec4(direct,direct,direct,1.0);
    //glFragColor = vec4(sphere_colour,1.0);
