...
// original https://www.shadertoy.com/view/ldXSDB
...
float logo( vec2 p )
{
    ...
    vec2 q = p;    
    #ifdef ANIMATE
    q.x *= 1.0 + 0.05*q.x*sin(2.0*time+0.0);
    q.y *= 1.0 + 0.05*q.x*sin(2.0*time+1.3);
    #endif    
...
float slogo( in vec2 p )
{
    ...
    for( int j=0; j<3; j++ )
    for( int i=0; i<3; i++ )
        s += logo( p + 3.5*vec2(i-1,j-1)/resolution.y );
    s /= 25.0;
    return s;
}
//-----------------------------------------------------------------------
...
void main( void )
{
    ...
    // calc normal    
    float a = slogo( p );
    float b = slogo( p + vec2(2.0,0.0)/resolution.y );
    float c = slogo( p + vec2(0.0,2.0)/resolution.y );
