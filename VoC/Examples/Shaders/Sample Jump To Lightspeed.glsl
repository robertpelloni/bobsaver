...
// original /* Created by Louis Sugy, 2019
...
{    
    ...
    vec2 resolution2 = resolution.xy * float(AA);
    vec2 uv = (2.0*coord - resolution2.xy) / resolution2.x;
    vec2 ad = cart2polar(uv);
    
    // Loop forever
    float time = mod(time, 8.0);
...
void main(void)
{
    ...
    for(int i = 0; i < AA; i++)
        for(int j = 0; j < AA; j++)
            avgcol += pixelVal(float(AA)*gl_FragCoord.xy + vec2(i, j));
        
    avgcol /= float(AA*AA);
    
    
    // Output to screen
    glFragColor = vec4(avgcol, 1.0);
}
