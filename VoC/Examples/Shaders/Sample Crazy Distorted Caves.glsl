...
// original https://www.shadertoy.com/view/wtBcWm
...
vec3 surface_color(vec3 p)
{
    p /= scale;
    p /= 200.0;
    return sin(vec3(sceneSDF(p/5.0,0.0),sceneSDF(p*3.0,0.0),sceneSDF(p*2.0,0.0)))*(2.0+sin(sceneSDF(p*11.0,0.0)))/4.0+vec3(.5);
}
...
void main(void)
{
    ...
    //}
    viewDir.yz *= rot(cos(time/5.0)/5.0);
    viewDir.xz *= rot(sin(time/5.0)/5.0);
    
    float speed = 10.0;
    vec3 eye = vec3(time*100.0,0,0);
