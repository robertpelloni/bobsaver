...
// original https://www.shadertoy.com/view/XdjyD1
...
void main(void)
{
    ...
    vec2 area = vec2(floor(13.0 / resolution.y * resolution.x), 13.0); // size of inner area

    vec2 p0 = gl_FragCoord.xy - resolution.xy / 2.0; // position (pixel)
    ...
    // big circle line
    c2 = mix(c2, vec3(1), saturate(2.0 - abs(max(circle, corner)) / scale));
