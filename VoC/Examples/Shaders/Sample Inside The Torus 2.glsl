...
// original https://www.shadertoy.com/view/XcS3zK
...
out vec4 glFragColor;
  ...
  vec2 toScreenspace(in vec2 p) {
    vec2 uv = (p - 0.5 * resolution.xy) / min(resolution.y, resolution.x);
    return uv;
  }
  mat2 R(float a) {
    float c = cos(a);
  ...
  Surface march( in Camera cam ){
    ...
    float ao = 0.; // march space AO. Simple weighted accumulator. Not really AO, but ¯\_(?)_/¯
    vec3 p; // ray position
