...
// original https://www.shadertoy.com/view/ttlfzf
...
float sceneSDF(vec3 p) {
    ...
    float i = 9.0;
    for(int k = 0; k < 2; k++){
        p = sin(p/i)*i-sin(p.yzx*i/(9.0)+time*9.0)/i;
        //float i = 10.0;
        //p /= 1.1;
...
void main(void)
{
    ...
    if (length(mouse*resolution.xy.xy) > 40.0) {
        viewDir.yz *= rot(3.14*0.5-mouse.x*resolution.xy.y/resolution.y*3.14);
        viewDir.xz *= rot(3.14-mouse.y*resolution.xy.x/resolution.x*3.14*2.0);
    }
    viewDir.yz *= rot(cos(time/5.0)/5.0); //wobble
    viewDir.xz *= rot(sin(time/5.0)/5.0); //wobble
    
    float speed = 10.0;
    vec3 eye = vec3(-35.0,1.0,time);
