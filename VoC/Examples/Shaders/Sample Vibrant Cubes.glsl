...
// original https://neort.io/art/c05e1q43p9f30ks59cs0
...
float dist1(vec3 p) {
  ...
  z.xy *= rotate(floor(z.z/interval_z)*.2 + time);
  z.z = mod(z.z, interval_z) - .5*interval_z;
  
  float r1 = rand2(id+floor(time)*.1);
...
float dist2(vec3 p) {
  ...
  float size = .5+pow(sin(time*10.)*.5+.5, 5.);
  size *= .5;
...
void main(){
  ...
  if(d<0.1) {
    ...
    float r1 = rand3(id+floor(time*10.)*.1);
