...
// original https://www.shadertoy.com/view/wsBXWw
...
vec2 map( vec3 pos ){
    ...
    float wave = -0.1 -  0.9*sin(5.*(pol.y+0.2*time))*sin(1.0*pol.z);
    float d2 = d1 + wave;
    float d = sdSub(d1,d2);
...
void main(void)
{
    ...
    vec3 ro = vec3( 3.0*cos(0.2*time), 0.0, 3.0*sin(0.2*time));
    vec3 ta = vec3( 0. , 0. , 0. );
    

    
    // camera matrix
    mat3 camMat = calcLookAtMatrix( ro, ta, sin(0.3*time) );  // 0.0 is the camera roll
