...
// original https://www.shadertoy.com/view/Ns2BWy
...
void main(void)
{
    ...
    // Thanks to FabriceNeyret2 for suggesting gl_FragCoord.xy/resolution.y
    vec2 uv = gl_FragCoord.xy/resolution.y * 64.0 + swayRandomized(COEFFS.zxy, (time * 0.1875) * COEFFS.yzx).xy * 32.0;
    // aTime, s, and c could be uniforms in some engines.
    float aTime = time * 0.0625;
    vec3 adj = vec3(-1.31, 1.41, 1.61);
    ...
    glFragColor = vec4(swayRandomized(COEFFS + 3.0, con * (3.14159265)) * 0.5 + 0.5,1.0);
}
