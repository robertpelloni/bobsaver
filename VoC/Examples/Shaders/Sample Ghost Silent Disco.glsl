...
// original https://www.shadertoy.com/view/ld3XWr
...
{    
    ...
    float a =fSeed*TWO_PI+time*(1.0+fSeed2*3.0)*sign(fSeed-0.5);
    vec2 sc = cossin(a);
       
    // deformation 
    p.xz +=cossin(p.y*sign(fSeed)+time*(5.0+fSeed2)+fSeed*TWO_PI).yx*0.2;
    
    // jump anim
    p.y-=0.7+sin(time*(1.0+fSeed2*5.0)+fSeed)*0.5;
    
    // Body
    float h = 1.0+fSeed2;
    float r = sin(p.y*PI+fSeed*TWO_PI+time*10.0)*(0.05+fSeed2*0.05-0.025)+0.25;    
    float d2Body =  sdVerticalCapsule(p, h, r);
    
    // Arms
    float r2 = r*2.5;
    float armH = h*0.6+fSeed2*0.3;
    float armL = h*0.4+fSeed*0.2 + abs(sin(time*(1.0+fSeed*1.0)+fSeed2));
    d2Body=opSmoothUnion(d2Body, sdCapsule(p, vec3(sc.x*r,armH,sc.y*r), vec3(sc.x*r2,armL,sc.y*r2) ,0.1), 0.2);
    d2Body=opSmoothUnion(d2Body, sdCapsule(p, vec3(-sc.x*r,armH,-sc.y*r), vec3(-sc.x*r2,armL,-sc.y*r2) ,0.1), 0.2);
    
    // Mouth
    float mouthR = max(0.0,sin(time*(0.7+fSeed2)+fSeed)*sin(time*(2.0+fSeed)+fSeed2)*0.15+0.1);
...
void main(void)
{
    vec2 uv = (gl_FragCoord.xy / resolution.xx) -vec2( 0.5,0.5*resolution.y/resolution.x);
    ...
    vec3 r = intersect(ro, rd, 0.3/resolution.y, 100.0);                    
    ...
    glFragColor=vec4( vec3(1.0)*diffuse*specular + vec3(diffuse*0.5*shadow+0.5)*color,1.0);
}
