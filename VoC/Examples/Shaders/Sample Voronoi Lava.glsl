...
// original https://www.shadertoy.com/view/MtKXWt
...
float voronoi(vec2 x) {
  ...
  for(int j = -1; j <= 1; j++) {
    ...
    vec2 o = hash(p + g);
    if(i == 0)
      o = 0.5 + 0.5*sin(6.28138*o + 0.2*time);
...
vec3 formula(vec2 p) {
  p *= mat2(cos(time*0.1), sin(time*0.1), -sin(time*0.1), cos(time*0.1));
  p += vec2(time*0.5, sin(time*0.7));
...
void main(void) {
  ...
  col += vec3(0.5, 0.0, 0.0)*smoothstep(0.0, 2.0, sin(mat.r*mat.b));
