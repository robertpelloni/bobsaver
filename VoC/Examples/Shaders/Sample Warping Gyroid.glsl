...
// original https://www.shadertoy.com/view/MdcfWH
...
vec2 map (vec3 p)
{
    ...
    vec3 w = vec3 (.0);
    vec2 d1 = vec2 (.0);
    mat3 m = rotY (20. * time) * rotZ (-20. * time);
...
{
    float pixelSize = 1. / resolution.x;
    bool forceHit = true;
...
void main(void)
{
    ...
    vec3 ro = 9. * vec3 (cos (.2 * time), 1.25, sin (.2 * time));
