...
//original https://www.shadertoy.com/view/Md23W3
...
void main(void) {
    ...
    ball_angle = t/3.0;
    forward = vec3(0.0,-cos(ball_angle),sin(ball_angle));
    side    = normalize(cross(forward, vec3(1.0, 0.0, 0.0)));
    up      = normalize(cross(forward, side));
    rot = mat3(forward,side,up);

    vec2 uv = (gl_FragCoord.xy / resolution.xy) * 2.0 - 1.0;
    
    vec3 dir = normalize(vec3(uv.x, uv.y * (resolution.y/resolution.x), 1.0));    
    
    vec3 cam = vec3(0.0,1.8,-8.0);
