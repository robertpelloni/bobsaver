...
// float r0 = cgol_test(res_r);
    ...
    if( nr0w >= 0.410 && nr0w <= 0.625 ) { r0 += s*(0.7+cs*00.0); res_g = 0.3; }
    if( nr1w >= 0.200 && nr1w <= 0.266 ) { r0 += s*(0.92); res_g = 1.0; }

    if( nr0w >= 0.437 && nr0w <= 0.464 ) { r0 -= s*(1.9); }
    if( nr1w >= 0.252 && nr1w <= 0.279 ) { r0 -= s*(1.0); }
