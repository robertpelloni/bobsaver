...
// original https://www.shadertoy.com/view/MllBzj
...
// variant of  https://shadertoy.com/view/llsfRj
// application from https://shadertoy.com/view/Xlffzj

#define S(v) smoothstep(2./resolution.y, 0., v )
...
{
    q = Mobius(q, Z1, Z2);
    return spiralZoom(q, vec2(-.5), 5., spiral, spiral2, .5, vec2(-1, 1)*time*.125);
}
float DrawInvMobiusSpiral(vec2 q, vec2 uv0, float r) // total inverse transform + draw iso-X
{
    vec2 v; float s = 0.;
    q = ispiralZoom(q, vec2(-.5), 5., spiral, spiral2, .5, vec2(-1, 1)*time*.125);
    v = iMobius(q, Z1, Z2);  
