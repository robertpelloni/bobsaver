...
// original https://www.shadertoy.com/view/fssfDM
...
vec3 distort(vec3 p) {
    ...
    float sc = 1.5;//exp(-0.5 * length(p));// + 0.5 * cos(0.5 * length(p) - time);
    
    float tp = 2.*pi/3.;
    //float val = length(p);//.;//cos(time)*p.x + cos(time + tp)*p.y + cos(time-tp)*p.z;
    
    float c  = cos(time + sc * smin(p.y, p.z) + pi * cos(0.1 * time));
    float c2 = cos(time + sc * smin(p.z, p.x) + pi * cos(0.1 * time + tp));
    float c3 = cos(time + sc * smin(p.x, p.y) + pi * cos(0.1 * time - tp));

    vec3 q = erot(normalize(p), normalize(vec3(c,c2,c3)), pi + 0.3 * cos(2. * length(p) - time));
...
float GetDist(vec3 p) {
    ...
    //p = mix(sabs(p) - 0., sabs(p) - 1., 0.5 + 0.5 * thc(4., time));
...
void main(void)
{
    ...
    if(d<MAX_DIST) {
        ...
        // p = distort(p);
        col *= 2.2 + 1.8 * thc(14., 24. * length(p) - 1. * time);
        col = clamp(col, 0., 1.);
        col *= 1.-exp(-1.1 - 0.5 * p.y);
        col *= 0.9 + 0.5 * n.y;
        vec3 e = vec3(1.);
        col *= pal(length(p) * 0.2 - 0.08 * time, e, e, e, 0.5 * vec3(0,1,2)/3.);
        col = clamp(col, 0., 1.);
