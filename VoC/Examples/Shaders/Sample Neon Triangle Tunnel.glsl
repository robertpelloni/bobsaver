...
// original https://www.shadertoy.com/view/7lScWV
...
vec4 map(in vec3 p) {
  ...
  for (float i = 0.0; i < 12.0; i++) {
    ...
    tAngle = inOutBack(0.0, 0.7, mod(time, spinDelta + 0.7) - spinTime)
      * 2.0 * PI / 3.0 + tAngle;
    tAngle += sin(time * smoothstep(0.3, 0.6, sin((tIndex + i) * 356.4))) * 0.1;
...
void main(void) {
  ...
  if (d > 0.0) {
...
#if 0
    {
      ...
      lightPos.xz += vec2(sin(time), cos(time) * 0.5);
