...
// original https://www.shadertoy.com/view/4dsyW8
...
float SH_4_4( in vec3 s ) { vec3 n = s.zxy; float z2 = n.z*n.z, r2 = dot(n, n);
                                            return  k15 * (35.0 * z2*z2 - 30.0 * z2*r2 + 3.0 * r2*r2); }
...
float SH_4_8( in vec3 s ) { vec3 n = s.zxy; float x2 = n.x*n.x, y2 = n.y*n.y;
                                            return  k17 * (x2 * (x2-3.0*y2) - y2*(3.0*x2 - y2)); }
...
void main(void)
{
    ...
    {        
        ...
        // camera
        float an = 0.314*TIME - 10.0*mouse.x*resolution.xy.x/resolution.x;
        float dist = 8.0;
