...
// original https://www.shadertoy.com/view/WslyzX
...
float scene(vec3 p)
{
    float flip = step(gl_FragCoord.xy.x,resolution.x*.5)*2.-1.;
    float time = flip*time;
...
void main(void)
{
    ...
    glFragColor.rgb = pow(trace(cam,dir),vec3(.45));
}
