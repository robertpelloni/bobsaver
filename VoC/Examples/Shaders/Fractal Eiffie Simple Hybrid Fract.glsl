...
// original https://www.shadertoy.com/view/4sdXDl
...
void Bulb(inout vec4 z, in vec4 c){//mandelBulb by twinbee
 ...
 float zo = asin(z.z / r) * k + time*0.15;
...
float rnd;
void randomize(in vec2 p){rnd=fract(float(frames)+sin(dot(p,vec2(13.3145,117.7391)))*42317.7654321);}

vec3 scene(vec3 ro, vec3 rd){
  vec3 L=normalize(vec3(0.4,0.025,0.5));
  vec3 bcol=sky(rd,L);
  vec4 col=vec4(0.0);//color accumulator
  float t=DE(ro)*rnd,d,od=1.0,px=1.0/resolution.x;
  ro+=rd*t;
...
void main(void) {
   ...
   float tim=time*0.3,r=2.0+cos(tim*0.7);
   ...
   glFragColor=vec4(scene(ro,rd)*2.0,1.0);
}
