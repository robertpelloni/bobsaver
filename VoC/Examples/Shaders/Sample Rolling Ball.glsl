...
// original https://www.shadertoy.com/view/lsfXz4
...
{
    float fogAmount = clamp(dis*dis* 0.0000005, 0.0, 1.0);
...
vec3 GrassOut(in vec3 rO, in vec3 rD, in vec3 mat, in vec3 normal, in float dist)
{
    ...
    for (int i = 0; i < 15; i++)
    {
        ...
        if (ret.x < rCoC)
        {
            ...
            vec3 gra = mix(vec3(.1, .3, 0.0), vec3(.35, .35, min(pow(ret.z, 4.0)*35.0, .35)),
...
{
    float m = 0.0;
    float gTime = (time*5.0+m+462.0)*.006;
    ...
    camTar      = CameraPath(gTime + .05);
    
    vec2 diff = (camTar.xz - CameraPath(gTime + .13).xz);
    ballRoll.x = -gTime * 66.0;
