...
#define time time
#define mouse*resolution.xy mouse
...
void main(void)
{
    ...
    vec3 org = vec3(0.0*.2+1.,+0.3+bass*.05,t+5.);
    ...
    color += vec4(1.0,0.3,0.0,1.0)/(ribbon1(p-n*.01)*20.+.75)*1.0;//pow(bass,2.)*3.;
    color += vec4(0.5,0.3,0.7,1.0)/(ribbon2(p-n*.01)*20.+.75)*1.0;//pow(texture( iChannel0, vec2(64./256.,0.25) ).x,2.)*2.;
    color += vec4(0.0,0.5,1.0,1.0)/(ribbon3(p-n*.01)*20.+.75)*1.0;//pow(texture( iChannel0, vec2(128./256.,0.25) ).x,2.)*5.;
    color += vec4(0.0,1.0,0.2,1.0)/(ribbon4(p-n*.01)*20.+.75)*2.0;//pow(texture( iChannel0, vec2(200./256.,0.25) ).x,2.)*5.;
    color *= AO(p,n);
