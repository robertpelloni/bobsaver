...
//original https://www.shadertoy.com/view/ld23DG
...
float DE(vec3 p)
{
    ...
    if(d2<d && d2<d1){
        if(d2<0.015){
            ...
            vec2 v=vec2(d2,(fract((t1*20.0+atan(b,c))*0.795775)-0.5)*0.05*len);
            d2=length(v)-0.01;
        }
        d=d2;
    }
...
void main() {
    ...
    float t=0.0,d=1.0,dm=100.0,tm=0.0;
    for(int i=0;i<32;i++){
        t+=d=DE(ro+rd*t);
        if(d<dm){dm=d;tm=t;}
    }
    ...
    glFragColor = vec4(clamp(col*1.5,0.0,1.0),1.0);
