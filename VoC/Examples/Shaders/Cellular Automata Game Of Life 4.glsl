...
// With some minor modifications by another Gabriel.
// Check out the "colonies" of each color that form,
// especially when viewing at 1.0 resolution.
...
if( distance( coord, mousePos) < 15. )
    ...
    */
    if( distance( coord, vec2(resolution.x/4.0 + resolution.x/20.0*sin(time*2.0),
                  resolution.y/2.0 + resolution.y/12.0*cos(time*2.0))) < 20. )
    {
        c = hueShift(coord.x/5., initialColor);
    }
    if( distance( coord, vec2(3.0*resolution.x/4.0 + resolution.x/20.0*cos(time*2.1),
                  resolution.y/2.0 + resolution.y/12.0*sin(time*2.1))) < 20. )
    {
        c = hueShift(coord.x/5., initialColor);
    }
