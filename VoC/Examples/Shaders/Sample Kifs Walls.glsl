...
// original https://www.shadertoy.com/view/mlS3DV
...
vec2 map( in vec3 p ) {
    ...
    for( int i=0; i<2; i++ )
    {
        t += s*.5*(cos(6.2831*q.x+time*5.) + cos(6.2831*q.y+time*2.));;
        s *= 0.5 + 0.1*t;
        q.xy = 0.97*mat2(1.6,-1.2,1.2,1.6)*q.xy + (t-0.5)*0.2;
    }
    
    float dP = abs(q.z  - 0.01 - (cos(q.x*4.+time)*.5+.5)*.02 );
    if (dP-.3 < d) mat = 1.;
    d = sminCubic( d,  dP, .6 );
    }
...
void main(void)
{
    vec3 ro = vec3(0., 0., 15. - time*2.5);
    camPos = ro;
    vec3 ta = vec3( 0.0, 0., 0.0 - time*2.5 );
    // camera matrix
    ...
    for( int n=0; n<AA; n++ )
    {
        ...
        rd.xy *= rot(sin(time*PI/7.5)*.4);
        rd.xz *= rot(sin(time*PI/7.5 * 2.)*.15);
