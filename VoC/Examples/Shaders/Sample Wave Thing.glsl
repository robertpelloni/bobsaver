...
// original https://www.shadertoy.com/view/NtScRh
...
float GetDist(vec3 p) {
    ...
    for (float i = 0.; i < 5.; i++) {
        ...
        p.y += 0.05 * cos(10. * p.x) * cos(2. * i * pi / 5. + 24. * length(p.xz) / pi + time);
...
void main(void)
{
    ...
    if(d<MAX_DIST) {
        ...
        vec3 e = vec3(0.5);
        col *= pal(mod(floor(0.25 * p.z + 0.25 * time), 2.) + -0. * time + 0. * h21(floor(1. * p.xz - 0.1 * time)) + mlength(p.xz), e, e, e, 0.5 * vec3(0,1,2)/3.);
        //col *= col + 0.5;
