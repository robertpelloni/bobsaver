...
//
// https://www.shadertoy.com/view/MdlGzn
//
  ...
  if (f<maxd){
    ...
    if (s.y==2.0){
      ...
      if (f<maxd){
        ...
        glFragColor=vec4((objColor*0.8+objColor2*0.2)*(1.0-f0*.03),1.0);
      } else glFragColor=vec4(objColor*0.8*(1.0-f0*.03),1.0);

    } else glFragColor=vec4(objColor*(1.0-f0*.03),1.0);
    
  } else glFragColor=vec4(0,0,0,1);
