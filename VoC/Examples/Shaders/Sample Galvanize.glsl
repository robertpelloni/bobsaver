...
// original https://www.shadertoy.com/view/4tc3zf
...
{
    return sqrt(sqrt(sqrt(worley(p*32. + 4.3 + time*.250) * sqrt(worley(p * 64. + 5.3 + time * -.125)) * sqrt(sqrt(worley(p * -128. +7.3))))));
}
...
float Mandelbulb(vec3 p)
{
    ...
    float parachute=(1.-min(1.8*abs(sin((time-5.0)*3.1415/scene)),1.0)); // Fallschirm
...
{
    m = length(max(abs(rotYaxis(rotXaxis(pos+vec3(0.0,-0.3,0.0),time),time*0.3))-vec3(0.35,0.35,0.35),0.0))-0.02; 
    m = smin (m, length(pos+vec3(0.0,-0.40,1.2+0.5*sin(0.8*time+0.0)))-0.4,7.4);
    m = smin (m, length(pos+vec3(0.0,-0.40,-1.2-0.5*sin(0.8*time+0.4)))-0.4,7.4);
    m = smin (m, length(pos+vec3(-1.2-0.5*sin(0.8*time+0.8),-0.40,0.0))-0.4,7.4);
    m = smin (m, length(pos+vec3(1.2+0.5*sin(0.8*time+1.2),-0.40,0.0))-0.4,7.4);
    m = smin (m, length(pos+vec3(0.0,-1.6+0.5*-sin(0.8*time+1.6),0.0))-0.4,7.4);
...
{
    return p.y+(0.025*sin(p.x*10.  +1.4*time  ))+(0.025*sin(p.z*12.3*cos(0.4-p.x)+  1.6*time  ))-0.05;
}
...
float map(in vec3 p)
{
    ...
    m = NewMenger(rotYaxis(rotXaxis(p-vec3(0.0,sin(time/0.63)+0.2,0.0),0.15*time),0.24*time));
    }
    if (efx == 2) {            // mandelbulb
    m = Mandelbulb(rotYaxis(rotXaxis(p,time*0.1),0.21*time));
    }
    if (efx == 3) {            // kalibox
    m = Kalibox(rotYaxis(rotXaxis(p,1.50),0.1*time));
    }
    if (efx == 4 || efx == 5) { // tunnel or swirl
    vec3 c = vec3(2.0, 8.0, 2.0);
    vec3 q = mod(p-vec3(1.0,0.1*time,1.0),c)-0.5*c;
    float kali = Kalibox(rotYaxis(q,0.04*time));
    m = max(kali,-sdCylinder(p,vec3(0.0,0.0,0.30+0.1*sin(time*0.2))) );
    }
    d = sminPoly (m, d, 0.04); 
       return d;
}
...
void main(void)
{
    ...
    if (time >=0. && time <=35. ) {efx=4; refleco=0; snowo=0;} 
    if (time >35. && time <=70. ) {efx=0; refleco=1; snowo=1;}
    if (time >70. && time <=105.) {efx=1; refleco=0; snowo=1;} 
    if (time >105.&& time <=140.) {efx=3; refleco=0; snowo=1;} 
    if (time >140.&& time <=175.) {efx=2; refleco=0; snowo=1;}  
    if (time >175.&& time <=210.) {efx=4; refleco=0; snowo=0;}   
    if (time >210.&& time <=245.) {efx=5; refleco=0; snowo=0;}  

    blend=min(2.0*abs(sin((time+0.0)*3.1415/scene)),1.0); 
    if (time >245.) blend = 0.;
...
// render:
    ...
    vec3 lig;    
    if (efx==4 || efx ==5 )      lig = normalize(vec3(-0.4*sin(time*0.15), 1.0, 0.5));
    else if (efx==3)              lig = normalize(vec3(-0.1*sin(time*0.2), 0.2, 0.4*sin(time*0.1)));    
...
// postprocessing
float klang1=0.75;
vec2 uv2=-0.3+2.*gl_FragCoord.xy/resolution.xy;
col-=0.20*(1.-klang1)*rand1(uv2.xy*time);                            
col*=.9+0.20*(1.-klang1)*sin(10.*time+uv2.x*resolution.x);    
col*=.9+0.20*(1.-klang1)*sin(10.*time+uv2.y*resolution.y);    
float Scr=1.-dot(uv2,uv2)*0.15;
vec2 uv3=gl_FragCoord.xy/resolution.xy;
float worl = fworley(uv3 * resolution.xy / 2100.);
...
if (uv3.y >=g2+0.07) {if (mod(uv3.x-0.06*time,0.18)<=0.16) col*=0.5;}
if (uv3.y >=g2+0.05) {if (mod(uv3.x-0.04*time,0.12)<=0.10) col*=0.6;}
if (uv3.y >=g2+0.03) {if (mod(uv3.x-0.02*time,0.08)<=0.06) col*=0.7;}
if (uv3.y >=g2+0.01) {if (mod(uv3.x-0.01*time,0.04)<=0.02) col*=0.8;}
if (uv3.y <=g1+0.10) {if (mod(uv3.x+0.01*time,0.04)<=0.02) col*=0.8;}
if (uv3.y <=g1+0.08) {if (mod(uv3.x+0.02*time,0.08)<=0.06) col*=0.7;}
if (uv3.y <=g1+0.06) {if (mod(uv3.x+0.04*time,0.12)<=0.10) col*=0.6;}
if (uv3.y <=g1+0.04) {if (mod(uv3.x+0.06*time,0.18)<=0.16) col*=0.5;}
if (uv3.y <=g1+0.02) col*=0.4;
if (uv3.y <=g1+0.00) col*=0.0;

if (snowo == 1) glFragColor = (vec4(col*1.0*Scr-1.6*snow(cv), 1.0)*blend)*vec4(1.0, 0.93, 1.0, 1.0);
else glFragColor = vec4(col*1.0*Scr, 1.0)*blend;
}
