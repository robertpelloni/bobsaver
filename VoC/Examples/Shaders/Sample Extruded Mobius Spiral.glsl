...
// original https://www.shadertoy.com/view/NlKXWt
...
float hm(in vec2 p){ 
     ...
     return (sin(6.2831*(p.y*2. + p.x) + time*2.)*.5 + .5); 
    ...
    //float h = hash21(p);
    //return (sin(6.2831*h + time*2.)*.5 + .5);
...
float map(vec3 p){
    ...
    #ifdef DOUBLE_SPIRAL
    vec2 z2 = rot2(-cos(time/3.)*.65*2.)*p.xy;    
...
float trace(in vec3 ro, in vec3 rd){
    ...
    for(int i = min(0, frames); i<128; i++){
