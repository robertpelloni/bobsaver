...
const float water = 0.1;//water level
const float scale = .1;//height of the rock 
float offTime;
float stime;
...
void main(void)
{
    offTime = time*2.0+97.0;  // Some cool place in time. (makes the animation a little faster
    stime=sin(offTime);
    //Camera animation
    vec2 vPos=(-1.0+2.0*(gl_FragCoord.xy/resolution.xy)) * 0.3;
    vec3 vuv=vec3(stime*0.01,.3,0);     //view up vector
    vec3 vrp=vec3(sin(offTime*0.027)*35.0, 1, cos(offTime*0.19)*20.0); //view reference point
    ...
    {
        glFragColor = vec4(getTerrain(scrCoord, rd, dis), 1.0);
    }else
    {
        glFragColor= vec4(getSky(rd, light), 1.0);
    }
}
