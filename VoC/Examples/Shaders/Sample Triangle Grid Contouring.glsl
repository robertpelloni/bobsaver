...
// original https://www.shadertoy.com/view/WtfGDX
...
// Aestheticallly, I kind of like it, but it's a little busy, so it's off by default.
...
vec3 simplexContour(vec2 p){
    ...
    // Wobbling the coordinates just a bit in order to a subtle hand drawn appearance.
    ...
    // Compare it to the underlying grey scale value, then smoothly threshold it.
    ns = smoothstep(0., 1., dot(col, vec3(.299, .587, .114)) - ns + .35);
    // Repeat the process with a rotated layer.
    q *= rot2(3.14159/3.);
    float ns2 = (n2D3G(q*4.*vec2(1./3., 3))*.64 + n2D3G(q*8.*vec2(1./3., 3))*.34)*.5 + .5;
    ns2 = smoothstep(0., 1., dot(col, vec3(.299, .587, .114)) - ns2 + .35);
    // Mix the two layers in some way to suit your needs.
    ns = min(ns*1.2, ns2); // Rough pencil sketch layer.
    //
    ...
    pt = rot2(6.2831/3.)*pt;
    float pat = clamp(cos(pt.x*6.2831*14. - offs)*2. + 1.5, 0., 1.);
    col *= pat*.4 + .8;
    */
...
void main(void)
{
    ...
    vec2 uv = (gl_FragCoord.xy - resolution.xy*.5)/min(650., resolution.y);
