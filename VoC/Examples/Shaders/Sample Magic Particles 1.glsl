...
#define nb_particles 150                                      // Number of particles on the screen at the same time. Be CAREFUL with big numbers of particles, 1000 is already a lot!
...
const float part_life_time_max = 2.0;                         // Maximum life time in s of a particle
...
{  
    ...
    time2 = time_factor*(time + timedelta);
    vec3 pcol = vec3(0.);
    // Main particles loop
...
void main(void)
{
    ...
    //vec3 pcolor = texture2D(iChannel0,uv).rgb*0.4;
