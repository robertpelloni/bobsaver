...
// original https://www.shadertoy.com/view/WtcXD8
...
#define ZOOM 5.
#define mx (50.*mouse*resolution.xy.x/resolution.x)
#define my (-time + 50.*mouse*resolution.xy.y/resolution.x)
...
float sdMain(vec3 p, vec2 idD){
    ...
    vec3 q = p;
    float steps = 5.;
    float m = sin(time + idD.x + idD.y*4.);
...
void main(void)
{
    ...
    if (hit){
        ...
        n.y *= -1.;
        n.g*=0.4;
        n.xy *= rot(0.2 + sin(time)*0.2 + uv.x*0.6);
