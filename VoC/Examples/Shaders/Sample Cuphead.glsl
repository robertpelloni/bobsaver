...
// original https://www.shadertoy.com/view/MllBWS
...
float straw(vec3 p) {
    ...
    p.xy = rot(p.xy + vec2(0, 0.2), 0.2*sin(14.0*time)) - vec2(0, 0.2);
...
float innereyes(vec3 p) {    
    ...
    p.z += 0.07*cos(10.0*time)*smoothstep(-0.2, 1.0, sin(time));
    p.z = abs(p.z) - 0.25;
...
void main(void)
{
    ...
    vec2 mo =  vec2(1.5, 1.0);
        //: (-resolution.xy + 4.0*mouse*resolution.xy.xy)/resolution.y;
