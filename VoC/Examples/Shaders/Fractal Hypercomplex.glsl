...
//original https://www.shadertoy.com/view/XslSWl
...
float map(vec3 p) {
    const float M = 0.7;
    float time = time + rsq(time*0.5) * 2.0;
    return julia(p,vec4( 
...
float getAO(vec3 p,vec3 n) {
    ...
    for(int i = 0; i < AO_SAMPLES; i++) {
        float fi = float(i);
        float h = 0.1+fi*INV_AO_SAMPLES*0.1;
        vec3 aon = n + vec3(sin(fi*42.5345),cos(fi*15.85234),sin(fi*7.742)) * 0.45;
        r += max(map(p+normalize(aon)*h) - TRESHOLD, 0.0);
    }
    r *= INV_AO_SAMPLES;
    return pow(clamp(r*5.0,0.0,1.0),0.5);
}
...
void main(void) {
    ...
    vec2 sc = vec2(sin(time2),cos(time2));
    ...
    vec3 color;
    color  = vec3((diffuse(n,l0,3.0) + specular(n,l0,dir,1.0)) * ORANGE);
    color += vec3((diffuse(n,l1,3.0) + specular(n,l1,dir,1.0)) * BLUE);    
