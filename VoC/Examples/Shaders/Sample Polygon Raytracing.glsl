...
// original https://www.shadertoy.com/view/ldsSzr
...
void scene(){
    ...
    for(float i=3.14*2000.0; i<3.14*4000.0; i+=3.14*400.0){
            ...
            ympyraPala.C=vec3(0.0, 4.+1.*sin(time*3.782562261), 0.0);
            ympyraPala.color=vec3(.31,.0,.0);
            resolveRayPolygonIntersection(ympyraPala);
    }
    for(float i=3.14*2000.0; i<3.14*4000.0; i+=3.14*400.0){
            polygon ympyraPala;
            ympyraPala.A=vec3(5.0*cos(i+3.14*400.), -3.0+1.*sin(time*3.782562261-1.), 5.0*sin(i+3.14*400.));
            ympyraPala.B=vec3(4.0*cos(i+3.14*400.), -1.0, 4.0*sin(i+3.14*400.));
            ympyraPala.C=vec3(4.0*cos(i), -1.0, 4.0*sin(i));
            ympyraPala.color=vec3(.31,.0,.0);
            resolveRayPolygonIntersection(ympyraPala);
            ympyraPala.C=vec3(5.0*cos(i+3.14*400.), -3.0+1.*sin(time*3.782562261-1.), 5.0*sin(i+3.14*400.));
            ympyraPala.B=vec3(5.0*cos(i), -3.0+1.*sin(time*3.782562261-1.0), 5.0*sin(i));
            ympyraPala.A=vec3(4.0*cos(i), -1.0, 4.0*sin(i));
            ympyraPala.color=vec3(.31,.0,.0);
            resolveRayPolygonIntersection(ympyraPala);
    }
    ...
    for(float j=-1.0; j<4.0; j++)
        for(float i=-2.0; i<2.0; i++){
            polygon tasoPalaA;
            tasoPalaA.C=vec3(4.0*i, -4.0+sin((i+j+time)*.71)*2., 4.0*j);
            tasoPalaA.B=vec3(4.0*i+4.0, -4.0+sin((i+j+time)*.71)*2., 4.0*j);
            tasoPalaA.A=vec3(4.0*i, -4.0+sin((i+j+time)*.71)*2., -4.0+4.0*j);
            tasoPalaA.color=vec3(.41,.41,.41);
            resolveRayPolygonIntersection(tasoPalaA);
            polygon tasoPalaB;
            tasoPalaB.C=vec3(4.0*i+4.0, -4.0+sin((i+j+time)*.71)*2., 4.0*j);
            tasoPalaB.B=vec3(4.0*i+4.0, -4.0+sin((i+j+time)*.71)*2., -4.0+4.0*j);
            tasoPalaB.A=vec3(4.0*i, -4.0+sin((i+j+time)*.71)*2., -4.0+4.0*j);
...
void main(void){
    ...
    closestPoint.normal=vec3(0.0);
    closestPoint.color=vec3(0.0);

    vec2 uv = gl_FragCoord.xy / resolution.xy;
    //float aspectratio = resolution.x / resolution.y;
    //vec3 noise=texture2D(iChannel0, uv+vec2(time)).xyz*rand(uv+vec2(time))*vec3(0.052);
    
    
    float aspectCorrection = (resolution.x/resolution.y);
    vec2 coordinate_entered = 2.0 * uv - 1.0;
    vec2 coord = vec2(aspectCorrection,1.0) *coordinate_entered;
    
    camera = vec3(-4.0*sin(time),3.0,22.0);
    ray = normalize(vec3(coord, -1.5));
    ray*=rotate_y((sin(time)*2.)*3.14/180.);
    sun.pos=vec3(0.,0.0,-5.*sin(time*0.166421));
    //camera=vec3(-4.0*sin(time),3.0,15.0);
    //ray=normalize(vec3((-1.+2.*uv)*vec2(aspectratio,1.),-1.));
    closestPoint.distanceFromCamera=100000.;
    color=vec3(0.);
    float angle=3.14*(sin(time*0.08631)*10.0)/180.0;
    ray*=rotate_y(angle);
    scene();

    glFragColor = vec4(color+vec3(-0.5),1.0);
}
