...
void main()
{
    ...
    position2 *= resolution.xy/(sin(time*1.0)*0.4+40.0);
    ...
    vec2 position = gl_FragCoord.xy / resolution.xy - vec2(cos(time*0.2)*1.01+20.0, sin(time*0.4)*1.2+0.4);

    float r = length(position+sin(time*sin(time*0.005)*0.2)*1.2);
    float a = atan(position.y, position.x);
    float t = time + 24.0/(r*r+1.0);
    
    float light = 14.0*abs(0.05*(sin(t)+sin(time+a*25.0)));
    vec3 color = vec3(-sin(r*15.0-a-time+sin(r+t)), sin(r*1.4+a-time+sin(r+t)), cos(r+a*6.0+a+time)-sin(r+t));
    ...
    fcolor = vec3(fcolor.r+fcolor.g, fcolor.r, fcolor.r*.95);
