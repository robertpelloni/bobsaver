...
// original https://www.shadertoy.com/view/ws2XRd
...
{    
    ...
    float clockLerp = clamp(mod(date.w * 2.,2.),0.,1.);
...
vec2 getFinalRotation(in vec2 uv, in vec2 id) 
{
    ...
    float segue = .8; 
    int fx = int(date.w / animationTime);
    float frac = mod(date.w, animationTime)/ animationTime;
