...
// original https://www.shadertoy.com/view/4sSBW3
...
float de( in vec3 p, in float r, out vec3 color ) {
    ...
    for (int x = -1 ; x <= 1 ; x++) {
        for (int y = -1 ; y <= 1 ; y++) {
            ...
            float height = 1.0 + perlin( time+rnd.x, rnd.y )*0.75;
...
void main(void) {
    ...
    vec3 from = vec3(time, 0.2*time, -5)+vec3(mouse*resolution.xy.xy*0.04, 0);
    ...
    glFragColor.rgb = mix(color, gloss.rgb, fres);
    glFragColor.a = 1.0;
