...
// original https://www.shadertoy.com/view/4sc3z2
...
{
    p = 2.0 * p - vec2(resolution.x/resolution.y, 1.0);
...
float map(vec3 pos)
{
    return length(pos - (vec3(0.0, 0.0, 1.5) + time * vec3(0.0, 0.0, SPEED))) - 1.0;
}
...
{
    vec3 pos = vec3(uv * vec2(resolution.x/resolution.y, 1.0), time * SPEED);
    float f;
...
void main(void)
{
    ...
    //if (mouse*resolution.xy.z > 0.0) {
        split += 2.0 * mouse.xy*resolution.xy/resolution.xy - 1.0;
    //}
    
    vec3 col = vec3(0.0, 0.0, 0.0);
    
    vec3 ro = vec3(0.0, 0.0, 0.0) + time * vec3(0.0, 0.0, SPEED);
    vec3 rd = vec3((p * 2.0 - 1.0) * vec2(resolution.x/resolution.y, 1.0), 1.0);
    col = marching(ro, rd, p, split);
    
    col = draw_circle_outline(p * vec2(resolution.x/resolution.y, 1.0), 0.9, col);
    col = mix(vec3(0.3, 0.0, 0.0), col, smoothstep(0.0, 0.005, abs(p.x - split.x)));
    col = mix(vec3(0.3, 0.0, 0.0), col, smoothstep(0.0, 0.005*resolution.x/resolution.y, abs(p.y - split.y)));
