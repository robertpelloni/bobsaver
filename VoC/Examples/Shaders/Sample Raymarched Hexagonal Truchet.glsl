...
// original https://www.shadertoy.com/view/4td3zj
...
/*
    ...
    which in turn is based on something Mattz did a while back. Both examples are below.
...
void main(void) {
    ...
    vec2 a = sin(vec2(1.570796, 0) + sin(time/4.)*.3); // Fabrice's observation.
    ...
    float p2 = 1.0 - smoothstep(0., .1, Voronoi(sp.xy*4. + vec2(time, cos(time/4.))));
