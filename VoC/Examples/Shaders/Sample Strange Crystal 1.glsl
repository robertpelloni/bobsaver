...
// original https://www.shadertoy.com/view/tsVXzh
...
vec3 insides(in vec3 ro, in vec3 rd, in float INNERRAD, in mat2 rot, out float tout) {
    ...
    //inner background
    vec3 ni2, no2;
    vec2 tb = box(ro, rd, vec3(INNERRAD), ni2, ni2);
    vec3 rob = ro + tb.y * rd;
    ...
    tout = mix(tb.y, t.x, step(0., t.x));
    return mix(mix(vec3(0.5), texb, step(0., tb.y)) * 0.5, tex * fac, step(0., t.x));
}
...
void main(void)
{
    ...
    float osc = smoothstep(0., 1., (clamp(abs(mod(time, t_osc*2.)/t_osc-1.), h_l, h_h) - h_l) / (h_h - h_l));
        ...
        for (int i=0; i<SAMPLES; i++) {
            ...
            for (int j=0; j<REFLECTIONS; j++) {
                ...
                if (tb > 0.) {
                    ...
                    vec3 ni2;
                    vec2 tout = box(ro_refr, rd2, BOXDIMS, ni2, no2);
                    no2 = n.zyx * no2.x + n.xzy * no2.y + n.yxz * no2.z;
                    vec3 rout = ro_refr + tout.y * rd2;
