...
// original https://www.shadertoy.com/view/4ddXW4
...
vec3 render(vec3 ro, vec3 rd) {
    ...
    if (m > -0.5) {
        ...
        col += 0.3 * sin(vec3(0.15, 0.02, 0.10) * time * 6.0);
...
void main(void) {
    ...
    float t = time * CameraSpeed + 4.0 * 60.0;
    vec3 ro = vec3(path(t), t);

    t += 0.5;
    vec3 target = vec3(path(t), t);
    vec3 dir = normalize(target - ro);
    vec3 up = vec3(-0.9309864, -0.33987653, 0.1332234) * rotationZ(time * 0.05);
