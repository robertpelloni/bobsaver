...
void main() {
    ...
    float nh1w = nh1[2] / nh1[1];
        ...
        if( nh1c >= 0.834 && nh1c <= 0.865 ) { res = 0.0; }

    float nh2s = 9.0;
    vec3 nh2 = nh_ring ( nh2s, 0.0, vec2(0.5, 3.0), vec2(0.0, 0.0) );
    float nh2c = nh2[0] / nh2[1];
    float nh2w = nh2[2] / nh2[1];
        if( nh2c >= 0.649 && nh2c <= 0.904 ) { res = 1.0; }
        if( nh2c >= 0.086 && nh2c <= 0.149 ) { res = 0.0; }
        if( nh2c >= 0.800 && nh2c <= 1.000 ) { res = 0.0; }
