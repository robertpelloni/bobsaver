...
// original https://www.shadertoy.com/view/tsGSzw
...
void main(void) {
    ...
    float time2 = time * 0.3;// + mouse*resolution.xy.x*0.01; // bteitler: Animation is based on time, but allows you to scrub the animation based on mouse movement
    ...
    vec3 ang = vec3(sin(time2*3.0)*0.1,sin(time2)*0.2+0.25,time2);
    ...
    vec3 hsv = rgb2hsv(glFragColor.xyz);    
