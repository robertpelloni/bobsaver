...
// original https://www.shadertoy.com/view/wljSzR
...
//#else
    //float yaw = mouse*resolution.xy.y - 0.15;
    //float angle = mouse*resolution.xy.x * 8.0;
//#endif
    
    vec3 from = (vec3(sin(angle), 0, cos(angle)) * cos(yaw) + vec3(0, sin(yaw) * 1.0, 0)) * 5.0; 
    //vec3 from = origin + vec3((mouse*resolution.xy.xy - vec2(0.5,0.0)) * vec2(15.0, 3.0), -5);
    vec3 to = vec3(0, -1, 0);
    vec3 up = vec3(0, 1, 0);
    ...
    glFragColor = trace(from, rot * normalize(ray));
}
