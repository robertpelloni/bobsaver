...
// original https://www.shadertoy.com/view/4sdGzs
...
float fCone(vec3 p, float radius, float height, vec3 direction) {
    p = reflect(p, normalize(lerp(vec3(0,1,0), -direction, .5)));
    //p -= vec3(0,height,0);
...
float fHole(
    ...
    vec3 AB = lerp(a, b, 0.5);
    vec3 AAB = lerp(a, b, w);
    vec3 ABB = lerp(a, b, 1. - w);
...
float spikes(vec3 p, float d) {
    d = fOpUnionRound(d, fCone(p, .05, 1.3, icoF1a), .12);
    d = fOpUnionRound(d, fCone(p, .05, 1.7, icoA1), .12);
    d = fOpUnionRound(d, fCone(p, .05, 1.8, icoB1), .12);
    return d;
}
