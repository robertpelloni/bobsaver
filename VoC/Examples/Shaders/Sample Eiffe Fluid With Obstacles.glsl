...
// original https://www.shadertoy.com/view/lllGDl
...
vec2 ReflectOffSurf(vec2 p, vec2 r){
    float d=max(DE(p),0.0);
    vec2 v=vec2(0.001,0.0);
    vec2 N=normalize(vec2(DE(p+v.xy)-DE(p-v.xy),DE(p+v.yx)-DE(p-v.yx)));
    d=clamp(sqrt(d)*1.1,0.0,1.0);
    r=mix(reflect(r,N),r*d,d);
    return r;
}
vec2 field(vec2 uv) {
    vec2 mouse = (mouse.x == 0. && mouse.y==0.) ? vec2(-0.15,-0.1) : mouse.xy/resolution.xy-vec2(0.5);
