							...
							__global int* height)
{
	...
	float time = 0;
    float u, v;
    calc_uv(&u, &v, x, y, width[0], height[0]);
	...
	int4 colint4 = convert_int4(col*255.0f);
	
	uchar4 coluchar4 = convert_uchar4(colint4);
	
	uchar ruchar = coluchar4.x;
	uchar guchar = coluchar4.y;
	uchar buchar = coluchar4.z;
	
	float fr=col.x*255.0f;
	float fg=col.y*255.0f;
	float fb=col.z*255.0f;

    // write pixel data to gpu
    //write_imagef(img, (int2)(x, y), col);
	
	col = (float4)(0.1f, 0.2f, 0.3f, 0.4f);
