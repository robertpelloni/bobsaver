...
// original https://www.shadertoy.com/view/Wt2fDR
...
void main(void)
{
    vec3 viewDir = rayDirection(45.0, resolution.xy);
        if (length(mouse*resolution.xy.xy) > 40.0) {
            //viewDir.yz *= rot(3.14*0.5-mouse.y*resolution.xy.y/resolution.y*3.14);
            viewDir.yz *= rot(3.14*0.5-0.44*3.14);
            //viewDir.xz *= rot(3.14-mouse.x*resolution.xy.x/resolution.x*3.14*2.0);
        }
    
    float speed = 10.0;
    float t1 = time/2.0+10.0;
