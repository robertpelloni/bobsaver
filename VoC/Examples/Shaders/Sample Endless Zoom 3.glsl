...
uniform float time;
uniform vec2 mouse; //сдвиги позиций
...
void main() {   
    ...
    for (int r=0; r<volsteps; r++) {
        ...
        if( r == volsteps-1 )
            fade *= sampleShift;
        v+=vec3(pow(s1,1.),s1*s1,pow(s1,4.))*a*brightness*fade; // coloring based on distance
...
//    backCol2.bg = mix(backCol2.gb, backCol2.bg, 0.5*(cos(time*0.01) + 1.0));    
    glFragColor = forCol2 + vec4(backCol2, 1.0);
    
    glFragColor.gb = vec2(0.0);
    glFragColor.r = sign(-tan(glFragColor.r));
}
