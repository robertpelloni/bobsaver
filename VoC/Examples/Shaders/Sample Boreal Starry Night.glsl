...
// original https://www.shadertoy.com/view/3slXDB
...
vec2 cmtVel = mod(time/modu+modu*.5, 2.) > 1. ? vec2(2., 1.4)*.5 : vec2(-2., 1.4)*.5;  // Speed component X,Y
...
void main(void)
{
...
#ifdef SHADERTOY
    mouseL = mouse*resolution.xy.xy / resolution.xy;
#endif
    ...
    vec2 ratio = resolution.xy / max(resolution.x, resolution.y);;
    vec2 uvs = uv * ratio;
...
////////////////////////////////////
         ...
         glFragColor =  vec4(boreal + sky, 1.) * ground;
}
