...
// original https://www.shadertoy.com/view/lly3W3
...
float udRoundBox(vec3 rp, vec3 b, float r) {
  return length(max(abs(rp) - b, 0.0)) - r;
}
...
float dfHead(vec3 rp) {
    //thin head to fit hat
    return sdEllipsoid(rp, vec3(1.4, 1.5, 1.5));
}

float dfHat(vec3 rp) {
    float muffs = sdCappedCone(rp.yzx, vec3(0.8, 0.8, 1.4));
    rp += vec3(0.0, -0.9, 0.0);
    float hat = udRoundBox(rp, vec3(1.0, 0.4, 1.0), 0.2);
    rp += vec3(0.0, 0.5, -0.4);
    float peak = udRoundBox(rp, vec3(1.0, 0.02, 1.4), 0.1);
    hat = min(hat, peak);
    return smin(hat, muffs, 0.2);   
}
...
float dfScene(vec3 rp) {
    ...
    float legs = dfLegs(rp);
    float feet = dfFeet(rp); 
    return min(min(min(torso, hands), min(head, hat)), min(legs, feet));
}
...
vec3 coatColour(vec3 rp) {
    vec3 coatCol = vec3(0.8, 0.4, 0.0);   
    if (rp.z > 0.0) {
        //seam
        if (rp.x > -0.05 && rp.x < 0.05) {
            coatCol = vec3(0.0);    
        }
        //pockets
        if (rp.y > -2.4 && rp.y < -1.8) {
            if (rp.x > -0.7 && rp.x < -0.3) {
                coatCol = vec3(0.0);    
            }
            if (rp.x > 0.3 && rp.x < 0.7) {
                coatCol = vec3(0.0);    
            }
        }
        if (rp.y > -2.35 && rp.y < -1.85) {
            if (rp.x > -0.65 && rp.x < -0.35) {
                coatCol = vec3(0.8, 0.4, 0.0);    
            }
            if (rp.x > 0.35 && rp.x < 0.65) {
                coatCol = vec3(0.8, 0.4, 0.0);    
            }
        }
...
vec3 faceColour(vec3 rp) {
    ...
    if (rp.z > 0.0) {
        ...
        if (rp.x > -0.3 && rp.x < 0.3) {
            if (rp.y > -0.4 && rp.y < -0.3) {
                 faceCol = vec3(0.0);   
            }
        }

        if (rp.x > -0.2 && rp.x < 0.2) {
            if (rp.y > -0.5 && rp.y < -0.3) {
                 faceCol = vec3(0.0);   
            }
        }
        if (rp.x > -0.15 && rp.x < 0.15) {
            if (rp.y > -0.45 && rp.y < -0.4) {
                 faceCol = vec3(1.0);   
            }
        }
    }
