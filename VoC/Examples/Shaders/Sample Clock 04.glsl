...
// original https://www.shadertoy.com/view/4dfSzn
...
{
    bool isDraw = false;
    float theTime = date.w;
    vec2 uv = gl_FragCoord.xy / resolution.xy;
    // draw the date
    isDraw = isDraw || isInNumber(uv,date.x/1000.0,vec4(0.05,0.575,0.07,0.3),0.015);
    isDraw = isDraw || isInNumber(uv,mod(date.x/100.0,10.0),vec4(0.15,0.575,0.07,0.3),0.015);
    isDraw = isDraw || isInNumber(uv,mod(date.x/10.0,10.0),vec4(0.25,0.575,0.07,0.3),0.015);
    isDraw = isDraw || isInNumber(uv,mod(date.x,10.0),vec4(0.35,0.575,0.07,0.3),0.015);
    isDraw = isDraw || isInRect(uv,vec4(0.455,0.575,0.03,0.06));
    isDraw = isDraw || isInNumber(uv,date.y/10.0,vec4(0.515,0.575,0.07,0.3),0.015);
    isDraw = isDraw || isInNumber(uv,mod(date.y,10.0),vec4(0.615,0.575,0.07,0.3),0.015);
    isDraw = isDraw || isInRect(uv,vec4(0.715,0.575,0.03,0.06));
    isDraw = isDraw || isInNumber(uv,date.z/10.0,vec4(0.78,0.575,0.07,0.3),0.015);
    isDraw = isDraw || isInNumber(uv,mod(date.z,10.0),vec4(0.88,0.575,0.07,0.3),0.015);
    // draw the time
