...
// original https://www.shadertoy.com/view/Xll3Rr
...
float FlyerDf (vec3 p, float dHit)
{
  ...
  if (d < dHit + 0.1) { dHit = SmoothMin (dHit, d, 0.1);  idObj = 2;  qHit = q; }
  q = p;
  q -= fusLen * vec3 (0., 0., 0.9);
  d = PrConeDf (q, fusLen * vec3 (0.04, 0.02, 0.05));
  if (d < dHit) { dHit = d;  idObj = 6;  qHit = q; }
  q = p;
...
vec3 FlyerCol (vec3 n)
{
  ...
  if (idObj >= 3 && idObj <= 5) {
    float s1, s2, s3;
    if (idObj == 3) { s1 = 1.6;  s2 = 6.; s3 = 11.; }
    else if (idObj == 4) { s1 = 1.;  s2 = 1.2; s3 = 4.1; }
    else if (idObj == 5) { s1 = 1.;  s2 = 0.9; s3 = 3.4; }
  ...
  if (idObj == 1) {
    ...
    else if (qHit.z > 0. && n.z > 0.9) col *= 0.1;
    else if (qHit.z < 0. && n.z < -0.9) col = vec3 (1., 0., 0.);
    else {
      qHit.z -= -0.3;
      col = mix (pCol, col,
  ...
  } else if (idObj == 6) col = vec3 (1., 0., 0.);
  idObj = 10;
  return col;
}
...
void main (void)
{
  qHitTransObj=vec3(0.0);
  qHit=vec3(0.0);
  vec2 uv = 2. * gl_FragCoord.xy / resolution.xy - 1.;
  uv.x *= resolution.x / resolution.y;
  trkOffset = vec2 (0.);
  float zmFac = 3.3;
  tCur = 18. * time + 100. * trkOffset.y;
