...
{
    pos.yz *= Rot((pos.z + 2.0)*sin(time*0.3)*0.2);
    pos.xy *= Rot(pos.z*sin(time*0.1)*0.25);
    pos.y -= 0.5 + sin(time*0.5)*0.2; 
...
void main(void)
{
    ...
    //{
    //    cameraOrigin.x = sin(mouse*resolution.xy.x*0.01) * 5.0;
    //    cameraOrigin.y = mouse*resolution.xy.y*0.05 - 10.0;
    //    cameraOrigin.z = cos(mouse*resolution.xy.x*0.01) * 5.0;  
    //}
    //else    
    //{
        cameraOrigin.x = sin(time*0.25 + 2.0) * (6.0 + sin(time * 0.1));
        cameraOrigin.y = sin(time*0.3) - 0.5;
