...
// original https://www.shadertoy.com/view/Ws3Bzf
...
float de(vec3 p) 
{
   ...
   // p2.z+=time;
    ...
    p2.xz*=rot(time*2.+id*10.);
 
    
    float obj1 = obj1(p2,vec3(0.8+sin(id+time*5.)*.5+.5,0.,0.0));
   ...
   //   obj1 = max(obj1, sphere(p2, sin(time)*.02+1.1));
    ...
    if (d == obj1) objcol = vec3(sin(id*5.+time)*.5+.5, sin(id*5.+time)*.1+.1, 0.0);
