...
// original https://www.shadertoy.com/view/lldfWH
...
void main(void)
{
    ...
    vec3 l = normalize(vec3(sin(time),cos(time),0.9));
    
    // roughness modifier using polar coords
    float rough = 0.3 + 0.7 * smoothstep(-0.2,0.2, sin(atan(h.w,h.z)*3.0+h.y*20.0));
    ...
    // glow
    float ga = pow(min(h.x*8.0,1.0), 1.0/8.0);
    ga = 1.0 - (1.0 - ga) * pow(vnoise(uv*2.0+vec2(time,0.0))*1.1, 4.0) *
