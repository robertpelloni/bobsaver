...
// original https://www.shadertoy.com/view/4slGz7
...
void main(void)
{
    ...
    vec3 ttilt = rm * vec3(-(mouse.x - resolution.x / 2.0) / resolution.x,
                      -(mouse.y - resolution.y / 2.0) / resolution.y * 0.5, 0.0);
    ...
    glFragColor.rgb = 1.5 * mix(fog, col, exp(-nearest * nearest * 0.1)) * v;
    
    glFragColor.rgb = mix(glFragColor.rgb, vec3(1.0, 1.0, 0.9) * (clamp(tdd * 0.3, 0.0, 1.0) +
                    torchFlicker() * torchOn() * pow(clamp(tdd - 0.2, 0.0, 1.0) * 2.0, 30.0)), thit);
    
    glFragColor.rgb *= 1.2;
    
    glFragColor.rgb += vec3(N(gl_FragCoord.xy) * 0.005);
    glFragColor.a = 1.0;
}
