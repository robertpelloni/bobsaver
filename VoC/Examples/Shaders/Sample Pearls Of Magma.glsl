...
// original https://www.shadertoy.com/view/lsS3zy
...
void main()
{
    float time2=time+10.63;
    
    mat3 m=rotateXMat(time2*0.5)*rotateYMat(time2*0.7);
    
    vec2 p=gl_FragCoord.xy/resolution.xy*2.0-vec2(1.0);
    ...
    if(t<20.0)
    {
        ...
        col=mix(vec3(0.0),vec3(1.7,0.6,0.2)*5.0*mix(0.6,1.0,(0.5+0.5*cos(time2*30.0))),
    ...
    col+=(0.03+cos(time2*7.0)*0.01)*mix(vec3(0.1,0.5,0.0)*0.3,vec3(1.2,0.7,0.1),0.5+0.5*cos(p.y*0.3+sin(p.x*0.1+time2)*6.0)*sin(p.y));
    
    glFragColor.rgb=sqrt(col);
}
