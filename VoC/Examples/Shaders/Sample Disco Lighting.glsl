...
// original https://www.shadertoy.com/view/lllXWr
...
vec3 Background(in vec3 rd){
    vec3 col=vec3(0.25)+vec3(0.125,0.05,0.125)*(rd+sin(rd*6.0+2.4*sin(rd.yzx*5.3+time)));
...
void main(void)
{
    SPREAD=0.1+sin(time)*0.09,SPIN=-time*0.6;
    ...
    // camera    
    vec3 ro = vec3(cos(time*0.3)*2.0,sin(time*0.2)-0.25,sin(time*0.3)*3.0); //camPath(time);
    vec3 ta = vec3(0.0);
