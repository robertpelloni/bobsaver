...
vec2 stamen( vec3 _p )
{
    vec2 dist = vec2( 1E2 );
    vec2 distC = dist;
    float phase = floor( atan( _p.z, _p.x )/PI/2.*13.-.9 );
    vec3 offset = vec3( .04, -.04, .04 ) * ( 1. + sin( t )*.1 );
    offset.y *= .5 + hash( vec2( phase*19.9127, 123.122 ) )*.5;
    offset.xz *= .7 + hash( vec2( phase*1.9873, 821.122 ) )*.6;
    vec3 p = _p - V.xyx*.01;
