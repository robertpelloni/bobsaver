...
void main()
{
   ...
   for(int i = 0; i < 20; ++i)
   {
      for(int j = 0; j < 20; ++j)
      {
         ...
         if(g.z > 0.0)
         {
            glFragColor.rgb += blob(g, p.xy) * 2.0 * (2.0) * 0.25;
         }
      }
   }

   glFragColor.rgb += blob(transform(vec3(10.0, 0.0, 50.0)), p.xy) * 400.0;
   glFragColor.rgb += blob(transform(vec3(0.0, 5.0, 50.0)), p.xy) * 0.2;
