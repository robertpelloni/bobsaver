// z+c
vec3 TriplexAdditionPhoenixMsltoe(in vec3 z, in vec3 c) {
	vec3 znew;
    znew.x=z.x+c.x;
    znew.y=z.y+c.y;
    znew.z=z.z+c.z;
...
vec3 MsltoeTriplexPowerPhoenixMsltoe(in vec3 z, in float p) {
	vec3 znew;
    radius=pow(radius,p);
    theta=theta*p;
    phi=phi*p;
...
vec3 TriplexMultiplicationPhoenixMsltoe(in vec3 z,in vec3 p) {
     vec3 znew;
...
void Iterate(inout vec3 z,in vec3 c) {
    ...
    // ^p
    znew=MsltoeTriplexPowerPhoenixMsltoe(z,power);
	znew2=TriplexMultiplicationPhoenixMsltoe(c,lastz);
	znew=znew+znew2;
	lastz=z;
	z=znew;
}
