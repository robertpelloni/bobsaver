...
// original https://www.shadertoy.com/view/Xs2SWy
...
{ 
    // y = -m(x-0.5)^2 + 1.0
    float xx = gl_FragCoord.y/resolution.y-0.5;
    xx = -5.000*xx*xx + 1.0;
    ...
    if( m >= 0.0 )
    {
        vec3 view_direction = normalize(-vec3(-0.5, -0.4, 0.5)+ro);
