...
// original https://www.shadertoy.com/view/fdlGDM
...
vec4 Brow(vec2 uv, float smile) {
    ...
    float y = uv.y;
    uv.y += .6*uv.x*mix(.5, .8, smile)-mix(.1, .3, smile) - .6;
    uv.x -= mix(.0, .1, smile)+ .1;
    uv -= .5;
...
vec4 Eye(vec2 uv, float side, vec2 m, float smile) {
    ...
    col.rgb = mix(col.rgb, vec3(.92), highlight);            // blend in highlight
...
vec4 Mouth(vec2 uv, float smile) {
    ...
    uv.y *= 1.5;
    uv.y -= uv.x*uv.x*-2.*smile; //2. -> -2.
    
    uv.x *= mix(2.5, 1.5, smile); //1. -> 1.5
...
vec4 Smiley(vec2 uv, vec2 m, float smile) {
    ...
    if(length(uv)<.5) {                    // only bother about pixels that are actually inside the head
        ...
        if(length(uv-vec2(.185, .325))<.18) {
            vec4 brow = Brow(within(uv, vec4(-.2, .2, .6, .45)), smile);
