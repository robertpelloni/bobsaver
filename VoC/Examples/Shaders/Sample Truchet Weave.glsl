...
// original https://www.shadertoy.com/view/4ltyRn
...
/*
    ...
    results in the spaghetti-like pattern you see. How one sets about randomizing 
    the rendering order depends upon the preferred technique of the individual, 
    but I've explained how I went about it in the code.
    ...
    By the way, there a "NO_WEAVE" option below that is mildy interesting. Plus,
    a "SHOW_SIMPLEX_GRID" option for anyone who requires a rough visual cell tile 
    construction aid.
...
// the weave involves random rendering order, so to take out the weaving effect,
...
vec2 hash22(vec2 p) { 
    ...
    return sin(p*6.283 + time*2.);
    //return abs(fract(p+ time*.5)-.5)*4.-1.; // Snooker.
    //return abs(cos(p*6.283 + time*2.))*2.-1.; // Bounce.
...
float hash21(vec2 p){
    ...
    //return sin(n*6.2831853 + time*.125)*.5 + .5;
    //return abs(fract(n + time*.125) - .5)*2.;
...
vec3 simplexWeave(vec2 p){
    ...
    // Something to that effect, anyway. I could save three hash below by using the following,
    // but it's a relatively cheap example, and I wanted to keep the noise seperate. By the way,
    // note that if you're after a cheap simplex noise value, the calculations don't have to
    // be particularly long. From here to the top, there's only a few lines, and the quality
    // is good enough.
    ...
    // The weave pattern itself.
    
    // Three random values -- taken at each of the triangle vertices, and ranging between
    // zero and one.
    ...
    // The torus rings. 
    // Toroidal axis width. Basicall, the weave pattern width.
    float tw = .25;
    ...
    float mid = length((p2 - p))*.5;
    // The three distance field functions: Stored in c.x, c.y and c.z.
    // Equivalent to: vec3 c =  cir - mid - tw; c = max(c, -(cir - mid + tw));
    vec3 cir = vec3(length(p), length(p1), length(p2));
    ...
    // I'm not sure how fond I am of this hacky logic, but it's easy enough to follow,
    ...
    if(h.x<h.y && h.x<h.z){ // X vertex is lowest.
        ...
        // Forgetting to do this, which I often do, sets me for a lot of debugging. :)
    ...
    // Rendering: Applying the layered distance field objects.
    ...
    // Applying some contrasty noise for a fake shadowy lighting effect.
    rimCol *= (smoothstep(0., .75, noise - .1) + .5);
    
    vec3 torCol = vec3(.2, .4, 1);
    a = clamp(cos(a*48. + time*0.)*1. + .5, 0., 1.)*.25 + .75;
...
void main(void) {
    ...
    vec2 uv = gl_FragCoord.xy/min(750., resolution.y);
   
    // Position with some scrolling.
    vec2 p = uv + vec2(.8660254, .5)*time/16.; 
