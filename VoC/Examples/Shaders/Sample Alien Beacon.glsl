...
// original https://www.shadertoy.com/view/ld2SzK
...
/*    if (time == 0.0)    // for debugging with manual camera
    {
        camPos = cXX.xyz;
    ...
    // debugging camera
    float mx=mouse.x/resolution.x*PI*2.0;// + iGlobalTime * 0.1;
    float my=-mouse.y/resolution.y*10.0;// + sin(iGlobalTime * 0.3)*0.2+0.2;//*PI/2.01;
    camPos += vec3(cos(my)*cos(mx),sin(my),cos(my)*sin(mx))*(5.2);     // prp

    // set time for moving camera along path
    float timeLine = time*0.2 + camPathOffset;
    ...
    vec3 worldPix = worldFacing + uv.x * sideNorm * (resolution.x/resolution.y) + uv.y * upNorm;//scrCoord
...
#ifdef MOVING_SUN
    vec3 sunDir = normalize(vec3(sin(time*0.047-1.5), cos(time*0.047-1.5), -0.5));
#else
    ...
    //if (t <= maxDepth)
    {
        ...
        vec3 ballGlow = vec3(0.1, 0.97, 0.1) * abs(SpiralNoise3D(vec3(time*1.3)));
