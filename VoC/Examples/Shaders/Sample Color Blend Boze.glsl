...
// original https://www.shadertoy.com/view/3tXyzl
...
surface map(vec3 p)
{
    ...
    for(int i = 0; i < int(num); i++){
        ...
        float len = sin(r * 1.85 + time * 3.) * 0.5 + 0.75;
        ...
        surface l = surface(sdCapsule(p, vec3(0,0,z), vec3(cos(r) * len, sin(r) * len, z), cos(r * 1.85 + time * 3.)*0.01 + 0.02), col, vec3(0), 0.1, 0.0, 0);
