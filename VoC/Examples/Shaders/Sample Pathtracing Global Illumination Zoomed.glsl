...
const vec3 mengerColor=vec3(0.95,0.95,0.95);
const float mengerReflect=0.5;

const vec3 mandelbulbColor=vec3(1.0,0.7,0.7);
const float mandelbulbReflect=0.0; //negative reflection means refract
const int mandelbulbIterations=7;
...
float DEBulb(vec3 z0)
{
    ...
    vec3 c=z0*4.0;
    vec3 z=z0*4.0;
    float dr = 4.0,r = length(z),zr,zo,zi,p=mandelbulbPower;
    for (int n = 0; n < mandelbulbIterations && r<2.0; n++) {
        zo = acos(z.z / r) * p;
        zi = atan(z.y, z.x) * p;
        zr = pow(r, p-1.0);
        dr = dr * zr * p + 1.0;
...
{//march the ray until you hit something or go out of bounds
    ...
    for(int i=0;i<MaxRaySteps && t<maxDepth;i++){
        h=map(ro+t*rd);
        t+=h.x*side;
        if(abs(h.x)<epsilon)return vec2(t,h.y);
...
void main(){
    ...
    vec3 pos= dir*-0.5+vec3(0.0,0.01,0.0);
