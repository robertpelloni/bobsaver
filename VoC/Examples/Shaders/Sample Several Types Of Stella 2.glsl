...
// original https://www.shadertoy.com/view/sdlBWS
...
float modifiedAbs(float x, float k){
    float h=tanh(x*k);
    return h*h*abs(x);    
}

vec2 modifiedAbs(vec2 x, float g){
    vec2 k= vec2(g);
    vec2 h=tanh(x*k);
    return h*h*abs(x);    
}

vec3 modifiedAbs(vec3 x, float g){
    vec3 k= vec3(g);
    vec3 h=tanh(x*k);
    return h*h*abs(x);    
}
...
void sfold(inout vec3 p, float n, float k) 
{
    ...
    for(int i=0;i<5;i++)
    {
        p.xy=modifiedAbs(p.xy,k);
        float g=dot(p,v);
        p-=(g-modifiedAbs(g,k))*v;
    }
}
