...
// original https://www.shadertoy.com/view/llXGD2
...
float scene (vec3 pos) {
    vec3 old = pos;
    pos.y += sin(1.0*pos.x/6.0 * 6.283 + time) * 0.125;
    pos.y += sin(1.0*pos.z/6.0 * 6.283 + time) * 0.25;
    pos.x += sin(2.0*pos.y/6.0 * 6.283 + time) * 0.125;
...
void main(void)
{
    ...
    vec3 ro = vec3(uv, time*4.0);
    vec3 rd = normalize(vec3(uv, 1.0)+vec3(sin(time*0.5)*0.5, cos(time*0.5)*0.25, 0.0));
    ...
    glFragColor = vec4(pow(color, vec3(1.0/1.8)), 1.0);
}
