...
// original https://www.shadertoy.com/view/fsySWm
...
vec2 blocks(vec3 p, vec3 scl, vec3 rd){
    ...
    if(any(isEdge)) {
        ...
        float expand = 1.05+0.45*(sin(length(center+100.0)*0.35+t*3.0)*0.5+0.5);
        //expand = 1.5; 
        if(isEdge.x){
...
void main(void)
{
    ...
    //ro.yz*=rot(2.0*(mouse*resolution.xy.y/resolution.y-0.5));
    ...
    if(hit){
        ...
        vec3 r = reflect(rd,n);
        vec3 e = vec3(0.5);
        vec3 al = pal(fract(d.y+t*0.8)*0.6-0.2,e*1.5,e,e*2.0,vec3(0,0.33,0.66));
        col = al;
...
void main(void)
{
    ...
    if(AA==1.0) {render(col,gl_FragCoord.xy); glFragColor = col; return;}
