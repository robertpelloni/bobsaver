...
// original https://www.shadertoy.com/view/XldcRl
...
float reboundValue( float time, float limit )
{
    float doubleValue = mod(time, limit*2.0);
    if(doubleValue>limit)
        return limit-(doubleValue-limit);
    return doubleValue;
}
...
float distScene (in vec3 p, out vec3 color, out vec3 material) {
    ...
    float armsPeriodtime = (speedPI2*0.275)-1.15+((mirrorX+1.0)*0.5*armsPeriod);
    float armRotSegmentA = reboundValue(armsPeriodtime,armsPeriod)-1.0;
    ...
    float armRotSegmentB0 = reboundValue(armsPeriodtime,armsPeriod)-0.7;
    ...
    float armRotSegmentB1 = reboundValue(armsPeriodtime,armsPeriod)-1.1;
    ...
    float armRotSegmentB2 = reboundValue(armsPeriodtime,armsPeriod)-0.1;
    ...
    float legsPeriodtime = ((speedPI2*0.375)-1.55)+((-mirrorX+1.0)*0.5*legsPeriod);
    float legRotSegmentA = reboundValue(legsPeriodtime,legsPeriod)-1.4;
    ...
    float legRotSegmentB1 = reboundValue(legsPeriodtime*0.85-1.1,legsPeriod*0.85);
...
void main(void) {
    ...
    float yawAngle = PI * (1.2) + clamp(reboundValue( time, PI2*1.5 ), PI, PI2);
    ...
    vec3 origin = (clamp(reboundValue( time, PI2*1.5 ), 0.0, PI)*10.0+(vec3 (0.0, 10.0, 0.0)) - cameraOrientation [2] * cameraDist);
