...
// original https://www.shadertoy.com/view/7d2yRD
...
float circle( vec2 uv, vec2 p, float r, float blur){
    ...
    uv.x *= resolution.x/resolution.y;//校正屏幕比例
    float d = length(uv-p);
...
pa.x*= resolution.x/resolution.y;
ba.x*= resolution.x/resolution.y;
...
uv -= o;
uv.x = abs(uv.x*resolution.x/resolution.y)-a;
