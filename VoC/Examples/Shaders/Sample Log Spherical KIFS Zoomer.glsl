...
// original https://www.shadertoy.com/view/ctcGRf
...
{    
    ...
    float sphere = length(p - vec3(1.8 + sin(time/3. + depth)*.6, 0, 0)) - .1; 
    ...
    float menger = sdKMC(p*2.9, 8, vec3(sin(time/53.))*.4, vec3(sin3(time/64.)*PI), vec4(2., 3.5, 4.5, 5.5)) / 2.9;
...
vec2 Map(in vec3 p) //Thanks dracusa, nice aticle <3 
{
    ...
    float t = time/10.;// + mouse*resolution.xy.x/resolution.x*3.;
    p.x -= t;
...
{
    float col = 0.;
    float dO = mix(MIN_DIST, MAX_DIST/2., S(.9, 1., sin(time/24.)*.5+.5));
    int steps = 0;
    ...
    for(int i = 0; i < MAX_STEPS; i++) 
    {
        ...
        if (dO > MAX_DIST || dS.x < dO / resolution.y) break;
    }
...
vec3 Palette(int index)
{
    switch (index)
    {
        ...
        case 3: return hsv2rgb_smooth(vec3(fract(time/21.), .65, .8));
    }
    return vec3(0.);
}
...
void main(void)
{
    ...
    col += pow(rmd.y / float(MAX_STEPS), 2.5) * normalize(ambCol) * 
            (GLOW_INT + (rmd.x < MAX_DIST ? 3.*S(.995, 1., sin(time/2. - length(p)/20.)) : 0.)); //glow wave
    
    glFragColor = PP(col, uv);
}