...
// original https://www.shadertoy.com/view/XttBWX
...
vec3 swing(vec3 p)
{
    p.xy*=rot(0.15*sin(1.5*time));

    p+=vec3(0.5*sin(1.5*time),0.,-0.3+2.*time);
    return p;
}
...
vec3 material_doraemon(vec3 rd, vec3 pos, vec3 nor)
{
    ...
    else if(obj_id==LIGHT)
    {
        ...
        float k=dot(sin(flp+cos(flp.yx*2.+time*2.)),vec2(.5));
...
void main(void)
{
    ...
    pixel_size=1.0/(resolution.y);
   
     // debugging camera
    //float x_rot=-mouse*resolution.xy.x/resolution.x*PI*2.0;
    //float y_rot=mouse*resolution.xy.y/resolution.y*3.14*0.5 + PI/2.0;
    //vec3 ro=vec3(0.,0.9,-2.*time)+vec3(cos(y_rot)*cos(x_rot),0.,cos(y_rot)*sin(x_rot))*10.;
    //vec3 ta=vec3(0.,0.,-2.*time);
   
    float time=mod(time,40.);
    vec3 ro,ro2;
    if(time < 20.)
    {
        ro=vec3(0.,.9,-9.);
    ...
    vec3 timetunnel=tunnel(rd,-time*8.,.05);
    vec3 menger=render_city(ro2,rd, 1.0-step(0.0,sin(0.05*(time+40.-14.)))).xyz;
    
    vec3 bg=mix(timetunnel,menger,1.-smoothstep(0.,1.,sin(0.1*(time+40.))));
