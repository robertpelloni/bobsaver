...
// original https://www.shadertoy.com/view/Wls3DB
 ...
 */
vec2 mp (vec3 p) {
    p.z = mod(p.z+time*12.0, 20.0)-8.0;
    ...
    for (float i = 0.0; i < 6.0; i++) {
        ...
        np.xy*= r2(0.4+s);
        np.zy*= r2(0.1+s*abs(cos(time/6.0)));
        np.xz*= r2(0.4*s*cos(time/10.0));
    }
...
void main(void)
{
    ...
    float bassLevel = 0.0;//(texture(iChannel0, vec2(0.0, 0.0)).r+texture(iChannel0, vec2(0.2, 0.0)).r)/2.0;
    ...
    col = fo = vec3(0.4)*cos(time/8.0) + vec3(0.1) * (1.0 - length(uv)*1.5 - 0.1);
    ...
    if (d > 0.0) {
           vec3 po = ro + rd*d,
             ...
             al = mix(vec3(1.0, 1.0,1.0), vec3(1.0, 0.0, 0.32),abs(cos(time/5.0)));
        
        if (sc.y < 5.0) {
            al = mix(vec3(1.0, 1.0,1.0), vec3(1.0, 0.0, 0.8),abs(cos(time/5.0)));
        }
