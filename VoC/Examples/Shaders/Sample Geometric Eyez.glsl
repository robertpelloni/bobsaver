...
// original https://www.shadertoy.com/view/Xd2BDm
...
void main(void)
{
    ...
    viewportMagnify = 1.; 
    //uv *= mat2(cos(time/6.+vec4(0.,1.6,-1.6,0.))) * exp(-time/8.);
    uv *= viewportMagnify;
    ...
    #if SHOW_SEGMENTS==1
        #define OPERATION1(x,y) glFragColor.rgb = mixColorLine(uv,glFragColor.rgb,EquerreColor,x,y,scale);
    ...
    vec2 TriCenterMix = (Tri.A + Tri.B + Tri.C)/3.;
        ...
        glFragColor.rgb *= 3.*(.5 + coeffDistPoint(uv,TriCenterMix,scale));
    #endif
    
    
    glFragColor.rgb = tanh(glFragColor.rgb*2./(1. + dot(uv,uv)/1e3 ) ); // LOG correction
}
