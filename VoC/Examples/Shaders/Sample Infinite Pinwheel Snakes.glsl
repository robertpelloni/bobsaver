...
// original https://www.shadertoy.com/view/4sSBRy
...
vec2 screenToViewport(vec2 uv)
{
    ...
    return (uv - resolution.xy/2.0 ) / resolution.x * viewportMagnify;
}
...
vec3 colorTriangle(vec2 uv_s,Pinwheel Tri,int i)
{
   ...
   // TriCenterMix = Tri.B;
    ...
    float k2 = (
            cos( (k - time*.1*(mod(Tri.ID,2.)*2.-1.) )*PI*5. )
        );
    col.rgb = col.gbr *
        smoothstep( 1. , 0. , tanh(abs(k2*3.)+.4) )
        * ( cos(
             (k - + Tri.ID2*iterK ) * vec3(7.,5.,3.) - time
           )*.5+.5 );
...
vec3 color(vec2 uv_s)
{
    ...
    float q;
    qLimit = .03;
    //qLimit = ( 1. / mix( 1. , 1./.14  , mouse*resolution.xy.x/resolution.x ) )/5.*viewportMagnify;
...
void main(void)
{
    ...
    glFragColor.rgb = tanh(glFragColor.rgb * 2e2 );
}
