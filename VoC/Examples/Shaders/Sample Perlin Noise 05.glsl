...
float noise(vec2 p, float freq ){
    float unit = (resolution.x / resolution.y)/freq;
...
void main( void ) {
    ...
    vec2 midp = vec2(aspectRatio, 1.0);
    
    float noise = pNoise(position * 2.0 + time * 0.1, 10);
    vec2 p2 = position;
    p2.x += noise;
    p2.y += noise;
    float noise2 = noise + pNoise(p2 * 2.0, 10);
    float noise3 = noise2 + pNoise(p2 * 4.0, 10);
    float noise4 = noise3 + pNoise(p2 * 8.0, 10);
    ...
    //vec3 color = vec3(noise4, noise4, noise4);
