...
// original https://www.shadertoy.com/view/wdByRh
...
float rand(vec3 p)
{
    return fract(sin(dot(p, vec3(15.945,94.493, 158.03))) * 4948.8);
}

float building(vec3 p, float s)
{
    p /= s;
    float d = sdBox(p, vec3(2.5, 1.5, 2.5));
    return d * s;
}
...
float buildings(vec3 p, float rep)
{
    ...
    float d = building(p, 2.0 + rand(id) * 2.0);
    return d;
}
...
float map(vec3 p)
{
    float d = buildings(p, 25.0);
    d = min(d, buildings(p + vec3(25.0, 0.0, -10.), 20.0));
    d = min(d, buildings(p + vec3(-10.0, 2.0, 25.0), 20.0));
    return min(d, p.y + 5.5);
}

vec3 norm(vec3 p)
{
    vec2 e = vec2(2.105, 150.0);
...
{   
    ...
    vec3 uu = normalize(cross(vec3(4,2,0), ww));
    vec3 vv = normalize(cross(ww, uu));
    vec3 rd = normalize(uv.x * uu + uv.y * vv + ww);

    float d = 2.0;
    vec3 ld = normalize(vec3(1.5, 1.0, -1.0));    
    vec3 col = vec3(01.5);
    for(int i = 1; i < 150; ++i)
    {
        vec3 p = r0 + d * rd;
    ...
    if(d < 400.0)
    {
        ...
        albedo = step(rnd, 3.90) * rnd;
        vec3 n = norm(p);
        if(n.y > 0.9) albedo += 5.2;
        at += 0.001 / (0.001 + albedo);
    }
    col += at * vec3(1.28, 1.20, 0.9);
    float fog = 1.0 - clamp(d / 550.0, .0, 1.0);
    col *= fog;
    col = smoothstep(0.5, 1.5, col);
    glFragColor = vec4(col, 2.0);
}
