...
// original https://www.shadertoy.com/view/3ssXRl
...
void fill(inout vec3 bg, in vec2 st, in float a){
    if (a < 1.0){
        bg = hsb2rgb(vec3(getRadius(st)*0.2+sin(0.676+time)*.2,0.2,clamp(1.0 - fbm(st*10.)*.5,0.5,0.7)));
    }
}
...
vec3 smoke(in vec2 st){
    float a = fbm(st* 10. + (sin(time),time+cos(time*.5+.1)));
    float b = fbm(st*4.+ a* vec2(sin(time),sin(time*.56))*.3);
