...
// original https://www.shadertoy.com/view/NtlSDs
...
// Created by inigo quilez - iq/2008
...
// "paint maths" (as opposed to doing "mathematical art").

// Also, this is the shader where I first used the now
// popular SDF based ambient occlusion estimation. You'll
// also find my first attemtps at smooth-minumum (used to
// blend the creature's head and tentacles together), and
// soft shadows (still not as good as the ones we use now).
// Otherwise, you'll find most of the familiar raymarching
// and SDF stuff - domain repetition, Value Noise, box SDFs
// and of course the usual raymarching machinery.
...
float fbm( vec3 pos )
{
    return 0.5000*noise( pos*1.0 ) + 
           0.2500*noise( pos*2.0 ) + 
           0.1250*noise( pos*4.0 ) +
           0.0625*noise( pos*8.0 );
}
...
#define NUMBALDOSAS 10
vec4 baldosas[NUMBALDOSAS];
...
void init_data( void  )
{
    ...
    for( int i=0; i<NUMBALDOSAS; i++ )
    {
        ...
        baldosas[i].x = cos(a);
        baldosas[i].y = sin(a);
        baldosas[i].z = 0.5 + 1.5*sfrand(kkr);
        baldosas[i].w = 1.0 + 1.0*sfrand(kkr);
    }
}
...
float map( vec3 pos, out uint sid, out uint submat )
{
    ...
    {
    for( int i=ZERO; i<NUMBALDOSAS; i++ )
    {
        float co = baldosas[i].x;
        float si = baldosas[i].y;
        float xc = pos.x - baldosas[i].z;
        float zc = pos.z - baldosas[i].w;
...
vec3 shade( vec3 pos, vec3 rd, uint matID, uint subMatID, float dis )
{
    ...
    if( matID==0u )
    {
        ...
        float fb = fbm( 16.0*vec3(pos[0]+xoff, pos[1], pos[2]) );
        ...
        float m = sin( 64.0*pos[2]*fx +  64.0*pos[0] + 4.0*fb );
    ...
    else if( matID==1u )
    {
        ...
        float m = sin( 64.0*pos[2] +  64.0*pos[0] + 4.0*fb );
    ...
    for( int aoi=ZERO; aoi<5; aoi++ )
    {
        ...
        totao += ao*sca;
        sca *= 0.5;
        //if( totao>1000.0+sin(time) ) break;
    }
...
void main(void)
{
    init_data();
#if AA<2    
    vec3 col = computePixel(gl_FragCoord.xy, resolution.xy, time );
#else
    vec3 col = vec3(0.0);
    for( int j=0; j<AA; j++ )
    for( int i=0; i<AA; i++ )
    {
        ...
        float time = time + (0.5/30.0)*float(j*AA+i)/float(AA*AA);
        col += computePixel(gl_FragCoord.xy+of, resolution.xy, time );
    }
    col /= float(AA*AA);
#endif
