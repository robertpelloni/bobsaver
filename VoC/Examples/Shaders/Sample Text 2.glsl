...
// original https://www.shadertoy.com/view/Mt2GWD
...
float print_number(float number, vec2 uv)
{
    ...
    for(int i = 3;i >= -1;i--)
    {
        ...
        {
            result += character(ch_per,uv);
        }
        
        if(abs(number) > pow(10.0, float(i)) || i == 0) //Clip off leading zeros.
        {
            result += character(get_digit(digit),uv);
        }
    }   
    return result;
}
...
float text(vec2 uv)
{
    ...
    float hour = floor(date.w/60.0/60.0);
    float minute = floor(mod(date.w/60.0,60.0));
    float second = floor(mod(date.w,60.0));
    ...
    TEXT_MODE = NORMAL;
    col += character(ch_col,uv);
    
    TEXT_MODE = UNDERLINE;
    col += print_integer(date.z,2,uv);
    col += character(ch_lsl,uv);
    col += print_integer(date.y+1.0,2,uv);
    col += character(ch_lsl,uv);
    col += print_integer(date.x,4,uv);
    ...
    TEXT_MODE = UNDERLINE;
    col += print_integer(resolution.x,0,uv); 
    col += character(ch_x,uv);
    col += print_integer(resolution.y,0,uv); 
