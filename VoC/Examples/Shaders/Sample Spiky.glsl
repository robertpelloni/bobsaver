...
// original https://www.shadertoy.com/view/MssXz4
...
void CamPolar( out vec3 pos, out vec3 ray, in vec3 origin, in vec2 rotation, in float distance, in float zoom )
{
    ...
    // ray in view space
    ray.xy = gl_FragCoord.xy - resolution.xy*.5;
...
float Spiky( vec3 p )
{
    ...
    vec4 q = QuatFromAxisAngle( normalize( vec3(cos(time*0.7),1,0) ), time*1.5 - 0.35*length(p) );
...
void main(void)
{
    vec2 camRot = vec2(0.4,2.6)+vec2(-2.5,8.5)*(mouse.yx/resolution.yx) ;
    camRot.x = max( camRot.x, 0. );
    vec3 vpos, vdir;
    vec3 targetPos = vec3(0.,0.,0.);
    CamPolar( vpos, vdir, targetPos, camRot, 25.0 + 5.*cos(time*0.1), 1.5 );
...
//        vec3 nor = ClassifyDir( pos );
        ...
        float l0Sh = 1. ;
        if( l0i >0.02 )
            l0Sh = softshadow( pos, -lightDir, 5., 40., 3. );
        vec3 l0 = l0i * l0Sh * lightCol; 
        ...
        // fog
        vec3 fogCol = vec3(1.,1.,1.);
        float lfog = length(pos-vpos) - 20. ; // start the fog a much further from the camera
        float fi = exp( -0.04*lfog );
        

        // choose the material floor or spiky        
//        if( pos.y < PlaneY+1. || lfog > 20. )
//        {
            // floor case !
//            col = mix( vec3(0.85), 0.5*sssCol, 1.-l0Sh ) ;
//        }
//        else
        {
            col = mix( l0+l1, l2, ao ) ;    
