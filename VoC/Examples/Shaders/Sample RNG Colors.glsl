...
// original https://www.shadertoy.com/view/MdyyRm
...
vec4 cash4(int x, int y, float seed) {
    ...
    vec4 res = mix_custom(a, b, fract(seed));

    vec4 hsluv_a = rgbToHsluv(res);
    vec4 hsluv_b = rgbToHsluv(res);
    vec4 hsluv_res = rgbToHsluv(res);
    hsluv_res.y = mix_custom(hsluv_a.y, hsluv_b.y, fract(seed));
    hsluv_res.z = mix_custom(hsluv_a.z, hsluv_b.z, fract(seed));
...
void main(void)
{
    ...
    vec4 rand = vec4(cash4(pos.x, pos.y, time));
    vec4 rgb = vec4(hsluv_fromLinear(rand.xyz), rand.w);
    glFragColor = rgb;
    
    /*glFragColor = toGamma(hsluvToRgb(rand.x * 360.0,
                           100.0,
                           50.0,
                           1.0));//;
*/}
