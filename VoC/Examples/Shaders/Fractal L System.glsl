...
void main( void ) { 
    ...
    vec3 color_increment =vec3(0.005,0.008,0.);
    vec2 pos1 = vec2(x1,y1);
    vec2 pos2 = vec2(x2,y2);
    vec2 c = vec2(0.5);
    glFragColor.xyz = vec3(line_segment(uv, pos1, d1, pos2, d2));
    ...
    glFragColor.rgb = max( glFragColor.xyz, max(stem_feedback, max(left_arm_feedback, right_arm_feedback)));
    
    glFragColor.rgb = mix( glFragColor.xyz, texture2D(backbuffer, uv).xyz, vec3(0.125)); // sort of a motion blur
