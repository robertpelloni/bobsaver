...
// original https://www.shadertoy.com/view/lsGcRD
...
void main(void)
{
  ...
  vec2 mp = mouse*resolution.xy.xy / R;
  ...
  int MODE = int(3.*mp.x);
  if (MODE==0) glFragColor = vec4(.2*c)+smoothstep(.05,.0,abs(c-5.*my))*vec4(1,0,0,0);
  if (MODE==1) glFragColor = vec4(.5+.5*sin(2.*c));
  if (MODE==2) glFragColor = vec4(sin(c),sin(c/2.),sin(c/4.),1.);
}
