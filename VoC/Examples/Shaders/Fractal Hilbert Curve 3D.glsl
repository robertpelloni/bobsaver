...
// original https://www.shadertoy.com/view/MljyRd
...
float sdf_inner(vec3 p){
    ...
    //float ang=sin(p.z+time*1.3)*sin(time*3.);
    float ang=p.z*2.;
    float thickness=pow(max(0.,sin(p.z*3.+time*3.)-.95)*20.,.4)*.03+.02;
    //float thickness=step(.93,sin(p.z*3.+time*3.))*.03+.02;
    //float thickness=.04;
...
void main(void)
{
    ...
    vec2 mouse=vec2(0.0);//(mouse*resolution.xy.xy*2.-resolution.xy)/resolution.x;
    vec2 click=vec2(0.0);//(mouse*resolution.xy.zw*2.-resolution.xy)/resolution.x;
    
    vec2 pan;
    //if(mouse*resolution.xy.z<=0.){
        //pan=vec2(time/3., sin(time/2.)*.4); // inside view
        pan=vec2(time/2, sin(time/4.)*.4); // outside view
    //}else{
    //    pan=mouse-click;
    //    pan.x*=6.;
    //    pan.y*=1.5;
    ...
    for(int i=0;i<STEPS;++i){
        ...
        pos+=dir*d;
        if(abs(d)<STOPDIST){
            glFragColor=vec4(vec3(sqrt(float(i)/float(STEPS)))*color,1.);
            return;
        }
        if(length(pos)>1e3){
            glFragColor=vec4(vec3(sqrt(float(i)/float(STEPS)))*vec3(1.,.5,.7),1.);
            return;
        }
    }
    glFragColor = vec4(.2,.5,.7, 1.0);
}
