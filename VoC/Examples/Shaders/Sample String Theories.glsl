...
// original https://www.shadertoy.com/view/7ds3DX
...
vec3 map( vec3 p )
{
    ...
    p.xz += 0.5*sincos(1.0+0.15*time+(p.y+11.0*ph)*1.108);

    vec3 p1 = p; p1.xz += 0.15*sincos(1.0*p.y-4.0*time+1.0);
    vec3 p2 = p; p2.xz += 0.15*sincos(1.0*p.y-1.0*time+2.0);
    vec3 p3 = p; p3.xz += 0.15*sincos(1.0*p.y-1.0*time+4.0);
    ...
    return opU( opU( vec3(h1.x-0.012,                                         ph + 4.0/3.0, h1.y), 
                     vec3(h2.x-0.012-0.05*cos(300.0*h2.y - time*4.0), ph + 5.0/3.0, h2.y) ), 
                     vec3(h3.x-0.012-0.02*cos(2000.0*h3.y - time*4.0), ph + 2.0/3.0, h3.y) );
}
...
{
    float px = 0.0001;//(2.0/resolution.y)*(1.0/3.0);
    float eps = px*t;
...
{    
    ...
    float px = (2.0/resolution.y)*(1.0/fl);
