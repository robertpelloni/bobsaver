...
// original https://www.shadertoy.com/view/4tjSR1
...
{
    gbase = vec2(1, 0);
    pixpos = Coord.xy / resolution.yy;
    pixpos -= vec2(1.0, 0.2);
    pixpos = supershapes(vec4(length(pixpos)*10.0, 1, 1, 1), atan(pixpos.y, pixpos.x)) + (Coord.xy / resolution.yy) * 1.0;
    pixpos = pixpos * (9.0 + sin(time)*4.0) - vec2(0.5, 0.5);
...
{
    gbase = vec2(1, 0);
    pixpos = (gl_FragCoord.xy / resolution.yy) * 1.5 + vec2(0.0, -0.3);
    pixpos -= vec2(sin(time*0.5)*3.0, 0.0);
    glFragColor = G() + o() + o2() + g() + l() + e() + bg(gl_FragCoord.xy);
}
