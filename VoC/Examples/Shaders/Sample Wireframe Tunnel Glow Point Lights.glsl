...
// original https://www.shadertoy.com/view/3s2Xzc
...
float scene(vec3 p) {
  ...
  vec3 _pt = repeat(
    (vec4(p, 1.)
       * translate(vec3(0., 0., mod(time, 1.5)))
       * rotateZ(time / 8.)
    ).xyz,
    1.5
  );
...
void main(void) {
  ...
  // camera settings
  vec3 lookAt = vec3(cos(time * .4) * .2, sin(time * .6) * .2, 0.);
