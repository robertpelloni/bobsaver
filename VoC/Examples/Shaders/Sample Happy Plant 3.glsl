...
// original https://www.shadertoy.com/view/4tdyRj
...
float distScene(in vec3 pos, out int object, out float colorVariation){
    ...
    pSpike.xz = rot(0.4 + 0.075 * sin(5. * time)) * pSpike.xz;
    pSpike.xy = rot(0.4 + 0.075 * sin(5. * time)) * pSpike.xy;
    ...
    pLayer.xy = rot(0.99 - 0.01 * sin(5. * time)) * pLayer.xy;
    ...
    pLayer.xz = repeat(pLayer.xz, np);
    pLayer.xy = rot(0.7 - 0.01 * sin(5. * time)) * pLayer.xy;
    pLayer.y = abs(pLayer.y);
    pLayer.z = smoothabs(pLayer.z, 0.01);
    radius = 0.09;
