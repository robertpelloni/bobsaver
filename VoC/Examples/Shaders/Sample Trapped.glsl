...
void main( void ) {  
    ...
    const float step = 0.0065; //segmentation
    const float loop = 800.0; //Longueur
    const float timeSCale = 1.75; // Vitesse
    ...
    for(float i=1.0;i<loop;i++){        
        ...
        blueGodColor += vec3(componentColor, componentColor/2.0, componentColor/4.0);
    }
