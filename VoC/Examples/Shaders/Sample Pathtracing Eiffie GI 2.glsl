...
uniform float bulbRefrRefl=0.0;
...
uniform float HitDistance=0.00001;
...
{//return distance estimate and object id 
    ...
    vec2 flr=vec2(z.x+1.0,2.0);//add as many ids as you like
    ...
    */
    ...
    return vec2(DEBulb(z-bulbPosition,8.0,1.0,5),1.0);

    /*
    vec2 flr=vec2(z.y+1.0,0.0);//add as many ids as you like
    vec2 box=vec2(DEBulb(z-bulbPosition,8.0,1.0,4),1.0);
    vec2 lit=vec2((UseEmissive)?mapL(z):1000.0,-2.0);//the id -2 is for emissive light
    minLiteDist=min(minLiteDist,lit.x);//save the closest distance to the light for bloom
    return min2(min2(box,lit),flr);//add as many objects as you like this way
    */
}
