...
// Attempt at optimizing the original shader to run on lower hardware...

// Perlin noise implementation by Omar El Sayyed.
//
// For more information, watch this awesome presentation by Ken Perlin: http://noisemachine.com/talk1/
//
...
float perlinNoise3D(int gridWidth, int gridHeight, int gridDepth, vec3 position) {
    ...
    vec3 hermit = smoothstep(0.0, 1.0, interpolationRatio);
    vec3 oneMinusHermit = smoothstep(0.0, 1.0, 1.0-interpolationRatio);

    vec4 temp = vec4(hermit.xy, oneMinusHermit.xy);
    vec4 hermits = temp.xzxz * temp.yyww;
        
    finalNoise  = dot(hermits, vec4(gradientValues[7], gradientValues[6], gradientValues[5], gradientValues[4])) * hermit.z;
    finalNoise += dot(hermits, vec4(gradientValues[3], gradientValues[2], gradientValues[1], gradientValues[0])) * oneMinusHermit.z;
...
const vec3 eyePosition = vec3(0.0, 0.0,-9.0);
...
void main() {
  ...
  if (t == -1.0) {
    if (timeInterval > 15.0) {
      ...
      // A bit of blur,
      glFragColor = getSumNeighbors() * 0.235;
    }
    return ;
  }
