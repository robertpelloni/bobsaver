...
float wave(float x, int i){
    float i_f=float(i);
    float fy = (3.3-0.5*i_f)*sin(x*2.*time*i_f);
    return fy * (0.4+0.3*cos(x*x));
}
...
void main(void)
{
    ...
    vec3 col = vec3(0.0);
    for(int i = 0; i<N; ++i){
        float i_f = float(i)*0.8;
        float y = d2y2(distance(4.*uv.y, wave(uv.x, i)),i_f);
        col += 0.8*y *hsv2rgb(vec3(0.15*time+i_f*0.1, 0.6,1.0));
        
    }
    
    glFragColor = vec4(vec3(yf)+col, 1.0);
}
