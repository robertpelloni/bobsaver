...
// original https://www.shadertoy.com/view/3llGzH
...
vec3 eye(vec2 fst, vec2 cst, vec2 mouse){
    float mouseDown = 0.0; //clamp(mouse*resolution.xy.z, 0.0, 1.0);
    float noise = rand(cst);
    
    float nt = time*2.0 * (noise + 0.8 ) +noise * 100.0;
    float fnt = floor(nt);
    ...
    float eyeOpen = (sin(time*2.0 + noise * 100.0) + 1.0) / 2.0;
...
void main(void) {
    ...
    col = max(col,col2);
    col += 0.1 * (rand(gl_FragCoord.xy/3.0 + time)-0.5);
    //col -= 0.1 * (rand(gl_FragCoord.xy/3.0 + time + 100.0));
