...
// original https://www.shadertoy.com/view/tdjczK
...
vec3 withMouse(vec3 p) {
        vec2 mouse = vec2(0.0);//mouse*resolution.xy.xy/resolution.xy;
...
vec3 allWindowsSkyscraperTexture(vec3 p, vec2 uv, vec3 normal, vec3 bid, float xr, float obj, float w, vec3 size) {
    ...
    if (normal.z == 0. && frame == 0.) {
        ...
        if (!isLight && n > .7 && abs(sin(bid.x + bid.y + fract(n*23422.)*110. + time/50.)) > fract(n*123.22)) {
            ...
            if (borders != 0.) {
                col = vec3(0.2);
                if (isLight) {
                    vec2 lights = vec2(sin(wuv.x + time + fract(bn * 3322.)*10.), sin(wuv.y + time + fract(bn * 3322.)*10.));
                    if (bn > .85) {
                        col.rb += lights;
                    } else {
                        col.rg += lights;
...
vec3 domeTexture(vec3 p) {
    ...
    if (id.y == 2.) {
        ...
        vec3 mc = step(ml, .1) * vec3(noise(5. + muv*4. + time/50., 5));
        if (ml > .1) {
...
vec2 cameraNextPosition() {
    float t = time * CAMERA_TIME_SCALE;
...
void scriptCamera() {
    ...
    camera.rotation = sin(time) * PI/64.;
...
void main(void) {
    ...
    vec2 uv = ((gl_FragCoord.xys.xy / resolution.xy) - .5) * vec2(resolution.x / resolution.y, 1.);
    ...
    glFragColor = vec4(min(vec3(1.), col), 1.);
    if (tr.obj != OBJ_DOME) {
        glFragColor *= fog;
    }
}
