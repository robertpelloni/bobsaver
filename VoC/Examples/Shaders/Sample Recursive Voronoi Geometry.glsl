...
// original https://www.shadertoy.com/view/MtBfzV
...
void main(void)
{
    ...
    vec2 t=uv*2.-1.;
       t.y/=resolution.x/resolution.y;
    vec3 ro=vec3(cos(time/10.)+.1,0.3,1.1),oro=ro;
    ...
    glFragColor.rgb+=diff*light(hitp,normalize(vec3(1.,.5,.7)),hitnorm,.2)*vec3(1.,1.,.8)*
        (.3+smoothstep(.05,.3,abs(fract((hitp.y*6.+hitp.x)*.7)-.5)));
    glFragColor.rgb+=diff*pow(light(hitp,normalize(vec3(-.1,0.,7)),hitnorm,2.5)/1.,.5)/3.*vec3(.8,.9,1.);

    glFragColor.rgb*=exp(-length(hitp.xy-oro.xy)/4.);
    glFragColor.rgb*=1.5;
    glFragColor.rgb+=.01*vec3(1,1,.7);
    glFragColor.rgb=pow(glFragColor.rgb*1.,vec3(1./2.4));
}
