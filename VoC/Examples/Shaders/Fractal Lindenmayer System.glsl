...
// original https://www.shadertoy.com/view/XtyGzh
...
// left-pads
int baseCounter (int base, int dec, int prec, int depth) 
{
 int r = 0;
 for (int i = 1; i <= MAXDEPTH; ++i) {
   r = int(mod(float(dec), float(base)));
   if (dec < base) r = dec;
   dec = dec / base;
   if (i == (depth-prec + 1)) return r;
 }
 return 0;   
}
