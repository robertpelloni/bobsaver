...
float circle(vec2 uv, vec2 pos, float d){
    ...
    return 1.0 - smoothstep(d, d + pixel * 1.0, length(uv - pos));
}
...
void main( void ) {
    ...
    layer1 += circle(uv, pos1 + d1*0.07, 0.030);
    layer1 += circle(uv, pos2 + d2*0.07, 0.030);
    
    
    
    layer1 += circle(uv, vec2(0.50,0.52), 0.16) - circle(uv, vec2(0.50,0.54), 0.16) ;
    float layer2 = circle(uv, vec2(0.50,0.55), 0.065);
    layer1+=layer2;
    glFragColor = vec4(layer1+layer2);
