...
// original https://www.shadertoy.com/view/3dfyDN
...
{   
    ...
    for(int i=1; i<10; i++){
        p.x+=0.3/float(i)*sin(float(i)*4.*p.y+time*speed*1.);//+mouse*resolution.xy.x/1000.;
        p.y+=0.3/float(i)*cos(float(i)*4.*p.x+time*speed*1.);//+mouse*resolution.xy.y/1000.;
    }
    //p.xy += time*10.;
