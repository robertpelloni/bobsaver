...
// original https://www.shadertoy.com/view/MllBR7
...
vec3 getCamera (vec3 eye, vec2 uv) {
  vec3 lookAt = vec3(0.);
  float click = 0.0;//clamp(mouse*resolution.xy.w,0.,1.);
  lookAt.x += mix(0.,((resolution.xy.x/resolution.x)*2.-1.) * 10., click);
  lookAt.y += mix(0.,resolution.xy.y/resolution.y * 10., click);
  float fov = .65;
