...
// original https://www.shadertoy.com/view/3ljGW3
...
#define EPSILON_NRM (0.15 / resolution.x)
...
void main(void) {
    ...
    float time = time * 0.3 + mouse.x*resolution.xy.x*0.01;
    ...
    // post
    glFragColor = vec4(pow(color,vec3(0.75)), 1.0)/pow(length(dist),0.15);
}
