...
// original https://www.shadertoy.com/view/4tV3WV
...
float dfHead(vec3 rp) {
    return sdEllipsoid(rp, vec3(1.9, 1.6, 1.5));
}

float dfHat(vec3 rp) {
    float hat = sdEllipsoid(rp, vec3(2.1, 1.8, 1.7));
    rp += vec3(0.0, 2.0, 0.0);
    float box = sdBox(rp, vec3(2.21));
    return max(hat, -box);
}

float dfRim(vec3 rp) {
    float rim = sdEllipsoid(rp, vec3(2.2, 1.9, 1.8));
    rp += vec3(0.0, 2.0, 0.0);
    float box = sdBox(rp, vec3(2.21));
    rim = max(rim, -box);
    rp += vec3(0.0, -0.2, 0.0);
    box = sdBox(rp, vec3(2.21));
    return max(rim, box);
}

float dfBobble(vec3 rp) {
    return sdSphere(rp, vec3(0.0, 2.2, 0.0), 0.6);
}

float dfTorso(vec3 rp) {
    rp += vec3(0.0, 2.1, 0.0);
    float body = sdEllipsoid(rp, vec3(2.4, 1.7, 1.7));
    rp += vec3(0.0, 3.0, 0.0);
    float box = sdBox(rp, vec3(2.41));
    rp += vec3(2.0, -3.4, 0.0);
    float leftArm = sdCappedCone(rp.yzx, vec3(0.44, 0.9, 0.7));
    rp += vec3(-4.0, 0.0, 0.0);
    float rightArm = sdCappedCone(rp.yzx, vec3(0.9, 0.44, 0.7));
    return smin(smin(max(body, -box), leftArm, 0.1), rightArm, 0.1);
}

float dfHands(vec3 rp) {
    rp += vec3(2.85, 1.7, 0.0);
...
float dfLegs(vec3 rp) {
    float crutch = sdSphere(rp, vec3(0.0, -1.9, 0.0), 1.4);
    rp += vec3(1.0, 2.8, 0.0);
    float leg1 = sdCappedCone(rp.xzy, vec3(0.80, 1.3, 0.8));
    rp += vec3(-2.0, 0.0, 0.0);
    float leg2 = sdCappedCone(rp.xzy, vec3(0.80, 1.3, 0.8));
    return smin(crutch, min(leg1, leg2), 0.1);
}

float dfFeet(vec3 rp) {
    rp += vec3(1.0, 3.4, 0.0);
    float foot = sdCappedCone(rp.xzy, vec3(1.0, 0.8, 0.3));
    rp += vec3(-2.0, 0.0, 0.0);
    return min(foot, sdCappedCone(rp.xzy, vec3(1.0, 0.8, 0.3)));
}
...
vec3 coatColour(vec3 rp) {
    ...
    if (rp.z > 0.0) {
        ...
        //buttons
        if (length(rp.xy - vec2(0.0, -1.2)) < 0.1) {
            coatCol = vec3(0.0);
        }
        if (length(rp.xy - vec2(0.0, -1.8)) < 0.1) {
            coatCol = vec3(0.0);
        }
        if (length(rp.xy - vec2(0.0, -2.4)) < 0.1) {
            coatCol = vec3(0.0);
        }
    }
    return coatCol;
...
vec3 faceColour(vec3 rp) {
    ...
    if (rp.z > 0.0) {
        //eyes
        if (length(rp.xy - vec2(0.47, 0.35)) < 0.45) {
            faceCol = vec3(0.0);
        }
        if (length(rp.xy - vec2(0.47, 0.35)) < 0.4) {
            faceCol = vec3(1.0);
        }
        if (length(rp.xy - vec2(0.35, 0.25)) < 0.1) {
            faceCol = vec3(0.0);
        }
        ...
        if (rp.x > -0.3 && rp.x < 0.3) {
            if (rp.y > -0.5 && rp.y < -0.4) {
                 faceCol = vec3(0.0);   
            }
        }
        if (length(rp.xy - vec2(0.0, -0.5)) < 0.1) {
            faceCol = vec3(0.0);
        }
        if (length(rp.xy - vec2(0.0, -0.5)) < 0.05) {
            faceCol = vec3(1.0);
        }
    }
    return faceCol;
...
vec3 marchScene(vec3 ro, vec3 rd) {
    ...
    vec2 df = DEFDF; //distance function and material
    
    vec3 fn = vec3(0, 1, 0); //floor normal
    float fd = raytraceFloor(ro, rd, fn, vec3(0, -3.71, 0)); //floor distance
    ...
    if (d < FAR_CLIP) { 
        ...
        if (df.y == HAT) {
            pc = vec3(0.52, 0.8, 1.0);   
        }
        if (df.y == LEGS) {
            pc = vec3(0.54, 0.27, 0.07);    
        }
        if (df.y == HEAD) {
            pc = faceColour(rp);;    
        }
