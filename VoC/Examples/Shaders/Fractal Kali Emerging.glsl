...
// original https://www.shadertoy.com/view/XlfGWl
...
#define FLOOR_COLOR vec3(.3,.15,.1)
#define ENERGY_COLOR vec3(1.,.7,.4)
#define BRIGHTNESS 1.5
#define GAMMA 1.2
#define SATURATION .8

#define detail .000028
#define t time*.25
...
vec2 de(vec3 pos) {
    float aa=smoothstep(0.,1.,clamp(cos(t*2.-pos.y*.4)*1.5,0.,1.))*3.14159;
    cc=cos(aa);ss=sin(aa);
    float hid=0.;
    vec3 tpos=pos;
...

{
    ...
    for (int i=0; i<15; i++) {
        fondo+=clamp(0.,1.,texture(pp+dir*float(i)*.02))*max(0.,1.-exp(-.03*float(i)));
    }
...
void main(void)
{
    ...
    glFragColor = vec4(color*vec3(.95,.94,.91),1.);
}
