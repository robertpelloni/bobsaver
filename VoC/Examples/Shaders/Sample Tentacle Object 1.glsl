...
// original https://www.shadertoy.com/view/3tXXRn
...
vec3 render()
{
    ...
    else
    {
        ...
        glFragColor.rgb *= mix(vec3(.75,1.,.75), vec3(1), smoothstep(0.1, .8, l));

        vec3 c = glFragColor.rgb;
        
        // Blue / green alternating pattern
        glFragColor.rgb = mix(c.bbb * vec3(.5,1.,.5), glFragColor.rgb, smoothstep(.3, .7, tri(l * 4.)));
        
        // Yellow tips
        glFragColor.rgb = mix(glFragColor.rgb, c.bbb * vec3(1,1,.5), smoothstep(1.4, 1.5, l));
        
        // Yellow tips self-illumination
        glFragColor.rgb += vec3(1,1,.4) * smoothstep(1.4, 1.5, l) * .11;

        // Specular highlight
        glFragColor.rgb += specshad * .9 * smoothstep(.4, .7, dot(r, normalize(vec3(1)))) * fr;
