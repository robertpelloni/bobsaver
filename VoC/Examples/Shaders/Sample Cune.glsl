...
// original https://www.shadertoy.com/view/ls2GDR
...
float siso(vec3 p)
{
    float h = time * 1.5 + sin(time * 1.3) * 0.3;
    float k = 0.05;
    vec2  m = vec2(6, 5);
    vec2  b = vec2(1, 0.5);
    for(int i = 0 ; i < 3; i++)
...
void main( void ) {
    ...
    dir.yz = rot(dir.yz, h * 0.1 + mouse.y * 0.01);
    dir.xy = rot(dir.xy, h * 0.1 + mouse.x * 0.01);
    ...
    glFragColor = vec4(col * (1.0 - pow(dot(uv*uv, uv*uv), 4.0)), 1.0);
