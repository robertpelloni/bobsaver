...
void main( void ) 
{
    ...
    glFragColor = encodeLevels(level*attenuation, lev(0,0,1));
    //glFragColor = encodeLevels(sin(gl_FragCoord.x/10.0)*sin(gl_FragCoord.y/10.0), lev(0,0,1));
    ...
    glFragColor.rg = vec2(log(1.0+f)/(0.7+0.7*length(v)));            // Uses factor in RG channels after transformation
}
