...
// original https://www.shadertoy.com/view/tdSXWt
...
void main(void) {
    ...
    vec3 ro = vec3( 5.0*cos(0.2*time-10.0), 1.0, 5.0*sin(0.2*time-10.0));
    vec3 ta = vec3( 0. , 0. , 0. );
    ...
    // create view ray
    vec3 rd = normalize( camMat * vec3(p.xy, 1.9+0.8*sin(0.15*time+12.0)) ); // 3.0 is the lens length
    
    // rotate camera with mouse
    mat3 rot = rotationXY(mouse*resolution.xy.xy * vec2( 0.01, -0.01 ) );
    rd = rot * rd;
    ro = rot * ro;
    vec3 col = vec3(0), sp;
