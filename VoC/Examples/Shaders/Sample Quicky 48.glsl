...
// original https://www.shadertoy.com/view/WsGBRd
...
void main(void)
{
    ...
    uv*=1.;
    float bpm = sqrt(fract(-length(uv)+time*130./60.*.5));
    //uv+=vec2(cos(time),sin(time))*.1;
    uv*=r(time*.1)*+bpm;
    uv= abs(uv)-.1;
    float f = 1.01+(sin(time)*.02+0.02);//sin(time+texture1D(spectrum1,.1).r*100.)*2.+2.5;
