...
// original https://www.shadertoy.com/view/ll2SRy
...
/*
    ...
    his various cube examples. He's a pretty clever guy, so he'll probably know how
    to do this ten times faster with ten times more efficiency. :)

    It doesn't look much like it, but Dukes' port of Las's "Cloudy Spikeball" also provided 
    inspiration.
    ...
    produce a reasonably clean looking, transparent scene.
    ...
    // Port of a demo by Las - Worth watching.
...
void main(void) {
    ...
    // There are a few ways to hide artifacts and inconsistances. Making things go fast is one of them. :)
    ...
    // Unit ray jitter is another way to hide artifacts. Can also trick the viewer into believing
    ...
    float thD = .035;// + smoothstep(0.8, 1.0, sin(time*0.375*0.5)*0.5+0.5)*0.015;
    ...
    for(float i=0.; i<56.; i++)    {
        ...
        // aD (accumulated distance). I interpolated aD on a whim, because it seemed to look nicer.
        // 1/.(1. + t*t*0.25) - Standard distance attenuation.
        ...
        // Kind of weird the way this works. I think by not allowing the ray to hone in properly,
        // provides a nice spread of values. The figures are based on a bit of knowledge versus
        // trial and error.
        t += max(d*.7, thD*1.5); 
    ...
    // Doing the same again, but this time mixing in some green.
