...
// original https://www.shadertoy.com/view/wlSfzh
...
void main(void) {
    float t = time/6.;
    vec3 pos = cam_pos(t);
    float small_dim = min(resolution.x, resolution.y);
    vec2 jiggle = vec2(0.25);
    vec3 color_sum = vec3(0.);
    for (int sgn_x = 0; sgn_x < 2; sgn_x++) {
        for (int sgn_y = 0; sgn_y < 2; sgn_y++) {
            vec2 screen_pt = 2.*(gl_FragCoord.xy + jiggle - 0.5*resolution.xy)/small_dim;
    ...
    glFragColor = vec4(color_sum/4., 1.);
}
