...
// original https://www.shadertoy.com/view/4tGXWd
...
float MandelBulb(vec3 pos, const int limitIterations)
{
    ...
    //float Power = 5.0 + cos(time*0.0125)*4.0;
...
vec3 EyeBallColor(vec3 p) {
    ...
    vec3 mouseVector = vec3(
        (mouse.x*resolution.x/resolution.x - 0.5) * 2.,
        (mouse.y*resolution.y/resolution.y - 0.5) * 2.,
        CAMERA_Z
    );
