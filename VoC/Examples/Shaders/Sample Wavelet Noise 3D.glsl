...
// original https://www.shadertoy.com/view/tlBczy
...
float scene(vec3 p) {
    ...
    p.x += WaveletNoise(p/2., time*3., 1.15)*.3;
    p.y += WaveletNoise(p/2.+10., time*3., 1.15)*.3;
    p.z += WaveletNoise(p/2.+20., time*3., 1.15)*.3;
    distorted_p = p;
...
void main(void)
{
    ...
    glFragColor.xyz = hit ? col : bg;
    glFragColor = sqrt(glFragColor);
    glFragColor *= 1.- dot(uv,uv)*.6;
}
