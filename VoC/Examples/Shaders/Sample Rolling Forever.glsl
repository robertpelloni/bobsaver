...
// original https://www.shadertoy.com/view/Xd2XWt
...
int idObj;
mat3 vuMat;
vec3 trkNv, ballNv=vec3(0.0), qHit=vec3(0.0), ltDir;
float tCur, posLin, radSpir, dhSpir, ballRad, rollVel, fnSpir, dLinSeg,
   dRotSeg, lenPath, trkAng, ballAng=0.0;
bool showFlap;
const int nBall = 6;
const int nSpir = 3;
...
vec3 BallPos (float d)
{
  ...
  if (d < 2. * dLRSeg) {
    if (d < dLinSeg) {
      ...
      d -= dLinSeg;
      a = d * da - 0.5 * pi;
      trkNv.xz = vec2 (cos (a), sin (a));
      p.xz = vec2 (posSpir, posSpir) + radSpir * trkNv.xz;
      p.y = htd - d * htSeg;
...
vec4 BallCol ()
{
  vec3 col;
  vec3 q = qHit;
  float aa = atan (ballNv.x, ballNv.z);
  q.xz = q.xz * cos (aa) * vec2 (1., 1.) + q.zx * sin (aa) * vec2 (-1., 1.);
  if (q.z * (mod (pi + atan (q.x, q.y) + ballAng, 2. * pi) - pi) > 0.)
     col = vec3 (0., 0.6, 0.);
