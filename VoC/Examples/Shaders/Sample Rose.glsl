...
{
    return fract(sin(dot(p, vec2(23.15898, 21.90563))) * 44549.5473453);
}
//--------------------------------------------------------------------------
...
vec3 DoLighting(in vec3 pos, in vec3 ray, in vec3 nor, in vec2 t)
{
    ...
    vec3 ref = reflect(nor, ray);
    float spec = pow(max(dot(sunLight, ref), 0.0), 8.0);
    col += vec3(.5, 0.3, .2) * spec * sha;
...
void main(void)
{
    ...
    {
        col = mix(GetSky(ray), col, normal.w);
    }
