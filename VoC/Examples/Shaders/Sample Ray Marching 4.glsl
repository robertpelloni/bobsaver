...
float SphereSponge(vec3 w)
{
    ...
    d = max(d,length(w * vec3(1.0,1.0,0.3)) - 0.6);
...
vec2 map( in vec3 pos )
{
  ...
  vec2 res = vec2( sdPlane(     pos), 1.0 ); 
    ...
    res = opU(res,vec2(SphereSponge(pos - vec3(0.0,0.6,0.0)),46.9 + 40.0 * cos(0.2 * time)));
...
void main( void )
{
  ...
  // camera  
  float angle = ROTATE ? time * .5 : 0.0;
  float rx = 3.0*cos(angle + 6.0*mo.x);
  float rz =  3.0*sin(angle + 6.0*mo.x);
  vec3 ro = vec3( rx, 0.5 + mouse.y , rz );
