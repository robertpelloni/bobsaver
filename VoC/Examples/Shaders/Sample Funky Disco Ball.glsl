...
// http://glslsandbox.com/e#40201.0
...
void main(void)
{
    ...
    float sphereColor    = 0.15 + 3.0 * pow( texture2D( iChannel0, vec2( 0.75, 0.25 ) ).x, 3.0 );
    
    if ( t1 > t0 )
    {
        t                 = t1;
        sphereRadius     = sphere1Radius;
        sphereColor        = 0.8 * pow( texture2D( iChannel0, vec2( 0.5, 0.25 ) ).x, 3.0 );
    ...
    if ( t2 > t1 )
    {
        ...
        sphereColor        = 0.1 * pow( texture2D( iChannel0, vec2( 0.0, 0.25 ) ).x, 3.0 );
    }
    ...
    float moodTime     = mod( time * 0.5, 3.0 );
    ...
    vec2 grainUV = ( gl_FragCoord.xy / ( 2.0 * resolution.xy ) + fract( vec2( time * 11.1, time * 31.3 ) ) );
    float grain = 0.0;//texture( iChannel1, grainUV ).x;
    color += vec3( ( 2.0 * grain - 1.0 ) * 0.04 * ( 1.0 - sphereColor * 0.5 ) );
    
    glFragColor=vec4( pow( color, vec3( 0.6 ) ), 1.0 );
}
