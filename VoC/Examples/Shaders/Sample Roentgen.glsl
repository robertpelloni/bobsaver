...
vec4 baseColor = vec4 (.5* abs(sin(time)), .4*abs(cos(time)), 1.0, 1.0);    
...
vec3 camera (vec3 p) 
{
    p.xz *= rot(PI*(cos(time) / resolution.y-.5) * sin(time));
    p.yz *= rot(PI*(.5* sin(time) / resolution.y-.5) * .2*cos(time));
    return p;
}
...
void main( void ) 
{
  vec2 uv = mouse.x*(gl_FragCoord.xy-.5*resolution.xy) / resolution.y;
