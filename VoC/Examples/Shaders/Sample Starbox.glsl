...
// Starbox - CoffeeBreakStudios.com
// Work in progress...
...
void main( void ) {
    ...
    float n = surface3(vec3(position, time * 0.05)*mat3(1,0,0,0,.8,.6,0,-.6,.8),1.0);
    float n2 = surface3(vec3(position, time * 0.04)*mat3(1,0,0,0,.8,.6,0,-.6,.8),.5);

        float lum = length(n);
        float lum2 = length(n2);

    vec3 tc = pow(vec3(1.-lum),vec3(28.,14.1,8.));
    vec3 tc2 = pow(vec3(1.1-lum2),vec3(5,11,5.));
    vec3 curr_color = tc + (tc2*0.4);
    ...
    if (rand(star.y * star.x) >= 2.1 && rand(star.y + star.x) >= .7) {
        ...
        curr_color = mix(vec3(0.8), curr_color, lm / 0.4);
    }
    
    glFragColor = vec4(curr_color.x,curr_color.y, curr_color.z, 1.0);
}
