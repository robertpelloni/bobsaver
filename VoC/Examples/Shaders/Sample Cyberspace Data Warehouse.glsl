...
// original https://www.shadertoy.com/view/NlK3Wt
...
void sphere (vec4 hx, vec2 st, float sm,  inout vec4 R) //memory sphere full of data
{
    ...
    float   T = mod(time+h21(hx.zw*20.)*20.,20.)        //local time
        ...
        ,   y = .4* ((T < 4.) ? sin((T-1.)*.52) :  //sphere position
                    ...
                    ((T < 8.5) ? sin((8.5-T)*.52) : 
                    0.))) - .06          
        ,   f = (.9 + noise(vec2(hx.x*50.+time*4.))*.3) //hole inner noise
    ...
    f = noise(hx.xy*vec2(12,7)+vec2(0,time*-4.))*.25+.5; //sphere noise
...
void pixel (float hh, float sm, vec2 st, vec2 s, float n,  vec4 R, inout vec4 C) //blinking pixels
{
    ...
    float   b = ((4.-n)*2.2+.8)*.05 //face lightness
        ...
        ,   T = mod(time+hh*20.,20.) //local timer
        ,   d = ((T < 3.) ? sin((T)*.52) :  //hole diameter
                        ((T < 6.5) ? 1. :   
                        ((T < 9.5) ? sin((9.5-T)*.52) : 
                                0.)))
        ,   f =  min(
                (pow(noise(id*hh*n+time*(.75+h21(id)*.15)*1.),8.)*2. //small picks
                + (noise(id*.2 + time*(.5+hh*n)*.5)-.1)) //big noise
...
void main(void)
{
    ...
    uv *= .8+sin(time*.3)*.25; //camera scale
    uv -= uv * pow(length(uv),2.5-sin(time*.3)*.5)*.025 +  //camera distortion
        vec2(time*.2,cos(time*.2));   //camera translate
