...
// original https://www.shadertoy.com/view/wdSfzh
...
// "Mother%&*$ers stole my 80s style!" - Steven Spielberg

vec2 z,v,e=vec2(.01,-.01);float t,tt,g,gg;vec3 sp,tp,ep,op,hp,po,no,ld,al;//Global vars. About as boring as conspiracy theorists conspiring to write fake conspiracies
...
float smax(float d1,float d2,float k){float h=clamp(0.5-0.5*(d2+d1)/k,0.,1.);return mix(d2,-d1,h)+k*h*(1.0-h);}//Remove geometries with smooth blend
...
vec4 texNoise(vec2 uv){ float f = 0.; 
...
{ 
  ...
  float fft=0.0; //texture( iChannel1, vec2(min(abs(op.z*.002)*.15,5.),0)).x; //Audio sync along z limited to first 5 fft samples (bass)
  ...
  sp.z=mod(sp.z,3.)-1.5; //Modulo along z to create many eggs along z axis
  ...
  h=vec2(length(ep.xz+tnoi*5.),3);  //Make electricitings
  ...
  h=vec2(length(tp.xy)-27.,6);  //CYLINDER FOR TUNNEL
...
void main(void)
{
  ...
  if(z.y>0.){ //Yeah we hit something, unlike you trying to throw a spear at a pig. We wouldnt have survive the ice age with you and your nerdy mates.
    ...
    co=mix(sp+al*(a(.1)*a(.3)+.2)*(vec3(.5,.2,.1)*dif+s(.5)*1.5),fo,min(fr,.5));//Building the final lighting result, compressing the %&*$ outta everything above into an RGB %&*$ sandwich
    co=mix(fo,co,exp(-.00007*t*t*t)); //Fog soften things, but it won't stop your annoying uncle from thinking "Bloody fiddling with bloody numbers, ain't gonna get you no real job, son. Real graft is what ye need, pal."
  } 
  glFragColor = vec4(pow(co+g*.2*vec3(.5,.2,.1)+gg*.1*vec3(.1,.2,.4),vec3(.55)),1);
}
