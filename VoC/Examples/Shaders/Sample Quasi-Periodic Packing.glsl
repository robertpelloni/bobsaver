...
// original https://www.shadertoy.com/view/7s3XD8
...
float DE(vec3 p) {
    ...
    for (int i = 0; i < MAX_ITER; i++) {
        bool cond = true;
        translate(p);
        cond = cond && try_reflect(p, coclusters[0], scale); 
        translate(p);
        cond = cond && try_reflect(p, coclusters[1], scale);
        translate(p);
        cond = cond && try_reflect(p, coclusters[2], scale);
        if (cond)
            break;
    }
...
void init() {
    coclusters[0] = Ball(vec3(0,1,1), 1., true);
    coclusters[1] = Ball(vec3(1,0,1), 1., true);
    coclusters[2] = Ball(vec3(1,1,0), 1., true);

    cluster = Ball(vec3(0,0,0), 1., false);
}
...
float map(vec2 p) {
    ...
    vec3 vz = vec3(1., sqrt(2.), sqrt(3.));
    vec3 vx = vec3(-sqrt(2.), 1., 0);
    vec3 vy = vec3(1., sqrt(2.), -sqrt(3.));
    vx = vx / sqrt(dot(vx,vx));
    vy = vy / sqrt(dot(vy,vy));
    vz = vz / sqrt(dot(vz,vz));
    vec3 q = p.x*vx + p.y*vy + time*0.3*vz;
    return k*DE(q) * strong_factor;
