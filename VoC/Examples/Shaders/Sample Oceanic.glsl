...
// original https://www.shadertoy.com/view/4sXGRM
...
void main(void)
{
  ...
  float time2 = (time+13.5)*.05;
  vec3 campos = camera( time2 );
  vec3 camtar = camera( time2 + 0.4 );
  campos.y = max(waterlevel+30.0, waterlevel+90.0 + 60.0*sin(time2*2.0));
  camtar.y = campos.y*0.9;

  float roll = 0.14*sin(time2*1.2);
