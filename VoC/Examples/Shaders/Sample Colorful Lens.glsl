...
// original https://www.shadertoy.com/view/tdGfz1
...
void main(void)
{
    ...
    // space distortion
    p = normalize(p) * sin(pow(length(p), 0.5)*PI - time * 0.3);
    ...
    for (int i = 0; i < disks; ++i)
    {
        ...
        // rotation
        p *= rotation(sin(time * 0.1) * 0.1 / falloff + 0.1 * time / falloff);
    ...
    for (int i = 0; i < dots; ++i)
    {
        ...
        float timeline = fract(ratio * 135.1654 + time * 0.5);
        float angle = TAU * ratio * 15.547 + time * 0.1;
