...
// original https://www.shadertoy.com/view/st3SD2
...
//Holy cube by kastorp

//RENDERING FROM: https://www.shadertoy.com/view/WdtcRs "infinite cube nightmare"  by Algorias
...
float boxFrame1( vec3 p, vec3 b, float e )
{
  p = abs(p  );
...
//cross holes
float boxFrame2( vec3 p, vec3 b, float e )
{
  p = abs(p  )-b;
  vec3 m=step(p,p.zxy)*step(p,p.yzx);
  vec3 q = abs(p+e)-e-m*b*.8;  
  return min(min(
      length(max(vec3(p.x,q.y,q.z),0.0))+min(max(p.x,max(q.y,q.z)),0.0),
      length(max(vec3(q.x,p.y,q.z),0.0))+min(max(q.x,max(p.y,q.z)),0.0)),
      length(max(vec3(q.x,q.y,p.z),0.0))+min(max(q.x,max(q.y,p.z)),0.0));
}
//cross holes and decorated
float boxFrame3( vec3 p, vec3 b, float e )
{
  p = abs(p  );
  vec3 m=step(p,p.zxy)*step(p,p.yzx);
  p -= b  -m*b*.6; 
  vec3 q = abs(p+e)-e-m*b*.2;  
  return min(min(
      length(max(vec3(p.x,q.y,q.z),0.0))+min(max(p.x,max(q.y,q.z)),0.0),
      length(max(vec3(q.x,p.y,q.z),0.0))+min(max(q.x,max(p.y,q.z)),0.0)),
      length(max(vec3(q.x,q.y,p.z),0.0))+min(max(q.x,max(q.y,p.z)),0.0));
...
// decorated agles 2
float boxFrame4( vec3 p, vec3 b, float e )
{
...
float cube(vec3 p, float b) {
  ...
  float e=.5, m=fract(time/15.);
 return 
     mix(
     mix(boxFrame3(p,vec3(b),e),boxFrame4(p,vec3(b),e),smoothstep(.2,.3,m)),
     mix(boxFrame1(p,vec3(b),e),boxFrame2(p,vec3(b),e),smoothstep(.7,.8,m)),
     smoothstep(.45,.55,m));
}
...
float dist(vec3 p) {
    ...
    rotate(p.xz, time );
    rotate(p.xy, 1.2*time );    
    float d = cube(p.xyz, 8.0);

    glow = vec3(0.2) + 0.1*sin(3.*p + 4.*time);
...
void main(void) {
    ...
    float camera_AoV = 30.0 * 2.*PI/360.0;
