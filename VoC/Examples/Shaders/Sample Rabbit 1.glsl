...
// original https://www.shadertoy.com/view/XlccWH
...
{
    float an=.5*.5*2.*6. +resolution.xy.x/resolution.x*6.;
...
void main(void)
{
    vec2 uv = (gl_FragCoord.xy/resolution.xy * 2. - 1.) * .75;
    uv.x *= resolution.x / resolution.y;
    vec2 vt=uv;
    float an;

    vec3 ro=vec3(0.,-.1,-8.+0.25*resolution.xy.y/resolution.y*2.);
    ...
    glFragColor.rgb=max(col,0.);
    glFragColor.rgb=sqrt(glFragColor.rgb+.01);
}
