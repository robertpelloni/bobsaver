...
// original https://www.shadertoy.com/view/tsBXW3
...
vec4 background(vec3 ray)
{
    ...
    vec4 nebulae = vec4(0.0);//texture(iChannel0, (uv*1.5 ));
...
void main(void)
{
    ...
    for( int i=0; i<AA; i++ )
    {
        ...
        vec3 pos = vec3(0.,0.05,-(20.*mouse*resolution.xy.xy/resolution.y-10.)*(20.*mouse*resolution.xy.xy/resolution.y-10.)*.05); 
        vec2 angle = vec2(time*0.1,.2);      
        angle.y = (2.*mouse.y*resolution.y/resolution.y)*3.14 + 0.1 + 3.14;
