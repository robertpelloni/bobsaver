...
// original https://www.shadertoy.com/view/4sSXDG
...
int idObj;
mat3 flyerMat;
vec3 flyerPos, engPos, qHit=vec3(0.0), qHitTransObj=vec3(0.0), sunDir, sunCol;
vec2 trkOffset;
...
float FlyerDf (vec3 p, float dHit)
{
  ...
  dHit /= szFac;
  p /= szFac;
  d = szFac * PrOBoxDf (p - vec3 (0., 3., -3.),
     vec3 (1.2 * wSpan, 6., 1.1 * fusLen));
  if (d < 0.3) {
    q = p;
    wr = q.z / fusLen;
...
void main (void)
{
  ...
  vel.y = 0.;
  vec3 acc = (fpF - 2. * flyerPos + fpB) / dt;
  acc.y = 0.;
