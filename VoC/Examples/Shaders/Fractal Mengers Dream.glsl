...
// original https://www.shadertoy.com/view/lsSSDV
...
{
    z  = abs(1.0-mod(z,2.0));
    float time2 = 0.3*time;
    float d = 1.0;
    for (int n = 0; n < Iterations; n++) 
    {
        z.xy = rotate(z.xy,2.0*cos( time2/8.0));
        z.yz = rotate(z.yz,2.0*sin( time2/200.0));
        z = abs(z);
...
void main(void)
{
    ...
    // animation    
    float time3 = time;
    time3 += 15.0*smoothstep(  15.0, 25.0,time );
    time3 += 20.0*smoothstep(  65.0, 80.0, time );
    time3 += 35.0*smoothstep( 105.0, 135.0, time );
    time3 += 20.0*smoothstep( 165.0, 180.0, time );
    time3 += 40.0*smoothstep( 220.0, 290.0, time );
    time3 +=  5.0*smoothstep( 320.0, 330.0, time );
    float time1 = (time3-10.0)*1.5 - 167.0;
    float time2 = time3;
    const float s = 1.1;
    mat4 mm;
    ...
    vec3 ro = 0.05*time*vec3(0.0,0.0,1.0) + 0.01*dro; 
