...
// original https://www.shadertoy.com/view/3tfSWs
...
float noiseInside(vec3 p){
  ...
  if(layer() > 4)
      p = p * vec3(1., .5 + p.y * .1, 1.) - vec3(0., time * 1.25, 0.);
...
vec3 makeClr(){
    ...
    float ang = (resolution.x - mouse.x*resolution.xy.x) * .01;
...
void main(void) {
    ...
    for(int y = 0; y < AA; ++y)
        for(int x = 0; x < AA; ++x){
            glFragColor.rgb += clamp(makeClr(), 0., 1.);
        }
    
    glFragColor.rgb /= float(AA * AA);
    if(layer() < 7)
        glFragColor.rgb = mix(vec3(1.), glFragColor.rgb, stripe());
}
