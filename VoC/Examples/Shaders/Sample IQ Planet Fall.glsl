...
// original https://www.shadertoy.com/view/lltBWB
...
// Change AA in lie 7 to 1 if it renders too slow for you
...
float raycast(in vec3 ro, in vec3 rd, in float tmin, in float tmax  )
{
    ...
    for( int i=0; i<512; i++ )
    {
        ...
        if( abs(h)<(0.15*t/resolution.x) ) break;
        t += h*0.5;
