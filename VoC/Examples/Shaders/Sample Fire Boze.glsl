...
// original https://www.shadertoy.com/view/3t2cRc
...
{
    //vec3 index = floor((pos +vec3(0,0,time*3.0) )* 2. + 0.5);
...
surface map(vec3 p)
{
    ...
    if(floor(abs(index.x) + abs(index.y) + abs(index.z)) > 0.)
    {
        ...
        //result = sdBoze(p + vec3(0, 0, 0), vec3(1), (sin(time * 2.0) * 0.5 + 0.5)*1.5);
...
float densitycalc(vec3 p){
    ...
    //surface r = sdBoze(p, vec3(1), (sin(time * 2.0) * 0.5 + 0.5)*1.5);
    surface r = sdBoze(p, vec3(1.2), 1.);
    //return fbm(p * 25. + time * 0.2) - min(r.dist, 1.) * 25.;
    return fbm(p * (25. + p.y * 25.) *vec3(1,0.5,1)+ vec3(0, time * -5.2, 0)) - min(r.dist, 1.) * (20. - p.y * 20.);
}

surface rayCast(vec3 pos, vec3 dir, float dist)
{
    //vec3 spherePos = vec3(0,sin(time * 0.5) * 0.5,0);
