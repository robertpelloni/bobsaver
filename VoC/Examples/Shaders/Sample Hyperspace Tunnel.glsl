...
#define TAU 6.28318
const float period = 3.7;
const float speed  = 2.0;
const float rotation_speed = 2.3;
...
{
    //p += vec2(sin(time * .7), cos(time * .45))*(.1) + mouse*resolution.xy.xy*.1/resolution.xy;
    float f = 0.0;
...
void main(void)
{
    ...
    vec2 mo = (2.0 * mouse*resolution.xy.xy - resolution.xy) / min(resolution.x, resolution.y);
    p += vec2(0.0, -0.1);
    
    //float ay = TAU * mod(time, 8.0) / 8.0;
    //ay = 45.0 * 0.01745;
    ...
    //polar.y = focal_depth / p_len + time * speed;
    ...
    float val = 0.45 + 0.55 * fBm3(
        vec3(vec2(1.0, 0.2) * polar, 0.15 * time));
