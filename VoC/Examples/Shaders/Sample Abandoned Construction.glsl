...
// original https://www.shadertoy.com/view/wdVGD3
...
/*
    ...
    in extruded heightmap form? Neither have I, but I began coding a brick pattern 
    on a 2D plane, and this is where I ended up. :D

    A lot of repetitive tiling examples utilize a square grid. I prefer them for the
    obvious reason that they're simpler to use. Of course, a lot of interesting 
    scenes require triangles and hexagons, so on occasion, I'll make use of those too.
    Like everyone else, these single regular polygons are the ones I stick to. However, 
    there are countless other tiling arrangements that remain underutilized, like this one.
    ...
    interesting 3D examples coming up, but for now, I'm going back to simple 2D examples.
...
vec2 hash22(vec2 p) {
    ...
    return p;
    #else
    return cos(p*6.283 + time);
    //return abs(fract(p+ time*.25)-.5)*2. - .5; // Snooker.
...
vec4 herringbone3D(vec3 q3){
    ...
    // Skewing vector. Each brick stagers down by this amount... for some
...
float trace(in vec3 ro, in vec3 rd){
    ...
    for(int i = min(frames, 0); i<128; i++){
...
void main(void) {
    ...
    // Camera Setup.     
    vec3 ro = vec3(0, 1.5, time*2.); // Camera position, doubling as the ray origin.
    ...
    if (t < FAR){
        ...
        vec3 sn = normal(sp, 1.); // Surface normal. 
        ...
        // Surface texel.
        vec3 tx = vec3(0.0);//tex3D(iChannel0, sp/2., sn);
        ...
        if(svObjID==1. || abs(bordCol0Col1)<bordW){
            ...
            vec3 bTx = vec3(0.0);//texture(iChannel0, svBID).xyz; bTx *= bTx;
