...
// original https://www.shadertoy.com/view/tdjSDw
...
float stripesCircle(vec2 uv, StripesCircleOpt opt, StripesOpt sopt) {
    uv = vec2(atan(uv.y, uv.x)/PI*opt.kappa+time*opt.omega+opt.phi, fit01(opt.minRadius, opt.maxRadius, length(uv))*2.-1.);
...
vec3 drawMainEye(vec3 col, vec2 uv, vec2 mouse, out float dist) {
    ...
    float eyeWhite = length(uv+innerCircleNoise-eyeCenter)-mix(.325, .31, noise(uv*8.+45.1+time*.5))*3.4*pow(r,1.1);
...
vec3 smallEye(vec2 uv, vec2 mouse, vec2 seed, vec3 eyeColor, out float dist) {
    vec2 innerCircleNoise = (noise2(seed+uv*3.+time*.2+uv.y*3.)-.5)*.03;
    
    vec2 r = rand2(seed);
    float freq = mix(.5,1.,r.x);
    float phi = r.y * 2. * PI;
    float open = fit01(.0, .002, sin(time*freq+phi)*.5+.5);
    ...
    float eyeWhite = -circle(uv+innerCircleNoise-eyeCenter, mix(.95, .99, noise(seed+uv*8.+45.1+time*.5))*innerRadius);
...
vec3 horn3(vec3 col, vec2 uv, float mainEye, float subEyes) {
    ...
    stripesMask = min(stripesMask, -eyes-mix(.0, .05, pow(noise(uv*10.-time*.1), 4.)));
...
vec3 horn6(vec3 col, vec2 uv, float mainEye, out float dist) {
    ...
    float s = stripes(rot(-.5)*uv*vec2(1.3,1.)-vec2(-time*.03,-1.2), StripesOpt(.5, 0.5, 1.));
    s = smin(s, stripesMask, .02);
...
vec3 horn9(vec3 col, vec2 uv, float mainEye, out float dist) {
    ...
    float s2 = stripes(uv*vec2(1.,3.)-vec2(time*.025+.1*uv.y,-.0), StripesOpt(0., -1.3, 0.));
    s = min(s, -s2);
...
vec3 horn10(vec3 col, vec2 uv, float mainEye, out float dist) {
    ...
    float s = stripes(uv*vec2(4.,4.)-vec2(time*.02,-.0), StripesOpt(1.5, 0.5, 0.));
    s = smin(s, stripesMask, .02);
...
vec3 horn12(vec3 col, vec2 uv, float mainEye, out float dist) {
    ...
    float s = stripes(uv*vec2(4.,4.)-vec2(time*.02,-.0), StripesOpt(.7, 0.5, 1.));
    s = smin(s, stripesMask, .02);
...
vec3 horn14(vec3 col, vec2 uv, float mainEye, float dh15, out float dist) {
    uv -= vec2(.445,.0);
    float mask = circle(uv+vec2(.0,-.7*uv.x*uv.x)-noise2(uv*10.+time*.1)*.01, .2);
    float innerMask = mask-.013;
    float stripesMask = innerMask-mix(.005, .02, noise(uv*10.+time));
...
vec3 horn17(vec3 col, vec2 uv, float mainEye, float dh11, float dh9, out float dist) {
    uv -= vec2(.525,-.25);
    vec2 cuv = uv+vec2(.0,-.7*uv.x*uv.x)+noise2(uv*9.-time*.15)*.005;
    float mask = circle(cuv, .12);
    ...
    float s = stripes((rot(-.3)*uv)*vec2(3.4,1.)+vec2(-time*.03,0.), StripesOpt(.9,.5,.0))*.25;
    stripesMask = min(stripesMask, -mainEye-.005);
...
void main(void)
{
    ...
    vec2 mouse = (mouse*resolution.xy.xy/resolution.xy-.5)*resolution.xy/resolution.y;
