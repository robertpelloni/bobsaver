...
// original https://www.shadertoy.com/view/wdsGzB
...
void main(void)
{
    ...
    vec3 ro = vec3(0,0, time * 3.); ro -= path(ro);
    ...
    glFragColor.rgb = mix( col, vec3(0), 1.0-exp( -0.01*d*d ) );
           
    vec2 q = gl_FragCoord.xy/si;
    glFragColor.rgb *= 0.5 + 0.5*pow( 16.0*q.x*q.y*(1.0-q.x)*(1.0-q.y), 0.25 );
}
