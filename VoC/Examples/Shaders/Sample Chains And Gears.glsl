...
void main( void )
{
    ...
    //GetCameraRayLookat( OrbitPoint(-mouse.x * 20.0, mouse.y) * 20.0, vec3(0.0, 0.0, 0.0), ray);
    GetCameraRayLookat( OrbitPoint(sin(time), abs(sin(time/2.0))) * (abs(cos(time/2.0))+0.2)*10.0, vec3(0.0, 0.0, 0.0), ray);
    //GetCameraRayLookat(vec3(0.0, 0.0, -5.0), vec3(0.0, 0.0, 0.0), ray);
    
    vec3 cScene = GetSceneColour( ray );
    
    float fExposure = 1.5;
    glFragColor = vec4( Tonemap(cScene * fExposure), 1.0 );
}
