...
float triangle(float x, float a) { 
    float output2 = 2.0*abs(  4.0*  ( (x/a) - floor( (x/a) + 1.5) ) ) - 2.0;
    return output2;
}
...
void main() {
         vec2 uv2 = (1920.0 / resolution.x) * gl_FragCoord.xy / vec2(512) - 1.;
    ...
    from.y += transverseSpeed*(11.0)*sin(0.01*time) +0.001*time;
    from.z += 0.00*time;    
    ...
    for (int r=0; r<volsteps; r++) {
        ...
        #ifdef cloud
        t3= (field(p3));
        t3 = smoothstep(0.3,0.1,atan(1.2*t3));
        #endif
        ...
        backCol2 += mix(4.2, 2., v2) * vec3(5.8 * t3 * t3 * t3, 1.4 * t3 * t3, t3) * fade;
    ...
    glFragColor = forCol2 + vec4(backCol2, 1);
}
