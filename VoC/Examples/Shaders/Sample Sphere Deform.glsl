...
// original https://www.shadertoy.com/view/MljBWw
...
{
    float deform = fbm(pos * 2.0+ time * 0.1) * 0.25;
    float d = sdSphere( pos, 0.35 + deform);
    return d;
}
...
void main(void)
{
    ...
    float Time = 0.45*(15.0 + time);

    // camera    
    //vec3 ro = vec3( 4.0*cos(Time+45.), 0.5, 2.0*sin(Time) );
    vec3 ro = vec3( 2.0*cos(Time), 0.5, 2.0*sin(Time) );
