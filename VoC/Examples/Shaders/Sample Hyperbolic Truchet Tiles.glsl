...
// original https://www.shadertoy.com/view/3llXR4
...
void main(void) {
    ...
    float kAlt = 0.1;//step(0.1, max(mouse*resolution.xy.z, mouse*resolution.xy.w));
    ...
    vec2 uv = (gl_FragCoord.xy - 0.5*resolution.xy) * px;
    ...
    if (dot(uv, uv) < 1.) {
        ...
        vec2 m = (mouse*resolution.xy.xy - 0.5*resolution.xy) * px;
        bool scroll = false;
