...
// original https://www.shadertoy.com/view/3lBSDz
...
void getColor(out vec4 glFragColor, in vec3 pos, in vec3 ray){
    int depth=10;
    int idx=0;
    int ri=0;
    for(int d=0;d<depth;d++){
        ...
        if(hit.y<-0.5){glFragColor=back[idx];break;}
        if(hit.z<0.1*r){
            glFragColor=edge[idx^int(hit.y)];
            break;
        }else if(d==depth-1){
            glFragColor=back[idx^int(hit.y)];            
        }else{
            idx^=int(hit.y);
            ri=ri*3+int(hit.y);
        }
...
void main(void)
{
    ...
    getColor(glFragColor, pos, ray);
}

void mainVR(out vec4 glFragColor, in vec2 gl_FragCoord, in vec3 pos, in vec3 ray){
    getColor(glFragColor, pos+vec3(0,0,1), ray);
}
