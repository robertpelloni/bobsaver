...
// original https://www.shadertoy.com/view/3ds3Rj
...
vec2 getsubpixel(int id)
{
    vec2 aa=vec2(floor((float(id)+0.1)*0.5),mod(float(id),2.0));
    return vec2((2.0*gl_FragCoord.xy+aa-resolution.xy)/resolution.y);
}
...
void main(void)
{
    ...
    for(int i=0;i<4;++i)
    {
        ...
        f=love_curve(p.x, (sin(time*0.5)*0.5+0.5)*16.);
        //df=(f-love_curve(p.x+eps, (sin(time)*0.5+0.5)*16.))/eps;
        df=grad_love_curve(p.x,(sin(time*0.5)*0.5+0.5)*16.);
