...
// original https://www.shadertoy.com/view/WtfSDf
...
void cam(inout vec3 p) {
  float t=time2*0.1 + curve(time2, 3.7, 10.0)*5.0;
  float t2=time2*0.17 + curve(time2, 2.7, 10.0)*3.0;
  p.xz *= rot(t);
  p.xy *= rot(t2);
...
{   
  ...
  time2 = mod(time * 0.6, 500.0) + 16.4;
    
  vec3 s=vec3((curve(time2, 4.7, 10.0)-0.5)*20.0,(curve(time2, 7.2, 10.0)-0.5)*10.0,-30.0);
  vec3 r=normalize(vec3(-uv, 0.8 + 0.7 * curve(time2, 1.2, 10.0)));
  ...
  #if 1
  float t3 = time2*0.3 - length(uv)*0.2;
  col.xy *= rot(t3);
  col.xz *= rot(t3*0.7);
  col=abs(col);
