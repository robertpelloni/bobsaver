...
// original https://www.shadertoy.com/view/lsSSzz
...
{
    float f = 0.0;
    f += 0.5000*noise( p ); p = m*p*2.02; p.y += 0.02*time;
    f += 0.2500*noise( p ); p = m*p*2.03; p.y -= 0.02*time;
    f += 0.1250*noise( p ); p = m*p*2.01; p.y += 0.02*time;
    f += 0.0625*noise( p );
    return f/0.9375;
}
...
void main( void )
{
    ...
    col *= 1.0 - smoothstep( 0.0, 1.0/resolution.y, abs(q.x - 0.5)-0.5 );
