...
// original https://www.shadertoy.com/view/7sK3DR
...
float hash21(vec2 p) {
    //@Dave_Hoskins - Hash without sine - but it's HEAVY
...
vec4 render(inout vec3 ro, inout vec3 rd, inout vec3 ref, bool last, inout float d) {
    ...
    if(d<MAX_DIST){
        ...
        if(m==3.) {
            h = hue((snh+fresnel)*3.25,.80,.15)*.5;
            ref = h-fresnel;
        }
        if(m==4.) {    
...
void main(void) { //WARNING - variables void ( out vec4 O, in vec2 F ) { need changing to glFragColor and gl_FragCoord.xy
    ...
    // mouse
    float x = 0.0; //M.xy == vec2(0) ? .0 : .07+(M.y/R.y * .0625 - .03125) * PI;
