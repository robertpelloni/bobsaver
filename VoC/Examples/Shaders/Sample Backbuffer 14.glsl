...
void main( void ) {
    ...
    glFragColor.g = blurH(uv).b;
    glFragColor.r = blurV(uv).g;
    vec2 d = pixelSize*4.;
    vec2 dx;
    dx.x = bilinear(backbuffer, uv-vec2(1.,0.)*d).r - bilinear(backbuffer, uv+vec2(1.,0.)*d).r;
    dx.y = bilinear(backbuffer, uv-vec2(0.,1.)*d).r - bilinear(backbuffer, uv+vec2(0.,1.)*d).r;
    vec2 uvr = uv - dx * pixelSize*3.;

    float r = bilinear(backbuffer,uvr).b;
    r += (r - bilinear(backbuffer,uvr).r)*23./256.;
    r -= 2./256. - mouse*0.5;
