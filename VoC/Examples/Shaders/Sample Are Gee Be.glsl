...
// original https://www.shadertoy.com/view/MlSXzG
...
#endif
    return mix(rgbFog, RgbLight() * 3.5, GSqr(saturate(dot(normalRay, NormalLight()))));
}
...
float UConeTraceScene(vec3 posRay, vec3 normalRay, float rConeWidth, float dS, bool fCrossCells)
{
    float sRay = 3.0;
    ...
    for (int iStep = 0; iStep < 50; ++iStep)
    {
        ...
        // Find sRay_new such that sRay_new == (sdf.m_s + sRay_old) - rConeWidth * sRay_new
        //    i.e., march until until new cone potentially touches surface
        //    Solution is: sRay_new := (sdf.m_s - sRay_old * rConeWidth) / (1.0 + rConeWidth)
        ...
        if (fCrossCells)
        {
            ...
            sRay += max(
                        dS,
                        (min(dSCellEdge + sEdgeSlop, sdf.m_s) - sRay * rConeWidth) /
                        (1.0 + rConeWidth));
        }
        else
        {
            sdf = SdfScene(pos, uRandom);
            sRay += max(dS, (sdf.m_s - sRay * rConeWidth) / (1.0 + rConeWidth));
        }

        uOcclusion = min(uOcclusion, saturate(0.5 * (sConeWidth + sdf.m_s) / sConeWidth));
...
float RSpecularLight(vec3 normalRay, vec3 normal, float gGgxAlpha, out float o_rDiffuse)
{
    ...
    vec3 normalHalf = normalize(NormalLight() - normalRay);
    ...
    float gVis = 1.0 / (GGgxVisRcp(gGgxAlphaSqr, dot(-normalRay, normal)) *
                        GGgxVisRcp(gGgxAlphaSqr, dot(NormalLight(), normal)));
...
vec3 RgbLightHit(vec3 posHit, vec3 normalRay, SHit hit)
{
    ...
    float uAmbient = UConeTraceScene(posHit, hit.m_normal, 1.0, 20.0, false);
    ...
    float gDotLight = dot(NormalLight(), hit.m_normal);
...
vec3 RgbIntersectScene(vec3 posRay, vec3 normalRay)
{
    ...
    for (int iBounce = 0; iBounce <= g_cBounce; ++iBounce)
    {
        ...
        else
        {
            ...
            float gDotLight = dot(normalRay, NormalLight());

            vec3 vecPerp = normalRay - gDotLight * NormalLight();
...
void main(void)
{
    ...
    g_vecURandomIS = VecHash2(vec2(gl_FragCoord + g_t));
    vec2 vecURandomDOF = VecHash2(vec2(gl_FragCoord * g_gPi + g_t * exp(1.0)));
    vec2 vecURandomAA = VecHash2(vec2(gl_FragCoord * exp(1.0) + g_t * g_gPi));
    float uRandomMB = UHash(vec2(gl_FragCoord * sqrt(2.0) + g_t * 0.5 * (1.0 + sqrt(5.0))));
    ...
    for (int iRay = 0; iRay < RAY_COUNT; ++iRay)
    {
        ...
        g_t = time - uRandomMB * g_dTExposure;
#endif
...
#endif
        vec2 xyPixelOffset = (gl_FragCoord.xy + dXyOffset);
        vec2 uvScreen = xyPixelOffset / resolution.xy;
        vec3 normalCm = vec3(1.0, 0.0, 0.0);
        vec2 vecAspect = vec2(-1.0, resolution.y / resolution.x);
        float gFov = 0.9;
    ...
    glFragColor.rgb = rgbColor;
#else
    glFragColor.rgb = RgbTonemap(rgbColor);
#endif

    // Vignette

    glFragColor.rgb *= 1.0 - smoothstep(0.8, 2.6, length((gl_FragCoord.xy / resolution.xy) * 2.0 - 1.0));
