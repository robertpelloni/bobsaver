...
void main(void)
{
    ...
    //lightning = smoothstep(1., 0.0,lightning);
    ...
    moon = clamp(moon, 0., 1.) + 0.3*smoothstep(0.80,0.956,1.-length(uv-moonp));
    ...
    newUV.x-=time*0.3;
    newUV.y+=time*3.;
    float rain = sn( vec2(newUV.x*20.1, newUV.y*40.1+newUV.x*400.1 ));
    rain = 0.1-rain;
    rain+=smoothstep(0.2,0.5,f4+lightning+0.1)*rain;
    rain += pow(length(uv-moonp),1.)*0.1;
    rain = clamp(rain, 0.,0.1);
    
    vec3 painting = ground*(clouds + rain);
    
    float r=1.-length(max(abs(gl_FragCoord.xy / resolution.xy*2.-1.)-.5,0.)); 
    painting*=r;
    
    glFragColor = vec4(painting, 1.);
}
