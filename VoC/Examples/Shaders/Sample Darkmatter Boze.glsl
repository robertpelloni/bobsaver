...
// original https://www.shadertoy.com/view/3dSBRt
...
vec3 materialize(vec3 p, vec3 ray, float depth, vec2 mat)
{
    ...
    vec3 col = min(vec3(0.12, 0., 0.4) + sinebow(fract(time * 1.5)) * vec3(0.1), 1.);
    vec3 nor = norm(p);
    vec3 sky = vec3(0.,0.,0.0);
    //vec3 sky = min(sinebow(fract(time * 2.5 + 0.5)) * vec3(0.35) + vec3(0.3) + pow(max(1.29 - p.y, 0.), 5.), 1.);
    //if(p.y >= 0.29) 
    if(depth > 100.)
    {
        col = sky;
        ...
        if(mat.y > 0.0){
            ...
            if(mat.y == MAT_BLACK) {
                col = vec3(sin(rand(index.xy) * M_PI2 + time * 2.) * 0.5 + 0.5, 0.0, 0.0);
                roughness = 1.0;
                metalness = 0.0;
            } else if(mat.y == MAT_FACE) {
                //col = vec3(1.0, 0.8, 0.6);
                //col = min(sinebow(fract(rand(vec2(index.x, index.y) * 32.) + time * 0.5)) + vec3(0.2), 1.);
                roughness = 0.0;
                metalness = 1.0;
                ...
                //col = min(sinebow(fract((index.x + index.y) * 0.175 + time * 0.5 + 0.5)) * vec3(0.3), 1.);
...
void main(void)
{
    ...
    float rotSpeed = time * -0.125;
    float x = mod(time * 0.5, 10000.);
