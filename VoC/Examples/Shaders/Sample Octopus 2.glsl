...
// original https://www.shadertoy.com/view/MdsBz2
...
float Scene( vec3 p )
{   
    p.z += cos( p.y * 0.2 + time ) * 0.11;
    p.x += sin( p.y * 5.0 + time ) * 0.05;    
    p.y += sin( time * 0.51 ) * 0.1;
    
    Rotate( p.yz, 0.45 + sin( time * 0.53 ) * 0.11 );
    Rotate( p.xz, 0.12 + sin( time * 0.79 ) * 0.09 );
