...
// original https://www.shadertoy.com/view/Ns2XD3
...
void main(void)
{
    ...
    
    {
        ...
        col = mix(col, col*0.5*vec3(1,1,0.) + vec3(1. ,0.5,0.5)+ sin(d*2.)*sh * vec3(0.6,0.5,1.)*1.,smoothstep(fwidth(d),0.,d - 0.1 + smm(time))*0.4);
        col = mix(col, col*0.5*vec3(1,1,0.) + vec3(1. ,0.6,0.5)+ sin(d*4.)*sh * vec3(0.6,0.5,1.)*1.,smoothstep(fwidth(d),0.,d - 0.2 + smm(time + 0.4))*0.12);
        
        col = mix(col,vec3(1. ,0.6,0.5)*1.5+ 1.*sin(-d*2.)*sh * vec3(0.6,0.5,1.)*1.,smoothstep(fwidth(d),0.,d+ smm(time + 0.1)*0.6));
        
        {
            p *= 1. + smm(time)*3.;
            float sm = sin(time)*0.02;
            //p *= rot(sm*14. + time + sin(time) + pi);
            //p *= rot( (time + sin(time))*float(mod(time,pi*8.)<pi*2.));
            //p*= rot(sin(time)*float(mod(time,pi) < pi));
            p*= rot((sin(time))*smoothstep(-0.5,1.,sin((time/2. + pi/2.))));
    ...
    }
    {
        ...
        for(float cl = 0.; cl < 14.; cl++){
            ...
            float sc = 1.2;
            float sl = 0.02*time*(1. + sin(cl*20.)*0.9);
            float slid = floor(sl);
            ...
            for(float i = 0.; i < 2.; i++){
                ...
                d = min(d,(length(q - vec2(0.1,0.)) - rad*0.5 - sin(id*20. + time + cos(id)*0.)*0.05*rad)/sc); 
    ...
    
    {
        ...
        for(float cl = 0.; cl < 54.; cl++){
            ...
            float sc = 1.2;
            float sl = 0.05*time*(1. + sin(cl*25.1)*0.2);
            float slid = floor(sl);
            ...
            //p.x = abs(p.x);
            float m =  sin(cl + time*4.)*0.3;
            float mm =  sin(cl + time*4. - 2.4)*0.3;
    ...
    
    {
        for(float cl = 0.; cl < 14.; cl++){
            ...
            float sc = 1.2;
            float sl = 0.02*time*(1. + sin(cl*204.124)*0.5);
            float slid = floor(sl);
            vec2 p = uv + vec2(0. + mod(sl,1.)*4. - 2.,sin(slid*15. + cl)*0.2 - 0.4);;
            p *= sc;
            //p.x *= 0.9;
    ...
    
    {
        ...
        for(float terr = 0.; terr < cnt; terr++){
            ...
            float tm = p.x*1. + terrIdx + time*(1. + terrIdx*0.)*.1 ;
        ...
        float  d = sdSegment( p + vec2(sin(time+md)*0.1,0.), vec2(-0.2,0.), vec2(0.2,0.)) - 0.01;
        */
        /*
        col = mix(col,vec3(
                 1. ,
