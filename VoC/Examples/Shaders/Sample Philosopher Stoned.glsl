...
/*
    ...
    tentative 8th:
    VIII. The All is Continuous; all things are connected without seam or gap.    

    What is Alchemy, but the transmutation of a thing from one state into another?
    And which is greater: that which is produced, or that which produces?
...
#define    Accuracy    2. / resolution.x    // ray marching surface threshold
#define NormAcc        2. / resolution.x    // surface normal accuracy
...
float mapDE(in vec3 p) {
    ...
    const float I = 64.;
    for(float i=0.; i<I; i++) {
        rotateXY(p, vec2(10.-.024273*time, .0045*time));
        //p = abs(p);
...
void main(void) {
    ...
    vec2 mPos    = (mouse*resolution.xy.xy-.5*res) / res.y;
    
    vec3 rayBeg        = vec3(0., 0., -3.);
    vec3 rayDir        = normalize(vec3(uv, 1.*2.));
    
    vec2 camRotXY = vec2(.75+.0353*time, .75+.0485*time);
