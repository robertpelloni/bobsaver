...
// original https://www.shadertoy.com/view/XdcyRX
...
void main(void) {
    ...
    float dx = 0.5 - uv.x;
    float dy = 0.5 - uv.y;
    dy *= resolution.y / resolution.x;
    
    float perlinTheta = (PI+atan(dy, -dx))/PI;
    float r = sqrt((dx*dx) + (dy*dy));
    r = centerToCorner - r;
    
    float perlin = perlinNoise(perlinTheta, r, time);
    
    float timeMod = mod(-time, 1.5);
    ...
    glFragColor = vec4(glowRing+c, glowRing, glowRing*0.5+(c*2.0), 0);
}
