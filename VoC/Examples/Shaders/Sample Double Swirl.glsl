...
// original https://www.shadertoy.com/view/MtcBzH
...
void main(void)
{
    vec2 mnuv = mouse*resolution.xy.xy/resolution.xy;
    vec2 mn = (2. * mouse*resolution.xy.xy - resolution.xy) / resolution.y;
    ...
    vec2 one = vec2(0.);
    if (ONE_OSC==1) {
        float t = sin(time*0.912)*0.3 + 0.5;
        one = t*inf + (1.-t)*zero;
    } else {
        one =  (inf + zero) * 0.5;
    }
    ...
    float swirl = 0.;
    if (SWIRL_OSC==1) {
        swirl = 2.*TAU*sin(time*0.5) * atan(logr);
    } else { 
    ...
    vec3 colVal = sin(ang*2.+ time*5. + offsetv);
    ...
    // switch modes
    vec3 col = mix(col_aa, smoothCol, 0.7+0.2*sin(time));//floor(mod(time*0.5,2.)));
