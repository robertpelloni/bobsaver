...
// original https://www.shadertoy.com/view/3ljBDK
...
void main(void)
{
    ...
    {
        vec2 u = (gl_FragCoord.xy+vec2(xx,yy)-.5*resolution.xy)/resolution.y;
        u *= (2.-.7*(s-.5)*(s-.5));
        vec4 m = march(vec3(u,-2.25+(s-.7)*(s-.3))*t,vec3(0,0,1)*t);
        f += vec2(smoothstep(4.4,2.,m.w/.7),1);
    }
    glFragColor = vec4(vec3(1,0,.25)*f.x/f.y,1);
}
