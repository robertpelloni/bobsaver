...
// original https://www.shadertoy.com/view/lsf3DH
...
{
        return length(position) - r ;//+ 0.53*sin(position.y*1.1 + mod(time2*0.2, 2.0*PI)-PI)
...
vec3 newPos = rotateY(rotateX(translate(position.xyz,disp),1.3*time2),1.0*time2);
...
float opCheapBend( vec3 p )
{
    ...
    float c = cos(0.9*(1.0+sin(time2*0.2))+p.y*0.018);
    float s = sin(0.9*(1.0+sin(time2*0.2))+p.y*0.018);
    mat2  m = mat2(c,-s,s,c);
    vec3  q = vec3(m*p.xy,p.z);
    float alpha =//(1.0+sin(time2*1.0))/4.0;
0.38;//(1.0+sin(time2*1.0))/4.0;

vec3 disp = vec3(0.0,0.0,6.0);

vec3 newPos = rotateY(rotateX(translate(q.xyz,disp),1.3*time2),1.0*time2);
...
float spikeball(vec3 p) {
    p = rotateX(p-vec3(0.0,0.0,6.0),time2*3.0);
   vec3 q=p;
   p = normalize(p);
   vec4 b = max(max(max(
   ...
   return length(q)-1.75*pow(1.5,b.x*(1.-mix(.3, 1., sin(time2*2.)*.5+.5)*b.x)) + 0.04* perlin(p*6.0);
}
...
float function(vec3 position) {
    ...
    vec3 pos = vec3(0.5*sin(time2*3.0)*3.0,2.0,0.5*cos(time2*3.0)*3.0);
    float b = //udRoundBox(rotateY(rotateX(position+vec3(0.0,-1.4,-3.4)+pos,time2*3.0),time2*3.0), vec3(1.4,1.4,1.4), 0.3);
opS( opRep(rotateY(rotateX(position+vec3(0.0,-1.4,-3.4)+pos,time2*3.0),time2*3.0), vec3(0.95,0.95,0.95)),udRoundBox(rotateY(rotateX(position+vec3(0.0,-1.4,-3.4)+pos,time2*3.0),time2*3.0), vec3(1.4,1.4,1.4), 0.3));// - 0.03* perlin(position*3.8);
...
{    
    ...
    width  = resolution.x;
    height = resolution.y;
    lightPosition = vec3(3.0*sin(time2*0.2),0.5,3.0*cos(time2*0.2))-vec3(0.0,-0.5,-2.0);
