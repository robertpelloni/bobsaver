...
// original https://www.shadertoy.com/view/Xdlczl
...
float getwaves(vec2 position){
    position *= 0.1;
    float w = wave(position, vec2(70.3, 60.4), 1.0, 4.5) * 0.8;
    w += wave(position, vec2(60.3, -55.4), 1.0, 5.0) * 0.8;

    w += wave(position, vec2(-74.3, 50.4), 1.0, 4.0) * 0.5;
    w += wave(position, vec2(-60.3, -70.4), 1.0, 4.0) * 0.5;
...
#define snoisesinpow5(a,b) pow(1.0 - abs(0.5 - supernoise3d(vec3(a, Time * 1.3 ))) * 2.0, b)
#define snoisesinpow6(a,b) pow(1.0 - abs(0.5 - supernoise3d(vec3(a, Time * 1.3))) * 2.0, b)
...
float getwavesHI(vec2 uv){
    return (getwaves(uv)) + 0.04 * heightwaterHI2(uv * 0.1);
}
...
vec3 atmosphere(vec3 r, vec3 r0, vec3 pSun, float iSun, float rPlanet, float rAtmos, vec3 kRlh, float kMie, float shRlh, float shMie, float g) {
    ...
    vec3 px = r0 + r * rs;
shMie *= ( (pow(fbmHI(px  + wind(px * 0.00000669) * 40000.0) * (supernoise3dX(px* 0.00000669 + time * 0.001)*0.5 + 0.5) * 1.3, 3.0) * 0.8 + 0.5));
...
vec2 cloudsDensity3D(vec3 pos){
    ...
    vec3 p = ps * 0.005;
    float density = fbmHxI( p + fbmHxI2(p * 0.5 + 0.1 * time) * 19.5 + time );
    ...
    float init = smoothstep(0.25, 0.40, density * mlt);
...
vec2 UV = vec2(0.0);
vec4 internalmarchconservative(vec3 atm, vec3 p1, vec3 p2){
...
void main(void)
{
    ...
    if(ray.y >= -0.01){
        ...
        vec4 clouds = internalmarchconservative(atm, ray * hitfloor, ray * hitceil);
    ...
    float hitfloor = rsi2(r, sphere1);
    float hitceil = rsi2(r, sphere2);
    vec4 clouds = internalmarchconservative(reflection, R * hitfloor, R * hitceil);
    
    vec3 C = fresnel * mix(reflection, clouds.xyz, clouds.a) * 2.0 + fresnel * sun(R);
    
     vec3 sd = rotmat(vec3(1.0, 1.0, 0.0), time * 0.25) * normalize(vec3(0.0, 1.0, 0.0)); 
    float superscat = pow(max(0.0, dot(RF, sd)), 8.0) ;
    C += vec3(0.5,0.9,0.8) * superscat * getSunColorDirectly(0.0) * 21.0;
