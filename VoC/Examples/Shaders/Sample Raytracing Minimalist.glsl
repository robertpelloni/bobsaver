...
// original https://www.shadertoy.com/view/MlXXDB
...
//increase the next 2 variables for better quality (but slower speed)
...
vec3 castFullRay(in vec3 origin, in vec3 direction, in int subframe) {
    ...
    seed = direction.xy * (float(subframe) + time + 1.0);
...
void main(void)
{    
    vec2 uv = 2.0 * gl_FragCoord.xy / resolution.y;
    uv -= vec2(resolution.x / resolution.y, 1.0);
    
    float myLocalTime = time * 0.2 + 2.0;
    ...
    for (int i = 0; i < MAX_RAYSAMPLES; i++) {
        ...
        vec3 direction = normalize(vec3(uv.xy + rand2n() / resolution.y * 2.0, 2.0));
