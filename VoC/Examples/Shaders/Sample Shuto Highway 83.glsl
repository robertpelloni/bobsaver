...
// original https://www.shadertoy.com/view/XdyyDV
...
vec3 test_sky( vec2 gl_FragCoord, vec3 l )
{
 vec2 u = gl_FragCoord.xy / min( resolution.x, resolution.y );
...
float heart_grid( vec2 sp, vec2 c )
{
 ...
 vec2 beat = impulse( vec2( 10. ), repeat( vec2( time, time + 0.08 ), vec2( 1.2 ) ) );
  ...
  if ( ct.type == 2 )
  {
   ...
   if ( ct.p.z > 2. + h2 - 0.0 )
   {
    ...
    if ( index == 0. )
    {
     // airliners
     vec2 c = vec2( 110., 110. );
     vec2 sp3 = rotate_with_unit_vector( sp, -vec2( 0.5, 0.866025403 ) ) + vec2( -time * 100., 0. );
     sp3 = repeat( sp3, c );
    ...
    {
     // hearts
     float hg = heart_grid( -perp( sp ) + vec2( sin( time * 3. ) * 8., 6. + time * 40. ), vec2( 32., 40. ) );
     emitter+=(vec3( 1., 0., 0. ))*(hg)*margin*1.;;
     float hg2 = heart_grid( ( rotate_with_unit_vector( -sp, vec2( 0.707106781, 0.707106781 ) ) * 2. ) + vec2( -time * 18., 6. ), vec2( 30., 30. ) );
    ...
    else if ( index == 2. )
    {
     ...
     sp3 *= 1.;
     sp3.y -= 100.;
     sp3 = rotate_with_angle( sp3, time );
    ...
    else if ( index == 3. )
    {
     ...
     float anim1 = ( sin( time * 8. ) + 1. ) * 0.5;
     vec2 sp1 = rotate_with_unit_vector( sp, -vec2( 0.707106781, 0.707106781 ) ) + vec2( 0., 18. - time * 20. + anim1 * 4. );
     float l1 = step( ika_grid( sp1, anim1, vec2( 15., 15. ) ), 0. );
     emitter+=(vec3( 0., 1., 1. ))*(l1)*margin*1.;;
     float anim2 = ( sin( time * 9. ) + 1. ) * 0.5;
     vec2 sp2 = rotate_with_unit_vector( sp, -vec2( 0.707106781, 0.707106781 ) ) + vec2( 5., 9. - time * 35. + anim1 * 4. );
...
//        nh.d = FLT_MAX; // uncomment to check actual size of building if they weren't clipped
 ...
 for ( int j = 0; j < 70 /*FORCE_LOOP*/+min(0,frames); ++j )
 {
...
void rayMarchCellObjects( Ray ray2, Ranges_x4 iv, bool permuted, inout ParametricBuildingHit hit, vec2 cell_index, bool shadow_trace )
{
 ...
 for ( int i = 0; i< ( 4 /*FORCE_LOOP*/+min(0,frames) ); ++i )
 {
...
//    if ( r2.y <= r2.x ) return; // non sensical per drop if we return...
 ...
 float split_case = -1.;
 // dda traverse
 for ( int i = 0; i < ( 37 /*FORCE_LOOP*/+min(0,frames) )
...
//        if ( hit.t > ct.t ) return; // fixme: no point in continuing, but we should just set dda end point instead
 ...
 // do ao in permuted space
 {
  ...
  { /* ao algo from http://www.iquilezles.org/www/material/nvscene2008/rwwtt.pdf macrofified to avoid repetition */ float _delta = 0.1, _a = 0.0, _b = 1.0; for ( int _i = 0; _i < 5 /*FORCE_LOOP*/+min(0,frames); _i++ ) { float _fi = float( _i ); float _ao_t = _delta * _fi;
...
TraceCityRetval traceCity( Ray ray, bool shadow_trace )
{
 ...
 if ( !/* SHADOW_EARLY_RET */(shadow_trace&&((ct.t)!=/* FLT_MAX */1000000.)) )
 {
  ...
  vec3 p, dd;
  // narrow fov need 200
  for ( int j = 0; j < 110 /*FORCE_LOOP*/+min(0,frames); ++j )
  {
   p = ray.o + t * ray.d;
...
mat4 look_around_mouse_control( mat4 camera, float pitch, float tan_half_fovy, float look_at_the_abyss )
{
 ...
 //if ( mouse*resolution.xy.z > 0.0 || false ) mm = ( mouse*resolution.xy.xy - resolution.xy * 0.5 ) / ( min( resolution.x, resolution.y ) * 0.5 );
 float mm_y = mm.y;
 mm.x = -mm.x;
...
//    glFragColor.rgb = test_sky( gl_FragCoord, l ); return; // contemplate sky probe, for the interested reader
//    glFragColor.rgb = get_building_palette( gl_FragCoord.xy / min( resolution.x, resolution.y ) ); return; // for the curious
 float time = time;
 ...
 TraceCityRetval ct;
 float shadow = 1.;
 for ( int i = 0; i < 2 /*FORCE_LOOP*/+min(0,frames) ; ++i )
 {
 ...
 float airliner = 0.;
 for ( int i = 0; i < 3 /*FORCE_LOOP*/+min(0,frames); ++i )
 {
 ...
 glFragColor.rgb = fade * shadeCity( ct, view_ray.o, view_ray.d, l, shadow, airliner, a_contrast, u, aspect, ground_ao );
}
