...
// original https://www.shadertoy.com/view/tslXWl
...
vec3 render(vec2 crd,float t, float tc){
    vec2 uv=2.*crd/resolution.xy-1.;
    float sz=resolution.x/5.;
...
void main(void) {
    ...
    for(float i=0.;i<AA;i++){
        for(float j=0.;j<AA;j++){
            c+=render(gl_FragCoord.xy+(vec2(i,j)/AA),t,tc);
        }
    }
    c/=AA*AA;
    #ifdef GRAYSCALE
        glFragColor = vec4(vec3(maxComp(c)),1.0);
    #else
