...
// original https://neort.io/art/bpsulu43p9fefb924p3g
...
void main() {
    ...
    vec2 st2 = vec2(cos(at),sin(at)) * (dot(lng,lng) * 0.5 + 10.0);
    vec2 id = st2 - st;
    vec2 id2 = st - id;

    vec3 color = vec3(1.0);
    color += snoise(id2 + time * 2.0);
    color.b += snoise(id2 + 2.0 - time * 5.0);
    color.rg += -length(modSt);
    color *= triangle_wave(st * 10.0,2.5);
    color.g *= triangle_wave(st * 10.0,1.5);
