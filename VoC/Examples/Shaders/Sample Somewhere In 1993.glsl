...
vec3 enpos()
{
    return vec3(sin(time2)*100.+50.,sin(time2)*30.+30.,300.+sin(time2*.9+sin(time2*0.88+0.2))*100.);
}
...
vec3 makeHud(in vec2 p, in float seek)
{
    ...
    vec3 hcol = (sin(vec3(0.35,0.4,0.48)*(3.2)*PALETTE)*0.5+.5);
    hcol.gb -= sk2;
    hcol.r += sk2;
    return hcol*(1.-rz);
}
...
{   
    ...
    vec3 enp = enpos();
    enp.z += time*50.;
    d = ou(d,vec2((oct(roty(p-enp, time*2.5))-6.)*0.66,7.9));
...
{    
    ...
    vec2 um = vec2(0);
    um.x = 0.5+(smoothstep(-2.,2.,sin(time2*.7-0.1))-0.5)*.1;
    um.y = sin(time2+1.)*0.02;
    
    //camera
    vec3 ro = vec3((smoothstep(-2., 2., sin(time2*0.7+1.57))-0.5)*50., sin(time2)*5.-1., time2*50.);
    um.x *= 3.;
    vec3 eye = normalize(vec3(cos(um.x),um.y*5.,sin(um.x)));
    vec3 right = normalize(vec3(cos(um.x+1.5708),0.,sin(um.x+1.5708)));
    mat2 ori = mm2( smoothstep(-.5,.5,sin(time*0.7+0.78))-.5 + smoothfloor(time2*0.04,.45)*6.28 );
    right.xy *= ori;
    ...
    //scanlines
    col *= (sin(p.y*1.3*resolution.x)*0.15)*(sin(p.y*10.+time2*410.)*0.65)+1.;
