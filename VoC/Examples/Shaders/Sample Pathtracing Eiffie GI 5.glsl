...
uniform vec3 skyColor=vec3(0.3,0.6,0.8);
uniform float fog=0.0;
uniform float ambience=0.6;
// Menger
...
uniform vec3 bulbPosition=vec3(0.0,0.0,0.5);
uniform float bulbRefrRefl=0.5;
uniform float bulbGloss=0.1;
...
const float maxDepth=50.0, OOMD=1.0/maxDepth;
...
{//return distance estimate and object id 
    ...
    vec2 flr=vec2(max(length(z.xz)-2.5,z.y+10.0),1.0);
    //vec2 glass=mapMagGlass(PYR(z-glassPosition,glassDirection));
    vec2 glass=vec2(10000,10000);
...
{//get material properties (color,refr/refl,gloss,spec,specExp)
    //return (item==0.0)?Mater0:(item==1.0)?Mater1:(item==2.0)?Mater2; //you can extend this with a texture lookup etc.
