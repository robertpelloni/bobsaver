...
// original https://www.shadertoy.com/view/tlBXWK
...
void main(void)
{
    ...
    for (float index = motion_frames; index > 0.; --index) {
        ...
        float time = time+(dither+index)/10./motion_frames;
        glFragColor += vec4(raymarch(eye, ray, time,total))/motion_frames;
    }
    glFragColor = vec4(pow(glFragColor.r, .3));
    
    // extra color
    glFragColor.rgb *= vec3(.8,.85,.8);
    float d = smoothstep(4.,0.,total);
    glFragColor.rgb += vec3(0.9,.5,.4) * d;
}
