...
// original https://www.shadertoy.com/view/Mls3D2
...
// Simple Ray marcher over distance field. To be improved on the BTEs and the approximations quality.
...
// See his articles at : http://www.iquilezles.org/www/index.htm
    ...
    vec4 renderScene(const ivec2 formatSize, const vec3 eyePos, const vec3 eyeTarget, const float focalLength, const float dMin, const float dNear, const float dMax)
    {
        vec2 revFrag = gl_FragCoord.xy;
        revFrag.y = resolution.y - revFrag.y;
        mat3 camera = computeCameraMatrix(eyePos, eyeTarget, 0.0);
        vec2 o = (revFrag/vec2(formatSize) - vec2(0.5, 0.5)) * vec2(1.0, -float(formatSize.y)/float(formatSize.x));
    ...
    {
        mat3 rotMat2 = mat3(     1, 0, 0,
                    0, cos(thetaRing2*time), sin(thetaRing2*time),
                    0, -sin(thetaRing2*time), cos(thetaRing2*time));
    
        mat3 rotMat3 = mat3(    cos(thetaRing3*time), sin(thetaRing3*time), 0,
                    -sin(thetaRing3*time), cos(thetaRing3*time), 0,
                    0, 0, 1);

        return    opUnion( vec2(plane(p), 1), 
                opUnion( vec2(sphere(p-lightPos, 0.005), 1024),
                opUnion( vec2(sphere(p-objectCenter, 0.5), 2),
                opUnion( vec2(torus82(p-objectCenter, vec2(1.0, 0.1)), 3),
                opUnion( vec2(torus82(rotMat2*(p-objectCenter), vec2(1.3, 0.1)), 4),
                          vec2(torus82(rotMat3*(p-objectCenter), vec2(1.6, 0.1)), 5)
                         )))));
    }
    ...
    float scattering(vec3 p, vec3 dir, float d)
    {
        ...
        return d*0.2*exp(-p.y*0.5)/(dLight*dLight);
    }
    ...
    vec4 renderMaterial(vec3 p, vec3 normal, float material, float dist, float absorption, float scattering)
    {
        ...
        if(material>0.0) // Valid object intersection
        {
            ...
            float l = length(u);
            u = u/l;
            float s = max(dot(-u, normal),0.0) * max(pow((max(dot(u, normalize(lightDir)), lightSpan)-lightSpan)/(1.0-lightSpan), lightProfile), 0.0) / max(l*l, 1.0) * (lightRadiance + sin(time)*lightRadiance/2.0);
        ...
        col.rgb = col.rgb * softShadow(p, lightPos, 1.0, 32.0) * max(1.0 - absorption,0.0) + scattering * (lightRadiance + sin(time)*lightRadiance/2.0) * lightCol/max(1.0,dist*dist);
...
void main(void)
    {
        vec4 c = renderScene(ivec2(resolution.xy), eyePos, eyeTarget, focalLength, 0.0, 0.001, 100.0);
        glFragColor = vec4(pow(c.rgb, vec3(1, 1, 1)/2.2), 1.0);
    }
