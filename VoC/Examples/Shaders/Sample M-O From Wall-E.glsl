...
// original https://www.shadertoy.com/view/WdVfRc
...
//
// Any remember 'M-O' from Pixar's 'Wall-E' movie?
...
float sdArm(vec3 p, float c) {
    return max(
        max(sdTaper(p.yz, vec3(.5, .2, 2)), abs(abs(p.x) - 1.55)) - 0.1, // Main arm.
        (p.z + 1.5 + sin(p.y * 7.) * 0.1) * c); // Front/back cut.
}

// The sine wave applied to the dirt track.
float trk(float z) { return sin(z * .2 - time2); }
...
Hit map(vec3 p) {
    ...
    // Wall lights.
    float d = sdBox(vec3(p.xy, mod(p.z - time2 * 8., 40.)) - vec3(20, 7.25, 20), vec3(.1, .1, 8));
    g += .01 / (.01 + d * d);
    ...
    p.yz *= rot(-arms);
    d = sdArm(p, -1.);
    p.z += arms + cos(time2 * 15.) * .1;
    minH(h, Hit(min(d, sdArm(p, 1.)), 0, op.xy));
    
    // Brush.
    float f, b = .2 + .8 * abs(sin(p.x * 18.)) * 0.1;
...
vec3 mat(Hit h, vec3 p, inout vec3 n) {
    if (h.id == 1) { // Floor
        vec2 t = h.t + vec2(trk(p.z), time2 * -5.);
    ...
    if (h.id == 6) { // Face
        ...
        if (t.y < -.3 && t.x < 1.) {
            ...
            t *= rot(.6 * arms);
            return .01 + vec3(1.5, 1.5, 0) * step(abs(t.x - .3), .25) * step(abs(t.y + .7), .1 - arms * 0.1) * l;
        }
    }
...
vec3 lights(vec3 p, vec3 rd, Hit h) {
    ...
    return mat(h, p, n) // Material color.
           * (
              (
                  max(0., dot(ld, n)) // Primary light.
                  + max(0., dot(ld * vec3(-1, 0, -1), n)) // Bounce light.
              )
...
vec3 march(vec3 ro, vec3 rd) {
    ...
    if (h.id == 1) {
        ...
        n = calcN(p);
        n.xz -= flrN(p.xz - vec2(0, time2 * 5.)) * .024;
...
{
    time2 = mod(time, 30.) - 1.;
    arms = (time2 < 0. ? smoothstep(-1., 0., time2) : abs(sin(time2 * 10.) * .1) + .9) * .38;
    time2 = max(0., time2);

    vec3 ro = vec3(-7, 4, -7. - sin(time2 * .3)),
         col = vec3(0);
#ifdef AA
    float dx, dy;
    for (dx = 0.; dx <= 1.; dx++)
    ...
    glFragColor.rgb = vig(pow(col, vec3(.45)), gl_FragCoord.xy);
}
