...
// original https://www.shadertoy.com/view/wljSz1
...
float map(vec3 p){
    ...
    // bumpy
    #ifdef BUMPY
    float f = 0.0;//pow(texture(iChannel2, p*0.1).b,4.0);
...
void main(void) {
    ...
    for(int x = 0; x < samples; ++x){
        // anti-aliasing
        vec2 p = -1.0 + 2.0 * (uv + (-0.5+(vec2(x, y)/float(samples)))/resolution.xy);
        p.x *= resolution.x/resolution.y;
        
        // motion blur
        float r = 0.0;//texelFetch(iChannel0, ivec2(mod(gl_FragCoord*float(samples)+vec2(x,y),1024.)),0).r;
        T = ot+(tm*r)/36.0;
        ...
        vec3 alb = mix((vec3(0.3,0.5,0.9)),
                       (vec3(0.75,0.9,0.4)),
                       0)*1.25;//texture(iChannel2, 0.04*mp).r)*1.25;
        #ifdef SPLOTCHES
    ...
    glFragColor.rgb = c/float(samples*samples);
    
    // "color grade" and gamma
    glFragColor.rgb = mix(vec3(dot(glFragColor.rgb, vec3(.2125,.7154,.0721))), glFragColor.rgb, 1.3);
    glFragColor.rgb = smoothstep(0.0, 1.25, glFragColor.rgb);
