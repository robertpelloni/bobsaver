...
// original https://www.shadertoy.com/view/7tfBzs
...
vec3 map(vec3 p) {
    ...
    if (r.x > d.x) r = d;
    p = m;
    d.x = length(p) - .45 - .1 * (sin(time * 10.) * .5 + .5);
    ...
    p = q; p.y -= 1.;
    d.x = min(d.x, length(p) - .15 - .05 * (sin(time * 10. + 1.5) * .5 + .5));
