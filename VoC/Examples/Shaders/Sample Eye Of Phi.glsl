...
// original https://www.shadertoy.com/view/7stfzB
...
{
    float t = time/PI*2.0;
    vec4 m = vec4(0.0); //mouse*resolution.xy; m.xy = m.xy*2.0/R-1.0; // ±1x, ±1y
    ...
    vec2 uv = (gl_FragCoord.xy-0.5*R)/R.y*SCALE*z; // apply cartesian, scale and zoom
    ...
    glFragColor = vec4(c*sqrt(c)*1.4, 1.0);
}
