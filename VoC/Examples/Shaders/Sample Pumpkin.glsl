...
// original https://www.shadertoy.com/view/lt3fWj
...
void main(void)
{
    ...
    cam.yz = rotate(cam.yz, .3+sin(time*.1)*.1);
    dir.yz = rotate(dir.yz, .3+sin(time*.1)*.1);

    cam.xz = rotate(cam.xz, time*.4);
    ...
    {
        vec3 fakeN = n;
        fakeN.xz = rotate(fakeN.xz, -time*.4);
        // pumpkin
