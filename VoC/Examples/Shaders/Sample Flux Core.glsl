...
// original https://www.shadertoy.com/view/ltlSWf
...
vec3 RayTrace(in vec2 Coord )
{
    ...
    // Map uv to [-1.0..1.0]
    vec2 uv = Coord.xy/resolution.xy * 2.0 - 1.0;
    ...
    // debugging camera
    float mx=mouse.x/resolution.x*PI*2.0;// + localTime * 0.166;
    float my=-mouse.y/resolution.y*10.0;// + sin(localTime * 0.3)*0.8+0.1;//*PI/2.01;
...
void main(void)
{
    ...
    for (float i = 0.0; i < antialiasingSamples; i++)
    {
        ...
        localTime += Hash11(v21(gl_FragCoord + seed)) * motionBlurLengthInSeconds;
        // Jitter the pixel position so we get antialiasing when we do multiple passes.
        vec2 jittered = gl_FragCoord.xy + vec2(
            Hash21(gl_FragCoord + seed),
            Hash21(gl_FragCoord*7.234567 + seed)
            );
        // don't antialias if only 1 sample.
        if (antialiasingSamples == 1.0) jittered = gl_FragCoord;
    ...
    localTime = time;
    finalColor = RayTrace(gl_FragCoord.xy);
#endif

    glFragColor = vec4(sqrt(clamp(finalColor, 0.0, 1.0)),1.0);
}
