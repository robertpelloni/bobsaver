...
// original https://www.shadertoy.com/view/4dXGDN
...
void main(void)
{
    ...
    vec3 from = 16.*normalize(vec3(sin(time2),cos(time2),.5+sin(time2)));
    vec3 up=vec3(1,sin(time2*2.)*.5,1);
    ...
    vec3 col=vec3(0.);
    float dist=0.;
    rotm=rot(normalize(vec3(1.)),time2*3.);
    for (int r=0; r<120; r++) {
