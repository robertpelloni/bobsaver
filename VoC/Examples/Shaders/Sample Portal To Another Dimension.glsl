...
// original https://www.shadertoy.com/view/4dcyzH
...
vec4 draw(vec2 pos) {
    ...
    for (float i = 0.0; i < num_circles; ++i) {
        ...
        vec2 st = vec2(-0.4, 0.0) + offset + pos / resolution.xy * vec2(resolution.x / resolution.y, 1.0);
...
void main(void)
{
    glFragColor = draw(gl_FragCoord.xy);
}
