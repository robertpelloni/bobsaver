...
// original https://www.shadertoy.com/view/MlcfWr
...
float cabeza(vec2 p) {
    ...
    p.x-=sec_baile*(-t*.05-ritmo2*.02-ritmo1 *.02);
...
void secuencia() {
    ...
    sec_lluvia=min(.9,time*.2)-sec_final*.3*smoothstep(.5,1.,rnd1(floor(time*1.5)));
...
void main(void)
{
    ...
    float text=0.0;//texture(iChannel1,uv).r;
    uv-=.5;
    uv*=.8+length(uv)*.4;
