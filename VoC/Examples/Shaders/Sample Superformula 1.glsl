...
// original https://www.shadertoy.com/view/XlXXDs
...
vec4 render( vec3 ro, vec3 rd ) {
    ...
    if(dist > renderRange.y) return vec4(0); // background
        ...
        vec3 lig = normalize(vec3(-2,3.5,-2.6)),
             nor = calcNormal(pos),
             ref = reflect(rd, nor);
        float dif = clamp(dot(nor, lig), 0., 1.),
              ...
              occ = calcAO(pos, nor),
              sss = calcSSS(pos, lig);
        dif *= calcSoftShadow(pos, lig, .0002, 1.74, 32);
        vec3 brdf = vec3(0);
        brdf += 0.8 * dif;
        brdf += 1.0 * spe * dif;
        brdf += 0.3 * amb * occ;
...
vec4 renderAA( vec3 ro, vec3 rd ) {
    ...
    vec4 c = vec4(0); // color
    vec2 o = vec2(13, 0); // offset
    o = rotate(o, vec2(0), PI/8.0);
    for(int i = 0; i < k; i++) {
        c += render(ro + o.x/resolution.x, rd) / float(k);
...
vec4 params( float t ){
    t=mod(t,11.0);
    if(t<1.0)return vec4(6., -1.2, 1.6, 2.5);
    if(t<2.0)return vec4(8., -1.5, 1.4, 1.3);
    if(t<3.0)return vec4(4., 1., 1.2, 1.62);
    if(t<4.0)return vec4(8., .5, 1.03, 1.91);
    if(t<5.0)return vec4(7.5, .87, .67, 1.06);
    if(t<6.0)return vec4(11., .4, .4, 1.3);
    if(t<7.0)return vec4(7, 1.3, .9, .57);
    if(t<8.0)return vec4(5., .95, 1.36, 1.73);
    if(t<9.0)return vec4(7.2, .6, 5.4, 3.8);
    if(t<10.)return vec4(10., .4, 5.2, 5.6);
    return vec4(2., 1., 1., 1.);
}
vec2 params_ab( float t ){
    t=mod(t,11.0);
    if(t<1.0)return vec2(.8, 1.);
    if(t<2.0)return vec2(1.5, .6);
    if(t<3.0)return vec2(.9, 1.9);
    if(t<4.0)return vec2(1., 1.7);
    if(t<5.0)return vec2(1.1, 1.3);
    if(t<6.0)return vec2(1., .94);
    if(t<7.0)return vec2(1.3, 1.);
    if(t<8.0)return vec2(1.4, .5);
    if(t<9.0)return vec2(.9, .9);
    if(t<10.)return vec2(1., 1.);
    return vec2(1.25, 1.1);
}

void main(void) {
    vec2 R = resolution.xy,
         p = (2.*gl_FragCoord.xy - R)/R.y, q = p,
         mo = -(mouse.xy / R - 0.5)*2.;
    p *= 3.0;
    
    // background
    glFragColor = vec4(vec3(.06), 1);
    
    // animation - from effie
    #ifdef ANIM
    float t = time*0.25*speed;
    S1 =    mix(params(t-1.),    params(t),    smoothstep(0.,1./speed*2.,fract(t)*2.));
    S1_ab = mix(params_ab(t-1.), params_ab(t), smoothstep(0.,1./speed*2.,fract(t)*2.));
    t /= 10.;
    S2 =    mix(params(t-1.),    params(t),    smoothstep(0.,1./speed*2.,fract(t)*2.));
    S2_ab = mix(params_ab(t-1.), params_ab(t), smoothstep(0.,1./speed*2.,fract(t)*2.));
    ...
    glFragColor = mix(glFragColor, vec4(col.rgb, 1.), col.a);
    // 2d shape 2
    ...
    vec3 camFow = vec3(vec3(0)-camPos);
    vec3 forward=normalize(camFow), up=normalize(vec3(0,1,0)), right=normalize(cross(up, forward));
    vec3 rayDir = mat3(right, up, forward) * normalize(vec3(q-vec2(0.48, -.125), R.y / R.x * 10.0)); rd = rayDir;
    #ifdef MSAA
