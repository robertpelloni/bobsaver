...
// float r0 = cgol_test(res_r);
    ...
    float s = 0.05;
    float r0 = res_r - s*(0.5);

    vec3 nr0 = nhd( vec2(10.0,5.0), psn, 0.0, 0 );
    float nr0w = nr0[0] / nr0[2];

    vec3 nr1 = nhd( vec2(20.0,10.0), psn, 0.0, 0 );
    float nr1w = nr1[0] / nr1[2];

    res_g = 0.0;
    res_b = 0.0;

    if( nr0w >= 0.397 && nr0w <= 0.612 ) { r0 += s*(0.70); res_g = 0.28; }
    if( nr0w >= 0.437 && nr0w <= 0.464 ) { r0 -= s*(1.90); res_b = 1.0; }

    if( nr1w >= 0.200 && nr1w <= 0.266 ) { r0 += s*(0.92); res_g = 0.55; }
    if( nr1w >= 0.253 && nr1w <= 0.279 ) { r0 -= s*(1.00); res_b = 0.3; }
    if( nr1w >= 0.120+cs && nr1w <= 0.180 ) { r0 -= s*(0.50); res_b = 1.0; }
