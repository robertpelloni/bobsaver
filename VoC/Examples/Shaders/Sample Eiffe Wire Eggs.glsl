...
// original https://www.shadertoy.com/view/4lS3zw
...
vec3 scene(in vec3 ro, in vec3 rd){
    float d=DE(ro)*rnd*0.15,t=d,od=1.0,pxl=1.6/resolution.y;
...
void main(void) {
    ...
    vec3 rd=normalize(vec3((2.0*gl_FragCoord.xy-resolution.xy)/resolution.y,1.0));
    float tim=time;
