...
// original https://www.shadertoy.com/view/tlSSDV
...
void initDynamics() {
    ...
    activeSpheres[0] = activeSphereGrid(time2);
    activeSpheres[1] = activeSphereGrid(time2+1.);
    ...
    jointYRot = mix(taa0, taa1, clamp(fract(time2)*2.-.5,0.,1.));    

    float tal = mix(length(ta0), length(ta1), clamp(fract(time2)*2.5-1.,0.,1.));

    vec2 target = vec2(tal,.5-.5*smoothstep(.35,.4,abs(fract(time2)-.5)));  
...
float getSphereLightIntensity(float num) {
    return num > .5 ?
        clamp(fract(time2)*10.-1., 0., 1.) :
        max(0., 1.-fract(time2)*10.); 
}
...
vec3 getLighting( vec3 p, vec3 normal ) {
    ...
    if (i > 0.) {
        l += sphereCol(time2) * (i * getLightIntensity(p, normal, sphereCenter(activeSpheres[0]), .375));
    } else {    
        i = getSphereLightIntensity(1.);
        if (i > 0.) {
            l += sphereCol(time2+1.) * (i * getLightIntensity(p, normal, sphereCenter(activeSpheres[1]), .25));
        }
    }
    
    vec3 robot = mix(sphereCol(time2), sphereCol(time2-1.), getSphereLightIntensity(0.));
...
void main(void) {
    ...
    vec2 mo = mouse*resolution.xy.xy == vec2(0) ? vec2(.4,-.1) : abs(mouse*resolution.xy.xy)/resolution.xy - .5;
