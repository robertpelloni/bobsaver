...
// original https://www.shadertoy.com/view/wsVyRt
...
void getMouse( inout vec3 p ) {
    float x = 0.0; //M.xy == vec2(0) ? 0. : -(M.y/R.y * .65 - .325) * PI;
    float y = 0.0; //M.xy == vec2(0) ? 0. :  (M.x/R.x * .45 - .225) * PI;
    p.zy *=r2(x);
    p.xz *=r2(y);   
}
...
vec3 getTiles(vec2 p) {
    ...
    float a = sin(dir * angle * 32. + time * 3.5);
    float b = sin(dir * angle * 12. - time * 4.5);
