...
// original https://www.shadertoy.com/view/ldB3DK
...
void main(void)
{
    ...
    ////////////////
    //Update World
    float localTime = mod(TIME_OFFSET+time*TIME_SCALE,9.0);
    vec3 tangent;
    ...
    ////////////////
    // Render
    vec2 puv = -1.0 + 2.0 * gl_FragCoord.xy / resolution.xy;
    vec2 p = vec2(puv.x * resolution.x/resolution.y,puv.y);
    ...
    vec2 mouseAngles = (mix(vec2(0.0),(mousePos*2.0)-1.0,0.0))*vec2(PI,PI*0.5);
