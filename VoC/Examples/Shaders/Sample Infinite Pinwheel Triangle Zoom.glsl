...
// original https://www.shadertoy.com/view/4lsczj
...
vec3 color(vec2 uv_s)
{
    ...
    int nbIterations = 16;//mouse*resolution.xy.z > .5 ? 3 : 2;
    ...
    float q;
    qLimit = ( 1. / mix( 1. , 1./.14  , mouse.x*resolution.x/resolution.x ) )/5.*viewportMagnify;
    qLimit = .05;
...
void main(void)
{
    ...
    //glFragColor.rgb = tanh(glFragColor.rgb * 26. / (.2 + qLimit/viewportMagnify*16.)  ); // LOG correction
}
