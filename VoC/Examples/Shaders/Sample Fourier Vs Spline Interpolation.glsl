...
// original https://www.shadertoy.com/view/MlGSz3
...
void main(void) {
    ...
    {
        float d = 1e5;
        float ani = min(mod((12.0 + time) / 10.1, 1.3), 1.0) * float(NUM);
   ...
   //------------------------------------------------------
    ...
    //------------------------------------------------------
    {
        float ani = min(mod((12.0 + time) / 10.1, 1.3), 1.0);
        float d = 1000.0;
        vec2 oq=vec2(0.0);
    ...
    col *= 1.0 - 0.3 * length(gl_FragCoord.xy / resolution.xy - 0.5);
