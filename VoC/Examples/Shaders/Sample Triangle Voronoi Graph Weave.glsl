...
// original https://www.shadertoy.com/view/XsycDd
...
vec2 hash22(vec2 p) { 
    ...
    return sin(p*6.2831853 + time/2.)*.24;
    #endif
}
...
float linePattern(vec2 p, vec2 a, vec2 b){
    ...
    float ln = clamp(cos(p.y*72.*2.)*1. - .5, 0., 1.);

    return ln*.25 + clamp(sin(p.y*72.)*3. + 2.95, 0., 1.)*.75 + .15; // Ridges.
...
void main(void) {
    ...
    vec2 uv2 = uv*vec2(resolution.y/resolution.x, 1);
    uv *= .95 + dot(uv2, uv2)*.1;
    ...
    vec2 p = uv*5. - vec2(0, -1)*time/8. - vec2(0, 1);
    #endif
...
*/
    ...
    float blink = smoothstep(.65, .75, sin(hash21(tri.id)*6.283 + time));
...
////    
    ...
    if(lineID == 2.) { // Diagonal edge line.
        ...
        if(lineFirst[2]){
            float lns = linePattern(p, tri.p0, tri.p2);
            float shade = clamp(1. - d2*4., 0., 1.)*.75 + clamp(d2*32., 0., 1.)*.4;
            d2 -= lw;
            col = mix(col, vec3(0), (1. - smoothstep(0., .1, d2 - .01))*.65);
            col = mix(col, vec3(0), 1. - smoothstep(0., .015, d2 - .0225));
            col = mix(col, lCol*lns*shade*.85, 1. - smoothstep(0., .015, d2));
        }
    }
        ...
        if(lineFirst[1]){
            float lns = linePattern(p, tri.n, tri.n2);
            float shade = clamp(1. - d1*4., 0., 1.)*.75 + clamp(d1*32., 0., 1.)*.4;
            d1 -= lw;
            col = mix(col, vec3(0), (1. - smoothstep(0., .1, d1 - .01))*.65);
            col = mix(col, vec3(0), 1. - smoothstep(0., .015, d1 - .0225));
            col = mix(col, lCol*lns*shade*.85, 1. - smoothstep(0., .015, d1));
        }
        ...
        if(lineFirst[0]){
            float lns = linePattern(p, tri.m, tri.m2);
            float shade = clamp(1. - d0*4., 0., 1.)*.75 + clamp(d0*32., 0., 1.)*.4;        
            d0 -= lw;
            col = mix(col, vec3(0), (1. - smoothstep(0., .1, d0 - .01))*.65);
            col = mix(col, vec3(0), 1. - smoothstep(0., .015, d0 - .0225));
            col = mix(col, lCol*lns*shade*.85, 1. - smoothstep(0., .015, d0));//vec3(1, .8, .25)
        }
    ...
    if(lineID == 2.) { // Diagonal edge line.
        ...
        if(!lineFirst[2]){
            ...
            float shade = clamp(1. - d2*4., 0., 1.)*.75 + clamp(d2*32., 0., 1.)*.4;
            d2 -= lw;
            col = mix(col, vec3(0), (1. - smoothstep(0., .1, d2 - .02))*.65);
            col = mix(col, vec3(0), 1. - smoothstep(0., .015, d2 - .0225));
            col = mix(col, lCol*lns*shade, 1. - smoothstep(0., .015, d2));
        }
    }
    else if(lineID == 1.){ // Vertical edge line.
        ...
        if(!lineFirst[1]){
            ...
            float shade = clamp(1. - d1*4., 0., 1.)*.75 + clamp(d1*32., 0., 1.)*.4;
            d1 -= lw;
            col = mix(col, vec3(0), (1. - smoothstep(0., .1, d1 - .02))*.65);
            col = mix(col, vec3(0), 1. - smoothstep(0., .015, d1 - .0225));
            col = mix(col, lCol*lns*shade, 1. - smoothstep(0., .015, d1));
        }
        ...
        if(!lineFirst[0]){
            ...
            float shade = clamp(1. - d0*4., 0., 1.)*.75 + clamp(d0*32., 0., 1.)*.4;        
            d0 -= lw;
            col = mix(col, vec3(0), (1. - smoothstep(0., .1, d0 - .02))*.65);
            col = mix(col, vec3(0), 1. - smoothstep(0., .015, d0 - .0225));
            col = mix(col, lCol*lns*shade, 1. - smoothstep(0., .015, d0));//vec3(1, .8, .25)
        }
    ...
    float shade = 1.;//clamp(1.1 - tVerts*4., 0., 1.);
    tVerts -= .09;
    //verts = tri.verts - .04;
    ...
    tVerts += .06;
    col = mix(col, vec3(0), 1. - smoothstep(0., .01, tVerts - .02));
    col = mix(col, vec3(1, .8, .6)*.8, 1. - smoothstep(0., .01, tVerts)); 
