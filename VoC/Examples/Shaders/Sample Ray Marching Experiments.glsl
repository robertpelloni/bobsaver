...
// original https://www.shadertoy.com/view/tllfzf
...
float sceneSDF(vec3 p) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        p = p * rotateY(time*.3);
        p = p * rotateX(time*.7);
        p = p * rotateZ(time*1.1);
    }
    return boxSDF(p, vec3(3.1 + .9 * sin(time * 3.)));
}
...
void main(void)
{
    ...
    vec2 mouse = mouse*resolution.xy.xy / 100.;
    vec3 eye = vec3( 50. * sin(time), 50. * cos(time), 0.);
