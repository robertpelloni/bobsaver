...
// original https://www.shadertoy.com/view/3dXSDB
...
{  
    ...
    float s= time*1.;   // add "movement speed" to the "blue" strands 
    ...
    mind = length(q)+.5+1.05*(length(fract(q*.5*(3.+3.*sin(oldvar*1. - time*1.)) )-.5)-1.215);
    // only the coloring:   
