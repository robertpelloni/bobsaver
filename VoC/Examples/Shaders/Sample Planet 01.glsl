...
void main( void ) {
    ...
    vec3 cityLights=vec3(0.0,0.0,0.0);

    if (zz >= 0.0) {
    
        //Rotate about the Y axis
        float angle = (0.09 * time) * PI;
        float temp1 = spherePos.x * cos(angle) - spherePos.z * sin(angle);
             float temp2 = spherePos.x * sin(angle) + spherePos.z * cos(angle);
        spherePos.x = temp1;
        spherePos.z = temp2;
    
        //Rotate about the X axis
        //angle = (0.1 * time * 2.-1.) * TWOPI;
        //temp1 = spherePos.y * cos(angle) - spherePos.z * sin(angle);
        //temp2 = spherePos.y * sin(angle) + spherePos.z * cos(angle);
        //spherePos.y = temp1;
        //spherePos.z = temp2;
        
        float n = clouds(spherePos, 6);
        float variance = 1.0 - (n * 0.8);
        float w = clouds(spherePos * 16., 4);
        
        color = vec3(15.0/255.0, 21.0/255.0, 51.0/255.) * variance; // add water color with some minor variations
        
        float terrain = smoothstep(0.15, 0.0, n); //block out some terrain
        
        vec3 terrainColor = vec3(26./255., 57./255., 15./255.) * variance; //green
        
        // mix in mountains
        terrainColor = mix(vec3(71.0/255., 61.0/255., 19./255.) * variance, terrainColor, smoothstep(0.1, .5, 1.0-n));
        terrainColor = mix(vec3(61.0/255., 51.0/255., 39./255.) * (1.0-variance), terrainColor, smoothstep(0.1, .3, 1.0-n));
