...
void main() {
    ...
    vec3 camangle = (vec3(0.5 - mouse.y, 0.0, mouse.x * 2.0 - 1.0)) * 3.1415926535 / 2.;
    p = rot(p, camangle);
    ...
    p *= 5.;
col = clamp(col, 0., 1.);
        p.x *= (sin(col.x) - cos(col.y) - tan(col.z)) * 2.;
        p.y *= (sin(col.x) + cos(col.y) - tan(col.z)) * 3.;
    float a = atan2(p.y, p.x);
    float r = 2.0;
    float wr = 0.1;
    float wn = 1.;
    float ws = 0.;
    float c = length(p) - r - wr * cos(wn * a - ws * time) + wr * atan(1.*time) * sin(4. * a - 1.*time);
    c = 0.1 / abs(c);
    glFragColor = clamp(vec4(c*(p.x+r),1.*c,c,1.), 0., 1.);
glFragColor += asin(col / 10.);
