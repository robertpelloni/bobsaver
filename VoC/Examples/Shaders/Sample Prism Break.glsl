...
// original https://www.shadertoy.com/view/XtSGRV
...
vec3 rok = vec3(0.35);
float m;
m = length(max(abs(rotYaxis(rotXaxis(pos+vec3(0.0,-0.3,0.0),time*0.3 ),time*0.15))-rok,0.0))-0.03; // Cube
m = min (m,length(max(abs(rotYaxis(rotXaxis(pos+vec3(0.0,-0.3, 1.2),time*0.21),time*0.24))-rok,0.0))-0.03); 
m = min (m,length(max(abs(rotYaxis(rotXaxis(pos+vec3(0.0,-0.3,-1.2),time*0.2 ),time*0.3 ))-rok,0.0))-0.03); 
m = min (m,length(max(abs(rotYaxis(rotXaxis(pos+vec3(1.2,-0.3, 0.0),time*0.17),time*0.26))-rok,0.0))-0.03); 
m = min (m,length(max(abs(rotYaxis(rotXaxis(pos+vec3(-1.2,-0.3,0.0),time*0.32),time*0.2 ))-rok,0.0))-0.03); 
return m;
}
...
{
return length(max(abs(rotXaxis(pos+vec3(0.0,-0.5,0.0),time*0.47))-vec3(0.55-0.025*(kl+0.4)*sin(pos.z*pos.x*pos.y*35.)),0.0))-0.025; // Cube
}
...
float menger(vec3 pos )
{
    ...
    float s = 1.63+0.07*sin(0.53*time)-0.3*pos.y;
    for( int m=0; m<2; m++ )
    {
...
if (ef==0)           m = Boxes(p); 
if (ef==1||ef==3)  m = menger(rotYaxis(p,0.12*time));
if (ef==2)           m = Singlebox(p+0.1*kl*rand1(gl_FragCoord.xy+time));
if (ef==4)           m = min(menger(rotYaxis(p,0.1*time)),sdBox(rotYaxis(rotXaxis(p+vec3(0.,0.2,0.),time),0.2*time),vec3(0.1,0.1,0.04)-0.002*sin(p.x*p.y*440.+time))-0.01);
return min (m, d); 
}
...
float softshadow(vec3 ro,vec3 rd) 
{
    ...
    {
        if(t > 20.) continue;
        h = map(ro + rd * t)+0.003*rand1(gl_FragCoord.xy+time);
        sh = min(sh, 4.0 * h / t);
        t += h;
    }
    return sh;
...
//  blend    
    float blend=min(2.*abs(sin((0.1*time)*3.1415/3.2)),1.0); // Blende
    vec2 uv,p;

//    zoom XY    
    if (ef==1||ef==3){
    uv.x = 1.0+(mod(gl_FragCoord.x-   sin(time)*gl_FragCoord.y   -(resolution.x/2.),    ((resolution.x/4.)* (-1.5*blend+0.501) +(resolution.x/4.)))-(1.*gl_FragCoord.x)  ) / resolution.x;
    uv.y = 1.0+(mod(gl_FragCoord.y+   sin(time)*gl_FragCoord.x   -(resolution.y/2.),    ((resolution.y/4.)* (-1.5*blend+0.501) +(resolution.y/4.)))-(1.*gl_FragCoord.y)  ) / resolution.y;
    }

//     zoom Y
    if (ef==0||ef==2)
    {
    uv.x = 1.0+ (mod(gl_FragCoord.x   -(resolution.x/2.),    ((resolution.x/4.)  * (-1.5*blend+0.501)  +(resolution.x/4.)))-1.*gl_FragCoord.x) / resolution.x;
    uv.y=  1.0-(gl_FragCoord.y /resolution.y);
    }
    p = (1.-uv) * 2.0 - 1.0;

//     without effect
    if (ef==4){ uv.xy = gl_FragCoord.xy /resolution.xy; p = uv * 2.0 - 1.0;}
   
    
    p.x *= resolution.x /resolution.y;
    float theta = sin(time*0.1) * 6.28;
    float x = 3.0 * cos(theta); 
    float z = 3.0 * sin(theta);

//  camera
    vec3 ro; 
    if (ef==0||ef==2) ro = vec3(x*2.0, 2.0+2.*sin((time+37.)*0.15), z*1.4);        //camera Cubes
    if (ef==1)                 ro = vec3(x*0.2+1.0, 4.0, 0.6*z-3.);                        //camera Menger
    if (ef==4)                 ro = vec3(0.0, 0.3+0.10*time, 0.001);                //camera Tunnel
    if (ef==3)                 ro = vec3(0.0, 36.-0.24*time, 0.001);                //camera Tunnel
...
//     lightning:
    ...
    if (ef==4||ef==2||ef==1) ligvec = vec3(0.5*sin(time*0.2), 0.2, -0.5*cos(time*0.3));
...
//     postprocessing
    ...
    col-=0.04*rand1(uv2.xy*time);                                    
    col*=.9+.2*sin(2.*uv2.y*resolution.y);    
    col-=1.-dot(uv,1.-uv)*2.4;
    glFragColor = vec4(col*blend, 1.0);
