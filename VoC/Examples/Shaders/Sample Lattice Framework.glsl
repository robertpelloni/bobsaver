...
// original https://www.shadertoy.com/view/lttGDn
...
/*
    ...
    I got bored the other day and coded up a simple lattice, complete with some joins to
    jazz it up a bit. At that point, I realized that it had a bit of a "Puls 256b" feel, 
    so added in some bolts to complete the process. I've never looked at the original
    Puls code, but it'd be interesting to see if it's more efficient. At 256 bytes, it's 
...
void main(void) {
    ...
    vec3 lk = camPath(time*speed + .1);  // "Look At" position.
    vec3 l = camPath(time*speed + 1.); // Light position, somewhere near the moving camera.
