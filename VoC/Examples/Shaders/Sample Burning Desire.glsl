...
// original https://www.shadertoy.com/view/XdlSRj
...
void main(void)
{
    ...
    vec2 vXT = -1.0 + 2.0 * gl_FragCoord.xy / resolution.xy;
    vXT.x *= resolution.x/resolution.y;
    ...
    glFragColor = mix(vec4(col,1.0),colXT, pow(glowXT*2.,4.));
...
void main()
{
        ...
        glFragColor = mix(vec4(0.), col, pow(glow*2.,4.));
        //glFragColor = mix(vec4(1.), mix(vec4(1.,.5,.1,1.),vec4(0.1,.5,1.,1.),p.y*.02+.4), pow(glow*2.,4.));
