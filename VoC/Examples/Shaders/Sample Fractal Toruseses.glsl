...
// original https://www.shadertoy.com/view/sd2fRy
...
vec3 getRo() {
    ...
    vec3 ro = vec3(sin(time), cos(time), 2.);
...
float GetDist(vec3 p) {  
   ...
   //p *= 0.25;
    ...
    float mx = 1. - fract(0.25 * time);
   // mx = test(fract(0.25 * time), 5.);
...
void main(void)
{
    ...
    //if(d<MAX_DIST) {
        ...
        //col += fresnel;
        float fresnel2 = pow(1.+dot(rd, n), 2.); 
    ...
    //}
    vec3 e = vec3(1);
    vec3 col2 = vec3(0.5 + 0.2 * cos(0.25 * time),1,0.3) * exp(rd.y-0.4);//0.8 * abs(rd);//pal(cross(rd,p)., e, e, e, vec3(0,1,2)/3.);
