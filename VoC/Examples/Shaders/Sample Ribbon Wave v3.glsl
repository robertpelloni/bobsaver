...
// original https://www.shadertoy.com/view/7tsfDX
...
void main(void)
{
   ...
   for(float i=-30.; i<30.; i++){
      if(mod(i,2.)==0.){
        col2 = vec3(1.);
        n = simplex3d(vec3(i*.01,10.0,20.0));
        offset += 2.5*n;
      }
      else
        col2 = vec3(0.);    
       
      curve = uv.y - .3*pow((1.-d),2.)*sin(12.*uv.x-time + offset) + i/100.;
      col = mix(col, col2, smoothstep(curve, curve+.01, 0.0));
    }
