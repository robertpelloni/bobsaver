...
// original https://www.shadertoy.com/view/fs3SWj
...
void main(void) {
    ...
    float scl = 6.0 * global_rot.x / resolution.y;
    ...
    vec2 lcoord = vec2(0);
        ...
        // and move light
        uv = (gl_FragCoord.xy - 0.5 * resolution.xy) / (max(resolution.x, resolution.y));
    ...
    // set light pos in 3D
    vec3 lpos = vec3(lcoord * 0.5 * resolution.xy * scl, cz);
    
    // camera pos in 3D
    const vec3 cpos = vec3(0, 0, cz);

    // map frag cords to scene coords (before global rotation)
    vec2 porig = (gl_FragCoord.xy + vec2(0.13, 0.17) - 0.5*resolution.xy) * scl;
