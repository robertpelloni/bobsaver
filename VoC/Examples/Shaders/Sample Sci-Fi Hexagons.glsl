...
// original https://www.shadertoy.com/view/Mlcczr
...
void main(void)
{
    vec2 uv = 3. * (2. * gl_FragCoord.xy - resolution.xy) / resolution.y;
    uv.x += time * .25;
    ...
    glFragColor.rgb = (smoothstep(.51, .51 - aa, totalDist) + pow(1. - max(0., .5 - totalDist), 20.) * 1.5)
