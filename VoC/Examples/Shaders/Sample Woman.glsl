...
// original https://www.shadertoy.com/view/4tdcWS
...
vec2 map(in vec3 pos) {
    ...
    //  left arm
    float d1 = sdCapsuleF(pos, vec3(-0.12, 0.56, .02), vec3(-0.11, 0.3425, -.045), .035, .025, f1);
    d1 = smin(d1, sdCapsuleF(pos, vec3(-0.11, 0.32, -.05), vec3(.07, .08, -0.11), .025, .022, f1), .015);
    ...
    float d4 = sdCapsuleF(pos, vec3(-0.145, .08, -0.17), vec3(0.205, .02, -0.10), .05, .019, f0);
...
float render(in vec3 ro, in vec3 rd, in vec2 uv) {
    ...
    if(t < 10.) {
        ...
        if (pos.x > .4 || pos.z > 1.) {
            ...
            occ = calcAO(pos, nor);
            sha = calcSoftshadow(pos, lig, .02, 1.5);
        }
...
void main(void) {
    float angle = -.18+ .18*sin(time*0.5);
    float tot = 0.;
    ...
    tot += .1 * hash12(gl_FragCoord.xy/resolution.xy);
    tot *= 1.35;
    
    glFragColor = vec4(min(tot*vec3(1, .97, .92), 1.), 1.);
}
