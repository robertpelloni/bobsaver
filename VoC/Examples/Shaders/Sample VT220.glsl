...
// original https://www.shadertoy.com/view/lt2SDK
...
#define NO_OF_LINES resolution.y*HEIGHT
#define SMOOTH 0.004
...
float somePlasma(vec2 uv)
{
    ...
    float color = 0.0;
    color += 0.7*sin(0.5*uv.x + time/5.0);
    color += 3.0*sin(1.6*uv.y + time/5.0);
    color += 1.0*sin(10.0*(uv.y * sin(time/2.0) + uv.x * cos(time/5.0)) + time/2.0);
    float cx = uv.x + 0.5*sin(time/2.0);
    float cy = uv.y + 0.5*cos(time/4.0);
    color += 0.4*sin(sqrt(100.0*cx*cx + 100.0*cy*cy + 1.0) + time);
    color += 0.9*sin(sqrt(75.0*cx*cx + 25.0*cy*cy + 1.0) + time);
    color += -1.4*sin(sqrt(256.0*cx*cx + 25.0*cy*cy + 1.0) + time);
...
#ifdef MOUSE_CURVE
    float r = 1.5*exp(1.0-mouse*resolution.xy.y/resolution.y);
#else
    float r = CURVE;
#endif
...
#ifdef MOUSE_MOVE
    uvS.x -= mouse*resolution.xy.x/resolution.x - 0.5;
#endif
...
#ifdef MOUSE_MOVE
    uvE.x -= mouse*resolution.xy.x/resolution.x - 0.5;
#endif
    
    // Inner Border
    for( int i=0; i<REFLECTION_BLUR_ITERATIONS; i++)
    {
        vec2 uvR = borderReflect(uvC, 0.05) + (rand(uvC + vec2(float(i),float(i)+0.1))-0.5)*REFLECTION_BLUR_SIZE;
