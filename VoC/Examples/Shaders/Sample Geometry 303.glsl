...
// original https://www.shadertoy.com/view/cs23DK
...
float map(vec3 p)
{
    ...
    p = fold(p,t);
    p.z-=1.;
    //p=rot(p,vec3(0,0,1),-mix(PI/2.,PI/18.,sin(time)*.5+.5));
    p=rot(p,vec3(0,0,1),-PI/2.*(sin(time)*.5+.5));
      vec3 a=vec3(.05,1.,.05);
    return length(p-clamp(p,-a,a))-.05;
}
