...
// original https://www.shadertoy.com/view/WdVyRd
...
void main(void)
{
    ...
    for(int i = 0; i < nbBall; ++i)
    {
        ...
        ballArray[i] = vec2(    triangleSignal(LateralOffset + LateralSpeed * time, ratio - ballRadius),
                                   sinAmplitude * (abs(sin(phaseOffset + phaseSpeed * time)) - 0.5) 
                           );
        float a = smoothFilter(sdCircle( ballArray[i] - p , ballRadius));
        col = mix(col, drawColor, a);           
    }
    
    // Sort Ball (Lame bubble)
