...
// original https://www.shadertoy.com/view/3dfGR2
...
#define SAMPLES (12+min(0,frames))
...
vec3 color(in ray r) {
    ...
    for (int i=0; i<MAX_RECURSION && world_hit(r, EPSILON, MAX_FLOAT, rec); i++) {
        ...
        ray shadowRay = ray(rec.p + rec.normal * 0.01, L);
...
void main(void) {
    ...
    vec4 frag_color = glFragColor;
    
    float aspect = resolution.x/resolution.y;
#ifdef MOVE_CAMERA
    vec3 lookfrom = vec3(278. + sin(time * .7)*200., 278, -800. + sin(time)*100.);
#else
    ...
    for (int i=0, l = SAMPLES; i<l; i++) {
        vec2 uv = (frag_coord + hash2(g_seed))/resolution.xy;
