...
// original https://www.shadertoy.com/view/lt3cz7
...
// thoroughly later, but in short, it's just a bunch of fiddly calls to create the gears objects. 
...
// which means time needs to be in a ratio of: 1./18: 1./15. : 1./12. You can see that implemented
// below.
//
...
// (the "mod(time/1.5" lines) in a hurry to get things working, but I'll take a closer look later 
...
float dist(vec3 p, float r1, float r2){
    ...
    //p.xy *= rot2(mod(time/1.5, 3.*6.2831/15.) + 6.2831/2.);
    p.xy *= rot2(mod(time/1.5, 6.2831/5.) + 3.14159265);
...
float dist2(vec3 p, float r1, float r2){
    ...
    ///p.xy *= rot2(mod(time/1.8, 3.*6.2831/18.) + 3.*6.2831/18.);
    p.xy *= rot2(mod(time/1.8, 6.2831/6.) + 6.2831/6.);
...
float dist3(vec3 p, float r1, float r2){
    ...
    // This is the same as the two function above, but the smaller cogs cross a boundary line
    // and directional polarity has to be considered.
    float dir = p.x < 0.? -1. : 1.;
    
    p.x = abs(p.x);
    vec3 q2 = p;
    p.xy *= rot2(mod(time/1.2*dir + 3.14159/12., 6.2831/12.) + 5.*6.2831/12.); // + 5.*3.14159/12.
    ...
    q = q2; 
    q.xy *= rot2(mod(time/1.2*dir, 6.2831/6.) + 2.*6.2831/6.);
    a = atan(q.y, abs(q.x));
    ia = floor(a/6.2831*6.) + .5;
    q.xy = rot2(ia*6.2831/6.)*q.xy;
...
void main(void)
{
    ...
    vec2 p = (gl_FragCoord.xy - resolution.xy*.5)/min(850., resolution.y);
    ...
    rd.xy *= rot2(sin(time/8.)*.2);
    rd.xz *= rot2(sin(time/4.)*.1);
    ...
    glFragColor = vec4(sqrt(clamp(sceneCol, 0., 1.)), 1);
}
