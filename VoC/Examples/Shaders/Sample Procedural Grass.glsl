...
// original https://www.shadertoy.com/view/MsV3D3
...
void main(void) //WARNING - variables void (out vec4 result, in vec2 gl_FragCoord) need changing to glFragColor and gl_FragCoord
{
    //result = vec4(0, 0, 0, 1);
    //result = vec4(0, 0, 0, 1);
    vec2 fragCoord=gl_FragCoord.xy;
    fragCoord -= resolution.xy * 0.5;
    vec2 uv = fragCoord.xy / resolution.y;
     
    vec3 c = mix(vec3(0.53, 0.63, 0.78), vec3(0.42, 0.52, 0.65), uv.y - uv.x * 0.5) * SKY_MULT;
    
    float elevation = 0.0;//mouse*resolution.xy.y / resolution.y;
