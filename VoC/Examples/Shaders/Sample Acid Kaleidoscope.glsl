...
// original https://www.shadertoy.com/view/XlSGzD
...
vec2 getTransform(in vec2 p, int which)
{
    if (which == 0) {
        ...
        return mapRects(rotate(time2*.3)*p*.5);
    } else {
        return rotate(time2*.3)*mapRects(mapRects(p));
    }
}
...
{
    return .2*sin(25.*th)*sin(time2);
}

vec4 tunnel(float th, float radius)
{
    return gradient(offset(th) + 2.*log(radius) - time2);
}
...
void main(void)
{
    ...
    glFragColor = tunnel(atan(p.y, p.x), 2.0 * length(p));
}
