...
// original https://www.shadertoy.com/view/lslXDn
...
vec3 hexgrid(vec2 p) {
    ...
    float pulse1 = smoothstep(-1.0, +1.0,
        sin(cdist+time*0.412-p.y*PI*0.11-p.x*PI*0.2));
    float pulse2 = smoothstep(-1.0, +1.0,
        sin(cdist+time*0.842-p.y*PI*0.42-p.x*PI*0.1));
    vec3 result = vec3(0.3, 0.8, 0.2);
    result += vec3(0.062, 0.41, 0.03)*pulse1;
    result += vec3(0.008, 0.23, 0.04)*pulse2;
    
    // add another pulse
    float pulse3 = (sin(time*3.0-p.y*0.2-p.x*PI*0.1)*.5+.5);
...
void distort(inout vec2 p) {
    p+=p*length(p)*0.9;
    p = vec2(atan(p.x, p.y)/PI*3.0, log(length(p))+time*0.15);
    p.x += p.y/PHI + time*0.1;
}
...
void main(void) {
    ...
    vec2 mouse2=(mouse.xy / resolution.xy - 0.5) * 3.0;
    //if (iMouse.z < 1.0) mouse = vec2(0.0);
    
    mat2 rotxz = rot(time*0.652+mouse2.x);
    mat2 rotxy = rot(sin(time*0.814)*.3+mouse2.y);
    ...
    #ifdef BLUE
    glFragColor.rgb = glFragColor.rbg;
    #endif
    
    #ifdef RED
    glFragColor.rgb = glFragColor.grb;
    #endif
}
