...
// original https://www.shadertoy.com/view/WlcyD7
...
// 'TIE Fighters' dean_the_coder (Twitter: @deanthecoder)
...
Hit sdWings(vec3 p) {
    ...
    p.z = abs(p.z) - 2.3;
    return Hit(min(
        sdTri(p, vec3(0), vec3(2, 3, 0), vec3(-2, 3, 0)),
        sdTri(p, vec3(0), vec3(3.3, 0, 0), vec3(2, 3, 0))) - .03,
        2, p);
}
...
Hit sdTerrain(vec3 p) {
    p.z -= T * 700.; // Number controls the terrain speed.

    vec2 d = sin(p.xz * vec2(.01476, .01345)) * 10. +
             sin((p.xz) * vec2(.05212, .04512)) * 2.;
...
float ao(vec3 p, vec3 n, float h) {
    return clamp(sdTies(p + h * n).d / h, 0., 1.);
}
...
vec3 lights(vec3 p, vec3 rd, float d, Hit h) {
    const vec3 sunCol = vec3(2, 1.8, 1.7);
    float sped = 4., am;
    vec3 c, n, ld = normalize(vec3(30, 50, -40) - p);
    ...
    if (h.id == 4) { // Snow
        ...
        if (h.id == 1 || h.id == 6) { // Metal
            c = vec3(.3 - n31(h.uv * 18.7) * .1);
            sped = .5;

            if (h.id == 6) // Cockpit.
                c *= 1. - .8 * step(abs(atan(h.uv.y, h.uv.z) - .8), .01);
        } else if (h.id == 2) { // Black wing area.
    ...
    // Primary light.
    float ldt = dot(ld, n),
          l = max(0., .2 + .8 * ldt) + // Primary light.
              max(0., .2 - .8 * ldt) * .3, // Bounce light.

    // Specular.
    spe = smoothstep(0., 1., pow(max(0., dot(rd, reflect(ld, n))), 50.)) * sped,

    // Shadow.
    sha = mix(.4, 1., calcShadow(p, ld));

    // Combine.
    vec3 lig = l * am * sha * sunCol +
               clamp(n.y, .05, 1.) * vec3(.9, .95, 1); // Sky light.
    return c * lig + spe;
}
...
vec3 march(vec3 ro, vec3 rd) {
    ...
    for (float i = 0.; i < 120.; i++) {
        ...
        if (abs(h.d) < .0015 || d > 600.)
            break;

        d += h.d; // No hit, so keep marching.
    }

    float gg = g; // Cache the 'glow'.
    vec3 c;
    if (d > 600.)
        c = vec3(.85, .9, 1);
    else
        c = mix(lights(p, rd, d, h), vec3(1), smoothstep(200., 540., d));
    c += gg * vec3(0, 1, 0);
    ...
    if (h.id == 3 || h.id == 1) {
        ...
        c = mix(c, d > 300. ? vec3(1) : lights(p, rd, d, h), alpha);
    }
...
void main(void)
{
    ...
    glFragColor = vec4(
                     vignette(pow(march(ro,
                         getRayDir(ro, lookAt, (gl_FragCoord.xy - .5 * resolution.xy) / resolution.y)),
                         vec3(.45)), gl_FragCoord.xy), 0);
}
