...
const float ra = 12;
const float rr = 3;
const float b1 = 0.357;
const float b2 = 0.365;
const float s1 = 0.098;
const float s2 = 0.445;
...
void main(void)
{
    ...
    for(float dx=-ra; dx<=ra; dx++) for(float dy=-ra; dy<=ra; dy++)
    {
        ...
        float val = texture(backbuffer, txy).x; 
    ...
    if(frames < 3)
        //c = hash13(vec3(gl_FragCoord,frames)) - texture(backbuffer, uv).x + 0.5;
        c = hash13(vec3(gl_FragCoord.xy,frames)) * 0.75;
