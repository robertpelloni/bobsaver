...
// original https://www.shadertoy.com/view/wtt3WX
...
vec4 eye(vec2 uv, vec2 up, vec2 spot1, vec2 spot2, float scale)
{
    ...
    float aa = scale * 4.0 / resolution.x;
...
vec4 flower(vec2 uv, vec2 id, float minSz, float maxSz, float scale)
{
    ...
    uv.x += 0.1 + rnd.x * 0.5;
    uv.y += 0.25 + rnd.y * 0.2;
    uv.y += sin(time * mix(0.4, 1.0, rnd.y) + (rnd.w * 45.0)) * 0.2;

    
    float aa2 = scale * 3.0 / resolution.x;
    ...
    else
    {
        ...
        if (centerDst < 0.36)
        {
            ...
            float aa = scale * 7.0 / resolution.x;
            float bar = thick * 1.7;
...
void main(void)
{
    ...
    float sc = 1.65 + cos(time* 0.1) * 0.25;
