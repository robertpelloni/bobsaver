...
// from http://www.shadertoy.com/view/4sXGDj
...
void MakeViewRay(out vec3 eye, out vec3 ray, in vec2 gl_FragCoord)
{
    vec2 ooR = 1./resolution.xy;
    vec2 q = gl_FragCoord.xy * ooR;
    vec2 p =  2.*q -1.;
    p.x *= resolution.x * ooR.y;
...
vec3 Light(vec3 X, vec3 n, vec3 V)
{
    ...
    //c = pow(textureCube(tex0,n).xyz,vec3(2.2));
    c=vec3(0); //ZZ
...
{
    vec3 viewP, viewD;
    MakeViewRay(viewP, viewD, gl_FragCoord.xy);
...
#if 0    
//    float debug_height = sin(time)*10.0;
    float debug_height = sin(time);
