...
// original https://www.shadertoy.com/view/wtBGDc
...
float scene(vec3 p) {
    vec3 point = vec3(abs(p.xy), p.z+0.03+sin(time*3.0)*0.005);
...
float wake(vec2 uv) {
    ...
    float dst = xwiggly*(wakeangledot > 0.0 ? abs(wakeangleflippeddot) : length(uvm));
    // if (wakeangleflippeddot > 0.0) return 0.0;
    return sign(wakeangleflippeddot)*sin(dst*120.0+time*2.0)*exp(-dst*20.0-wakeangledot*5.0);
}
...
float randomWave(vec2 uv) {
    ...
    float wave = 0.0;
    for (int j = 1; j < 20; j++) {
        wave += sin(float(j)*dot(uv, normalize(getVec3()).xy)+getFloat()*3.14*2.0+time*0.2*sqrt(float(j)))/pow(float(j+1),1.5);
    }
...
float heightmap(vec2 uv) {
    ...
    float height = randomWave(uv*2.0+vec2(time/4.0,0.0))*0.12;
    // float maxdist = 0.05;
...
void shadeBoat(inout Ray ray) {
    //this code is super spaghetti and I'm so %&*$ing sorry
...
void main(void) {
        ...
        vec2 uv_base = (gl_FragCoord.xy/resolution.xy*2.0-1.0)/2.0;
        uv_base.x *= resolution.x/resolution.y;
        ...
        float hightoffset = 0.0;
        //if (mouse*resolution.xy.z > 0.0) {
        //    angle = 3.0*3.14*mouse.x*resolution.xy.x/resolution.x;
        //    hightoffset = mouse.y*resolution.xy.y/resolution.y*2.0-1.0;
        //}

        for (int i = 0; i < SAMPLES; i++) {
            vec2 uv = uv_base + getVec3().xy/500.0;
            vec3 cameraOrigin = vec3(5.0*sin(angle), 5.0*cos(angle), hightoffset + heightmap(vec2(3.5, 3.5))+1.5) + normalize(getVec3())*0.04;
        ...
        glFragColor = vec4(pow(log(col+1.0), vec3(1.3))*1.25, 1.0); //colour grading
