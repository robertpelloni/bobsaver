...
// original https://www.shadertoy.com/view/Ms23zw
...
{//voronoi grass and fbm clouds
    ...
    vec2 f=voronoi(z0.xz*10.0+sin(y*10.0+z0.zx)*0.1*y+vec2(abs(sin(time2*5.0+0.5*z0.z)),sin(time2*6.6+z0.x))*y*y*2.0);
    float d=f.x*0.1-0.02+(y+fract(sin(f.y)*43758.5453123))*0.02;
    d=min(max(d,y-1.0),y);
    float dC=fbm(z0+vec3(time2*0.25))*0.5+sin(z0.z*0.4)*0.1+abs(z0.y-4.0)*0.58-0.4;
    return min(d,dC);
}
