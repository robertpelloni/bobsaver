...
// original https://www.shadertoy.com/view/WtBGDz
...
/*
    ...
    The forground distance field is just some simple Truchet tiling, and the
    ...
    show that it's possible to render in a lot of the static vector graphic styles 
...
float dist(vec2 p, float rnd){
    ...
    // Leaves.
        ...
        p = rot2(6.2831*rnd + fract(rnd*57. + .37)*time/2.)*p;
...
void main(void) {
    ...
    vec2 uv = (gl_FragCoord.xy - resolution.xy*.5)/min(800., resolution.y);
    
    // Scaling and translation.
    float gSc = 5.;
    vec2 p = uv*gSc + vec2(.5, 0)*time;
    // Rotation, for something different.
    //vec2 p = rot2(3.14159/6.)*(uv*gSc + vec2(.5, 0)*time);
...
///////   
    ...
    for(int j = 3; j>=-3; j--){ // Top to bottom rendering, for the 3D distance look.
        for(int i = 3; i>=-3; i--){ // Right to left.
            ...
            vec2 s = uv*gSc*sc - vec2(1., .5)*time;
            ...
            ofs = sin(ofs*6.2831 + vec2(1.57, 0) + time/2.);
