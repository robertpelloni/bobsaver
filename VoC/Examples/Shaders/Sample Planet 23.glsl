...
// original https://www.shadertoy.com/view/3lXfRl
...
vec3 getColorForCoord() {
    ...
    vec3 w = normalize(vec3(gl_FragCoord.xy - resolution.xy * 0.5, (resolution.y) / (-2.0 * tan(verticalFieldOfView / 2.0))));
    ...
    if (t < inf) {
        ...
        q = vec3(fbm(X + 0.025*time), fbm(X), fbm(X));
        r = vec3(fbm(X + 1.0*q + 0.01*time), fbm(X + q), fbm(X + q));
...
void main(void)
{
    ...
    glFragColor.rgb =
        getColorForCoord(gl_FragCoord.xy) * 3. -
        getColorForCoord(gl_FragCoord.xy + vec2(1.0, 0.0)) * 0.5 -
        getColorForCoord(gl_FragCoord.xy + vec2(0.0, 1.0)) * 0.5 -
        getColorForCoord(gl_FragCoord.xy - vec2(1.0, 0.0)) * 0.5 -
        getColorForCoord(gl_FragCoord.xy - vec2(0.0, 1.0)) * 0.5;
#else
    // just use a single pass
    glFragColor.rgb = getColorForCoord();
#endif
    
    float alpha = (glFragColor.r + glFragColor.g + glFragColor.b)/3.0;
    glFragColor.a = alpha;
}
