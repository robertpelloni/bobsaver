...
vec2 inObj(in vec3 p){
    ...
    float cos1x = cos(time*modder*9.0);
    float sin1x = sin(time*modder*8.0);
    float cos2x = cos(time*modder*7.0);
    float sin2x = sin(time*modder*6.0);
    float cos3x = cos(time*modder*5.0);
    float sin3x = sin(time*modder*4.0);
    float cos4x = cos(time*modder*3.0);
    float sin4x = sin(time*modder*2.0);
...
void main(void){
  ...
  if (f<MAX_DEPTH){
    c=vec3(1.0,0.1,0.4);
      c.x = c.x*sin(f+cos(f+time*1.2)+time*1.1)*0.9;
      c.y = 0.1;
      c.z += cos(f+sin(f+time*2.1)+time*5.3)*0.8;
      c.x += 0.2;
      c.z += 0.4;
    float b=max(dot(n,normalize(E-p)),0.1);
    glFragColor=vec4((b*c+pow(b,100.0))*(1.0-f*.001),1.0);//simple phong LightPosition=CameraPosition
  }
  else glFragColor=vec4(0,0,0.0,1); //background color
}
