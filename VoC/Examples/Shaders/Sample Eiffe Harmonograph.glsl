...
// original https://www.shadertoy.com/view/Xls3zf
...
float DE(in vec3 p0){
    ...
    dB=min(dB,min(dPen,min(dPenP,min(dPaperP,dPenA))));
    if(id<0){
        if(dW<dB && dW<dPaper)id=1;
        else if(dB<dPaper)id=2;
        else id=3;
    }
    return min(dW,min(dB,dPaper));
}
...
vec3 Color(vec3 ro, vec3 rd, float t, float px, vec3 col){
    ...
    if(id==1){sc=vec3(0.3,0.3,0.3);//sc=texture2D(tex,ro.zx+vec2(0.5)).rgb;
    }else if(id==2){sc=vec3(0.8,0.4,0.2);
    }else sc=HarmonographIt((ro.xz+vec2(0.0,0.5)-paper.xz*0.5)*10.0, mod(time,20.0)-0.1,px*t);
    float h=0.2*dot(N,R);
    sc*=vec3(0.8+h,1.0,0.8-h);
    float sh=clamp(shadao(ro,L,px*t)+0.4,0.0,1.0);
...
void main() {
    ...
    vec3 rd=lookat(vec3(0.0,0.4,-0.5)-ro)*normalize(vec3((2.0*gl_FragCoord.xy-size.xy)/size.y,3.0));
    //ro=eye;rd=normalize(dir);
    float t=DE(ro)*rndStart(),d=0.0,dm=10.0,tm=0.0;
    vec3 col=Sky(rd);
    for(int i=0;i<64;i++){
        t+=d;
        d=DE(ro+rd*t);
