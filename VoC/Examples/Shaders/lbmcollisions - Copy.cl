__kernel void LBMCollisionsKernel( __global float * n0,
							       __global float * nN,
							       __global float * nS,
							       __global float * nE,
							       __global float * nW,
							       ...
							       __global float * xvel,
							       __global float * yvel,
							       __global float * speed2,
							       __global float * barrier,
							       __global float * omega)
{	
	int index=get_global_id(0);
    if (barrier[index]<0.5) {
		float n=n0[index]+nN[index]+nS[index]+nE[index]+nW[index]+nNW[index]+nNE[index]+nSW[index]+nSE[index];
		density[index]=n;
		float one9th=1.0/9.0;
		float one36th=1.0/36.0;
		float one9thn=one9th*n;
		float one36thn=one36th*n;
		float four9ths=4.0/9.0;
		float vx=0.0;
		float vy=0.0;
