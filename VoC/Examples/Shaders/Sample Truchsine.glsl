...
// original https://www.shadertoy.com/view/stlBDl
...
void main(void)
{
    ...
    for(float i=0.;i<64.;i++){
        vec2 d = sdf(rp);
        if(d.y ==2. && d.x <.5){
            float act = step(.5,sin(rp.z-time));
               if(act==0.) {
                    rp+=rd*.1;
                    continue;
               }
                acc += vec3(cos(rp.z+rp.x)*.5+.5,.1,sin(rp.z)*.5+.5)*act*exp(fract(time+floor(rp.z)*.5)*-abs(d.x)*i*i)/20.;
    ...
    glFragColor = vec4(col+sqrt(acc),1.0);
}
