...
// original https://www.shadertoy.com/view/WsscDl
...
{
    float cv = (cycle(time, 60.0, 1.0, 0.5, 0.5) + cycle(time, 13.44, 1.0, 1.0, 0.25))/2.0;
    ...
    vec3 d2 = hash3(vec3(-1.0, -13.0, 0.0)) * time * 2.25;
...
void main(void)
{
    ...
    vec2 clip_coord = ((gl_FragCoord.xy / resolution.xy) - 0.5) * 2.0;

    mat3 view = mat3(cos(time), -sin(time), 0.0,
    ...
    //vec3 Ld = normalize(vec3(0.2 * cos(time), 0.2 * sin(time), 0.1));
    ...
    for (i = 0.0; i < st0_count; i += 1.0)
    {
        ...
        t = t0 + dt0*i*
            (1.0 -
             hash3(vec3(fract(gl_FragCoord.xy*time*1.5), time*20.0)).y
             *ev0);
        vec3 p = ro + t*rd;
        ...
        {
            float st1_t = (j + 1.0 + 0.152*(hash3(vec3(gl_FragCoord.xy*0.001, 0.0)).x * 0.5 + 0.5))*dt1;
