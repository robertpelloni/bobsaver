...
// original https://www.shadertoy.com/view/4slSWf
...
// Uncomment the following define to see Bender under regular 3D lighting
...
vec4 map( vec3 p )
{
    ...
    d1 = opU( d1, sdCone(     (p-vec3(2.2,1.75,1.0))*vec3(1.0,-1.0,-1.0), vec3( normalize(vec2(0.8,0.4)), 0.45 ) ) );
    ...
    // arms
    vec3 d4;
    d4 =          vec3( sdSphere( q-vec3( 0.88,0.15,0.0), 0.17 ), 0.0, 15.0 );
    d4 = opU( d4, vec3( sdBezier( vec3(0.9,0.15,0.0), vec3(2.0,-0.5,0.0), vec3(2.2,2.0,1.0), q, 0.11 ), 16.0) );
...
void main( void )
{
    ...
    float sep = resolution.x * smoothstep( 0.5, 0.8, cos(1.57+0.25*6.2831*time));
    //if( iMouse.z>0.0 ) sep = iMouse.x;
    float doCartoon = step( sep, gl_FragCoord.x ); 
    
    //-----------------------------------------------------
    // camera
    //-----------------------------------------------------
    float an = 5.7 + 0.62831*time;
    vec3 po = vec3(1.0 + 3.5*sin(an),0.6,3.5*cos(an));
    vec3 ta = vec3(1.0,1.5,0.0);

    vec3 ro,  rd;  generateRay( ro,  rd,  po, ta, gl_FragCoord.xy );
    vec3 ro2, rd2; generateRay( ro2, rd2, po, ta, gl_FragCoord.xy+vec2(2.0,0.0) );
    vec3 ro3, rd3; generateRay( ro3, rd3, po, ta, gl_FragCoord.xy+vec2(0.0,2.0) );
    ...
    // separator
    col *= smoothstep( 1.0, 2.0, abs(gl_FragCoord.x-sep) );
