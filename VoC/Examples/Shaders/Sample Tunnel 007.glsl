...
vec2 pos = gl_FragCoord.xy / resolution.xy * 2.0 - vec2(1.0);
   ...
   for(int i = n; i > 0; i -= 1)
   {
      ...
      {
         glFragColor.rgb = mix(glFragColor.rgb, col * 0.4, g);
      }
      else
      {
         glFragColor.rgb = mix(glFragColor.rgb,
   ...
   if(point.y < -0.7)
      glFragColor.rgb = mix(fluid_fog_colour, glFragColor.rgb,
         fluid_fog_amount);

   glFragColor.rgb = mix(fog_colour, glFragColor.rgb,
         fog_amount);
   ...
   if(light_pos.z > 0.0)
{
   ...
   glFragColor.rgb +=  vec3(1.1, 1.0, 0.7) * clamp((1.0 - light_pos.z / 3.5) * 20.0, 0.0, 1.0) * vec3(0.1 /
         (light_pos.z * distance(pos.xy * vec2(1.3333, 1.0), light_pos.xy)));
}

glFragColor.rgb = pow(glFragColor.rgb, vec3(0.6));
}
