...
// original https://www.shadertoy.com/view/MsXGR2
...
float speed,ground_x,ground_y,ground_z;
...
void main(void)
    {
    speed=time;
    ground_x=0.0;//+0.125*sin(PI*speed*0.25);
    ground_y=0.0;//+0.125*cos(PI*speed*0.25);
    ground_z=4.0*sin(PI*speed*0.0625);//+speed*0.5;

    vec2 position=(gl_FragCoord.xy/resolution.xy);
    vec2 p=-1.0+2.0*position;
    vec3 dir=normalize(vec3(p*vec2(1.0/resolution.y*resolution.x,1.0),1.0));    // screen ratio (x,y) fov (z)
    ...
    color=vec3(color.x*0.9-0.1*cos(p.x*resolution.x),color.y*0.95+0.05*sin(p.y*resolution.x/2.0),color.z*0.9+0.1*cos(PI/2.0+p.x*resolution.x));
    /* return color */
