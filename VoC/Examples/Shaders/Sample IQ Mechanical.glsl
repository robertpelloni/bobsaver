...
// original https://www.shadertoy.com/view/XslXW2
...
void main(void)
{
    ...
    for( int i=0; i<numSamples; i++ )
    {
        ...
        float time2 = time - nt*(1.0/24.0);
        
        float si = mod( floor(time*0.25), 2.0 );

        
        float ftime = pow(clamp(4.0*fract(time2),0.0,1.0),4.0);
        float itime = floor(time2);
        float atime = (itime + ftime)*1.0 + time2;
        
        vec2 ce = vec2(0.0,0.0);
        ce.x = -0.3 + 1.25*(ftime - fract(time2));

        ce.y = 0.15-0.15*smoothstep(0.0,0.3,abs(fract(time2+0.65)-0.5))*(1.0-si);
