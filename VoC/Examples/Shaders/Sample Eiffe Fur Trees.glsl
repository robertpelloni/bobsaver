...
// original https://www.shadertoy.com/view/lts3zr
...
vec3 mcol=vec3(0.0);//material color
...
float DE(in vec3 z0){
    ...
    if(dt<d){
        d=dt;
        mcol=vec3(0.5,0.1,0.0)*cyl;
    }else{
        mcol=vec3(0.5,0.6,0.2)*(cyl+z0.y*0.38+0.41);
...
void main(){
    ...
    if(rd.y<0.0){
        ro+=rd*tG;
        float s=1.0,dst=0.1;
        t=DE(ro)*rndStart(gl_FragCoord.xy);
        for(int i=0;i<4;i++){
