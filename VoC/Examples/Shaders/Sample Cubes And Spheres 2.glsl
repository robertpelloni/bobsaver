...
void main(void){
  ...
  vec3 finalColor=vec3(c1.xyz*0.75+c2.xyz*0.25)*(1.0/maxaa)+backpixel.xyz*((maxaa-1.0)/maxaa);
  finalColor+=rand3d_1d(vec3(position*time,time))*(0.5/maxaa); //some noise helps erase old images, just 8 bits color depth...
