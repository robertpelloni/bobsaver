...
// original https://www.shadertoy.com/view/wlfGzB
...
float DE(vec3 z){z+=noyz(z*200.0)*0.01; 
 ...
 float tim=(time2+float(i)),arm=sin(float(i))*0.25;//0.2 
...
void main(void) { //WARNING - variables void (out vec4 O, in vec2 U){ need changing to glFragColor and gl_FragCoord
  ...
  vec3 ro=vec3(0.1,-0.25,-3.0),rd=normalize(vec3((2.0*U.xy-resolution.xy)/resolution.y,0.4));
  time2=time*6.0; 
  float t=DE(ro)*noyz(vec3(U.xy,time2)),d,a=0.0; 
  for(int i=0;i<48;i++){ 
    t+=d=DE(ro+t*rd); 
    a+=0.02;
    if(t>6.0 || d<0.01)break; 
  }
  t*=1.0/6.0*(noyz(rd*2.0+vec3(0.0,-time2,time2))+noyz(rd*7.3+vec3(0.0,-time2*0.3,0.0))); 
  glFragColor=vec4(mix(vec3(0.3,0.0,0.3+rd.y*0.5),vec3(0.3,0.9,0.4),(t*t+a)*0.5)*pow(dot(rd,vec3(0.3,0.1,1.0)),3.0),1.0);
}
