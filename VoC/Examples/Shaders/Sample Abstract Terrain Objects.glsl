...
// original https://www.shadertoy.com/view/ttXGWH
...
vec3 getSky(vec3 ro, vec3 rd, vec3 lp){
    ...
    vec3 p = (ro + rd*FAR)/1. + vec3(0, 0, time);
...
void main(void) {
    ...
    // Camera Setup.
    vec3 ro = vec3(0, 1.25, time*2.); // Camera position, doubling as the ray origin.
    ...
    */
    ...
    float overlay = 1. + .35*sin(u2.x*3.14159*resolution.y/1.5);
    overlay *= 1. + .35*sin(u2.y*3.14159*resolution.y/1.5); 
    ...
    */
    ...
    glFragColor = vec4(sqrt(max(sceneColor, 0.)), 1);
}
