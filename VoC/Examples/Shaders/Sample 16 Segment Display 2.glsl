...
// original https://www.shadertoy.com/view/ltfXD2
...
void main(void)
{
    vec2 uv = ( gl_FragCoord.xy / resolution.xy ) * 16.0;
    ...
    float achar = 0.0; //Character to print
    ...
    float anim_time = clamp(time * 0.3, 0.0, 1.0) * float(NUM_CHARS);
    
    achar = mix(0.0, achar, clamp(anim_time - index, 0.0, 1.0));
    
    float dist = dchar(achar, uvc);
