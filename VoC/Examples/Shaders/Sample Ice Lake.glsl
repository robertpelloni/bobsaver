...
// original https://www.shadertoy.com/view/MtlSz8
...
{
    vec2 dir = ((mouse.xy/resolution.xy) * vec2(1.0,-0.5) + vec2(0.0,0.5)) * 6.2831;
...
vec3 doBackground( in vec3 dir)
{
    ...
    float sun = pow(dot(dir,normalize(vec3(0.2,0.2,0.8)))*0.5+0.5,32.0);
    vec2 p = dir.xz/dir.y+vec2(time*0.05,0.0);
    float clouds = (noise(p*6.0)*0.5+noise(p*16.0)*0.3+noise(p*32.0)*0.2) * noise(p) * pow(1.0-sky,4.0);
...
{
    float snow = min(pow(noise(p.xz)*0.6+noise(p.xz*2.0)*0.25+noise(p.xz*4.0)*0.15+0.4,64.0),1.0);
