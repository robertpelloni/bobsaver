...
// original https://www.shadertoy.com/view/MtdXRS
...
vec3 texrand(in vec2 n, in float mipLevel) {
    //return texture2D(iChannel2, n, mipLevel).xyz;
...
SDResult sceneDist(in Ray ray) {
    SDResult result;
    float t = time + 10.0;
    R(ray.origin.yz, cos(ray.origin.z - t*0.9) / t);
...
vec3 backgroundColour (in Ray ray, in float divergence) {
   ...
   // base = mix(vec3(0.05, 0.1, 0.2), vec3(0.0), x);
    ...
    if (ray.dir.y >= 0.0) {
        ...
        vec2 floorCoord = floorPos(ray, -30.0) + vec2(0, time * 100.0);
...
debugValue += glassTravelDist / 2.0;
                    ...
                    colour = mix(
                        colour, 
                        textureCube(iChannel1, reflect(ray.dir, n)), 
                        vec4(fresnel) * impact);
*/
                    colour = mix(
                        colour,
...
//debugValue += 0.5;
   ...
   // colour += textureCube(iChannel0, ray.dir) * impact;
...
vec3 camPath(in float time) {
    ...
    vec3 camPos = vec3(sin(time * 0.3) * 2.0, sin(time * 0.1) * 2.0 + 4.0, time);
