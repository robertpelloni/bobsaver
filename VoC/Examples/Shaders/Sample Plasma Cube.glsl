...
// original https://www.shadertoy.com/view/4d3SRN
...
void main(void) {
    ...
    glFragColor = vec4(0.0);
      ...
      vec3 c=vec3(0.0), p=vec3(0.0), N, vuv, vrp, prp;    
    ...
    if (f < maxd){
        ...
        glFragColor += vec4(max((b * c + pow(b, 64.0)) * (1.0 - f *.01), 0.0), 1.0);
        
        
      } else {
        
        glFragColor = vec4(vec3(normalize(scp) / 2.), 0.) + glow ;//* vec4(1., 1., 0.,1.);// vec4(boxmap(-u * scp * rotationZ(k / 4.) * rotationX(k), 1.)) / 4. + glow;
    }
}     
