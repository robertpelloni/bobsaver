...
//original https://www.shadertoy.com/view/Mll3WB
...
vec4 scene(in vec3 ro, in vec3 rd){
    ...
    float rnd=fract(sin(gl_FragCoord.x+cos(gl_FragCoord.y))*4317.6219);
    float t=rnd,d;
    ro+=rd*t;
    for(int i=0;i<32;i++){
        t+=d=DE(ro);
...
void main() {
    ...
    vec3 rd=lookat(-ro,vec3(0.0,1.0,0.0))*normalize(vec3((2.0*(gl_FragCoord.xy)-rez.xy)/rez.y,1.0));
