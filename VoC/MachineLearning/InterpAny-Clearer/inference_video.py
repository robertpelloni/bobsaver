...
if __name__ == '__main__':
    ...
    parser.add_argument('--fps', type=int, help='fps of output movie')
    ...
    if args.model == 'RIFE':
        ...
        if args.variant == 'T':
            ...
            cmd = 'python inference_video_plus.py --video "{}" --save_dir "{}" --checkpoint {} --fps {} --num {}'.format(
                args.video, args.save_dir, args.checkpoint, args.fps, ' '.join([str(x) for x in args.num])
            )
        if args.variant == 'D':
            args.checkpoint = osp.join(args.checkpoint, 'train_sdi_log')
            cmd = 'python inference_video_plus_sdi.py --video "{}" --save_dir "{}" --checkpoint {} --fps {} --num {}'.format(
                args.video, args.save_dir, args.checkpoint, args.fps, ' '.join([str(x) for x in args.num])
            )
        if args.variant == 'TR' or args.variant == 'DR':
            args.checkpoint = osp.join(args.checkpoint, 'train_sdi_log')
            cmd = 'python inference_video_plus_sdi_recur.py --video "{}" --save_dir "{}" --checkpoint {} --fps {} --num {} --iters {}'.format(
                args.video, args.save_dir, args.checkpoint, args.fps, ' '.join([str(x) for x in args.num]), args.iters
            )
        os.system(cmd)
    if args.model == 'IFRNet' or args.model == 'AMT-S':
        ...
        if args.variant == 'T':
            cmd = 'python inference_video_plus.py --video "{}" --save_dir "{}" --checkpoint {} --fps {} --cfg_name {} --num {}'.format(
                args.video, args.save_dir, args.checkpoint, args.fps, cfg_name, ' '.join([str(x) for x in args.num])
            )
        if args.variant == 'D':
            cmd = 'python inference_video_plus_sdi.py --video "{}" --save_dir "{}" --checkpoint {} --fps {} --cfg_name {} --num {}'.format(
                args.video, args.save_dir, args.checkpoint, args.fps, cfg_name, ' '.join([str(x) for x in args.num])
            )
        if args.variant == 'TR' or args.variant == 'DR':
            cmd = 'python inference_video_plus_sdi_recur.py --video "{}" --save_dir "{}" --checkpoint {} --fps {} --cfg_name {} --num {} --iters {}'.format(
                args.video, args.save_dir, args.checkpoint, args.fps, cfg_name, ' '.join([str(x) for x in args.num]), args.iters
            )
        os.system(cmd)
