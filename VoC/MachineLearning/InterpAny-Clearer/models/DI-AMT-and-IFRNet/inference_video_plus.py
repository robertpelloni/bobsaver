...
def interpolate(I0, I1, num):
    ...
    embt = torch.arange(1 / (num + 1), 1, 1 / (num + 1)).float().view(1, num, 1, 1).to(device)

    for i in range(num):
        mid = model(I0, I1, embt[:, i: i + 1, ...], eval=True)['imgt_pred']
...
if __name__ == '__main__':
    ...
    for video_path in video_list:
        ...
        videoWriter = cv2.VideoWriter(save_path, fourcc, args.fps, (gif_imgs[0].shape[1], gif_imgs[0].shape[0]))
        for img in gif_imgs:
