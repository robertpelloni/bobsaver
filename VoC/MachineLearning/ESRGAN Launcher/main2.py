...
#edited for multiple image file processing
...
class ESRGAN(object):
    ...
    def main(self, argv):
        ...
        for model_upscale in self.models_upscale:
            ...
            #get the png files
            fnames = []
            for dirpath, _, filenames in walklevel(args.input,0):
                for filename in filenames:
                    ...
                    if filename[-5:] == '.JPEG':
                        fnames.append(filename)
            print(f"{len(fnames)} images found")
            ...
            for filename in fnames:
                ...
                print("Time elapsed: %s(s) Time left: %s(s)\nEstimated finish time: %s"%prstime)
                print('')
