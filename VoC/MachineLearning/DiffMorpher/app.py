...
with gr.Blocks() as demo:
    ...
    original_image_0, original_image_1 = gr.State(Image.open("assets/monalisa.jpeg").convert("RGB").resize((512,512), Image.BILINEAR)), gr.State(Image.open("assets/pearlgirl.jpg").convert("RGB").resize((512,512), Image.BILINEAR))
    # key_points_0, key_points_1 = gr.State([]), gr.State([])
    # to_change_points = gr.State([])
    
    with gr.Row():
        with gr.Column():
            input_img_0 = gr.Image(type="numpy", label="Input image A", value="assets/monalisa.jpeg", show_label=True, height=LENGTH, width=LENGTH, interactive=True)
            prompt_0 = gr.Textbox(label="Prompt for image A", value="a painting of a woman", interactive=True)
            with gr.Row():
        ...
        with gr.Column():
            input_img_1 = gr.Image(type="numpy", label="Input image B ", value="assets/pearlgirl.jpg", show_label=True, height=LENGTH, width=LENGTH, interactive=True)
            prompt_1 = gr.Textbox(label="Prompt for image B", value="a painting of a woman", interactive=True)
            with gr.Row():
    ...
    with gr.Accordion(label="Algorithm Parameters"):
        with gr.Tab("Basic Settings"):
            with gr.Row():
                ...
                model_path = gr.Text(value="Manojb/stable-diffusion-2-1-base",
