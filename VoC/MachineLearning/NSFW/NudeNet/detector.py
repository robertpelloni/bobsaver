...
class Detector:
    ...
    def __init__(self, model_name="default"):
        ...
        self.detection_model = onnxruntime.InferenceSession('detector_v2_default_checkpoint.onnx')

        #self.classes = [c.strip() for c in open(classes_path).readlines() if c.strip()]
        self.classes = [c.strip() for c in open('detector_v2_default_classes').readlines() if c.strip()]
...
if __name__ == "__main__":
    ...
    sys.stdout.write("\n")
    sys.stdout.write(str(m.detect(args.input_image)))
    sys.stdout.write("\n")
    sys.stdout.flush()
