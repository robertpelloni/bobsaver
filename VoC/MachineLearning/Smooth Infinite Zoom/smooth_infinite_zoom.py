...
# Original file is located at https://colab.research.google.com/github/BalintKomjati/smooth-infinite-zoom/blob/main/smooth_infinite_zoom.ipynb
...
def parse_args():
    ...
    parser.add_argument("--model", type=str, help="inpainting model to use")
    parser.add_argument("--width", type=int, help="image width and height, in pixel space")
    parser.add_argument("--num_interpol_frames", type=int, help="frames between each outpainting frame")
    parser.add_argument("--num_outpainting_steps", type=int, help="outpainting frame count")
    ...
    parser.add_argument("--output", type=str, help="output movie")
...
def shrink_and_paste_on_blank(current_image, mask_width):
  ...
  #blank_image[mask_width:height-mask_height, mask_width:width-mask_width,:] = prev_image
...
#model_id = "diffusers/stable-diffusion-xl-1.0-inpainting-0.1" #@param
#model_id = "Sanster/Realistic_Vision_V1.4-inpainting"
model_id = args2.model
...
prompt = args2.prompt
negative_prompt = args2.negative_prompt #"frame, ugly, blur" #@param
...
num_outpainting_steps = args2.num_outpainting_steps
...
#mask_height = int(128*height/width) #@param
#@markdown Number of images to be interpolated between each outpainting step:
num_interpol_frames = args2.num_interpol_frames
...
write_video(args2.output, all_frames, fps)
