...
#            maybe_print(iter, total_loss[0], total_dream_losses)
            for octave, octave_sizes in enumerate(octave_list, 1):
                ...
                num_calls = [0]
                def feval():
#                    maybe_print_octave_iter(iter, num_calls[0], octave, params.octave_iter, dream_losses)
                    num_calls[0] += 1
                    optimizer.zero_grad()
                    net(img)
                    loss = 0
...
def maybe_print_octave_iter(iter, t, n, total, dream_losses):
    if params.print_octave_iter > 0 and t % params.print_octave_iter == 0 and t<=total:
#        print("Octave iter "+str(n) +" iteration " + str(t) + " / "+ str(total))
        print("Iteration "+str(iter) + "/"+ str(params.num_iterations)+" Octave "+ str(n)+ "/"+ str(params.num_octaves)+" Octave iteration " + str(t) + "/"+ str(total))
        '''
        for i, loss_module in enumerate(dream_losses):
