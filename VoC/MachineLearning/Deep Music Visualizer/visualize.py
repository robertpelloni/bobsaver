...
if args.song:
    song=args.song
    sys.stdout.write("Reading audio ...\n")
    sys.stdout.flush()
...
for i in range(len(gradm)):   
    ...
    sys.stdout.write(f"Generating input vectors. {i+1} of {len(gradm)}\n")
    sys.stdout.flush()
...
for i in range(frame_lim):
    ...
    sys.stdout.write(f"Generating movie frames.  Step {i+1} of {frame_lim}\n")
    sys.stdout.flush()
    ...
    for out in output_cpu:    
        ...
        #@alstroemeria313 fix
        #im = np.round(np.clip((out + 1) * 127.5, 0, 255)).astype(np.uint8)
        im = np.round(np.clip((out + 1) * 127.5, 0, 255)).transpose(1,2,0)
        frames.append(im)
...
#clip.write_videofile(outname,audio_codec='aac',logger=None)
