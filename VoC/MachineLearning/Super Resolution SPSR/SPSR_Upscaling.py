...
#@markdown Define the model and various parts it uses
...
def pixelshuffle_block(in_nc, out_nc, upscale_factor=2, kernel_size=3, stride=1, bias=True,                         pad_type='zero', norm_type=None, act_type='relu'):
    '''
    Pixel shuffle layer
...
def upconv_blcok(in_nc, out_nc, upscale_factor=2, kernel_size=3, stride=1, bias=True,                 pad_type='zero', norm_type=None, act_type='relu', mode='nearest'):
    # Up conv
...
class SPSRNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, upscale=4, norm_type=None,             act_type='leakyrelu', mode='CNA', upsample_mode='upconv'):
        ...
        self.model = sequential(fea_conv, ShortcutBlock(sequential(*rb_blocks, LR_conv)),            *upsampler, self.HR_conv0_new)
        ...
        self.b_block_1 = RRDB(nf*2, kernel_size=3, gc=32, stride=1, bias=True, pad_type='zero',             norm_type=norm_type, act_type=act_type, mode='CNA')
        

        self.b_concat_2 = conv_block(2*nf, nf, kernel_size=3, norm_type=None, act_type = None)
        self.b_block_2 = RRDB(nf*2, kernel_size=3, gc=32, stride=1, bias=True, pad_type='zero',             norm_type=norm_type, act_type=act_type, mode='CNA')


        self.b_concat_3 = conv_block(2*nf, nf, kernel_size=3, norm_type=None, act_type = None)
        self.b_block_3 = RRDB(nf*2, kernel_size=3, gc=32, stride=1, bias=True, pad_type='zero',             norm_type=norm_type, act_type=act_type, mode='CNA')


        self.b_concat_4 = conv_block(2*nf, nf, kernel_size=3, norm_type=None, act_type = None)
        self.b_block_4 = RRDB(nf*2, kernel_size=3, gc=32, stride=1, bias=True, pad_type='zero',             norm_type=norm_type, act_type=act_type, mode='CNA')
        ...
        self.f_block = RRDB(nf*2, kernel_size=3, gc=32, stride=1, bias=True, pad_type='zero',             norm_type=norm_type, act_type=act_type, mode='CNA')
...
#@markdown <br>(in half-precision for K80 GPUs)
model_eval = SPSRNet(in_nc=3, out_nc=3, nf=64, nb=23, gc=32, upscale=4, norm_type=None)
model_eval.eval().requires_grad_(False);

state_dict = torch.load("spsr.pth", map_location='cpu')
model_eval.load_state_dict(state_dict)
model_eval.eval().requires_grad_(False);
model_eval.to(device);
model_eval.half().eval().requires_grad_(False);
del state_dict
clear_mem()
...
# Upload a file to the file browser on the left and put the filename in there.
# You can resize it before upscaling by adding (width, height) after the filename
# For example: OpenImage("20211009_115256.jpg", (288,512))
# not specifying output size gives x4 upscaling
test_img = OpenImage(args.input_image)
# display(ToImage(test_img[0]))


# In[5]:

sys.stdout.write("SPSR upscaling ...\n")
sys.stdout.flush()


torch.set_grad_enabled(False)
clear_mem()
model_out = model_eval(test_img.half())
ToImage(model_out[1].clamp(0,1)[0]).save(args.output_image)
...
# x_out_branch
# display(ToImage(model_out[0][0].clamp(0,1)))
# x_out
#display(ToImage(model_out[1][0].clamp(0,1)))
# x_grad
# display(ToImage(model_out[2][0].clamp(0,1)))
