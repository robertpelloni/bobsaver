...
def stylize(content_img, style_imgs, init_img, frame=None):
  with tf.device(args.device), tf.Session() as sess:
    ...
    # style loss
    sys.stdout.write("Calculating style losses...\n\n")
    sys.stdout.flush()
    if args.style_mask:
    ...
    # content loss
    sys.stdout.write("Calculating content loss...\n\n")
    sys.stdout.flush()
    L_content = sum_content_losses(sess, net, content_img)
    
    # denoising loss
    sys.stdout.write("Calculating denoising loss...\n\n")
    sys.stdout.flush()
    L_tv = tf.image.total_variation(net['input'])
    
    # loss weights
    sys.stdout.write("Calculating loss weights...\n\n")
    sys.stdout.flush()
    ...
    # total loss
    sys.stdout.write("Calculating total loss...\n\n")
    sys.stdout.flush()
    ...
    sys.stdout.write("Getting optimizer...\n\n")
    sys.stdout.flush()
...
def minimize_with_adam(sess, net, optimizer, init_img, loss):
  ...
  while (iterations < args.max_iterations):
    ...
    curr_loss = loss.eval()
    sys.stdout.write("Iteration {}\tf=  {}".format(iterations, curr_loss)+"\n\n")
    sys.stdout.flush()
    iterations += 1
  ...
  for frame in range(args.start_frame, args.end_frame+1):
    with tf.Graph().as_default():
      ...
      if frame == 1:
        ...
        sys.stdout.write('Frame {} elapsed time: {}\n\n'.format(frame, tock - tick))
        sys.stdout.flush()
      else:
        content_frame = get_content_frame(frame)
        style_imgs = get_style_images(content_frame)
