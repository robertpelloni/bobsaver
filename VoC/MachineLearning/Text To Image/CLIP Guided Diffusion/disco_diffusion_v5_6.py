...
kaliyuga_pixel_art_model_names = ['pixelart_expanded', 'pixelart_hard_256', 'pixelart_soft_256', 'pixelart_4k', 'pulp_scifi']
...
kaliyuga_pulpscifi_model_names = ['pulp_scifi']
...
if ViTH14: clip_models.append(open_clip.create_model('ViT-H-14', pretrained='laion2b_s32b_b79k').eval().requires_grad_(False).to(device))
if ViTG14: clip_models.append(open_clip.create_model('ViT-g-14', pretrained='laion2b_s12b_b42k').eval().requires_grad_(False).to(device))
...
#Add support for easing_functions
import subprocess
multipip_res = subprocess.run(['pip', 'install', 'easing_functions', 'matplotlib'], stdout=subprocess.PIPE).stdout.decode('utf-8')
print(multipip_res)
...
def easing_normalize(arr, t_min, t_max):
    norm_arr = []
    for i in arr: norm_arr.append((((i - min(arr))*(-abs(t_max - t_min)))/(max(arr) - min(arr))) + max(t_min, t_max))
    return norm_arr
def dd_easing_int(start=1, end=9, steps=1000, easing_fn="ExponentialEaseInOut"): return list(np.rint(easing_normalize(list(map(eval(easing_fn + "(start=start, end=end, duration=steps)"), np.arange(0, steps, 1))), start, end)).astype(int))
def dd_easing_float(start=9, end=1, steps=1000, easing_fn="ExponentialEaseInOut"): return list(easing_normalize(list(map(eval(easing_fn + "(start=start, end=end, duration=steps)"), np.arange(0, steps, 1))), start, end))
cut_easing_fn = args3.cut_easing_fn #@param ["LinearInOut", "QuadEaseInOut", "CubicEaseInOut", "QuarticEaseInOut", "QuinticEaseInOut", "SineEaseInOut", "CircularEaseInOut", "ExponentialEaseInOut", "ElasticEaseInOut", "BackEaseInOut", "BounceEaseInOut"]


#VOC START 3 - DO NOT DELETE
cut_overview = "dd_easing_int(start=0, end=10, steps=1000, easing_fn=cut_easing_fn)"
cut_innercut = "dd_easing_int(start=10, end=2, steps=1000, easing_fn=cut_easing_fn)"
cut_ic_pow = "[2]*200+dd_easing_int(start=20, end=2, steps=800, easing_fn=cut_easing_fn)"
...
#plot easing graphs
plt.plot(np.arange(0, 1000, 1), eval(cut_overview), label='overview')
plt.plot(np.arange(0, 1000, 1), eval(cut_innercut), label='inner')
...
if diffusion_model.startswith("custom_"):
