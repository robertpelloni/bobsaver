# Vonclank's version of Danbgo233's original that would not run without errors locally
# Original file is located at https://colab.research.google.com/drive/1QgH9TvQMXR3PpEGBcHnghtEcwFDXLaYE
...
def parse_args():
  ...
  parser.add_argument('--model1', type=int, help='Flag for which models to use.')
  parser.add_argument('--model2', type=int, help='Flag for which models to use.')
  parser.add_argument('--model3', type=int, help='Flag for which models to use.')
  parser.add_argument('--model4', type=int, help='Flag for which models to use.')
  parser.add_argument('--model5', type=int, help='Flag for which models to use.')
  parser.add_argument('--model6', type=int, help='Flag for which models to use.')
  parser.add_argument('--model7', type=int, help='Flag for which models to use.')
  parser.add_argument('--model8', type=int, help='Flag for which models to use.')
  parser.add_argument('--yfcc1', type=int, help='Flag for which models to use.')
  parser.add_argument('--yfcc2', type=int, help='Flag for which models to use.')
  parser.add_argument('--openimages', type=int, help='Flag for which models to use.')
  parser.add_argument('--augs', type=int, help='Use augs.')
  parser.add_argument('--flipaugs', type=int, help='Flip augs.')
  parser.add_argument('--lerp', type=int, help='Do lerp.')
...
sys.stdout.write("Setting secondary model ...\n")
sys.stdout.flush()
...
if args.yfcc1 == 1:
    model_list.append("yfcc_1")
    pace.append({"model_name":"yfcc_1", "guided":True, "mag_adjust":1})
if args.yfcc2 == 1:
    model_list.append("yfcc_2")
    pace.append({"model_name":"yfcc_2", "guided":True, "mag_adjust":1})
if args.openimages == 1:
    model_list.append("openimages")
    pace.append({"model_name":"openimages", "guided":True, "mag_adjust":1})
...
if use_LIT:
    for model_name in model_list:
        ...
        if model_name != "openimages":
            ...
            model[model_name] = LitModule.from_params(model_name,
                                      lambda: model[model_name],
                                      device="cuda")
        elif model_name == "openimages":
            openai, diffusion = create_model_and_diffusion(**model_config)
            openai.load_state_dict(torch.load('openimages.pth', map_location='cpu'))
            ...
            openai = LitModule.from_params("openimages",
                                      lambda: openai,
                                      device="cuda")
...
clip_size = {}
for i in clip_list:
    sys.stdout.write(f"Loading {i} CLIP model ...\n")
    sys.stdout.flush()
...
def cond_sample(model, x, steps, eta, extra_args, cond_fn):
    ...
    for i in range(len(steps)):
        ...
        #print(mag)
        if torch.isnan(mag):
            sys.stdout.write('NAN error caught\n')
            sys.stdout.flush()
            #print("ERROR2")
            continue
        ...
        if itt % args.update == 0:
            ...
            image = TF.to_pil_image(pred[0].add(1).div(2).clamp(0, 1))
...
#             clamp_max = clamp_index[1000-t]
            ...
            if mag==0:
                sys.stdout.write('0 mag error caught\n')
                sys.stdout.flush()
                #print("ERROR")
                return(grad)
            if t>=0:
...
#os.makedirs('/content/diff/', exist_ok=True)

#torch.cuda.empty_cache()
#gc.collect()
def do_run():
    global target_embeds, weights, init, makecutouts, progress, textprogress, progress2, batch_num,taskname,itt
    if seed is not None:
    ...
    if init_image is not None:
        ...
        if mask_scale > 0:
            ...
            init = init.resize((args.sizex, args.sizey), Image.BILINEAR)
            ...
            init = init.resize((args.sizex, args.sizey), Image.LANCZOS)
            init = TF.to_tensor(init).to(device)
...
if args.seed_image is not None:
    ...
    init_image = args.seed_image# "content/init/96.jpg"   # This can be an URL or Colab local path and must be in quotes.
else:
...
#sys.stdout.write(f"init_image = {init_image}\n")
#sys.stdout.flush()
...
anti_jpg=0
w,h = args.sizex,args.sizey #512,640


tv_scales = [0]+[2000]*3
tv_scale_2 = 0

step = args.iterations #250
steps_pow=1
...
if args.lerp == 1:
    lerp=True #False # Vram heavy!!
else:
    lerp=False #False # Vram heavy!!
