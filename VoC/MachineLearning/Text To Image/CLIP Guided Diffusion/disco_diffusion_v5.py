# Disco Diffusion v5 [w/ 3D animation]
...
def parse_args():
  ...
  parser.add_argument('--near_plane', type=float, help='3D near plane')
  parser.add_argument('--far_plane', type=float, help='3D far plane')
  parser.add_argument('--fov', type=float, help='3D FOV')
  parser.add_argument('--frames_scale', type=str, help='3D frames scale')
  ...
  parser.add_argument('--turbo_steps', type=int, help='Turbo steps.')
  parser.add_argument('--max_frames', type=int, help='Max frames count for 3D.')
  parser.add_argument('--perlin_init', type=int, help='Turbo steps.')
  parser.add_argument('--perlin_mode', type=str, help='Turbo steps.')
            ...
            with image_display:
              ...
              if itt % args2.update == 0 or cur_t == -1:
                  for k, image in enumerate(sample['pred_xstart']):
                      ...
                      #sys.stdout.flush()
                      if (args2.frame_dir is not None) and (args.animation_mode == "3D") and (itt == args2.iterations-skip_steps):
                          sys.stdout.flush()
...
if animation_mode == "Video Input":
  ...
  #sys.stdout.flush()
  os.system(f'ffmpeg.exe -i {video_init_path} -vf {vf} -vsync vfr -q:v 2 -loglevel error -stats {videoFramesFolder}/%04d.jpg')
