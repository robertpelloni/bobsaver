# Pixel Art Diffusion v3.0 (With Disco Symmetry)
# Original file is located at https://colab.research.google.com/github/KaliYuga-ai/Pixel-Art-Diffusion/blob/main/Pixel_Art_Diffusion_v3_0_(With_Disco_Symmetry).ipynb
...
sys.path.append('./guided-diffusion-5-4')
sys.path.append('./ResizeRight')
sys.path.append('./MiDaS')
sys.path.append('./disco-diffusion-5-4')
...
def parse_args():
  ...
  parser.add_argument('--noise_injector', type=int)
  parser.add_argument('--noise_injector_threshold', type=int)
  parser.add_argument('--noise_injector_mix', type=float)
  parser.add_argument('--noise_injector_px_size', type=int)
  parser.add_argument('--noise_injector_spice', type=float)
  parser.add_argument('--noise_injector_mask_feather', type=int)
  parser.add_argument('--noise_injector_sigma', type=int)
  parser.add_argument('--copy_palette', type=int)
  parser.add_argument('--copy_palette_image', type=str)
...
def gitclone(url):
    ...
    #print(res)
    print(f'avoided a git clone of {url}')

def pipi(modulestr):
    #res = subprocess.run(['pip', 'install', modulestr], stdout=subprocess.PIPE).stdout.decode('utf-8')
    #print(res)
    print(f'avoided a pip for {modulestr}')

def pipie(modulestr):
    #res = subprocess.run(['git', 'install', '-e', modulestr], stdout=subprocess.PIPE).stdout.decode('utf-8')
    #print(res)
    print(f'avoided a pipie for {modulestr}')

def wget(url, outputdir):
    #res = subprocess.run(['wget', url, '-P', f'{outputdir}'], stdout=subprocess.PIPE).stdout.decode('utf-8')
    #print(res)
    print(f'avoided a wget for {url}')
...
def init_midas_depth_model(midas_model_type="dpt_large", optimize=True):
    ...
    sys.stdout.write(f"Initializing MiDaS '{midas_model_type}' depth model...\n")
    sys.stdout.flush()
    # load network
    ...
    if midas_model_type == "dpt_large": # DPT-Large
        ...
        sys.stdout.write(f"midas_model_type '{midas_model_type}' not implemented\n")
        sys.stdout.flush()
        assert False
    ...
    sys.stdout.write(f"MiDaS '{midas_model_type}' depth model initialized.\n")
    sys.stdout.flush()
...
def do_run():
  ...
  for frame_num in range(args.start_frame, args.max_frames):
          ...
          if turbo_mode:
            if frame_num == turbo_preroll: #start tracking oldframe
              ...
              if frame_num % int(turbo_steps) != 0: 
                sys.stdout.write('turbo skip this frame: skipping clip diffusion steps\n')
                sys.stdout.flush()
                filename = f'{args.batch_name}({args.batchNum})_{frame_num:04}.png'
                blend_factor = ((frame_num % int(turbo_steps))+1)/int(turbo_steps)
                sys.stdout.write('turbo skip this frame: skipping clip diffusion steps and saving blended frame')
                sys.stdout.flush()
                ...
                if file_list:
                    ...
                    count_string = "00001"
                voc_frame_filename = args2.frame_dir+"\FRA"+count_string+".png"
                ...
                sys.stdout.write('clip/diff this frame - generate clip diff image\n')
                sys.stdout.flush()
      ...
      sys.stdout.write(f'{args.image_prompts_series}\n')
      sys.stdout.flush()
      ...
      sys.stdout.write(f'Frame {frame_num} Prompt: {frame_prompt}\n')
      sys.stdout.flush()
      ...
      if init_image is not None:
          ...
          if args.noise_injector == True and args.animation_mode == "3D":
            ...
            #sys.stdout.write('edge mix is...',round(edge_mix,1))
            sys.stdout.flush()
            if edge_mix < args.noise_injector_threshold:
              ...
              sys.stdout.write('edge mix was below threshold, noise was added\n')
              sys.stdout.flush()
              ...
              sys.stdout.write('edge mix was above threshold, no noise was added\n')
              sys.stdout.flush()
            ...
            with image_display:
              if j % args.display_rate == 0 or cur_t == -1 or intermediateStep == True:
                  for k, image in enumerate(sample['pred_xstart']):
                      ...
                      #if itt % args.display_rate == 0 or cur_t == -1:
                      ...
                      if cur_t == -1:
                        ...
                        #Save next VoC FRAnnnnn.PNG frame file
                        if args2.frame_dir != None: shutil.copyfile(f'{batchFolder}/{filename}',voc_frame_filename)
...
diffusion_model = args2.diffusion_model #"pixel_art_diffusion_hard_256" #@param ["pixel_art_diffusion_hard_256", "pixel_art_diffusion_soft_256", "pixelartdiffusion4k", "PADexpanded"]

#@markdown If you enable the secondary model, beware: secondary *REALLY* likes making everything it can into buildings.
if args2.use_secondary_model == 1:
...
elif diffusion_model == 'pixelart_hard_256':
    model_config.update({
...
elif diffusion_model == 'pixelart_soft_256':
    model_config.update({
          'attention_resolutions': '16',
          'class_cond': False,
          'diffusion_steps': 1000,
...
elif diffusion_model == 'pixelart_4k':
    model_config.update({
          'attention_resolutions': '16',
          'class_cond': False,
          'diffusion_steps': 1000,
...
elif diffusion_model == 'pixelart_expanded':
    model_config.update({
          'attention_resolutions': '16',
          'class_cond': False,
          'diffusion_steps': 1000,
...
if use_secondary_model:
    sys.stdout.write('Loading secondary model ...\n')
    sys.stdout.flush()
...
batch_name = args2.batch_name #'Pixel Art Diffusion' #@param{type: 'string'}
steps =  args2.iterations #250#@param [25,50,100,150,250,500,1000]{type: 'raw', allow-input: true}
#width_height = [475, 320]#@param{type: 'raw'}
width_height = [args2.sizex, args2.sizey]#@param{type: 'raw'}
clip_guidance_scale = args2.clip_guidance_scale #7000 #@param{type: 'number'}
tv_scale = args2.tv_scale # 0#@param{type: 'number'}
range_scale = args2.range_scale #  300#@param{type: 'number'}
sat_scale = args2.sat_scale #  0#@param{type: 'number'}
sat_scale_buffer =   .05#@param{type: 'number'}
if sat_scale_buffer < 0.0 or sat_scale_buffer > .2:
  sat_scale_buffer = 0
  sys.stdout.write('sat_scale_buffer out of range. Automatically reset to 0.0\n')
  sys.stdout.flush()
cutn_batches =  args2.cutn_batches # 4#@param{type: 'number'}
if args2.skip_augs == 1:
...
if soft_limiter_knee < 0.5 or soft_limiter_knee > .999:
  ...
  sys.stdout.write('soft_limiter_knee out of range. Automatically reset to 0.98\n')
  sys.stdout.flush()

#@markdown ---
#@markdown ####**Init Settings:**
init_image = args2.init_image #None #@param{type: 'string'}
init_scale = args2.init_scale #1000 #@param{type: 'integer'}
skip_steps = args2.skip_steps #0 #@param{type: 'integer'}
...
if side_x != width_height[0] or side_y != width_height[1]:
    sys.stdout.write(f'Changing output size to {side_x}x{side_y}. Dimensions must by multiples of 64.\n')
    sys.stdout.flush()
"""
side_x = args2.sizex;
side_y = args2.sizey;
...
animation_mode = args2.animation_mode #'None' #@param ['None', '2D', '3D', 'Video Input'] {type:'string'}
...
video_init_path = args2.video_init_path #"training.mp4" #@param {type: 'string'}
extract_nth_frame = args2.extract_nth_frame #2 #@param {type: 'number'}
if args2.video_init_seed_continuity == 1:
...
near_plane = args2.near_plane #100#@param {type:"number"}
...
turbo_steps = args2.turbo_steps #"3" #@param ["2","3","4","5","6"] {type:"string"}
turbo_preroll = args2.turbo_preroll #10 # frames

#insist turbo be used only w 3d anim.
if turbo_mode and animation_mode != '3D':
    sys.stdout.write('=====\n')
    sys.stdout.write('Turbo mode only available with 3D animations. Disabling Turbo.\n')
    sys.stdout.write('=====\n')
    sys.stdout.flush()
    turbo_mode = False
...
frames_scale = args2.frames_scale #0 #@param{type: 'integer'}
#@markdown `frame_skip_steps` will blur the previous frame - higher values will flicker less but struggle to add enough new detail to zoom into.
frames_skip_steps = args2.frames_skip_steps #'70%' #@param ['40%', '50%', '60%', '65%', '70%', '75%','80%'] {type: 'string'}
...
if args2.copy_palette == 1:
    copy_palette = True#@param{type: 'boolean'}
else:
    copy_palette = False#@param{type: 'boolean'}
copy_palette_image = args2.copy_palette_image #"" #@param{type: 'string'}
...
vr_eye_angle = args2.vr_eye_angle #0.5 #@param{type:"number"}
#@markdown interpupillary distance (between the eyes)
vr_ipd = args2.vr_ipd #5.0 #@param{type:"number"}

#insist VR be used only w 3d anim.
if vr_mode and animation_mode != '3D':
    sys.stdout.write('=====\n')
    sys.stdout.write('VR mode only available with 3D animations. Disabling VR.\n')
    sys.stdout.write('=====\n')
    vr_mode = False
...
eta = args2.eta #.75#@param{type: 'number'}
if args2.clamp_grad == 1:
...
clamp_max = args2.clamp_max # .2#@param{type: 'number'}
...
cut_overview = args2.cut_overview #"[15]*100+[15]*100+[12]*100+[12]*100+[6]*100+[4]*100+[2]*200+[0]*200" #@param {type: 'string'}       
cut_innercut = args2.cut_innercut #"[1]*100+[1]*100+[4]*100+[4]*100+[8]*100+[8]*100+[10]*200+[10]*200"#@param {type: 'string'}  
...
cut_ic_pow = args2.cut_ic_pow #"[12]*300+[12]*100+[12]*50+[12]*50+[10]*100+[10]*100+[10]*300"#@param {type: 'string'}  
cut_icgray_p = args2.cut_icgray_p #"[0.87]*100+[0.78]*50+[0.73]*50+[0.64]*60+[0.56]*40+[0.50]*50+[0.33]*100+[0.19]*150+[0]*400"#@param {type: 'string'}

#@markdown ####**Transformation Settings:**
if args2.use_vertical_symmetry == 1:
    use_vertical_symmetry = True #@param {type:"boolean"}
else:
    use_vertical_symmetry = False #@param {type:"boolean"}

if args2.use_horizontal_symmetry == 1:
    use_horizontal_symmetry = True #@param {type:"boolean"}
else:
    use_horizontal_symmetry = False #@param {type:"boolean"}

transformation_percent = [args2.transformation_percent] #@param
...
n_batches = args2.n_batches # 1#@param{type: 'number'}
...
if args2.resume_run == 1:
    resume_run = True #@param{type: 'boolean'}
else:
    resume_run = False #@param{type: 'boolean'}
run_to_resume = args2.run_to_resume #'latest' #@param{type: 'string'}
resume_from_frame = args2.resume_from_frame #'latest' #@param{type: 'string'}
if args2.retain_overwritten_frames == 1:
...
sys.stdout.write(f'Starting Run: {batch_name}({batchNum}) at frame {start_frame}\n')
sys.stdout.flush()
...
sys.stdout.write('Prepping model...\n')
sys.stdout.flush()
...
try:
    ...
    #sys.stdout.write('Seed used:', seed)
    gc.collect()
    torch.cuda.empty_cache()
