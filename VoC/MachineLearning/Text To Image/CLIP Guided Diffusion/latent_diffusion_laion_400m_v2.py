        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                        ...
                        if opt.skip_save == 0:
                            for x_sample in x_samples_ddim:
                                ...
                                Image.fromarray(x_sample.astype(np.uint8)).save(
                                    ...
                                    #os.path.join(outpath, f'{opt.prompt} [Latent Diffusion LAION_400M v2 {opt.sampler}] {base_count:05}.png'))
                                    f"{opt.image_file[:-4]} {base_count:05}.png")
                                base_count += 1
                ...
                if not opt.skip_grid:
                    ...
                    #Image.fromarray(grid.astype(np.uint8)).save(os.path.join(outpath, f'{opt.prompt} [Latent Diffusion LAION_400M v2 {opt.sampler}].png'))
                    Image.fromarray(grid.astype(np.uint8)).save(opt.image_file)
                    grid_count += 1
