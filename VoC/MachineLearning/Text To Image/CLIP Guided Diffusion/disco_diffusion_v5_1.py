...
def parse_args():
  ...
  parser.add_argument('--turbo_preroll', type=int, help='Turbo preroll steps.')
  ...
  parser.add_argument('--video_init_path', type=str, help='Source video')
  parser.add_argument('--video_continuity', type=int, help='Video init seed continuity.')
  parser.add_argument('--vr_mode', type=int, help='VR mode.')
...
def do_run():
  ...
  for frame_num in range(args.start_frame, args.max_frames):
      ...
      if (args.animation_mode == "3D") or (args.animation_mode == "Video Input"):
          ...
          if turbo_mode:
            if frame_num == turbo_preroll: #start tracking oldframe
              ...
              if frame_num % int(turbo_steps) != 0: 
                ...
                #####################################################################
                if args2.frame_dir is not None and (args.animation_mode == "3D" or args.animation_mode == "Video Input"):
                  sys.stdout.flush()
                  ...
                  if vr_mode == True:
                      ...
                      cv2.imwrite(save_name,blendedImage)

                  sys.stdout.flush()
                  sys.stdout.write('3D frame saved\n')
                  sys.stdout.flush()
                #####################################################################
      ...
      #print(f'Frame {frame_num+1} Prompt: {frame_prompt}', encoding='utf-8')
      newstr=f'Frame {frame_num+1} Prompt: {frame_prompt}'
            ...
            with image_display:
              if j % args.display_rate == 0 or cur_t == -1 or intermediateStep == True:
                  for k, image in enumerate(sample['pred_xstart']):
                      ...
                      if (args2.frame_dir is not None) and (args.animation_mode is not "None") and (itt == args2.iterations-skip_steps):
                          sys.stdout.flush()
                          sys.stdout.write('Saving 3D frame...\n')
                          sys.stdout.flush()
                          import os
                      ...
                      if j % args.display_rate == 0 or cur_t == -1:
                        ...
                        #####################################################################
                        #save Video Input processed frame
                        #####################################################################
                        if (args2.frame_dir is not None) and (j==total_steps) and (args.animation_mode == "Video Input"):
                            
                            sys.stdout.flush()
                            sys.stdout.write('Saving video input frame ...\n')
                            sys.stdout.flush()
                      ...
                      if args.steps_per_checkpoint is not None:
                        if j % args.steps_per_checkpoint == 0 and j > 0:
                          if args.intermediates_in_subfolder is True:
                            ...
                            #save next movie frame
                            #####################################################################
                            if (args2.frame_dir is not None) and (args.animation_mode == "3D"):
                              sys.stdout.flush()
                              sys.stdout.write('Saving 3D frame...\n')
                              sys.stdout.flush()
                              import os
...
def generate_eye_views(trans_scale,batchFolder,filename,frame_num,midas_model, midas_transform):
   for i in range(2):
      ...
      eye_file_path = args2.frame_dir+f"/FRA{frame_num:05}" + ('_L' if i==0 else '_R')+'.png'
...
timestep_respacing = '100' #@param ['25','50','100','150','250','500','1000','ddim25','ddim50', 'ddim75', 'ddim100','ddim150','ddim250','ddim500','ddim1000']  
...
width_height = [args2.sizex, args2.sizey]#[1280, 768]#@param{type: 'raw'}
clip_guidance_scale = args2.guidancescale #5000 #@param{type: 'number'}
tv_scale =  args2.tvscale#0#@param{type: 'number'}
range_scale =  args2.rangescale#  150#@param{type: 'number'}
sat_scale =  args2.saturationscale# 0#@param{type: 'number'}
cutn_batches = args2.cutnbatches  #4  #@param{type: 'number'}
...
if animation_mode == "Video Input":
  ...
  #os.system(f'ffmpeg -i {video_init_path} -vf {vf} -vsync vfr -q:v 2 -loglevel error -stats {videoFramesFolder}/%04d.jpg')
  os.system(f'ffmpeg -i "{video_init_path}" -vf {vf} -vsync vfr -q:v 2 -loglevel error -stats "{videoFramesFolder}/%04d.jpg"')
...
angle = "0:(0)"#@param {type:"string"}angle = args2.angle #"0:(0)"#@param {type:"string"}
...
if args2.turbo_mode==1:
    ...
    turbo_mode = False
turbo_steps = args2.turbo_steps #@param ["2","3","4","5","6"] {type:"string"}
...
vr_mode = args2.vr_mode #@param {type:"boolean"}
...
cut_overview = args2.cutoverview #"[12]*400+[4]*600" #@param {type: 'string'}       
cut_innercut = args2.cutinnercut #"[4]*400+[12]*600"#@param {type: 'string'}  
cut_ic_pow = args2.cuticpower #1#@param {type: 'number'}  
cut_icgray_p = args2.cuticgray #"[0.2]*400+[0]*600"#@param {type: 'string'}
...
display_rate = args2.update # 50 #@param{type: 'number'}
