...
# Original file is located at https://colab.research.google.com/github/zippy731/disco-diffusion-turbo/blob/skunk/Disco_Diffusion.ipynb
...
def parse_args():
  ...
  parser.add_argument('--sat_scale_buffer', type=float)
  parser.add_argument('--cutn_batches', type=int)
  parser.add_argument('--skip_augs', type=int)
  parser.add_argument('--soft_limiter_on', type=int)
  parser.add_argument('--soft_limiter_knee', type=float)
  ...
  parser.add_argument('--skip_end_steps', type=int)
  ...
  parser.add_argument('--frames_skip_end_steps', type=int)
...
def do_run():
  ...
  for frame_num in range(args.start_frame, args.max_frames):
      ...
      if init_image is not None:
          ...
          if (args.noise_injector == True or args.copy_palette == True) and frame_num > 0:
              ...
              if j % args.display_rate == 0 or cur_t == -1 or intermediateStep == True:
                  for k, image in enumerate(sample['pred_xstart']):
                      ...
                      if cur_t == -1:
                        ...
                        #Save next VoC FRAnnnnn.PNG frame file
                        if args2.frame_dir != None: 
                            #sys.stdout.write(f'*****Source file {batchFolder}/{filename}\n')
                            #sys.stdout.write(f'*****Target file {voc_frame_filename}\n')
                            #sys.stdout.flush()
...
batch_name = args2.batch_name #'Time_To_Disco_ColorFX' #@param{type: 'string'}
steps = args2.iterations #@param [25,50,100,150,250,500,1000]{type: 'raw', allow-input: true}
...
range_scale = args2.range_scale #  0#@param{type: 'number'}
sat_scale = args2.sat_scale # 10000#@param{type: 'number'}
sat_scale_buffer = args2.sat_scale_buffer #  .05#@param{type: 'number'}
if sat_scale_buffer < 0.0 or sat_scale_buffer > .2:
  sys.stdout.write(f'sat_scale_buffer of {sat_scale_buffer} is out of range. Automatically reset to 0.0\n')
  sys.stdout.flush()
  sat_scale_buffer = 0
cutn_batches = args2.cutn_batches #1  #@param{type: 'number'}
...
if args2.soft_limiter_on == 1:
  soft_limiter_on = True#@param{type: 'boolean'}
else:
  soft_limiter_on = False#@param{type: 'boolean'}
soft_limiter_knee = args2.soft_limiter_knee #.98 #@param{type: 'number'}
...
#@markdown ---
skip_end_steps = args2.skip_end_steps #0 #@param{type: 'integer'}
...
translation_x = "0:(0)"
translation_y = "0:(0)"
translation_z = "0:(1), 500:(2.5)"
rotation_3d_x = "0:(0)"
rotation_3d_y = "0:(0)"
rotation_3d_z = "0:(0.3), 500:(0.9)"
...
fov = args2.fov #80#@param {type:"number"}
...
frames_skip_steps = args2.frames_skip_steps #'75%' #@param ['40%', '50%', '60%', '65%', '70%', '75%','80%'] {type: 'string'}
...
frames_skip_end_steps = args2.frames_skip_end_steps #10 #@param{type: 'integer'}
...
noise_injector_threshold = args2.noise_injector_threshold # 35#@param{type: 'integer'}
#@markdown `noise_injector_threshold` if edge% falls below this %, will boost next frame. 20 is a good starting value.
noise_injector_mix = args2.noise_injector_mix #0.75 #@param{type: 'number'}
#@markdown `noise_injector_mix` overall wet/dry mix for noise effect 0 - 1.0 
noise_injector_px_size = args2.noise_injector_px_size #4 #@param{type: 'integer'}
...
copy_palette_image = args2.copy_palette_image #"/content/drive/MyDrive/AI/Disco_Diffusion/init_images/beach.jpg" #@param{type: 'string'}
...
if type(intermediate_saves) is not list:
    if intermediate_saves:
        ...
        sys.stdout.write(f'Will save every {steps_per_checkpoint} steps\n')
        sys.stdout.flush()
    else:
...
cut_ic_pow = args2.cut_ic_pow #1#@param {type: 'number'}  
...
0: ["collage of memorabilia, 60s Kitsch and psychedelia"]
