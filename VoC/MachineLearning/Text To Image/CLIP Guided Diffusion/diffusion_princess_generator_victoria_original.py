...
# https://github.com/Dango233/PrincessGenerator/blob/main/Diffusion_V_Princess_Generator_Victoria.ipynb


# https://set.zlkj.in/models/diffusion/512x512_diffusion_uncond_openimages_epoch28_withfilter.pt download to openimages.pth
...
#             clamp_max = clamp_index[1000-t]
            ...
            tv_losses = tv_loss(x_in).sum() * tv_scales[0] +                tv_loss(F.interpolate(x_in, scale_factor= 1/2)).sum()* tv_scales[1] +                 tv_loss(F.interpolate(x_in, scale_factor = 1/4)).sum()* tv_scales[2] +                 tv_loss(F.interpolate(x_in, scale_factor = 1/8)).sum()* tv_scales[3] 
            ...
            loss =  tv_losses  + range_losses  +                 bsq_loss * bsq_scale 
