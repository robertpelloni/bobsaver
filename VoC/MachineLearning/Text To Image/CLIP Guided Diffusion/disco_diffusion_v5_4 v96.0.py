...
# Optical flow code from https://colab.research.google.com/github/Sxela/DiscoDiffusion-Warp/blob/main/Disco_Diffusion_v5_2_Warp.ipynb
        ...
        if video_init_flow_warp:
          ...
          if frame_num > 0: 
            ...
            if video_init_check_consistency:
                weights_path = f"{flo_folder}/{frame1_path.split('/')[-1]}-21_cc.npy" 
...
diffusion_model = args2.diffusion_model #"512x512_diffusion_uncond_finetune_008100" #@param ["256x256_diffusion_uncond", "512x512_diffusion_uncond_finetune_008100"]
if args2.use_secondary_model == 1:
  use_secondary_model = True #@param {type: 'boolean'}
else:
  use_secondary_model = False #@param {type: 'boolean'}
diffusion_sampling_mode = args2.diffusion_sampling_mode #'ddim' #@param ['plms','ddim']  

#@markdown #####**Custom model:**
custom_path = args2.custom_path #'/content/drive/MyDrive/deep_learning/ddpm/ema_0.9999_058000.pt'#@param {type: 'string'}
...
batch_name = args2.batch_name #'TimeToDisco' #@param{type: 'string'}
...
clip_guidance_scale = args2.clip_guidance_scale #5000 #@param{type: 'number'}
tv_scale = args2.tv_scale #0#@param{type: 'number'}
range_scale = args2.range_scale #150#@param{type: 'number'}
...
video_init_steps = args2.video_init_steps #100 #@param [25,50,100,150,250,500,1000]{type: 'raw', allow-input: true}
video_init_clip_guidance_scale = args2.video_init_clip_guidance_scale #1000 #@param{type: 'number'}
video_init_tv_scale = args2.video_init_tv_scale #0.1#@param{type: 'number'}
video_init_range_scale = args2.video_init_range_scale #150#@param{type: 'number'}
video_init_sat_scale = args2.video_init_sat_scale #300#@param{type: 'number'}
video_init_cutn_batches = args2.video_init_cutn_batches #4#@param{type: 'number'}
video_init_skip_steps = args2.video_init_skip_steps #50 #@param{type: 'integer'}
...
skip_steps = args2.skip_steps #10 #@param{type: 'integer'}
...
"""
video_init_path = args2.video_init_path
extract_nth_frame = args2.extract_nth_frame #2 #@param {type: 'number'}
persistent_frame_output_in_batch_folder = args2.persistent_frame_output_in_batch_folder #True #@param {type: 'boolean'}
video_init_seed_continuity = args2.video_init_seed_continuity #False #@param {type: 'boolean'}
#@markdown #####**Video Optical Flow Settings:**

if args2.video_init_flow_warp == 1:
    video_init_flow_warp = True #@param {type: 'boolean'}
else:
    video_init_flow_warp = True #@param {type: 'boolean'}

# Call optical flow from video frames and warp prev frame with flow
video_init_flow_blend = args2.video_init_flow_blend # 0.999#@param {type: 'number'} #0 - take next frame, 1 - take prev warped frame
video_init_check_consistency = args2.video_init_check_consistency #False #Insert param here when ready
video_init_blend_mode = args2.video_init_blend_mode #"optical flow" #@param ['None', 'linear', 'optical flow']
...
frames_skip_steps = args2.frames_skip_steps #'60%' #@param ['40%', '50%', '60%', '70%', '80%'] {type: 'string'}

#@markdown ####**Video Init Coherency Settings:**
#@markdown `frame_scale` tries to guide the new frame to looking like the old one. A good default is 1500.
video_init_frames_scale = args2.video_init_frames_scale #15000 #@param{type: 'integer'}
#@markdown `frame_skip_steps` will blur the previous frame - higher values will flicker less but struggle to add enough new detail to zoom into.
video_init_frames_skip_steps = args2.video_init_frames_skip_steps #'70%' #@param ['40%', '50%', '60%', '70%', '80%'] {type: 'string'}
...
if animation_mode == 'Video Input':
    ...
    def load_cc(path):
        ...
        print('weight min max mean std', weights.shape, weights.min(), weights.max(), weights.mean(), weights.std())
        return weights
...
if animation_mode == "Video Input":
    ...
    sys.stdout.write(f'***DEBUG*** Video Input 01\n')
    sys.stdout.flush()

    force_flow_generation = args2.force_flow_generation #False #@param {type:'boolean'}
    in_path = videoFramesFolder
    flo_folder = f'{in_path}/out_flo_fwd'
    ...
    if (animation_mode == 'Video Input') and (video_init_flow_warp):
        ...
        if (len(flows)>0) and not force_flow_generation:
            ...
            sys.stdout.write(f'***DEBUG*** Video Input 02\n')
            sys.stdout.flush()
        ...
        if (len(flows)==0) or force_flow_generation:
            ...
            if len(frames)>=2:
                ...
                raft_model = torch.nn.DataParallel(RAFT(args3))
...
eta = args2.eta #0.8#@param{type: 'number'}
if args2.clamp_grad == 1:
    clamp_grad = True #@param{type: 'boolean'}
else:
    clamp_grad = False #@param{type: 'boolean'}
clamp_max = args2.clamp_max #0.05 #@param{type: 'number'}

### EXTRA ADVANCED SETTINGS:
if args2.randomize_class == 1:
...
if args2.clip_denoised == 1:
    ...
    clip_denoised = False
if args2.fuzzy_prompt == 1:
    fuzzy_prompt = True
else:
    fuzzy_prompt = False
rand_mag = args2.rand_mag #0.05
...
cut_overview = args2.cut_overview #"[12]*400+[4]*600" #@param {type: 'string'}       
cut_innercut = args2.cut_innercut #"[4]*400+[12]*600"#@param {type: 'string'}  
cut_ic_pow = args2.cut_ic_pow #1#@param {type: 'string'}  
cut_icgray_p = args2.cut_icgray_p #"[0.2]*400+[0]*600"#@param {type: 'string'}
...
display_rate = args2.update #20 #@param{type: 'number'}
n_batches = args2.n_batches #50 #@param{type: 'number'}
...
gobig_skip_percent = 60#@param{type: 'number'}
...
if go_big == True:
  if go_big_init == "" or go_big_init == "None":
    go_big_init = None
  ...
  if go_big_init != None:
    ...
    temp_image = Image.open(go_big_init)
...
slices_todo = (go_big_scale * go_big_scale) + 1
if go_big_scale > 2:
  slices_todo += 1 # add an extra slice to help ensure comically large images don't run out of memory
...
def slice(source):
    ...
    while remainder > 0:
        ...
        print('adding an extra slice to cover remainder of image')
    slice_width += overlap
    i = 0
    slices = []
    x = 0
...
if go_big:
  ...
  #print(f'debug: after slicing, slices_todo is {slices_todo}')
