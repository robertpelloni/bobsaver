...
def load_model_from_config(config, ckpt, verbose=False):
    ...
    if len(u) > 0 and verbose:
        print("unexpected keys:", flush=True)
        print(u, flush=True)
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("--every_n_train_steps", type=int, help="How many steps to save each checkpoint after.")
...
class SetupCallback(Callback):
    ...
    def on_pretrain_routine_start(self, trainer, pl_module):
        if trainer.global_rank == 0:
            ...
            print("Project config", flush=True)
            print(OmegaConf.to_yaml(self.config), flush=True)
            OmegaConf.save(self.config,
                           os.path.join(self.cfgdir, "{}-project.yaml".format(self.now)))

            print("Lightning config", flush=True)
            print(OmegaConf.to_yaml(self.lightning_config), flush=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not "gpus" in trainer_config:
            ...
            print(f"Running on GPUs {gpuinfo}", flush=True)
            cpu = False
        ...
        if hasattr(model, "monitor"):
            print(f"Monitoring {model.monitor} as checkpoint metric.", flush=True)
        ...
        if 'metrics_over_trainsteps_checkpoint' in callbacks_cfg:
            print(
                'Caution: Saving checkpoints every n train steps without deleting. This might require some free space.', flush=True)
        ...
        for k in data.datasets:
            print(f"{k}, {data.datasets[k].__class__.__name__}, {len(data.datasets[k])}", flush=True)
        ...
        print(f"accumulate_grad_batches = {accumulate_grad_batches}", flush=True)
        lightning_config.trainer.accumulate_grad_batches = accumulate_grad_batches
        if opt.scale_lr:
            model.learning_rate = accumulate_grad_batches * ngpu * bs * base_lr
            print(
                "Setting learning rate to {:.2e} = {} (accumulate_grad_batches) * {} (num_gpus) * {} (batchsize) * {:.2e} (base_lr)".format(
                    model.learning_rate, accumulate_grad_batches, ngpu, bs, base_lr), flush=True)
        else:
            model.learning_rate = base_lr
            print("++++ NOT USING LR SCALING ++++", flush=True)
            print(f"Setting learning rate to {model.learning_rate:.2e}", flush=True)
