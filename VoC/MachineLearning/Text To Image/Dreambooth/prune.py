...
def parse_args():
    ...
    parser.add_argument("--model_filename", type=str, help="Full path to model")
...
def prune_it(p, keep_only_ema=False):
    print(f"prunin' in path: {p}", flush=True)
    size_initial = os.path.getsize(p)
    nsd = dict()
    sd = torch.load(p, map_location="cpu")
    print(sd.keys(), flush=True)
    for k in sd.keys():
        if k != "optimizer_states":
            nsd[k] = sd[k]
    else:
        print(f"removing optimizer states for path {p}", flush=True)
    if "global_step" in sd:
        print(f"This is global step {sd['global_step']}.", flush=True)
    if keep_only_ema:
    ...
    print(f"saving pruned checkpoint at: {fn}", flush=True)
    torch.save(nsd, fn)
