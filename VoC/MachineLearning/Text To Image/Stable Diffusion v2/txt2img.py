...
def parse_args():
    ...
    parser.add_argument("--sampler",type=str,help="plms, dpm or ddim",)
...
def main(opt):
    ...
    if opt.sampler == 'plms':
        sys.stdout.write("Setting PLMS sampler ...\n")
        sys.stdout.flush()
        sampler = PLMSSampler(model)

    if opt.sampler == 'dpm':
        sys.stdout.write("Setting DPM sampler ...\n")
        sys.stdout.flush()
        sampler = DPMSolverSampler(model)

    if opt.sampler == 'ddim':
        sys.stdout.write("Setting DDIM sampler ...\n")
        sys.stdout.flush()
    ...
    with torch.no_grad(), \
        ...
        model.ema_scope():
            ...
            for n in range(opt.n_iter):
                ...
                sys.stdout.write(f"Generating samples {n+1} of {opt.n_iter}...\n")
                sys.stdout.flush()
                ...
                for prompts in data:
                    ...
                    if not opt.skip_save:
                        for x_sample in x_samples:
                            ...
                            curseed = opt.seed + image_count
                            image_count = image_count + 1
                            file_name=os.path.join(sample_path, f"{opt.image_file[:-4]} {curseed}.png")
            ...
            if opt.n_samples > 1:
                file_name=os.path.join(outpath, f"{opt.image_file[:-4]} {opt.seed} grid.png")
            else:
                file_name=os.path.join(outpath, f"{opt.image_file[:-4]} {opt.seed}.png")
            ...
            grid.save(os.path.join(outpath,get_unique_filename(file_name)))
