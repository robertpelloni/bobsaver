...
def AugmentLoss(img, clip_model, text, replicate=10, interp_mode='bilinear', policy=POLICY):
    ...
    img_aug = F.interpolate(img_aug, size=clip_model.visual.input_resolution, mode=interp_mode)
...
def NaiveSemanticLoss(img, clip_model, text, interp_mode='bilinear'):
    ...
    img = F.interpolate(img, size=clip_model.visual.input_resolution, mode=interp_mode)
...
class FuseDreamBaseGenerator():
    def __init__(self, G, G_config, G_batch_size=10, clip_mode="ViT-B/16", interp_mode='bilinear'):
    ...
    def generate_basis(self, text, init_iters=500, num_basis=5, image_name='Progress.png', frame_dir=None, update_iters=10):
        ...
        with torch.no_grad():
            ...
            for i in range(init_iters):
                ...
                image_tensors = F.interpolate(image_tensors, size=self.clip_model.visual.input_resolution, mode=self.interp_mode)
                ...
                sys.stdout.write("Warmup {}/{}".format(i+1,init_iters)+"\n")
                sys.stdout.flush()
                """
                if (i+1) % update_iters == 0:
                    sys.stdout.flush()
                    sys.stdout.write("Saving progress ...\n")
                    sys.stdout.flush()
                    
                    save_image(self.G(z_init, self.G.shared(y_init)), image_name,1)
    ...
    def optimize_clip_score(self, z_init_cllt, y_init_cllt, text, latent_noise=False, augment=True, opt_iters=500, optimize_y=False, image_name='Progress.png', frame_dir=None, update_iters=10):
        ...
        for i in range(opt_iters):
            ...
            if (i+1) % update_iters == 0:
                ...
                save_image(self.G(z_init_ans, y_init_ans), image_name,1)
                ...
                if frame_dir is not None:
                    ...
                    if file_list:
                        ...
                        count_string = "00001"
                    save_name = frame_dir+"\FRA"+count_string+".png"
                    save_image(self.G(z_init_ans, y_init_ans), save_name,1)
