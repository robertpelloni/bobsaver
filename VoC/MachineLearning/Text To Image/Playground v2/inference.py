...
if args2.process_video is not None:
    ...
    sys.stdout.write("Setting up init image pipeline with {args2.scheduler} ...\n")
    sys.stdout.flush()
    pipe = AutoPipelineForImage2Image.from_pretrained(
        f"playgroundai/playground-{args2.model}-1024px-aesthetic", torch_dtype=torch.float16, use_safetensors=True
    )
    ...
    # apply hidiffusion
    if args2.hidiffusion == 1:
        sys.stdout.flush()
        sys.stdout.write('Applying HiDiffusion ...\n')
        sys.stdout.flush()
    ...
    for i in range(len(onlyfiles)):
        ...
        init_image = load_image(onlyfiles[i])
...
# diffusers docs https://huggingface.co/docs/diffusers/using-diffusers/img2img
...
if args2.in_script_movie == 0:
    ...
    if args2.init_image is None:
        ...
        pipe = DiffusionPipeline.from_pretrained(
            f"playgroundai/playground-{args2.model}-1024px-aesthetic",
...
if args2.in_script_movie == 1:
    ...
    sys.stdout.write(f"Setting up DiffusionPipeline ...\n")
    sys.stdout.flush()
    pipe = DiffusionPipeline.from_pretrained(
        f"playgroundai/playground-{args2.model}-1024px-aesthetic",
        torch_dtype=torch.float16,
    ...
    """
    # apply hidiffusion - does not work for the image2image pipe
    if args2.hidiffusion == 1:
        sys.stdout.flush()
        sys.stdout.write('Applying HiDiffusion ...\n')
        sys.stdout.flush()
        apply_hidiffusion(pipe2)
    """
