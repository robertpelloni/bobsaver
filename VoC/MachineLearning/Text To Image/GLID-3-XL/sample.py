...
parser.add_argument('--image_file', type = str) # output image file name from VoC

args = parser.parse_args()

#DISABLED INPAINTING FOR NOW AS PYQT5 NOT COMPATIBLE WITH THE VOC_BASE ENVIRONMENT
...
def do_run():
    ...
    if args.edit:
        ...
        if args.mask:
            ...
            sys.stdout.write("Draw the area for inpainting, then close the window\n")
            sys.stdout.flush()
    ...
    def save_sample(i, sample, clip_score=False):
        for k, image in enumerate(sample['pred_xstart'][:args.batch_size]):
            ...
            basename = basename[:-4]
            filename = f'{dirname}/{basename} {i * args.batch_size + k:05}.png'
            out.save(filename)
    ...
    for i in range(args.num_batches):
        ...
        sys.stdout.write("Batch {}".format(i+1)+"\n")
        sys.stdout.flush()
        ...
        for j, sample in enumerate(samples):
            sys.stdout.write("Iteration {}".format(j+1)+"\n")
            sys.stdout.flush()
