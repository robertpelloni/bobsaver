# CLIPDraw AI Telephone.ipynb
# Original file is located at https://colab.research.google.com/github/kvfrans/clipdraw/blob/main/clipdraw.ipynb
...
args = lambda: None
args.num_paths = args2.numpaths # 256
args.num_iter = args2.iterations
args.max_width = args2.maxwidth
...
canvas_width, canvas_height = args2.sizex, args2.sizey #768, 768 #224, 224
...
sys.stdout.write("Setting optimizers ...\n")
sys.stdout.flush()

# Optimizers
points_optim = torch.optim.Adam(points_vars, lr=args2.pointslr) #1.0
width_optim = torch.optim.Adam(stroke_width_vars, lr=args2.widthlr) #0.1
color_optim = torch.optim.Adam(color_vars, lr=args2.colorlr) #0.01
...
for t in range(args2.iterations):
    ...
    #if t % 5 == 0:
    #    pydiffvg.imwrite(img.cpu(), './iter_{}.png'.format(int(t/5)), gamma=gamma)
    img = img[:, :, :3]
    img = img.unsqueeze(0)
