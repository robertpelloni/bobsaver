...
def get_all(img):
  ...
  print(img.shape)
  cards = gallery(cards)
  imageio.imwrite('progress.png', np.array(cards))
  file_path = '/content/CLIP/frames/{}.png'.format(str(len(os.listdir('/content/CLIP/frames'))).zfill(5))
  imageio.imwrite(file_path, np.array(cards))
  return display.Image('progress.png')
...
class BigGANConfig(object):
    ...
    """
    def __init__(self,
                 output_dim=args.image_width, #128
                 z_dim=args.image_width, #128
                 class_embed_dim=args.image_width, #128
                 channel_width=args.image_width, #128
                 num_classes=1000,
                 layers=[(False, 16, 16),
                         (True, 16, 16),
                         (False, 16, 16),
...
#text = 'an armchair in the shape of an avocado' #@param {type:"string"}
prompt = args.input_phrase
...
model_name = 'biggan-deep-' + str(res)
