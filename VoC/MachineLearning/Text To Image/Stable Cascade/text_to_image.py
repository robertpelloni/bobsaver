...
def parse_args():
    ...
    parser.add_argument("--priorguidancescale", type=float)
    parser.add_argument("--priorsteps", type=int)
    parser.add_argument("--decoderguidancescale", type=float)
    parser.add_argument("--decodersteps", type=int)
    parser.add_argument("--init_image", type=str)
    parser.add_argument("--init_strength", type=float)
    ...
    parser.add_argument("--zoom_factor", type=float, help="zoom compensation for non square frames")
...
#######################################################################################################################
# Functions for single image without and with init images
#######################################################################################################################

def single_image(this_prompt,this_seed,this_guidance):
    sys.stdout.write("Setting up StableCascadePriorPipeline pipeline ...\n")
    sys.stdout.flush()
    ...
    sys.stdout.write("Creating generator ...\n")
    sys.stdout.flush()

    generator = torch.Generator(device=device).manual_seed(this_seed)

    sys.stdout.write("Running prior pipeline ...\n")
    sys.stdout.flush()
    ...
    prior_output = prior(
        ...
        width=args2.width,
        height=args2.height,
        guidance_scale=args2.priorguidancescale, #4.0,
        num_inference_steps=args2.priorsteps, #20,
        num_images_per_prompt=num_images_per_prompt
    )

    sys.stdout.write("Cleaning up prior memory ...\n")
    sys.stdout.flush()
    ...
    sys.stdout.write("Setting up StableCascadeDecoderPipeline pipeline ...\n")
    sys.stdout.flush()
    ...
    sys.stdout.write("Running decoder pipeline ...\n")
    sys.stdout.flush()

    decoder_output = decoder(image_embeddings=prior_output.image_embeddings.half(),
        prompt=this_prompt,
        negative_prompt=args2.negative_prompt,
        guidance_scale=this_guidance, #args2.decoderguidancescale, #0.0,
        output_type="pil",
        num_inference_steps=args2.decodersteps #10
    ).images

    sys.stdout.write(f"Saving image to {args2.image_file} ...\n")
    sys.stdout.flush()

    decoder_output[0].save(args2.image_file)
    
    sys.stdout.write("Done\n")
    sys.stdout.flush()

def single_image_with_init(this_prompt,this_seed,input_image,output_image):

    torch.manual_seed(this_seed)
    torch.random.manual_seed(this_seed)
    np.random.seed(this_seed)

    sys.stdout.write(f'Prompt "{this_prompt}"\n')
    sys.stdout.write(f'Seed {this_seed}\n')
    #sys.stdout.write(f"IMAGE {init_image_file_name}\n")
    #sys.stdout.flush()
    ...
    sys.stdout.write("Loading extras and models ...\n")
    sys.stdout.flush()
    ...
    sys.stdout.write(f"Loading init image {input_image} ...\n")
    sys.stdout.flush()

    images = resize_image(load_image(input_image)).unsqueeze(0).expand(batch_size, -1, -1, -1).to(device)
    batch = {'images': images}

    caption = this_prompt
    height, width = args2.height, args2.width
    stage_c_latent_shape, stage_b_latent_shape = calculate_latent_sizes(height, width, batch_size=batch_size)

    effnet_latents = core.encode_latents(batch, models, extras)
    t = torch.ones(effnet_latents.size(0), device=device) * args2.init_strength
    noised = extras.gdf.diffuse(effnet_latents, t=t)[0]

    sys.stdout.write("Preparing conditions ...\n")
    sys.stdout.flush()

    # Stage C Parameters
    extras.sampling_configs['cfg'] = args2.priorguidancescale #4
    extras.sampling_configs['shift'] = 2
    extras.sampling_configs['timesteps'] = int(args2.priorsteps * args2.init_strength) #args2.priorsteps #20
    extras.sampling_configs['t_start'] = args2.init_strength #1.0
    extras.sampling_configs['x_init'] = noised
    
    # Stage B Parameters
    extras_b.sampling_configs['cfg'] = args2.decoderguidancescale #1.1
    extras_b.sampling_configs['shift'] = 1
    extras_b.sampling_configs['timesteps'] = args2.decodersteps #10
    ...
    #sys.stdout.write(f"Saving image to {args2.image_file} ...\n")
    sys.stdout.write(f"Saving image to {output_image} ...\n")
    sys.stdout.flush()

    sampled = np.clip(sampled.cpu(), a_min=0.0, a_max=1.0)
    pilImg = transforms.ToPILImage(mode="RGB")(sampled[0])
    #pilImg.save(args2.image_file)
    pilImg.save(output_image)
    pilImg.close
...
if args2.in_script_movie == 0:
    ...
    #######################################################################################################################
    # Single image with no init image - uses diffusers for speed
    #######################################################################################################################

    if args2.init_image is None:
        single_image(args2.prompt,args2.seed,args2.decoderguidancescale)
        sys.exit()
        
    #######################################################################################################################
    # Single image with init image
    #######################################################################################################################

    if args2.init_image is not None:
        single_image_with_init(args2.prompt,args2.seed,args2.init_image,args2.image_file)
        sys.exit()
...
if args2.in_script_movie == 1:
    ...
    sys.stdout.write(f"Generating initial frame ...\n")
    sys.stdout.flush()

    current_seed = args2.seed

    prompt = set_prompt(0,args2.prompt)

    if args2.init_image is None:
        single_image(prompt,current_seed,0)
        
    if args2.init_image is not None:
        single_image_with_init(prompt,current_seed,args2.init_image)
    
    if args2.color_matching == 1:
        src = args2.image_file
        tgt = args2.frame_dir+"first_frame.png"
        #sys.stdout.write(f"*** COPY SRC {src}\n")
        #sys.stdout.write(f"*** COPY TGT {tgt}\n")
        #sys.stdout.flush()
    ...
    while (frames<(args2.total_frames+1)):
        ...
        numpy_array = np.array(load_image(args2.image_file))
        ...
        if args2.zoom_after_sharpen == 0:
            #if args2.zoom != 1:
            if args2.zoom != "0:1":
            #if abs(float(args2.zoom)-1)>0.0001:
                sys.stdout.flush()
        ...
        if args2.randomize_seed == 1:
            #seed_everything(args2.seed+frames)
            
            #torch.random.manual_seed(args2.seed+frames)
            #np.random.seed(args2.seed+frames)
            
            current_seed = args2.seed+frames


        sys.stdout.flush()
        #sys.stdout.write('Generating frame ...\n')
        #sys.stdout.write(f'Prompt: "{prompt}"\n')
        #sys.stdout.write(f'Seed: "{current_seed}"\n')
        sys.stdout.flush()
        ...
        if frames % args2.render_every == 0:
            ...
            if args2.hard_cuts == 0:
                generator = torch.Generator(device="cuda").manual_seed(args2.seed+frames)
                images = pipe2(
                    prompt=prompt,
                    negative_prompt=args2.negative_prompt,
                    guidance_scale=args2.guidance_scale,
                    num_images_per_prompt=args2.num_images_per_prompt,
                    image=images,
                    num_inference_steps=args2.steps,
                    strength=args2.init_image_strength,
                    target_size=(args2.w,args2.h),
                    generator=generator
                    ).images[0]
            else:
        ...
        single_image_with_init(prompt,current_seed,args2.image_file,args2.image_file)
        #single_image_with_init(prompt,current_seed,args2.image_file)
