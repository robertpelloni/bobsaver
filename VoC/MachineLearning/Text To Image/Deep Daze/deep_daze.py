# original code https://github.com/lucidrains/deep-daze
...
class DeepDaze(nn.Module):
    def __init__(
            ...
            total_batches,
            batch_size,
            num_layers=8, # originally 8
            image_width=512,
            loss_coef=100, # originally 100
            theta_initial=None,
            theta_hidden=None,
            lower_bound_cutout=0.1, # should be smaller than 0.8
            upper_bound_cutout=1.0,
            saturate_bound=False, # originally False
            gauss_sampling=False,
            gauss_mean=0.6,
            gauss_std=0.2,
            do_cutout=True, # was True
            center_bias=False,
            center_focus=2,
            hidden_size=256,
        ...
        w0 = default(theta_hidden, 30.) # originally 30
        w0_initial = default(theta_initial, 30.) # originally 30
        ...
        self.saturate_limit = 0.75  # originally 0.75 - cutouts above 0.75 lead to destabilization
...
class Imagine(nn.Module):
    def __init__(
            ...
            *,
            ...
            img=None,
            clip_encoding=None,
            lr=0.000005, # originally 1e-5
            batch_size=32, # (32 in last VOC) originally 4 - increasing this seems to lead to sharper images - 32 seems to be a happy medium between detail and speed
            gradient_accumulate_every=1, # (1 in last VOC) originally 4 - setting to 1 makes it faster and gives sharper images
            save_every=100,
            image_width=512,
            num_layers=16, # (24 in last VOC) was 16 originally - lower values make larger sized features - higher values make more detailed/tiled like images
            epochs=20,
            iterations=1050,
            save_progress=True,
            seed=None,
            ...
            theta_hidden=None,
            model_name="ViT-B/32",
            lower_bound_cutout=0.1, # should be smaller than 0.8 originally 0.1
            upper_bound_cutout=1.0,
            saturate_bound=False, # originally False
            averaging_weight=0.3, # originally 0.3
            create_story=False,
            story_start_words=5,
            story_words_per_epoch=5,
            story_separator=None,
            gauss_sampling=False, # originally False
            gauss_mean=0.6, # originally 0.6
            gauss_std=0.2, # originally 0.2
            do_cutout=True, # originally True
            center_bias=True, # originally False
            center_focus=2,
            optimizer="AdamP", # originally AdamP, other options Adam or DiffGrad
            jit=True,
            hidden_size=256, # originally this was 256 - 512 gives richer/deeper colors
            save_gif=False,
        ...
        if exists(seed):
            sys.stdout.write(f'Setting seed to {seed} ...\n')
            sys.stdout.flush()
        ...
        sys.stdout.write('Setting model ...\n')
        sys.stdout.flush()
    ...
    def save_image(self, epoch, iteration, img=None):
        ...
        out_file = open(self.image_file, 'wb')
        pil_img = T.ToPILImage()(img.squeeze())
        #tweak display image
        pil_img = torchvision.transforms.functional.adjust_brightness(pil_img,0.7)
        pil_img = torchvision.transforms.functional.adjust_contrast(pil_img,6.0)
        pil_img = torchvision.transforms.functional.adjust_sharpness(pil_img,9.0)
        ...
        if self.frame_dir is not None:
            #extension = os.path.splitext(self.image_file)[1]
            #sys.stdout.write(f"Extension for frame file is {extension}\n")
            #sys.stdout.flush()
            #file_list = glob.glob(r"C:\Data\*.csv")
            #import glob
            #import re
            #file_list = glob.glob(self.frame_dir+"FRA?????"+extension)
            #file_list = [file for file in os.listdir(self.frame_dir) if re.search('.*\.png', file)]  
            
            #file_list = [file for file in os.listdir(self.frame_dir) if file.startswith("FRA") and file.endswith(extension)]  
            file_list = []
            ...
            if file_list:
                ...
                count_string = "00001"
            save_name = self.frame_dir+"\FRA"+count_string+".png"

            out_file = open(save_name, 'wb')
            pil_img = T.ToPILImage()(img.squeeze())
            #tweak display image
            pil_img = torchvision.transforms.functional.adjust_brightness(pil_img,0.7)
            pil_img = torchvision.transforms.functional.adjust_contrast(pil_img,6.0)
    ...
    def forward(self):
        ...
        itt = 1
        for asatreat in range(self.iterations):
            _,total_loss,step_loss = self.train_step(1, itt)
            itt+=1
