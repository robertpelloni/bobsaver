# Optimized ruDALLE v2.0
# Original file is located at https://colab.research.google.com/drive/1F7Irb-00N5NU2lEb3aH5Yzh8RH0c8K1T
...
sys.stdout.write("Getting DALL-E ...\n")
sys.stdout.flush()
...
def dalle_sa_forward(self, hidden_states, ltor_mask, has_cache=False, use_cache=False,):
    ...
    if use_cache and has_cache:
        ...
        # query_layer = torch.cat((self.past_query, query_layer), dim=-2)  # Disabled, main source of speedup
        key_layer = torch.cat((self.past_key, key_layer), dim=-2)
    if use_cache and has_cache and self.past_attentions.shape[-1] > t:
        # self.past_key[..., t-1:t, :] = key_layer
        # self.past_value[..., t-1:t, :] = value_layer
        # key_layer = self.past_key[..., :t, :]
        # value_layer = self.past_value[..., :t, :]
        attention_scores = calculate_attention_scores(
            query_layer=query_layer, key_layer=key_layer, ltor_mask=ltor_mask
        )
        self.past_attentions[:, :, t-1:t, :t] = attention_scores
        ...
        allocate_every = 1024
        n = -(-t // allocate_every) * allocate_every  # allocate every 1024
        r = -(-n//t)
        # print(r, self.past_attentions.shape, attention_scores.shape)
        self.past_attentions = (attention_scores if not has_cache else self.past_attentions).clone().repeat(1, 1, r, r) - 10000.0
        self.past_attentions[:, :, :t, :t] = attention_scores
        # self.past_key = key_layer.clone().repeat(1, 1, r, 1)
        # self.past_value = value_layer.clone().repeat(1, 1, r, 1)
        if has_cache:
            query_layer = torch.cat((self.past_query, query_layer), dim=-2)
    attention_scores = self.past_attentions[:, :, :t, :t]
...
def dalle_mlp_forward_(self, hidden_states, has_cache=False, use_cache=False):
    ...
    if use_cache:
        if has_cache and dont_alloc:
            ...
            # print("alloc")
            n = -(-t // 1024) * 1024  # allocate every 1024
            # print(n)
...
#text = 'синий кот' #@param {type:"string"}
