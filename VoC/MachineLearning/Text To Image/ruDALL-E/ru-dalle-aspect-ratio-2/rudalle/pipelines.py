...
def show(pil_images, nrow=4, size=14, save_dir=None, show=True, image_name=None):
    """
    ...
    if save_dir is not None:
        ...
        for k in range(len(pil_images)):
            ...
            if file_list:
                ...
                promptlen=len(image_name)+1
                count_value = int(last_name[promptlen:promptlen+5])+1
                count_string = f"{count_value:05d}"
            else:
                count_string = "00001"
            save_name = save_dir+"\\"+image_name+" "+count_string+".png"
    ...
    for i, img in enumerate(imgs):
        ...
        if thiscount > 1:
            if save_dir is not None:
                ...
                img.save(join(save_dir, f'{image_name}.png'))
        if show:
