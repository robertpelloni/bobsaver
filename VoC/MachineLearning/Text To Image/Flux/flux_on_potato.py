...
if args2.init_image != None:
    ...
    seed_image = load_image(args2.init_image)
    #pipeline = FluxImg2ImgPipeline.from_pretrained("eramth/flux-4bit",torch_dtype=torch.float16).to("cuda")
    pipeline = FluxImg2ImgPipeline.from_pretrained("eramth/flux-4bit").to("cuda")
    pipeline.vae.enable_tiling()

    #kw = {"image":seed_image,"prompt":args2.prompt,"guidance_scale":3.5,"num_inference_steps":args2.steps}
    #image = pipeline(**kw,strength=args2.init_image_strength,generator=torch.Generator(pipeline.device).manual_seed(args2.seed)).images[0]
    image = pipeline(
        image=seed_image,
        prompt=args2.prompt,
