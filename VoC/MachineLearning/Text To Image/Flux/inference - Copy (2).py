...
if args2.in_script_movie == 1:
    ...
    images = pipe.generate(
        ...
        prompt=prompt,
        seed=args2.seed,
        guidance=args2.guidance_scale,
        strength=args2.init_image_strength,
        height=args2.h,
        width=args2.w,
        num_steps=args2.steps,
    )
    """
    ...
    """
    with open(args2.frame_dir+"first_frame.png", "wb") as f:
        f.write(images.getvalue())    
    with open(args2.frame_dir+"last_frame.png", "wb") as f:
        f.write(images.getvalue())    
    """
    image.save(args2.frame_dir+"first_frame.png")
    image.save(args2.frame_dir+"last_frame.png")
    
    lastprompt = set_prompt(1,prompt)

    #steps = int(args2.steps*(1/args2.init_image_strength))

    sys.stdout.write(f"Setting up img2img pipeline ...\n")
    sys.stdout.flush()
    
    pipeline = FluxImg2ImgPipeline.from_pretrained("eramth/flux-4bit").to(device)
    ...
    while (frames<(args2.total_frames+1)):
        ...
        this_image = Image.open(args2.frame_dir+"last_frame.png")
        numpy_array = np.array(this_image)
        #this_image.save(args2.frame_dir+"after_load.png")
        ...
        images.save(args2.frame_dir+"seed_image.png")
        seed_image = load_image(args2.frame_dir+"seed_image.png")
        ...
        if frames % args2.render_every == 0:
            if args2.hard_cuts == 0:
                ...
                image = pipeline(
                    image=seed_image,
                    #image=load_image(args2.frame_dir+"seed_image.png"),
                    prompt=prompt,
                    ...
                    width=args2.w,
                    num_inference_steps=args2.steps,
                    generator=torch.Generator(pipeline.device).manual_seed(args2.seed),
                ).images[0]
                #with open(args2.frame_dir+"last_frame.png", "wb") as f:
                #    f.write(images.getvalue())    
                image.save(args2.frame_dir+"last_frame.png")
