...
sys.path.append('./latent-diffusion-1-3')
...
config = OmegaConf.load("./latent-diffusion-1-3/configs/latent-diffusion/txt2img-1p4B-eval.yaml")  # TODO: Optionally download from same location as ckpt and chnage this logic
...
def parse_prompt(prompt):
    ...
    #sys.stdout.write(f'*** vals[0]={vals[0]} vals[1]={vals[1]}\n')
    sys.stdout.flush()
...
def do_run():
  #  with torch.cuda.amp.autocast():
        global progress, target_embeds, weights, zero_embed, init, scale_factor, itt, image_count
        scale_factor = 1
        make_cutouts = {}
        for i in clip_list:
            ...
            with torch.cuda.amp.autocast():
                with model.ema_scope():
                    ...
                    for n in range(opt.n_iter):
                        ...
                        #import shutil
                        #if os.path.exists("GFPGAN/results/"):
                        #    shutil.rmtree("GFPGAN/results/")
                        ...
                        for custom_schedule in custom_schedules:
                            ...
                            if type(custom_schedule) != type(""):
                                ...
                                if method == "latent":
                                    sys.stdout.write('Starting latent upscaling ...\n')
...
if args.latent_tv_loss==1:
    latent_tv_loss = True #Applies the TV Loss in the Latent space instead of pixel, improves generation quality
else:
    latent_tv_loss = False #Applies the TV Loss in the Latent space instead of pixel, improves generation quality
...
how_many_batches = args.how_many_batches #1 #@param{type: 'integer'}
...
if(custom_settings != None and custom_settings != '' and custom_settings != 'path/to/settings.cfg'):
...
if(((init_image != None) and (init_image != 'None') and (init_image != '')) and (starting_timestep != 1) and (custom_schedule_setting[0][1] == 1000)):
  custom_schedule_setting[0] = [custom_schedule_setting[0][0], custom_schedule_setting[0][1]*starting_timestep, custom_schedule_setting[0][2]]
...
for n in range(how_many_batches): 
    ...
    opt.n_iter = 1 # Old way for batching, avoid touching. The batching is now done by the for-loop we are in. Setting a value > 1 here will cause that batching also happens within the do_run() method, which for some reason fails at cleaning CUDA cache and causes out of memory error. 
    opt.n_samples =  args.n_samples # How many samples in parallel. Seems to break upscaling.
    torch.cuda.empty_cache()
    gc.collect()
