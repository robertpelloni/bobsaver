...
# Original file is located at https://colab.research.google.com/github/multimodalart/MajestyDiffusion/blob/main/latent.ipynb
...
import transformers
import gc
sys.path.append('./latent-diffusion-1-6')
...
def parse_args():
  ...
  parser.add_argument('--latent_prompts', type=str)
  parser.add_argument('--latent_negatives', type=str)

  parser.add_argument('--gfpgan_model', type=str)
  parser.add_argument('--latent_diffusion_model', type=str)
  parser.add_argument('--ViT_B32', type=int)
  parser.add_argument('--ViT_B16', type=int)
  parser.add_argument('--ViT_L14', type=int)
  parser.add_argument('--ViT_L14_laion',type=int)
  ...
  parser.add_argument('--use_cond_fn', type=int)
  parser.add_argument('--latent_tv_loss', type=int)
  ...
  parser.add_argument('--scale_div', type=float)
  parser.add_argument('--opt_mag_mul', type=float)
  parser.add_argument('--opt_ddim_eta', type=float)
  parser.add_argument('--opt_eta_end', type=float)
  parser.add_argument('--opt_temperature', type=float)
  parser.add_argument('--grad_center', type=int)
  parser.add_argument('--grad_scale', type=float)
  parser.add_argument('--anti_jpg', type=float)
  parser.add_argument('--RGB_min', type=float)
  parser.add_argument('--RGB_max', type=float)
  parser.add_argument('--flip_aug', type=int)
  
  parser.add_argument('--cut_blur_n', type=int)
  parser.add_argument('--plms', type=int)
  parser.add_argument('--score_modifier', type=int)
  parser.add_argument('--threshold_percentile',type=float)
  parser.add_argument('--grad_blur', type=int)
  ...
  parser.add_argument('--latent_diffusion_guidance_scale', type=float)
  parser.add_argument('--clip_guidance_scale', type=float)
  parser.add_argument('--aesthetic_loss_scale', type=float)
  ...
  parser.add_argument('--init_brightness', type=float)
  parser.add_argument('--starting_timestep', type=float)
  parser.add_argument('--init_rotate', type=int)
  parser.add_argument('--mask_rotate', type=int)
  parser.add_argument('--init_noise', type=float)
  parser.add_argument('--init_magnitude', type=float)
  parser.add_argument('--n_samples', type=int)
  parser.add_argument('--how_many_batches', type=int)
  parser.add_argument('--cut_blur_kernel', type=int)
  parser.add_argument('--ths_method', type=str)
  parser.add_argument('--threshold', type=int)
  parser.add_argument('--var_range', type=float)
  parser.add_argument('--mean_range', type=float)
  parser.add_argument('--compress_steps', type=int)
  parser.add_argument('--compress_factor', type=float)
  parser.add_argument('--punish_steps', type=int)
  parser.add_argument('--punish_factor', type=float)
  parser.add_argument('--custom_settings', type=str)
...
latent_diffusion_model = args.latent_diffusion_model #'original' #@param ["original", "finetuned"]
...
def load_model_from_config(config, ckpt, verbose=False, latent_diffusion_model="original"):
    ...
    print(f"Using {latent_diffusion_model} latent diffusion model")
    ...
    if(latent_diffusion_model == "ongo"):
      del sd  
    ...
    if(latent_diffusion_model == "erlich"):
      del sd  
    ...
    #if(latent_diffusion_model == "puck (fine tuned in pixel art)"):
    if(latent_diffusion_model == "puck"):
      del sd
      sd_finetuned = torch.load(f"{model_path}/puck.pt")
      sd_finetuned["input_blocks.0.0.weight"] = sd_finetuned["input_blocks.0.0.weight"][:,0:4,:,:]
      model.model.diffusion_model.load_state_dict(sd_finetuned, strict=False)
      del sd_finetuned
      torch.cuda.empty_cache()
...
config = OmegaConf.load("./latent-diffusion-1-6/configs/latent-diffusion/txt2img-1p4B-eval.yaml")  # TODO: Optionally download from same location as ckpt and chnage this logic
...
def parse_prompt(prompt):
    #if prompt.startswith('http://') or prompt.startswith('https://') or prompt.startswith("E:") or prompt.startswith("C:") or prompt.startswith("D:") or prompt.startswith("G:"):
    if prompt.startswith('http://') or prompt.startswith('https://') or (prompt != '' and prompt[1] == ':'):
...
def display_handler(x,i,cadance = 5, decode = True):
    global progress, image_grid, writer, img_tensor, im, itt, image_count
    ...
    if args.n_samples == 1:
        ...
        if itt%args.update==0:
            ...
            if i%cadance==0:
                ...
                with io.BytesIO() as output:
                    ...
                    im.save(args.image_file, format = 'png')
                    if args.how_many_batches > 1:
                        #auto-numbered image filename
                        tmpname = args.image_file
                        tmpname = tmpname[:len(tmpname)-4]
                        tmpname = tmpname + ' ' + str(image_count)
                        tmpname = tmpname + '.png'
        ...
        if itt%args.update==0:
            ...
            if i%cadance==0:
                ...
                #Here you split each image grid into a single image
                no_pad_grid = make_grid(torch.clamp((x+1.0)/2.0, min=0.0, max=1.0),round(x.shape[0]**0.5), padding=0)
                chunks = no_pad_grid.chunk(opt.n_samples,1)
                j = 0
                for chunk in chunks:
                    im = T.ToPILImage()(chunk)
                    #im.save("probably_you_want_to_do_image_file_000j.png", format = "png")
                    tmpname = args.image_file
                    tmpname = tmpname[:len(tmpname)-4]
                    tmpname = tmpname + ' ' + str(j+1)
                    tmpname = tmpname + '.png'
                    im.save(tmpname, format = "png")
                    j+=1
                grid = 255. * rearrange(grid, 'c h w -> h w c').detach().cpu().numpy()
                image_grid = grid.copy(order = "C")
                #You don't need BytesIO here 
                im = Image.fromarray(grid.astype(np.uint8))
                im.save(args.image_file, format = 'png')
...
def do_run():
        global has_purged
        if(has_purged):
            global clip_model, clip_size, clip_tokenize, clip_normalize, clip_list, itt, image_count
            clip_model, clip_size, clip_tokenize, clip_normalize, clip_list = full_clip_load(clip_load_list)
            has_purged = False
  #  with torch.cuda.amp.autocast():
        global progress,target_embeds, weights, zero_embed, init, scale_factor, cur_step, uc, c, itt
        cur_step = 0
        scale_factor = 1
        make_cutouts = {}
        for i in clip_list:
            ...
            with torch.cuda.amp.autocast():
                with model.ema_scope():
                    ...
                    for n in range(opt.n_iter):
                        ...
                        #sys.stdout.write("***** NEXT BATCH IMAGE *****\n")
                        #sys.stdout.flush()
                            ...
                            if type(custom_schedule) != type(""):
                                ...
                                if ('|' in args.prompt):
                                    samples_ddim, _ = sampler.sample(S=opt.ddim_steps,
                                                                 ...
                                                                 #fails for multiple prompts witn more than one pipe character
                                                                 #clip_embed = target_embeds["ViT-L-14--openai"] if "ViT-L-14--openai" in clip_list else None
                                                                 #works for more than one pipe char, fails for no pipe chars
                                                                 clip_embed = (target_embeds["ViT-L-14--openai"] * weights["ViT-L-14--openai"].squeeze().unsqueeze(1)).sum(0,keepdim = True) / weights["ViT-L-14--openai"].sum() if "ViT-L-14--openai" in clip_list else None
                                                                )
                                else:
                                    samples_ddim, _ = sampler.sample(S=opt.ddim_steps,
                                                                 conditioning=c,
                                ...
                                if method == "RGB":
                                    ...
                                    sys.stdout.write('Finished latent upscaling\n')
                                if method == "gfpgan":
                                    scale_factor = float(scale_factor)
                                    last_step_upscale = True
                                    temp_file_name = "temp_"+f"{str(round(time.time()))}.png"
                                    ...
                                    gfpgancmd = f"cd GFPGAN&python  -B -W ignore inference_gfpgan.py -i ..\{temp_file} -o results -v {GFP_ver} -s {GFP_factor} --model_name {args.gfpgan_model}&cd.."
                                    
                                    sys.stdout.write('Executing GFPGAN ...\n')
                                    sys.stdout.write(f'{gfpgancmd}\n')
                                    sys.stdout.flush()
                                    
                                    os.system(gfpgancmd)
                                    
                                    sys.stdout.write('Finished GFPGAN\n\n')
                                    sys.stdout.flush()
                        ...
                        if(last_step_upscale and method == 'gfpgan'):
                          ...
                          sys.stdout.write("Saving progress ...\n")
                          sys.stdout.flush()
                          latest_upscale.save(args.image_file, format = "png")
                          sys.stdout.flush()
                          sys.stdout.write("Progress saved\n")
                          sys.stdout.flush()
                        else:
                          ...
                          sys.stdout.write("Saving progress ...\n")
                          sys.stdout.flush()
                          Image.fromarray(image_grid.astype(np.uint8)).save(args.image_file, format = "png")
                          sys.stdout.flush()
                          sys.stdout.write("Progress saved\n")
                          sys.stdout.flush()
...
cut_blur_kernel = args.cut_blur_kernel #3
cut_ic_pow = args.cut_ic_pow #.1
cutn_batches = args.cutn_batches #1
ths_method = args.ths_method

symmetric_loss_scale = args.symmetric_loss_scale #0 #Apply symmetric loss

#Latent Diffusion Advanced Settings
scale_div = args.scale_div #0.5 # Use when latent upscale to correct satuation problem
opt_mag_mul = args.opt_mag_mul #15 #Magnify grad before clamping
if(args.plms == 1):
  opt_plms = True
else:
  opt_plms = False
opt_ddim_eta = args.opt_ddim_eta
opt_eta_end = args.opt_eta_end # linear variation of eta
opt_temperature = args.opt_temperature #.95 
...
if args.grad_center==1:
    ...
    grad_center = False
grad_scale = args.grad_scale #1 #5 Lower value result in more coherent and detailed result, higher value makes it focus on more dominent concept
#Restraints the model from explodign despite larger clamp
if(args.score_modifier == 1):
  score_modifier = True
else:
  score_modifier = False
threshold_percentile = args.threshold_percentile #.85
threshold = args.threshold #1 
score_corrector_setting = ["latent",""]
var_range = args.var_range #0.5
mean_range = args.mean_range #0.75
#Noise settings
...
#How much you want this to influence your result
experimental_aesthetic_embeddings_weight = args.experimental_aesthetic_embeddings_weight #0.5
#9 are good aesthetic embeddings, 0 are bad ones
experimental_aesthetic_embeddings_score = args.experimental_aesthetic_embeddings_score #9
...
if(args.grad_blur == 1):
  ...
  grad_blur = False
compress_steps = args.compress_steps #200
compress_factor = args.compress_factor #0.1
punish_steps = args.punish_steps #200
punish_factor = args.punish_factor #0.5
...
clip_prompts = [ phrase.strip() for phrase in args.prompt.split("|") ]

#sys.stdout.write(f'*** clip_prompts={clip_prompts}\n')
#sys.stdout.flush()

#Prompt for Latent Diffusion
latent_prompts = [ args.latent_prompts ]
#latent_prompts = [ phrase.strip() for phrase in args.latent_prompts.split("|") ]

#sys.stdout.write(f'*** latent_prompts={latent_prompts}\n')
#sys.stdout.flush()

#Negative prompts for Latent Diffusion
latent_negatives = [ args.latent_negatives ]
#latent_negatives = [ phrase.strip() for phrase in args.latent_negatives.split("|") ]
...
#this next prompt works fine, 2 image prompts from web addresses, BUT passing this in from VoC fails?!
#image_prompts =['https://upload.wikimedia.org/wikipedia/commons/thumb/e/ec/Mona_Lisa%2C_by_Leonardo_da_Vinci%2C_from_C2RMF_retouched.jpg/687px-Mona_Lisa%2C_by_Leonardo_da_Vinci%2C_from_C2RMF_retouched.jpg','https://upload.wikimedia.org/wikipedia/commons/thumb/0/0f/1665_Girl_with_a_Pearl_Earring.jpg/1200px-1665_Girl_with_a_Pearl_Earring.jpg']
...
width = args.sizex # 256#@param{type: 'integer'}
height = args.sizey # 256#@param{type: 'integer'}

latent_diffusion_guidance_scale = args.latent_diffusion_guidance_scale #1.75 #@param {type:"number"}
clip_guidance_scale = args.clip_guidance_scale #5000 #@param{type: 'integer'}

how_many_batches = args.how_many_batches #@param{type: 'integer'}
...
init_scale = args.init_scale #1000 #@param{type: 'integer'}
init_brightness = args.init_brightness #0.0 #@param{type: 'number'}
#@markdown How much extra noise to add to the init image, independently from skipping timesteps
init_noise = args.init_noise #0 #@param{type: 'integer'}
#Init image advanced settings
if args.init_rotate==1:
    init_rotate = True
else:
    init_rotate = False
if args.mask_rotate==1:
    mask_rotate = True
else:
    mask_rotate = False
init_magnitude = args.init_magnitude #0.15
...
custom_settings = args.custom_settings #@param{type:'string'}
...
for n in range(how_many_batches):
  ...
  opt.n_samples =  args.n_samples #1 #How many implaes in parallel. Breaks upscaling
  torch.cuda.empty_cache()
  gc.collect()
