...
sys.path.append('./deforum-stable-diffusion-07')
sys.path.append('./deforum-stable-diffusion-07/src')
...
def parse_args():
    ...
    parser.add_argument("--hybrid_video_generate_inputframes",  type=int, help="?")
    parser.add_argument("--hybrid_video_use_first_frame_as_init_image",  type=int, help="?")
    parser.add_argument("--hybrid_video_motion",  type=str, help="?")
    parser.add_argument("--hybrid_video_flow_method",  type=str, help="?")
    parser.add_argument("--hybrid_video_composite",  type=int, help="?")
    parser.add_argument("--hybrid_video_comp_mask_type",  type=str, help="?")
    parser.add_argument("--hybrid_video_comp_mask_inverse",  type=int, help="?")
    parser.add_argument("--hybrid_video_comp_mask_equalize",  type=str, help="?")
    parser.add_argument("--hybrid_video_comp_mask_auto_contrast",  type=int, help="?")
    parser.add_argument("--hybrid_video_comp_save_extra_frames",  type=int, help="?")
    parser.add_argument("--hybrid_video_use_video_as_mse_image",  type=int, help="?")
...
def Root():
    ...
    configs_path = "./deforum-stable-diffusion-07/configs" #@param {type:"string"}
    ...
    model_config = args2.config #"v2-inference-v.yaml" #@param ["custom","v2-inference.yaml","v2-inference-v.yaml","v1-inference.yaml"]
    model_checkpoint =  args2.ckpt #"v2-1_768-ema-pruned.ckpt" #@param ["custom","v2-1_768-ema-pruned.ckpt","v2-1_512-ema-pruned.ckpt","768-v-ema.ckpt","512-base-ema.ckpt","v1-5-pruned.ckpt","v1-5-pruned-emaonly.ckpt","sd-v1-4-full-ema.ckpt","sd-v1-4.ckpt","sd-v1-3-full-ema.ckpt","sd-v1-3.ckpt","sd-v1-2-full-ema.ckpt","sd-v1-2.ckpt","sd-v1-1-full-ema.ckpt","sd-v1-1.ckpt", "robo-diffusion-v1.ckpt","wd-v1-3-float16.ckpt"]
...
#VOC FINISH 2 - DO NOT DELETE
    ...
    if args2.hybrid_video_generate_inputframes == 1:
        hybrid_video_generate_inputframes = True #False #@param {type:"boolean"}
    else:
        hybrid_video_generate_inputframes = False #False #@param {type:"boolean"}
    if args2.hybrid_video_use_first_frame_as_init_image == 1:
    ...
    hybrid_video_motion = args2.hybrid_video_motion #"None" #@param ['None','Optical Flow','Perspective','Affine']
    hybrid_video_flow_method = args2.hybrid_video_flow_method #"Farneback" #@param ['Farneback','DenseRLOF','SF']
    if args2.hybrid_video_composite == 1:
        hybrid_video_composite = True #@param {type:"boolean"}
    else:
        hybrid_video_composite = False #@param {type:"boolean"}
    hybrid_video_comp_mask_type = args2.hybrid_video_comp_mask_type #"None" #@param ['None', 'Depth', 'Video Depth', 'Blend', 'Difference']
    if args2.hybrid_video_comp_mask_inverse == 1:
        hybrid_video_comp_mask_inverse = True #@param {type:"boolean"}
    else:
        hybrid_video_comp_mask_inverse = False #@param {type:"boolean"}
    hybrid_video_comp_mask_equalize = args2.hybrid_video_comp_mask_equalize #"None" #@param  ['None','Before','After','Both']
    if args2.hybrid_video_comp_mask_auto_contrast == 1:
    ...
    if args2.hybrid_video_comp_save_extra_frames == 1:
        hybrid_video_comp_save_extra_frames = True #@param {type:"boolean"}
    else:
        hybrid_video_comp_save_extra_frames = False #@param {type:"boolean"}
    if args2.hybrid_video_use_video_as_mse_image == 1:
        hybrid_video_use_video_as_mse_image = True #@param {type:"boolean"}
    else:
...
def DeforumArgs():
    ...
    sampler = args2.sampler #'dpmpp_2s_a' #@param ["klms","dpm2","dpm2_ancestral","heun","euler","euler_ancestral","plms", "ddim", "dpm_fast", "dpm_adaptive", "dpmpp_2s_a", "dpmpp_2m"]
    ...
    strength = args2.strength #0.1 #@param {type:"number"}
    ...
    frame_start_number = args2.frame_start_number
    self_driven_caption = args2.self_driven_caption
