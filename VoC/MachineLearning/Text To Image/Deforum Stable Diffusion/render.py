...
def save_8_16_or_32bpc_image(image, outdir, filename, bit_depth_output): 
    ...
    print(f'\nImage saved as {os.path.join(outdir, filename)}\n')
...
def render_image_batch(root, args, prompts, cond_prompts, uncond_prompts):
    ...
    for iprompt, (cond_prompt, uncond_prompt) in enumerate(zip(cond_prompts,uncond_prompts)):
            ...
            for image in init_array: # iterates the init images
                ...
                for image in results:
                    ...
                    if args.save_samples:
                        ...
                        #sys.stdout.flush()
                        sys.stdout.write(f'\nSaving individual image "{filename}"...\n')
                        #sys.stdout.flush()
                        ...
                        sys.stdout.write(f'\nSaved individual image "{filename}"\n')
                        sys.stdout.flush()
        ...
        if args.make_grid:
            ...
            print(f'\nImage saved as {os.path.join(args.outdir, filename)}\n')
            display.clear_output(wait=True)            
