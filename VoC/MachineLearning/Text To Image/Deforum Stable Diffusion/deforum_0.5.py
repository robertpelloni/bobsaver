    ...
    def view_sample_step(self, latents, path_name_modifier=''):
        if self.save_sample_per_step or self.show_sample_per_step:
            ...
            if self.show_sample_per_step:
                sys.stdout.write(f"{path_name_modifier}\n")
                sys.stdout.flush()
...
def parse_weight(match, frame = 0)->float:
    ...
    if check_is_number(w_raw):
        ...
        t = frame
        if len(w_raw) < 3:
            sys.stdout.write('the value inside `-characters cannot represent a math function\n')
            sys.stdout.flush()
            return 1
...
def generate(args, frame = 0, return_latent=False, return_sample=False, return_c=False):
    ...
    if not args.use_init and args.strength > 0 and args.strength_0_no_init:
        ...
        sys.stdout.write("If you want to force strength > 0 with no init, please set strength_0_no_init to False.\n")
        sys.stdout.flush()
        args.strength = 0
    ...
    if args.use_mask:
        ...
        print(f"Preparing mask {args.mask_file}...",flush=True)
...
model_checkpoint = args2.ckpt #"sd-v1-4.ckpt" #@param ["custom","sd-v1-4-full-ema.ckpt","sd-v1-4.ckpt","sd-v1-3-full-ema.ckpt","sd-v1-3.ckpt","sd-v1-2-full-ema.ckpt","sd-v1-2.ckpt","sd-v1-1-full-ema.ckpt","sd-v1-1.ckpt", "robo-diffusion-v1.ckpt","waifu-diffusion-v1-3.ckpt"]
...
if embedding_type == ".pt":
  ...
  if os.path.isfile(file_path):
    ...
    sys.stdout.write('using .pt embedding\n')
    sys.stdout.flush()
  ...
  if os.path.isfile(file_path):
    ...
    sys.stdout.write('using .bin embedding\n')
    sys.stdout.flush()
...
def render_animation(args, anim_args):
    ...
    while frame_idx < anim_args.max_frames:
        ...
        sys.stdout.flush
        sys.stdout.write(f"\nRendering animation frame {frame_idx} of {anim_args.max_frames}\n")
        sys.stdout.flush
        ...
        if turbo_steps > 1:
            ...
            for tween_frame_idx in range(tween_frame_start_idx, frame_idx):
                ...
                sys.stdout.write(f"  creating in between frame {tween_frame_idx} tween:{tween:0.2f}\n")
                sys.stdout.flush()
        ...
        sys.stdout.write(f"{args.prompt} {args.seed}\n")
        sys.stdout.flush()
        if not using_vid_init:
            sys.stdout.write(f"Angle: {keys.angle_series[frame_idx]} Zoom: {keys.zoom_series[frame_idx]}\n")
            sys.stdout.write(f"Tx: {keys.translation_x_series[frame_idx]} Ty: {keys.translation_y_series[frame_idx]} Tz: {keys.translation_z_series[frame_idx]}\n")
            sys.stdout.write(f"Rx: {keys.rotation_3d_x_series[frame_idx]} Ry: {keys.rotation_3d_y_series[frame_idx]} Rz: {keys.rotation_3d_z_series[frame_idx]}\n")
            sys.stdout.flush()
...
def vid2frames(video_path, frames_path, n=1, overwrite=True):      
    if not os.path.exists(frames_path) or overwrite: 
      ...
      while success:
        ...
        success,image = vidcap.read()
        count += 1
      sys.stdout.write("Converted %d frames" % count)
      sys.stdout.flush()
    else: sys.stdout.write("Frames already unpacked\n")
...
def render_input_video(args, anim_args):
    ...
    if anim_args.use_mask_video:
        ...
        sys.stdout.write(f"Exporting Video Frames (1 every {anim_args.extract_nth_frame}) frames to {mask_in_frame_path}...\n")
        sys.stdout.flush()
...
if override_settings_with_file:
    sys.stdout.write(f"reading custom settings from {custom_settings_file}\n")
    sys.stdout.flush()
    if not os.path.isfile(custom_settings_file):
        sys.stdout.write('The custom settings file does not exist. The in-notebook settings will be used instead\n')
        sys.stdout.flush()
    else:
        with open(custom_settings_file, "r") as f:
            ...
            for i, k in enumerate(args_dict):
                if k in jdata:
                    ...
                    sys.stdout.write(f"key {k} doesn't exist in the custom settings data! using the default value of {args_dict[k]}\n")
                    sys.stdout.flush()
            for i, k in enumerate(anim_args_dict):
                if k in jdata:
                    anim_args_dict[k] = jdata[k]
                else:
                    sys.stdout.write(f"key {k} doesn't exist in the custom settings data! using the default value of {anim_args_dict[k]}\n")
                    sys.stdout.flush()
            sys.stdout.write(args_dict)
            sys.stdout.write(anim_args_dict)
            sys.stdout.flush()
