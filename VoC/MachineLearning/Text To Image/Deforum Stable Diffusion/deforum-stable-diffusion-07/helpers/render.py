...
import cv2
import numpy as np
from PIL import Image, ImageChops, ImageOps, ImageEnhance, ImageDraw, ImageFont
import pathlib
...
def render_image_batch(args, prompts, root):
    ...
    for iprompt, prompt in enumerate(prompts):  
            ...
            for image in init_array: # iterates the init images
                ...
                for image in results:
                    ...
                    if args.save_samples:
                        ...
                        sys.stdout.flush()
                        sys.stdout.write(f'args.image_file =  "{args.image_file}"...\n')
                        sys.stdout.flush()

                        if len(prompts) == 1:
                            filename = f"{args.image_file[:-4]} {args.seed}.png"
                        else:
                            filename = f"{os.path.dirname(args.image_file)}\\{args.prompt} {args.seed}.png"
        ...
        if args.make_grid:
            if len(prompts) == 1:
                if args.n_batch > 1:
                    filename=f"{args.image_file[:-4]} {args.seed} grid.png"
                else:
                    filename=f"{args.image_file[:-4]} {args.seed}.png"
            else:
                filename = f"{os.path.dirname(args.image_file)}\\{args.prompt} {args.seed}.png"
...
def caption_image(args,filename):
    #caption image
    if args.self_driven_caption == 1:
        img = Image.open(filename)
        draw = ImageDraw.Draw(img)
        myFont = ImageFont.truetype("Roboto-Black.ttf", 16)
        w, h = draw.textsize(args.prompt)
        w, h = myFont.getsize(args.prompt)
        xpos = (img.width//2)-(w//2)
        ypos = (img.height - 32)
        draw.text((xpos-1, ypos-1),args.prompt,(0,0,0),font=myFont)
        draw.text((xpos-1, ypos+1),args.prompt,(0,0,0),font=myFont)
        draw.text((xpos+1, ypos-1),args.prompt,(0,0,0),font=myFont)
        draw.text((xpos+1, ypos+1),args.prompt,(0,0,0),font=myFont)
        draw.text((xpos, ypos),args.prompt,(255,255,0),font=myFont)
        img.save(filename)
...
def render_animation(args, anim_args, animation_prompts, root):
    ...
    if anim_args.animation_mode in ['2D','3D']:
        if anim_args.hybrid_video_composite or anim_args.hybrid_video_motion in ['Affine', 'Perspective', 'Optical Flow']:
            ...
            #hybrid_frame_path = f"{args.outdir}\hybridframes"
            #print(f"**********{hybrid_frame_path}**********")
    ...
    while frame_idx < anim_args.max_frames:
        ...
        if turbo_steps > 1:
            ...
            for tween_frame_idx in range(tween_frame_start_idx, frame_idx):
                ...
                filename = f"FRA{tween_frame_idx+1+args.frame_start_number:05}.png"
                
                cv2.imwrite(os.path.join(args.outdir, filename), cv2.cvtColor(img.astype(np.uint8), cv2.COLOR_RGB2BGR))
                
                caption_image(args, os.path.join(args.outdir, filename))
        ...
        filename = f"FRA{frame_idx+1+args.frame_start_number:05}.png"
...
def render_interpolation(args, anim_args, animation_prompts, root):
    ...
    for i, prompt in animation_prompts.items():
        ...
        sys.stdout.write(f'Preparing interpolation keyframe {i} for "{prompt}"\n')
        sys.stdout.write(f"There will be no display for this frame\n")
        sys.stdout.flush()
