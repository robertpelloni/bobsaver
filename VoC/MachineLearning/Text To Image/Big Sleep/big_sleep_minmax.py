# Big sleep Minmax
# Original file is located at https://colab.research.google.com/drive/12CnlS6lRGtieWujXs3GQ_OlghmFyl8ch
...
def parse_args():
  ...
  parser.add_argument('--minimize_phrase', type=str, help='Text to minimize.')
...
#with open('clip.out') as f:
#  entries = f.read.split('\n')

#@markdown Your text prompt to maximize:
promptInputMaximize = args.input_phrase
#@markdown Your text prompt to minimize:
promptInputMinimize = args.minimize_phrase #@param{type:"string"}
#@markdown Show latest trained image every n interations:
displ_freq = args.save_every #@param{type:"number"}
...
'''
optimizer = torch.optim.Adam(lats.parameters(), args.learning_rate)
eps = 0
