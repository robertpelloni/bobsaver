...
class BigSleep(nn.Module):
    ...
    def forward(self, text_embeds, text_min_embeds=[], return_loss = True):
        ...
        for ch in range(num_cutouts):
            # sample cutout size
            size = int(width * torch.zeros(1,).normal_(mean=.8, std=.3).clip(.5, .95)) # originally clip(.5, .95)
            # get cutout
...
class Imagine(nn.Module):
    def __init__(
        ...
        *,
        ...
        iterations = 1050,
        save_progress = False,
        bilinear = False,  # originally False
        open_folder = True,
        seed = None,
        append_seed = False,
        torch_deterministic = False, # originally False
        max_classes = None,
        class_temperature = 2.,
        save_date_time = False,
        save_best = False,
        experimental_resample = False, # originally False
        ema_decay = 0.99,
        num_cutouts = 128, # originally 128
        center_bias = False,
        clip_model = "ViT-B/32",
        ...
        sys.stdout.flush()
        #perceptor, normalize_image = load('ViT-B/32', jit = False)
        perceptor, normalize_image = load(clip_model, jit = False)
    ...
    def train_step(self, epoch, i, pbar=None):
        ...
        sys.stdout.write("Iteration {} Loss {}".format(i,loss)+"\n")
        sys.stdout.flush()
        ...
        if (i) % self.save_every == 0:
            ...
            with torch.no_grad():
                ...
                #save_image(image, 'progress.png')
                save_image(image, self.image_file)
                ...
                import numpy as np
                im_arr = do_image_warping(np.asarray(image),-1,4,0,2,10)
                ...
                #self.lr = lr
                normed_img = self.clip_transform(im_arr).unsqueeze(0).cuda()
                #with torch.no_grad():
    ...
    def forward(self):
        ...
        for asatreat in range(self.iterations):
            _,total_loss,step_loss = self.train_step(1, itt, None)
            itt+=1
