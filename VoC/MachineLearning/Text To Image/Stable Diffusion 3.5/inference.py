...
def parse_args():
    ...
    parser.add_argument("--quantization", type=int, help="0 no 1 yes")
...
if args2.in_script_movie == 0:
    ...
    if args2.init_image is None:
        sys.stdout.write(f"Setting up StableDiffusion3Pipeline ...\n")
        sys.stdout.flush()
        
        #no quantization
        if args2.quantization == 0:
            pipe = StableDiffusion3Pipeline.from_pretrained("stable-diffusion-3.5-large", torch_dtype=torch.float16)
        else:
            #quantization for less VRAM https://huggingface.co/stabilityai/stable-diffusion-3.5-large
            nf4_config = BitsAndBytesConfig(load_in_4bit=True, bnb_4bit_quant_type="nf4", bnb_4bit_compute_dtype=torch.bfloat16)
            model_nf4 = SD3Transformer2DModel.from_pretrained("stable-diffusion-3.5-large", subfolder="transformer", quantization_config=nf4_config, torch_dtype=torch.bfloat16)
            pipe = StableDiffusion3Pipeline.from_pretrained("stable-diffusion-3.5-large", transformer=model_nf4, torch_dtype=torch.bfloat16, max_sequence_length=512)
    ...
    #load lora
    if args2.lora is not None:
        sys.stdout.write(f"Setting up StableDiffusion3Pipeline LoRA ...\n")
        sys.stdout.flush()
        pipe.load_lora_weights(args2.lora)

    #if args2.img2img == 1 or args2.init_image is not None:
    if args2.img2img_model is not None or args2.init_image is not None:
        sys.stdout.write(f"Setting up StableDiffusion3Img2ImgPipeline ...\n")
        sys.stdout.flush()
        ...
        #quantization for less VRAM https://huggingface.co/stabilityai/stable-diffusion-3.5-large
        #does not work with img2img pipeline
        #nf4_config = BitsAndBytesConfig(load_in_4bit=True, bnb_4bit_quant_type="nf4", bnb_4bit_compute_dtype=torch.bfloat16)
        #model_nf4 = SD3Transformer2DModel.from_pretrained("stable-diffusion-3.5-large", subfolder="transformer", quantization_config=nf4_config, torch_dtype=torch.bfloat16)
        #pipe2 = StableDiffusion3Pipeline.from_pretrained("stable-diffusion-3.5-large", transformer=model_nf4, torch_dtype=torch.bfloat16)
        pipe2 = StableDiffusion3Img2ImgPipeline.from_pretrained(
            ...
            variant="fp16",
            text_encoder_3=None, #save VRAM
            tokenizer_3=None, #save VRAM
        )
    ...
    if args2.init_image is None:
        ...
        if args2.img2img_model is None:
            ...
            sys.stdout.flush()
            images = pipe2(prompt=args2.prompt,negative_prompt=args2.negative_prompt,width=args2.width,height=args2.height,num_images_per_prompt=args2.num_images_per_prompt, image=latents,num_inference_steps=args2.steps,strength=args2.init_image_strength).images[0]
    else:
        sys.stdout.write("Loading init image ...\n")
        sys.stdout.flush()
        ...
        sys.stdout.write("Refining init image ...\n")
        sys.stdout.flush()
        images = pipe2(prompt=args2.prompt,negative_prompt=args2.negative_prompt,width=args2.width,height=args2.height,guidance_scale=args2.guidance_scale,num_images_per_prompt=args2.num_images_per_prompt, image=initimage,num_inference_steps=args2.steps,strength=args2.init_image_strength).images[0]
...
if args2.in_script_movie == 1:
    ...
    if args2.quantization == 0:
        ...
        nf4_config = BitsAndBytesConfig(load_in_4bit=True, bnb_4bit_quant_type="nf4", bnb_4bit_compute_dtype=torch.bfloat16)
        model_nf4 = SD3Transformer2DModel.from_pretrained("stable-diffusion-3.5-large", subfolder="transformer", quantization_config=nf4_config, torch_dtype=torch.bfloat16)
        pipe = StableDiffusion3Pipeline.from_pretrained("stable-diffusion-3.5-large", transformer=model_nf4, torch_dtype=torch.bfloat16)
    ...
    #load lora
    if args2.lora is not None:
        sys.stdout.write(f"Setting up StableDiffusion3Img2ImgPipeline LoRA ...\n")
        sys.stdout.flush()
        pipe2.load_lora_weights(args2.lora)
    ...
    sys.stdout.write("Refining image ...\n")
    sys.stdout.flush()
    images = pipe2(prompt=prompt,negative_prompt=args2.negative_prompt,width=args2.width,height=args2.height,num_images_per_prompt=args2.num_images_per_prompt, image=latents,num_inference_steps=args2.steps,strength=args2.init_image_strength).images[0]
    ...
    while (frames<(args2.total_frames+1)):
        ...
        if frames % args2.render_every == 0:
            ...
            if args2.hard_cuts == 0:
                images = pipe2(prompt=prompt,negative_prompt=args2.negative_prompt,width=args2.width,height=args2.height,guidance_scale=args2.guidance_scale,num_images_per_prompt=args2.num_images_per_prompt, image=images,num_inference_steps=args2.steps,strength=args2.init_image_strength).images[0]
            else:
                if prompt == lastprompt:
                   ...
                   torch.random.manual_seed(args2.seed)
                   images = pipe2(prompt=prompt,negative_prompt=args2.negative_prompt,width=args2.width,height=args2.height,guidance_scale=args2.guidance_scale,num_images_per_prompt=args2.num_images_per_prompt, image=images,num_inference_steps=args2.steps,strength=1.0).images[0]
                   lastprompt=prompt
        else:
            sys.stdout.write('\n')
