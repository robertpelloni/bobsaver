# Mse regulized VQGANCLIP zquantize with forms
# Original file is located at https://colab.research.google.com/drive/1hf1seGOZctOJUznkhJNblLluXHbWLKZh
...
#@title Parameters { form-width: "750px" }
promt = args2.prompt
#@markdown SEED : Same input with different seed will produce different 'same' images
sdsd = args2.seed #@param {type:"slider", min:1, max:3000000, step:1}
#@markdown --------------------------------------
#@markdown Step: Default is 0.1 <br>
#@markdown This is a learning curve. Lower: more accurate, slower; Higher: less accurate, faster.
step = 0.2 #@param {type:"slider", min:0.02, max:2, step:0.01}
#@markdown MSE epoches : Default is 5 <br>
#@markdown This parameter 'guides' the generation, stabilizing it, increasing it leads to better readability of shapes and overall image
mse_ep = 45 #@param {type:"slider", min:5, max:150, step:1}
#@markdown MSE decay epoches : Default is 50 <br>
#@markdown This parameter defines how much epoches will be 'guided' (MSE_EP x 10 usually.)
mse_epdec = 125 #@param {type:"slider", min:50, max:850, step:1}

#@markdown Cutout value: default is 64 <br>
#@markdown Affects on 'Creativity': less cutout will lead to more random/creative results, sometimes barely readable, while higher values (90+) lead to very stable, photo-like outputs
cutout1 = 128 #@param {type:"slider", min:1, max:128, step:1}
#@markdown ----------------------------- 
#@markdown <b>Modifiers </b> <br>
#@markdown Modifier [Flip Horizontal]; Default: 0.5
fliphorizntl = 0.95 #@param {type:"slider", min:0.0, max:1.0, step:0.05}
#@markdown Modifier [Affine Random Degrees]; Default: 30 <br>
#@markdown This thing affects on micro-pieces rotation, low numbers as 5 could make image rough and look like its been cut
afdeg = 75 #@param {type:"slider", min:0, max:100, step:5}
#@markdown Modifier [Affine Translate]; Default: 0.1
aftran = 0.55 #@param {type:"slider", min:0.0, max:0.6, step:0.05}
#@markdown Modifier [Affine Padding]; Default: 0.8
afpad = 0.5 #@param {type:"slider", min:0.25, max:1, step:0.05}
#@markdown Modifier [Random Perspective]; Default: 0.2
persprand = 0.2 #@param {type:"slider", min:0.05, max:0.5, step:0.05}

#@markdown -----------------------------
#@markdown Display each n epoch: <br>
#@markdown This is how frequent updates of progress will appear during generation.
showeach = 25 #@param {type:"slider", min:5, max:100, step:5}
#@markdown Epochs number. Default: 400, Infinite: -1 <br>
#@markdown Amount of iterations neural network will iterate on input. Decent results happens already at 350-450
maxepochs = 410 #@param {type:"slider", min:50, max:800, step:15}
#@markdown -----------------------------
noisef1 = 0.1
#@markdown Output Resolution. Default is: 640x512.
resX = args2.sizex #640 #@param {type:"slider", min:128, max:640, step:1}
resY = args2.sizey #512 #@param {type:"slider", min:128, max:512, step:1}

args = argparse.Namespace(
    
    prompts=[promt],
    size=[resX, resY],   #default is [640, 512]
    init_image= None,
    init_weight= 0.5,
...
augs = nn.Sequential(
    ...
    K.RandomHorizontalFlip(p=fliphorizntl),
    K.RandomAffine(degrees=afdeg, translate=aftran, p=afpad, padding_mode='border'), # padding_mode=2
...
def ascend_txt():
    ...
    if args.record_generation:
      with torch.no_grad():
        ...
        TF.to_pil_image(out_a[0].cpu()).save('Progress.png')
        vid_index += 1
    """
...
for i in range(args2.iterations):
    train(itt)
    if itt > 0 and itt%args.mse_decay_rate==0 and itt <= args.mse_decay_rate * args.mse_epoches:
        z = EMATensor(z.average, args.ema_val)
        opt = optim.Adam(z.parameters(), lr=args.step_size, weight_decay=0.00000000)
    itt+=1
