# Improved Multi Perceptor VQGAN + CLIP [Public]
...
def parse_args():
  ...
  parser.add_argument('--learning_rate_epoch', type=float, help='Learning rate epoch.')
  ...
  parser.add_argument('--cut_size_mode', type=str, help='cut_size_mode')
  parser.add_argument('--cut_map_weight', type=float, help='cut_map_weight')
  parser.add_argument('--cut_weight_pow', type=float, help='cut_weight_pow')
  parser.add_argument('--cut_std_factor', type=float, help='cut_std_factor')
  parser.add_argument('--cut_oversample', type=float, help='cut_oversample')
  ...
  parser.add_argument('--donoiseburst', type=int, help='Do noise burst.')
  parser.add_argument('--noise_frac_base', type=float, help='noise_frac_base')
  parser.add_argument('--noise_frac_burst', type=float, help='noise_frac_burst')
  parser.add_argument('--noise_burst_start', type=int, help='noise_burst_start')
  parser.add_argument('--noise_burst_end', type=int, help='noise_burst_end')
  parser.add_argument('--noise_burst_freq', type=int, help='noise_burst_freq')
  parser.add_argument('--noise_burst_length', type=int, help='noise_burst_length')
  parser.add_argument('--rand_init_mode', type=str, help='rand_init_mode')
  parser.add_argument('--perlin_octaves', type=int, help='perlin_octaves')
  parser.add_argument('--perlin_weight', type=float, help='perlin_weight')
  parser.add_argument('--pyramid_octaves', type=int, help='pyramid_octaves')
  parser.add_argument('--pyramid_decay', type=float, help='pyramid_decay')
  parser.add_argument('--ema_val', type=float, help='ema_val')
  parser.add_argument('--mse_weight', type=float, help='mse_weight')
  parser.add_argument('--mse_epoch_step', type=int, help='mse_epoch_step')
  parser.add_argument('--mse_epoch_count', type=int, help='mse_epoch_count')
  parser.add_argument('--tv_weight', type=float, help='tv_weight')
  parser.add_argument('--use_ema_tensor', type=int, help='use_ema_tensor')
  parser.add_argument('--seed_image', type=str, help='Initial seed image.', default=None)
  parser.add_argument('--animation', type=int, help='Enable in script zoom and rotate')
  parser.add_argument('--anim_start', type=int, help='Animation start iteration')
  parser.add_argument('--anim_step', type=int, help='Iterations per animation frame')
  parser.add_argument('--zoom_amount', type=int, help='Animation zoom')
  parser.add_argument('--rotate_amount', type=float, help='Animation rotate')
...
should_display_init = False #@param {type:'boolean'}
display_every = args2.update#50 #@param {type:'number'}
...
<font size=5>❗</font> *You need to run these cells even if you are not using these features.*
"""

#@title ### ✒️ Prompt Writing Tips
#@markdown 
...
if args2.donoiseburst==1:
    DoNoiseBurst = True
else:
    DoNoiseBurst = False

noise_frac_base = args2.noise_frac_base #0.1 #@param {type:'number'}
#@markdown  ---
noise_frac_burst = args2.noise_frac_burst #0.3 #@param {type:'number'}
noise_burst_start = args2.noise_burst_start #150 #@param {type:'number'}
noise_burst_end = args2.noise_burst_end #350 #@param {type:'number'}
noise_burst_freq = args2.noise_burst_freq #100 #@param {type:'number'}
noise_burst_length = args2.noise_burst_length #100 #@param {type:'number'}
...
cut_sizes_should_use_large_dimension = False #@param {type:'boolean'}
cut_size_mode = args2.cut_size_mode #"trunc_norm" #@param [ "normal", "trunc_norm", "cut_pow", "cut_map"]

cut_map_weight = args2.cut_map_weight #0.66#@param {type:'number'}
cut_map_size_weights = [0.6, 0.4, 0.2, 0.12] #@param

std_factor = args2.cut_std_factor #1.5#@param {type:'number'}
#@markdown - Used in truncated normal to pick random cut sizes

cut_oversample = args2.cut_oversample #0#@param {type:"slider", min:0, max:1, step:0.01}
...
if args2.animation==1:
    UseAnimationOverride = True #@param {type:'boolean'}
else:
...
anim_start = args2.anim_start #100  #@param {type:'number'}
anim_step = args2.anim_step #3 #@param {type:'number'}

#@markdown Animation parameters for basic animation. Modify the code in this cell to create more complex behaviors.
zoom_amount = args2.zoom_amount #4  #@param {type:'number'}
rotate_amount = args2.rotate_amount #-0.5  #@param {type:'number'}
...
cut_weight_pow = args2.cut_weight_pow #0.5 #@param {type:'number'}
...
### MSE Loss Behavior
mse_epoch_step = args2.mse_epoch_step #50 #@param {type:'number'}
mse_epoch_count = args2.mse_epoch_count #5 #@param {type:'number'}
...
gen_seed = args2.seed #-1#@param {type:'number'}
...
max_iter = args2.iterations #500 #@param {type:'number'}
...
rand_init_mode = args2.rand_init_mode #'VQGAN ZRand'#@param [ "VQGAN ZRand", "Perlin Noise", "Pyramid Noise"]
perlin_octaves = args2.perlin_octaves #7#@param {type:"slider", min:1, max:8, step:1}
perlin_weight = args2.perlin_weight #0.22#@param {type:"slider", min:0, max:1, step:0.01}
pyramid_octaves = args2.perlin_octaves #5#@param {type:"slider", min:1, max:8, step:1}
pyramid_decay = args2.pyramid_decay #0.99#@param {type:"slider", min:0, max:1, step:0.01}
ema_val = args2.ema_val #0.99
...
clip_model2 = args2.clip_model_2 #'None' #@param ["None","ViT-B/16", "ViT-B/32", "RN50x16", "RN50x4"]
if clip_model2 == "None":
  clip_model2 = None 
...
learning_rate_epoch = args2.learning_rate_epoch#0.2#@param {type:'number'}
