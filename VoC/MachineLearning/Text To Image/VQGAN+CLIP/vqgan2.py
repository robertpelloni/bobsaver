...
# Original file is located at https://colab.research.google.com/drive/15UwYDsnNeldJFHJ9NdgYBYeo6xPmSelP
...
def parse_args():
  ...
  parser.add_argument('--tv_weight', type=float, help='TV weight.')
  ...
  parser.add_argument('--softmax', type=bool, help='Softmax.')
...
args = argparse.Namespace(
    ...
    tv_weight=args2.tv_weight, # originally 0.
    #clip_model='ViT-B/32',
...
def checkin(i, losses):
    ...
    if args2.r is not None:
        ...
        #logits = torch.randn([toksY * toksX, n_toks], device=device, requires_grad=True)
        #logits = TF.to_tensor(im_arr).to(device).unsqueeze(0) * 2 - 1
        logits = TF.to_tensor(model.encode(TF.to_tensor(im_arr).to(device).unsqueeze(0) * 2 - 1))
