...
def do_init(args):
    ...
    sys.stdout.write("Loading VQGAN model "+args.vqgan_config+" ...\n")
    sys.stdout.flush()
    ...
    #drawer.load_model('vqgan_imagenet_f16_16384.yaml','vqgan_imagenet_f16_16384.ckpt', device)
...
def checkin(args, iter, losses):
    ...
    outim = img.resize((args.size[0], args.size[1]), Image.LANCZOS)
    outim.save(outfile, pnginfo=info)
    
    
    
    #img.save('Progress.png', pnginfo=info)
...
def train(args, cur_it):
    ...
    if cur_it==0:
        ...
        sys.stdout.write("Iteration {}".format(cur_it)+"\n")
        sys.stdout.flush()
...
def setup_parser():
    ...
    vq_parser.add_argument("-vqgan","--vqgan_model", type=str, help="VQGAN model", default='vqgan_imagenet_f16_16384', dest='vqgan_model')
