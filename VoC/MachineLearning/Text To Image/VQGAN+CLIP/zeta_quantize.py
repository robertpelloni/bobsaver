# zeta_quantize.ipynb
# Original file is located at https://colab.research.google.com/gist/afiaka87/a97cca3b54c02209b94ff805224f9eb5/zeta_quantize.ipynb
...
prompts = args2.prompt #"an orange hue sunset in the background. a surfer riding a surfboard under a huge wave." #@param {type:"string"}
vqgan_model = args2.vqgan_model #"vqgan_imagenet_f16_16384" #@param ["vqgan_imagenet_f16_16384", "vqgan_imagenet_f16_1024", "gumbelvqgan_oi_f8_8192"]
noise_prompt_seeds = [] 
...
initial_image=args2.seed_image
initial_weight=0.2
clip_model = args2.clip_model
learning_rate=args2.learning_rate
num_random_cutouts=args2.cutn
#@markdown `cutout_power` "lower = more global coherence"
cutout_power=args2.cut_power
display_frequency=args2.update

num_iterations=args2.iterations
...
def checkin(iter, losses):
    ...
    if args2.r is not None:
        ...
        z.requires_grad_(True)
        opt = optim.Adam([z], lr=args.step_size, betas=(0.9, 0.96), weight_decay=1e-8, amsgrad=True)
...
def train(iter):
    ...
    sys.stdout.write("Iteration {}".format(iter)+"\n")
    sys.stdout.flush()
    
    if iter % args2.update == 0:
