...
def display_callback(info):
    ...
    if (info['i']) % display_every == 0:
        ...
        utils.to_pil_image(info['pred']).save(image_name)

        sys.stdout.flush()
        sys.stdout.write("Progress saved\n")
        sys.stdout.flush()
    """
    sys.stdout.write(f'Callback Iteration {info["i"]+1}\n')
    sys.stdout.flush()
    if (info['i']+1) % display_every == 0:
        ...
        cur_batch_size = 1
        outs = run(info["x"][info["i"]:info["i"]+cur_batch_size], info["steps"])
        for j, out in enumerate(outs):
            #utils.to_pil_image(out).save(f'out_{i + j:05}.png')
            utils.to_pil_image(out).save(args.image_file)
...
def main():
    ...
    p.add_argument('--model', type=str, default='cc12m_1_cfg',
    ...
    p.add_argument('--image_file', type=str, help='output image filename')
    args = p.parse_args()
    ...
    def run(x, steps):
        if args.method == 'ddpm':
            return sampling.sample(cfg_model_fn, x, steps, 1., {}, callback=display_callback)
        if args.method == 'ddim':
            return sampling.sample(cfg_model_fn, x, steps, args.eta, {}, callback=display_callback)
        if args.method == 'prk':
            return sampling.prk_sample(cfg_model_fn, x, steps, {}, callback=display_callback)
        if args.method == 'plms':
            return sampling.plms_sample(cfg_model_fn, x, steps, {}, callback=display_callback)
        if args.method == 'pie':
            return sampling.pie_sample(cfg_model_fn, x, steps, {}, callback=display_callback)
        if args.method == 'plms2':
            return sampling.plms2_sample(cfg_model_fn, x, steps, {}, callback=display_callback)
        assert False
        ...
        for i in trange(0, n, batch_size):
            ...
            if args.batch-size>1:
                for j, out in enumerate(outs):
                    ...
                    utils.to_pil_image(out).save(f'{dirname}\\out_{i + j:05}.png')
            """
