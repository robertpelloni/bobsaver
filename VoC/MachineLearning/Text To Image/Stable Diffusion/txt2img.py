            ...
            with model.ema_scope():
                ...
                for n in range(args.n_iter):
                    ...
                    for prompts in data:
                        uc = None
                        if args.scale != 1.0:
                            uc = model.get_learned_conditioning(batch_size * [args.negative_prompt])
                    ...
                    if args.n_samples > 0:
                        if not args.skip_save:
                            for ite, x_sample in enumerate(x_samples):
                                ...
                                #also save to a txt file that VoC can read after script finishes to load the image
                                #for some reason the sys.stdout.write calls do not always output text that VoC can parse_args
                                #so the preview image on the Text-to-Image dialog is not updated
                                #using a txt file here means VoC can open the txt file and load the image once the script ends
                                with open('last_image.txt', 'w') as f:
                                    f.write(f'{get_unique_filename(file_name)}')#.stdout.encode('utf-8')
                                #foo = str(get_unique_filename(file_name))
                                #print(f'***** foo={foo.decode("utf-8")}')
                                #f = open('last_image.txt', 'w', encoding="utf-8")
                                #f.write(str(foo).encode('utf-8'))
                                #f.close()
                ...
                if not args.skip_grid:
                    ...
                    #using a txt file here means VoC can open the txt file and load the image once the script ends
                    with open('last_image.txt', 'w') as f:
                        f.write(f'{get_unique_filename(file_name)}')
                    #foo = get_unique_filename(file_name)
                    #print(f'***** foo={foo.decode("utf-8")}')
                    #f = open('last_image.txt', 'w', encoding="utf-8")
                    #f.write(str(foo).encode('utf-8'))
                    #f.close()
