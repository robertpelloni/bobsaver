...
if args2.in_script_movie == 0:
    ...
    if args2.img2img_model is not None or args2.init_image is not None:
        ...
        pipe2 = StableDiffusion3Img2ImgPipeline.from_pretrained("models", torch_dtype=torch.float16, use_safetensors=True, variant="fp16")
    ...
    if args2.init_image is None:
        ...
        sys.stdout.write("Refining init image ...\n")
        sys.stdout.flush()
        images = pipe2(prompt=args2.prompt,negative_prompt=args2.negative_prompt,guidance_scale=args2.guidance_scale,num_images_per_prompt=args2.num_images_per_prompt, image=initimage,num_inference_steps=args2.steps,strength=args2.init_image_strength).images[0]
    ...
    while (frames<(args2.total_frames+1)):
        ...
        if frames % args2.render_every == 0:
            ...
            if args2.hard_cuts == 0:
                images = pipe2(prompt=prompt,negative_prompt=args2.negative_prompt,guidance_scale=args2.guidance_scale,num_images_per_prompt=args2.num_images_per_prompt, image=images,num_inference_steps=args2.steps,strength=args2.init_image_strength).images[0]
            else:
                if prompt == lastprompt:
                   ...
                   torch.random.manual_seed(args2.seed)
                   images = pipe2(prompt=prompt,negative_prompt=args2.negative_prompt,guidance_scale=args2.guidance_scale,num_images_per_prompt=args2.num_images_per_prompt, image=images,num_inference_steps=args2.steps,strength=1.0).images[0]
                   lastprompt=prompt
        else:
            sys.stdout.write('\n')
