...
def init_paths(input_ns):
    ...
    input_ns.inverse_path = os.path.join(const.GENERATED_DATA_PATH, 'inverses', input_ns.video_name, f'{input_ns.preprocess_name}_{input_ns.grid_size}x{input_ns.grid_size}_{input_ns.pad}')
...
def install_civitai_model(model_id):
    ...
    print(model_name)
    #print(f'RUNNING python {convert_py_path} --checkpoint_path {model_name}  --dump_path {diffusers_path_model_name}  --from_safetensors')
...
def run(*args):
    ...
    print(f'list_of_inputs[22] = {list_of_inputs[22]}')
    input_ns.model_id = const.MODEL_IDS[list_of_inputs[22]] 
    #input_ns.model_id = list_of_inputs[22] 

    #if(len(list_of_inputs[22]) > 0):
    #    words = list_of_inputs[22].split()
    ...
    CN.init_models(input_ns.hf_cn_path, input_ns.hf_path, input_ns.preprocess_name, input_ns.model_id)
    #CN.init_models(f'"{input_ns.hf_cn_path}"', input_ns.hf_path, input_ns.preprocess_name, input_ns.model_id)
