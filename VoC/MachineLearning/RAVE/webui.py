...
def init_paths(input_ns):
    ...
    #clear pad as it points to D: for some reason and causes the script to die
    #input_ns.pad=1
    
    print(f'const.GENERATED_DATA_PATH = {const.GENERATED_DATA_PATH}')
    print(f'input_ns.hf_path = {input_ns.hf_path}')
    print(f'input_ns.video_name = {input_ns.video_name}')
    print(f'input_ns.preprocess_name = {input_ns.preprocess_name}')
    print(f'input_ns.model_id = {input_ns.model_id}')
    print(f'input_ns.grid_size = {input_ns.grid_size}')
    print(f'input_ns.preprocess_name = {input_ns.preprocess_name}')
    print(f'input_ns.pad = {input_ns.pad}')

    words = input_ns.model_id.split('\\')

    input_ns.inverse_path = os.path.join(const.GENERATED_DATA_PATH, 'inverses', input_ns.video_name, f'{input_ns.preprocess_name}_{words[-1]}_{input_ns.grid_size}x{input_ns.grid_size}_{input_ns.pad}')
    ...
    #input_ns.inverse_path = os.path.join(const.GENERATED_DATA_PATH, 'inverses', input_ns.video_name, f'{input_ns.preprocess_name}_{input_ns.model_id}_{input_ns.grid_size}x{input_ns.grid_size}')
    #input_ns.control_path = os.path.join(const.GENERATED_DATA_PATH, 'controls', input_ns.video_name, f'{input_ns.preprocess_name}_{input_ns.grid_size}x{input_ns.grid_size}')
    
    print(f'input_ns.hf_cn_path = {input_ns.hf_cn_path}')
    print(f'input_ns.hf_path = {input_ns.hf_path}')
    print(f'input_ns.inverse_path = {input_ns.inverse_path}')
    print(f'input_ns.control_path = {input_ns.control_path}')
...
def install_civitai_model(model_id):
    ...
    os.system(f'wget https://civitai.com/api/download/models/{model_id} --content-disposition --directory "{install_path_model}"')
    ...
    os.system(f'python {convert_py_path} --checkpoint_path "{model_name}"  --dump_path "{diffusers_path_model_name}"  --from_safetensors')
...
def run(*args):
    ...
    if str(input_ns.model_id) != 'None':
        ...
        #words = input_ns.model_id.split('\\')
        #input_ns.model_id = words[-2]+'/'+words[-1]
        #input_ns.model_id = words[-1]
        #input_ns.model_id = words[-2]
        #print(f'*****input_ns.model_id = {input_ns.model_id}')
