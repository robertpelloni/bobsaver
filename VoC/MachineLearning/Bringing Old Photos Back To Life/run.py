...
def run_cmd(command):
    sys.stdout.write(f"run_cmd {command}\n")
    sys.stdout.flush()
    try:
        call(command, shell=True)
    except KeyboardInterrupt:
        sys.stdout.write("Process interrupted\n")
        sys.stdout.flush()
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    sys.stdout.write("Running Stage 1: Overall restoration\n")
    sys.stdout.flush()
    os.chdir("./Global")
    ...
    if not opts.with_scratch:
        ...
        mask_dir = stage_1_output_dir+"\\masks"
        new_input = mask_dir+"\\input"
        new_mask = mask_dir+"\\mask"
        ...
        sys.stdout.write(f"Running stage_1_command_1 {stage_1_command_1} ...\n")
        sys.stdout.flush()
        
        run_cmd(stage_1_command_1)

        sys.stdout.write(f"Running stage_1_command_2 {stage_1_command_2} ...\n")
        sys.stdout.flush()
    ...
    sys.stdout.write("Finish Stage 1 ...\n")
    sys.stdout.flush()

    ## Stage 2: Face Detection

    sys.stdout.write("Running Stage 2: Face Detection\n")
    sys.stdout.flush()
    ...
    sys.stdout.write("Finish Stage 2 ...\n")
    sys.stdout.flush()

    ## Stage 3: Face Restore
    sys.stdout.write("Running Stage 3: Face Enhancement\n")
    sys.stdout.flush()
    ...
    if opts.HR:
        ...
        stage_3_command = (
            'python test_face.py'
            +' --old_face_folder "'+ stage_3_input_face+'"'
            +' --old_face_label_folder "'+ stage_3_input_mask+'"'
            +' --tensorboard_log --name "'+ opts.checkpoint_name+'"'
            +' --gpu_ids '+ gpu1
            +' --load_size 512 --label_nc 18 --no_instance --preprocess_mode resize --batchSize 1 --results_dir "'+ stage_3_output_dir+'"'
            +' --no_parsing_map'
        ) 
    else:
        """
        ...
        stage_3_command = (
            ...
            +' --tensorboard_log --name "'+ opts.checkpoint_name+'"'
            +' --gpu_ids '+ gpu1
            +' --load_size 256 --label_nc 18 --no_instance --preprocess_mode resize --batchSize 4 --results_dir "'+ stage_3_output_dir+'"'
            +' --no_parsing_map'
        )
    run_cmd(stage_3_command)
    sys.stdout.write("Finish Stage 3 ...\n")
    sys.stdout.flush()

    ## Stage 4: Warp back
    sys.stdout.write("Running Stage 4: Blending\n")
    sys.stdout.flush()
    os.chdir(".././Face_Detection")
    ...
    if opts.HR:
        ...
        """
        stage_4_command = (
            'python align_warp_back_multiple_dlib_HR.py'
    ...
    sys.stdout.write("Finish Stage 4 ...\n")
    sys.stdout.flush()

    sys.stdout.write("All the processing is done. Please check the results.\n")
    sys.stdout.flush()
