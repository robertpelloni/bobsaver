...
def extract_frames(video, outDir):
    ...
    if IS_WINDOWS:
        ffmpeg_path = os.path.join(args.ffmpeg_dir, "ffmpeg.exe")
    else:
        ffmpeg_path = "ffmpeg"
...
def create_video(dir):
    ...
    retn = os.system('"'+'"{}" -r "{}" -i "{}/%d.png" -c:v libx264 -preset:v veryslow -profile:v high -crf 15 -pix_fmt yuvj420p -an -y "{}"'.format(ffmpeg_path, args.fps, dir, args.output)+'"')
...
def download_file_from_google_drive(id, destination):
    sys.stdout.write("\nDownloading required Super-SloMo model\n")
    sys.stdout.write("This only needs to happen once the first time Super-SloMo is used\n")
    sys.stdout.write("Please wait...\n")
    sys.stdout.flush()
    
    URL = "http://voc.x-marks-the-spot.net/models/SuperSloMo.ckpt"
...
file_id = '1IvobLDbRiBgZr3ryCRrWL8xDbMZ-KnpF'
destination = 'SuperSloMo.ckpt'
