...
from blender_utils.node import arrange_nodes
...
def build_environment_texture_background(world: bpy.types.World, rotation: float = 0.0) -> None:
    world.use_nodes = True
    node_tree = world.node_tree
...
def set_environment_texture_background(world: bpy.types.World, hdri_path: str, rotation: float = 0.0) -> None:
    world.use_nodes = True
    node_tree = world.node_tree
    # for i in node_tree.nodes:
    #     print(i)
    environment_texture_node = node_tree.nodes["Environment Texture"]
    environment_texture_node.image = bpy.data.images.load(hdri_path)
