...
def main():
    ...
    parser.add_argument('--model_path', type=str, default='model_zoo/swin2sr/Swin2SR_ClassicalSR_X2_64.pth')
    ...
    parser.add_argument('--output_location', type=int, default=1, help='where to save the result')
    parser.add_argument('--overwrite_source_image', default=False, action='store_true', help='overwrite the source image with upscaled image')
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        print(f'args.save_dir = {save_dir}')
        model_name = os.path.basename(args.model_path)
        model_name = model_name[:-4]
        """
        #output to destination directory
        if (args.output_location==1):
            output_file = f'{save_dir}/{imgname}_{model_name}.png'
            print(f'Saving image to {output_file}')
            cv2.imwrite(output_file, output)
        #output to destination directory
        if (args.output_location==2):
            output_file = f'{save_dir}/{imgname}_{model_name}.png'
            print(f'Saving image to {output_file}')
            cv2.imwrite(output_file, output)
        #overwrite original
        if (args.output_location==3):
            head, tail = os.path.split(path)
            output_file = f'{save_dir}{tail}'
            print(f'Saving image to {output_file}')
            cv2.imwrite(output_file, output)
        """
        if args.overwrite_source_image==True:
            output_file = f'{save_dir}/{imgname}.png'
        else:
            output_file = f'{save_dir}/{imgname}_{model_name}.png'
        print(f'Saving image to {output_file}')
        cv2.imwrite(output_file, output)
